---
id: "hld-interviews-payment-financial-systems-design-digital-wallet-service"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Payment Financial Systems"
subSection: ""
title: "Design Digital Wallet Service"
slug: "hld-interviews-payment-financial-systems-design-digital-wallet-service"
summary: "What makes digital wallets fascinating from an engineering perspective is the zero tolerance for errors. In most systems, a bug might cause a bad user experience or data inconsistency that can be fixed later. In a wallet system, a bug could mean m..."
eli10: "Imagine Design Digital Wallet Service as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Digital Wallet Service Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TB
    ...
    subgraph "Supporting Features"
        direction LR
        BAL[Balance Check<br/>Real-time accuracy]:::secondary
        HIST[Transaction History<br/>Complete audit trail]:::secondary
    end

    subgraph "Core Operations"
        direction LR
        TU[Top-up<br/>External → Wallet]:::green
        TR[Transfer<br/>Wallet → Wallet]:::primary
        WD[Withdraw<br/>Wallet → External]:::orange
    end
...
flowchart LR
    subgraph "Daily Traffic"
        direction TB
        T1[50M transactions/day<br/>580 TPS average]:::primary
    end

    subgraph "Peak Traffic (5x)"
        direction TB
        T2[2,900 TPS<br/>during high activity]:::orange
    end
...
```mermaid
flowchart LR
    subgraph "Storage Growth (1 Year)"
        direction TB
        W[Wallets<br/>20 GB]:::green
        T[Transactions<br/>5.5 TB]:::orange
        L[Ledger Entries<br/>5.5 TB]:::orange
    end
...
flowchart LR
    ...
    subgraph "Write Operations"
        POST1[POST /wallets/id/topup<br/>Add money]:::green
        POST2[POST /wallets/id/withdraw<br/>Remove money]:::orange
        POST3[POST /transfers<br/>Send to another user]:::purple
    end

    subgraph "Read Operations"
        GET1[GET /wallets/id/balance<br/>Check balance]:::primary
        GET2[GET /wallets/id/transactions<br/>View history]:::primary
    end
...
flowchart TB
    ...
    subgraph O2["Option 2: Computed Balance"]
        C1["Transaction<br/>History"]:::orange
        C2["Sum all transactions<br/>O(n) computation"]:::red
    end
	
    subgraph O1["Option 1: Stored Balance"]
        S1["Wallet Record<br/>balance: $500"]:::primary
        S2["Single row read<br/>O(1) lookup"]:::green
    end	
...
sequenceDiagram
    ...
    Client->>API: GET /wallets/{id}/balance
    Note over API: Authenticate user
    API->>WS: Forward request
    WS->>Cache: Check cached balance
    alt Cache Hit
        Cache-->>WS: Return balance
        Note over WS: Verify cache is fresh
    else Cache Miss
        WS->>DB: Query wallet
        DB-->>WS: Wallet record
        WS->>Cache: Populate cache
    end
    WS-->>Client: Return balance
```
...
sequenceDiagram
    ...
    Client->>API: POST /transfers (Alice→Bob, $100)
    API->>TS: Forward request
    TS->>IS: Check idempotency key
    alt Already Processed
        IS-->>TS: Return cached result
        TS-->>Client: Return cached response
    else New Request
        TS->>IS: Mark key as "processing"
        TS->>DB: BEGIN TRANSACTION
        TS->>DB: SELECT ... FOR UPDATE (lock both wallets)
        TS->>DB: Verify Alice.balance >= $100
        alt Insufficient Balance
            TS->>DB: ROLLBACK
            TS-->>Client: 400 Insufficient Balance
        else Balance OK
            TS->>DB: UPDATE Alice.balance -= $100
            TS->>DB: UPDATE Bob.balance += $100
            TS->>DB: INSERT transaction record
            TS->>LS: Create ledger entries
            TS->>DB: COMMIT
            TS->>IS: Mark key as "completed" with result
            TS->>Cache: Invalidate Alice & Bob balance cache
...
sequenceDiagram
    ...
    participant Stripe as External Processor
    participant DB as Database
    participant LS as Ledger Service

    Client->>TS: POST /topup ($100, card_token)
    TS->>DB: Create pending transaction
    TS->>PG: Charge card
    PG->>Stripe: Process payment
    Stripe-->>PG: Payment approved
    PG-->>TS: Success with charge ID
    TS->>DB: BEGIN TRANSACTION
    TS->>DB: Credit wallet += $100
    TS->>DB: Update transaction (completed)
    TS->>LS: Create ledger entries
    TS->>DB: COMMIT
    TS-->>Client: Success with new balance
```
...
sequenceDiagram
    ...
    Client->>TS: POST /withdraw ($200, bank_id)
    TS->>DB: BEGIN TRANSACTION
    TS->>DB: Verify balance >= $200
    TS->>DB: Debit wallet -= $200
    TS->>DB: Create pending withdrawal
    TS->>LS: Create ledger entries
    TS->>DB: COMMIT
    TS->>MQ: Queue withdrawal job
    TS-->>Client: Return pending status

    Note over MQ,Bank: Asynchronous Processing (1-3 days)
    MQ->>BTS: Process withdrawal
    BTS->>Bank: Initiate ACH transfer
    Bank-->>BTS: Transfer initiated
    BTS->>DB: Update status (processing)
    Note over Bank: 1-3 business days later
    Bank-->>BTS: Transfer completed
    BTS->>DB: Update status (completed)
```
...
flowchart TB
    ...
    subgraph Data Layer
        IS[(Idempotency Store<br/>Redis)]
        CACHE[(Balance Cache<br/>Redis)]
        WDB[(Wallet Database<br/>PostgreSQL)]
        LDB[(Ledger Database<br/>PostgreSQL)]
    end
    ...
    style MA fill:#00ceff,stroke:#000,color:#000
    style WA fill:#00ceff,stroke:#000,color:#000
    style AG fill:#38d9a9,stroke:#000,color:#000
    style WS fill:#00ceff,stroke:#000,color:#000
    style TS fill:#00ceff,stroke:#000,color:#000
    style LS fill:#ffa94d,stroke:#000,color:#000
    style PG fill:#da77f2,stroke:#000,color:#000
    style BTS fill:#da77f2,stroke:#000,color:#000
    style IS fill:#ff8787,stroke:#000,color:#000
    style CACHE fill:#ff8787,stroke:#000,color:#000
    style WDB fill:#9775fa,stroke:#000,color:#000
    style LDB fill:#9775fa,stroke:#000,color:#000
    style MQ fill:#ffd43b,stroke:#000,color:#000
    style PP fill:#f783ac,stroke:#000,color:#000
    style BANK fill:#f783ac,stroke:#000,color:#000
```
...
```mermaid
flowchart LR
    subgraph "PostgreSQL for Wallet Data"
        A[ACID Transactions]:::green
        B[Strong Consistency]:::green
        C[Row-Level Locking]:::green
        D[NUMERIC Precision]:::green
    end
...
sequenceDiagram
    ...
    Note over A,B: Alice's balance: $100
    A->>DB: SELECT balance WHERE wallet_id='alice'
    DB-->>A: balance = $100
    B->>DB: SELECT balance WHERE wallet_id='alice'
    DB-->>B: balance = $100
    Note over A: $100 >= $100" Yes!
    Note over B: $100 >= $100" Yes!
    A->>DB: UPDATE balance = 0
    B->>DB: UPDATE balance = -100
    Note over A,B: Alice now has -$100 (double-spent!)
```
...
sequenceDiagram
    ...
    A->>DB: SELECT ... FOR UPDATE (acquires lock)
    B->>DB: SELECT ... FOR UPDATE (blocks, waiting)
    A->>DB: UPDATE balance = 0
    A->>DB: COMMIT (releases lock)
    Note over B: Lock acquired!
    DB-->>B: balance = 0
    Note over B: $0 >= $100" No!
    B-->>B: Return "insufficient balance"
```
...
flowchart TD
    subgraph "The Bottleneck"
        T1[Transaction 1]:::primary --> L[Lock on<br/>Merchant Wallet]:::red
        T2[Transaction 2]:::primary --> L
        T3[Transaction 3]:::primary --> L
        T4[Transaction N...]:::primary --> L
    end

    L --> DB[(Single Row<br/>Update)]:::purple
...
```mermaid
flowchart LR
    subgraph "100ms batch window"
        T1[Payment $50]:::primary
        T2[Payment $75]:::primary
        T3[Payment $25]:::primary
    end

    T1 --> Q[Batch Queue]:::orange
    T2 --> Q
    T3 --> Q

    Q --> P[Batch Processor<br/>Sum = $150]:::secondary
    P --> DB[(Single UPDATE<br/>balance += 150)]:::purple
...
```mermaid
flowchart TB
    subgraph "Merchant Wallet (Total: $40,000)"
        S1[Sub-wallet 1<br/>$10,000]:::orange
        S2[Sub-wallet 2<br/>$10,000]:::orange
        S3[Sub-wallet 3<br/>$10,000]:::orange
        S4[Sub-wallet 4<br/>$10,000]:::orange
    end

    P1[Payment 1]:::primary --> S1
    P2[Payment 2]:::primary --> S2
    P3[Payment 3]:::primary --> S3
    P4[Payment 4]:::primary --> S4
...
flowchart LR
    subgraph "Ledger Entries"
        E1[Debit: Alice $100]:::red
        E2[Credit: Bob $100]:::green
    end

    E1 --> |"Sum = 0"| BAL[Balanced]:::primary
...
```mermaid
flowchart TD
    A[Request arrives<br/>with idempotency key]:::primary --> B{Key exists<br/>in store"}
    B -->|Yes, completed| C[Return cached<br/>success response]:::green
    B -->|Yes, failed| D[Return cached<br/>error response]:::red
    B -->|Yes, processing| E[Return 409<br/>try again later]:::orange
    B -->|No| F[Insert key as<br/>'processing']:::primary
    F --> G{Insert<br/>succeeded"}
    G -->|No, duplicate| H[Race lost, retry lookup]:::orange
    G -->|Yes| I[Process transaction]:::primary
    I --> J{Success"}
    J -->|Yes| K[Update: completed<br/>+ cache response]:::green
    J -->|No| L[Update: failed<br/>+ cache error]:::red
    K --> M[Return success]:::green
    L --> N[Return error]:::red
...
stateDiagram-v2
    ...
    pending --> processing: Validation passed
    pending --> failed: Validation failed
    processing --> completed: All operations successful
...
```mermaid
flowchart TD
    A[Debit Wallet for Withdrawal] --> B[Call Bank API]
    B --> C{Bank API<br/>Success"}
    C -->|Yes| D[Mark Completed]
    C -->|No: Definite Failure| E[Credit Wallet Back<br/>Mark Failed]
    C -->|Timeout/Unknown| F[Mark Pending<br/>Reconcile Later]
    ...
    style F fill:#ffd43b,stroke:#000,color:#000
```
...
```mermaid
flowchart LR
    A[Transaction<br/>Request] --> B[Velocity<br/>Check]
    B --> C[Amount<br/>Limits]
    C --> D[Device/IP<br/>Check]
    D --> E[ML Risk<br/>Score]
    E --> F{Risk<br/>Level}
    F -->|Low| G[Approve]
    F -->|Medium| H[Additional<br/>Verification]
    F -->|High| I[Block]
    ...
    style E fill:#9775fa,stroke:#000,color:#000
    style G fill:#69db7c,stroke:#000,color:#000
    style H fill:#ffd43b,stroke:#000,color:#000
    style I fill:#ff8787,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    A[Transaction Service] --> B[Risk Service]
    B --> C[Rule Engine]
    B --> D[ML Model Service]
    B --> E[Velocity Cache<br/>Redis]
    C --> F{Decision}
    D --> F
    F --> G[Return Risk Score<br/>+ Actions]

    style A fill:#00ceff,stroke:#000,color:#000
    style B fill:#ffa94d,stroke:#000,color:#000
    style C fill:#38d9a9,stroke:#000,color:#000
    style D fill:#9775fa,stroke:#000,color:#000
    style E fill:#ff8787,stroke:#000,color:#000
```
