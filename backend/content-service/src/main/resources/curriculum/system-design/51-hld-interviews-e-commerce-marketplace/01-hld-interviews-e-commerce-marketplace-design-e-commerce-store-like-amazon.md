---
id: "hld-interviews-e-commerce-marketplace-design-e-commerce-store-like-amazon"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - E commerce Marketplace"
subSection: ""
title: "Design E-commerce Store like Amazon"
slug: "hld-interviews-e-commerce-marketplace-design-e-commerce-store-like-amazon"
summary: "This system design problem covers a wide range of distributed systems concepts including search, inventory management, payment processing, and handling traffic spikes."
eli10: "Imagine Design E-commerce Store like Amazon as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design E-commerce Store like Amazon Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Product Discovery"
        S[Search & Browse]:::primary
        P[Product Details]:::primary
    end

    subgraph "Purchase Flow"
        C[Shopping Cart]:::orange
        CH[Checkout]:::orange
        O[Order & Tracking]:::green
    end

    subgraph "Behind the Scenes"
        I[Inventory Management]:::purple
    end
...
flowchart TB
    subgraph "Daily Scale"
        direction TB
        U[100M DAU]:::primary
        P[100M Products]:::primary
    end

    subgraph "Read Traffic"
        V[2B page views/day<br/>23,000 QPS avg]:::green
        VP[230,000 QPS peak]:::green
    end

    subgraph "Write Traffic"

        O[2M orders/day<br/>23 QPS avg]:::orange
        OP[230 QPS peak]:::orange
    end
...
flowchart TB
    subgraph "Traffic Pattern"
        R[Reads: 23,000 QPS]:::green
        W[Writes: 23 QPS]:::orange
        Ratio[1000:1 Read/Write Ratio]:::primary
    end

    subgraph "Design Implications"
        C[Heavy caching<br/>CDN + Redis]:::green
        S[Separate read/write paths]:::primary
        I[Inventory needs<br/>special handling]:::red
    end
...
flowchart LR
    subgraph "Product Discovery"
        S[GET /products/search<br/>Find products]:::primary
        D[GET /products/id<br/>View details]:::primary
    end

    subgraph "Shopping Cart"
        CA[POST /cart/items<br/>Add item]:::orange
        CU[PUT /cart/items/id<br/>Update quantity]:::orange
        CR[DELETE /cart/items/id<br/>Remove item]:::orange
    end

    subgraph "Checkout"
        O[POST /orders<br/>Place order]:::green
        OS[GET /orders/id<br/>Track status]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Read Path (23,000 QPS)"
        direction LR
        R1[Search]:::primary
        R2[Browse]:::primary
        R3[View Product]:::primary
    end

    subgraph "Write Path (23 QPS)"
        direction LR
        W1[Add to Cart]:::orange
        W2[Checkout]:::orange
        W3[Create Order]:::orange
    end

    R1 & R2 & R3 --> Cache[(Cache Layer)]:::green
    Cache --> DB1[(Read Replicas)]:::purple

    W1 & W2 & W3 --> Primary[(Primary DB)]:::purple
...
flowchart LR
    ...
    U -->|"1. GET /products/search"q=wireless+headphones"| LB
    LB --> AG
    AG -->|"2. Route to search"| SS
    SS -->|"3. Check cache"| Redis
    SS -->|"4. Query if miss"| ES
...
sequenceDiagram
    ...
    Client->>API: GET /products/prod_abc123
    API->>PS: Forward request
    PS->>Cache: Check cache for product
    alt Cache Hit
        Cache-->>PS: Return cached product
    else Cache Miss
        PS->>DB: Query product data
        DB-->>PS: Return product
        PS->>Cache: Populate cache
    end
    PS-->>API: Return product details
    API-->>Client: 200 OK with product JSON
    Client->>CDN: Load product images
    CDN-->>Client: Return images
```
...
sequenceDiagram
    ...
    participant Cache as Redis (Cart)
    participant DB as Cart DB

    Client->>API: POST /cart/items {product_id, quantity: 2}
    API->>CS: Forward request
    CS->>IS: Check availability for product
    IS-->>CS: Available (156 in stock)

    alt Sufficient Stock
        CS->>Cache: Add item to cart
        Cache-->>CS: Updated cart
        opt Logged-in User
            CS->>DB: Persist cart async
        end
        CS-->>API: Return updated cart
        API-->>Client: 200 OK with cart JSON
...
sequenceDiagram
    ...
    Client->>API: POST /orders {cart_id, address, payment}
    API->>OS: Create order request

    Note over OS: Step 1: Reserve Inventory
    OS->>IS: Reserve items for cart
    IS-->>OS: Reservation confirmed (reservation_id)

    Note over OS: Step 2: Process Payment
    OS->>PS: Charge customer
    PS-->>OS: Payment successful (payment_id)

    Note over OS: Step 3: Create Order
    OS->>DB: Insert order record
    DB-->>OS: Order created (order_id)

    Note over OS: Step 4: Confirm & Publish
    OS->>IS: Confirm reservation (deduct stock)
    ...
    MQ-->>MQ: Email, fulfillment, analytics
```
...
```mermaid
flowchart TD
    Start[Start Checkout]:::primary --> Reserve[Reserve Inventory]:::green
    Reserve -->|Success| Payment[Process Payment]:::green
    Reserve -->|Failure| Err1[Return: Out of Stock]:::red

    Payment -->|Success| CreateOrder[Create Order]:::green
    Payment -->|Failure| Release1[Release Inventory]:::orange
    Release1 --> Err2[Return: Payment Failed]:::red

    CreateOrder -->|Success| Confirm[Confirm & Notify]:::green
    CreateOrder -->|Failure| Refund[Refund Payment]:::orange
    Refund --> Release2[Release Inventory]:::orange
    Release2 --> Err3[Return: Order Failed]:::red

    Confirm --> Done[Order Complete]:::primary
...
flowchart TB
    ...
    subgraph Services["Application Services"]
        direction TB
        Search[Search Service]:::primary
        Product[Product Service]:::primary
        Cart[Cart Service]:::primary
        Order[Order Service]:::primary
        Inventory[Inventory Service]:::orange
        Payment[Payment Service]:::orange
    end

    subgraph Async["Async Processing"]
        Kafka[Kafka]:::secondary
        Notification[Notification<br/>Service]:::green
        Fulfillment[Fulfillment<br/>Service]:::green
    end

    subgraph Data["Data Layer"]
        ES[(Elasticsearch)]:::purple
        ProductDB[(Product DB)]:::purple
        OrderDB[(Order DB)]:::purple
        InventoryDB[(Inventory DB)]:::purple
        Redis[(Redis)]:::orange
    end

    subgraph External["External Services"]
        PayProc[Payment<br/>Processor]:::pink
    end
...
flowchart TB
    ...
    subgraph "Best Fit"
        NoSQL[(NoSQL<br/>DynamoDB)]:::purple
        SQL[(SQL<br/>PostgreSQL)]:::purple
        Redis[(Redis<br/>Counters)]:::orange
...
flowchart LR
    subgraph "Indexing Pipeline"
        P[(Product DB)]:::purple --> CDC[CDC / Debezium]:::secondary
        CDC --> Kafka[Kafka]:::secondary
        Kafka --> Indexer[Indexer Service]:::primary
        Indexer --> ES[(Elasticsearch)]:::purple
    end
...
```mermaid
flowchart TB
    Query[User Query: 'wireless headphones']:::rose

    subgraph "Scoring Components"
        BM25[Text Relevance<br/>BM25 score]:::primary
        Pop[Popularity<br/>Sales, CTR]:::orange
        Biz[Business Rules<br/>Sponsored, Prime]:::green
        Personal[Personalization<br/>User history]:::purple
    end

    Query --> BM25 & Pop & Biz & Personal

    BM25 & Pop & Biz & Personal --> Final[Final Score<br/>Weighted combination]:::primary

    Final --> Results[Ranked Results]:::green
...
sequenceDiagram
    ...
    U1->>DB: SELECT FOR UPDATE (acquires lock)
    Note over DB: Row locked
    U2->>DB: SELECT FOR UPDATE
    Note over U2: Waiting...

    U1->>DB: UPDATE available_qty - 1
    U1->>DB: COMMIT (releases lock)
    Note over DB: Lock released

    U2->>DB: Lock acquired
    U2->>DB: UPDATE available_qty - 1
    U2->>DB: COMMIT
```
...
```mermaid
flowchart LR
    Start[User starts checkout]:::rose --> Reserve[Reserve inventory<br/>Move from available to reserved]:::primary
    Reserve --> Timer{10 min<br/>timer}
    Timer -->|Payment succeeds| Confirm[Confirm reservation<br/>Create order]:::green
    Timer -->|Timeout or failure| Release[Release back<br/>to available]:::orange
...
flowchart TB
    subgraph "Layer 1: Edge"
        CDN[CDN Caching]:::green
        RL[Rate Limiting]:::green
    end

    subgraph "Layer 2: Application"
        Queue[Request Queuing]:::orange
        Circuit[Circuit Breakers]:::orange
    end

    subgraph "Layer 3: Data"
        Redis[Redis Counters]:::red
        Tokens[Pre-computed Tokens]:::red
    end

    subgraph "Layer 4: Graceful Degradation"
        Degrade[Disable non-essential<br/>features]:::purple
    end

    CDN --> Queue --> Redis --> Degrade
...
```mermaid
flowchart TB
    Users[Users]:::rose --> CDN[CDN<br/>Cached product pages]:::green
    CDN -->|Checkout requests| LB[Load Balancer<br/>Rate limiting]:::secondary

    LB --> Queue[Request Queue<br/>Kafka]:::orange
    Queue --> Workers[Order Workers<br/>Controlled rate]:::primary

    Workers --> Redis[(Redis<br/>Token/Counter)]:::red
    Redis -->|Success| Order[Create Order]:::green
    Redis -->|Sold Out| Reject[Return 'Sold Out']:::red

    Order --> DB[(PostgreSQL)]:::purple
...
flowchart LR
    subgraph "Forward Path"
        S1[Reserve<br/>Inventory]:::green --> S2[Process<br/>Payment]:::green
        S2 --> S3[Create<br/>Order]:::green
        S3 --> S4[Notify<br/>Customer]:::green
    end

    subgraph "Compensation Path"
        C1[Release<br/>Inventory]:::orange
        C2[Refund<br/>Payment]:::orange
        C3[Cancel<br/>Order]:::orange
    end

    S1 -.->|"If payment fails"| C1
    S2 -.->|"If order fails"| C2
    S2 -.->|"If order fails"| C1
    S3 -.->|"If notify fails"| C3
...
```mermaid
flowchart LR
    Req[Request arrives]:::rose --> Check{Idempotency key<br/>in database"}
    Check -->|Yes| Return[Return cached response]:::green
    Check -->|No| Process[Process order]:::primary
    Process --> Store[Store result with key]:::primary
    Store --> Respond[Return response]:::green
...
flowchart TB
    subgraph "Offline Pipeline (Batch)"
        Events[User Events]:::rose --> Kafka[Kafka]:::secondary
        Kafka --> Process[Event Processor]:::primary
        Process --> Features[(Feature Store)]:::purple
        Features --> Train[ML Training]:::orange
        Train --> Models[(Model Store)]:::purple
    end

    subgraph "Online Pipeline (Real-time)"
        Request[Rec Request]:::rose --> Service[Recommendation Service]:::primary
        Service --> Models
        Service --> Features
        Service --> Cache[(Redis Cache)]:::orange
        Service --> Response[Ranked Results]:::green
    end
...
flowchart LR
    subgraph "Challenges"
        C1[High Read Traffic<br/>23,000+ QPS]:::rose
        C2[Product Search<br/>Relevance + Speed]:::rose
        C3[Inventory<br/>No Overselling]:::rose
        C4[Flash Sales<br/>50x Traffic Spikes]:::rose
        C5[Order Reliability<br/>No Double Charges]:::rose
    end

    subgraph "Solutions"
        S1[CDN + Redis + Replicas]:::green
        S2[Elasticsearch + Ranking]:::green
        S3[Reservations + Redis Counters]:::green
        S4[Queuing + Rate Limiting]:::green
        S5[Saga + Idempotency]:::green
    end
