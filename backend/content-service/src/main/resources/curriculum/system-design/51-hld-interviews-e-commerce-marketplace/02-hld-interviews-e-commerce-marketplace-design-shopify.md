---
id: "hld-interviews-e-commerce-marketplace-design-shopify"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - E commerce Marketplace"
subSection: ""
title: "Design Shopify"
slug: "hld-interviews-e-commerce-marketplace-design-shopify"
summary: "This system design problem covers multi-tenancy, inventory management, payment processing, and handling traffic spikes during flash sales. It tests your ability to design a system that balances consistency (inventory, payments) with high availabil..."
eli10: "Imagine Design Shopify as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Shopify Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Daily Traffic"
        direction TB
        W[Orders<br/>10M/day<br/>115 QPS avg]:::orange
        R[Page Views<br/>1B/day<br/>11.5K QPS avg]:::primary
    end

    subgraph "Peak Traffic (3x)"
        direction TB
        WP[350 QPS<br/>orders]:::orange
        RP[35K QPS<br/>reads]:::primary
    end

    subgraph "Flash Sale (10x peak)"
        direction TB
        WF[3,500 QPS<br/>orders]:::red
        RF[350K QPS<br/>reads]:::red
    end
...
flowchart LR
    ...
    subgraph "Customer APIs"
        C1[GET /products<br/>Browse catalog]:::primary
        C2[POST /cart/items<br/>Add to cart]:::primary
        C3[POST /checkout<br/>Place order]:::green
        C4[GET /orders/:id<br/>Track order]:::primary
    end

    subgraph "Merchant APIs"
        M1[POST /products<br/>Create product]:::orange
        M2[PUT /products/:id<br/>Update product]:::orange
        M3[GET /orders<br/>View orders]:::orange
    end
...
```mermaid
flowchart TD
    subgraph "Write Path (1x traffic)"
		direction LR
        W1[Customer]:::rose --> W2[API Gateway]:::secondary
        W2 --> W3[Services]:::primary
        W3 --> W4[(Database)]:::purple
    end
...
```mermaid
flowchart TD
    subgraph "Read Path (100x traffic)"
		direction LR
        R1[Customer]:::rose --> R2[CDN Edge]:::green
        R2 -.->|cache miss| R3[Cache Layer]:::orange
        R3 -.->|cache miss| R4[(Database)]:::purple
    end
...
flowchart LR
    ...
    U -->|"1. Request product page"| CDN
    CDN -->|"2. API request"| LB
    LB -->|"3. Route to instance"| SF
    SF -->|"4. Check cache"| Redis
    SF -->|"5. Cache miss"| DB
    SF -->|"6. Return product"| U
...
sequenceDiagram
    ...
    Customer->>CDN: GET /products/tshirt-123
    Note over CDN: Serve cached images, CSS, JS
    CDN->>LB: Forward API request
    LB->>SF: Route to available instance
    SF->>Cache: Get product:tshirt-123
    alt Cache Hit
        Cache-->>SF: Return cached product
        Note over SF: 95% of requests end here
    else Cache Miss
        SF->>DB: SELECT * FROM products WHERE id = 'tshirt-123'
        DB-->>SF: Return product data
        SF->>Cache: SET product:tshirt-123 (TTL: 1 hour)
    end
    SF-->>Customer: Return product JSON
```
...
flowchart TB
    ...
    subgraph Application["Application Layer"]
        CART[Cart Service]:::primary
        INV[Inventory Service]:::primary
    end

    subgraph Caching["Cache Layer"]
        Redis[(Redis<br/>Cart + Reservations)]:::orange
    end

    subgraph Storage["Storage Layer"]
        IDB[(Inventory DB)]:::purple
    end

    U -->|"1. Add to cart"| CART
    CART -->|"2. Reserve inventory"| INV
    INV -->|"3. Atomic check & reserve"| IDB
    INV -->|"4. Track reservation TTL"| Redis
    CART -->|"5. Update cart"| Redis
    CART -->|"6. Confirm addition"| U
...
sequenceDiagram
    ...
    Customer->>Cart: Add 2 blue t-shirts (M) to cart
    Cart->>Inv: Reserve 2 units of variant_123
    Inv->>DB: BEGIN TRANSACTION
    Note over Inv,DB: Check: available - reserved >= 2
    alt Sufficient Stock
        Inv->>DB: UPDATE inventory SET reserved = reserved + 2
        Inv->>DB: COMMIT
        Inv->>Redis: Set reservation TTL (15 min)
        Inv-->>Cart: Reservation successful
        Cart->>Redis: Update cart with items
        Cart-->>Customer: "Added to cart"
    else Insufficient Stock
        Inv->>DB: ROLLBACK
        Inv-->>Cart: Insufficient inventory
        Cart-->>Customer: "Sorry, only X available"
    end
```
...
> **Why reserve at add-to-cart time"**
>
...
flowchart TB
    ...
    subgraph Application["Application Layer"]
        CHK[Checkout Service]:::primary
        CART[Cart Service]:::secondary
        INV[Inventory Service]:::secondary
        ORD[Order Service]:::primary
    end

    subgraph External["External Services"]
        PAY[Payment Gateway<br/>Stripe/PayPal]:::pink
    end
    ...
    subgraph Messaging["Async Processing"]
        KFK[Kafka]:::orange
        EMAIL[Email Service]:::secondary
        NOTIF[Merchant Notifications]:::secondary
    end

    U -->|"1. Place order"| CHK
    CHK -->|"2. Validate cart"| CART
    CHK -->|"3. Confirm inventory"| INV
    CHK -->|"4. Process payment"| PAY
    CHK -->|"5. Create order"| ORD
    ORD -->|"6. Save order"| ODB
    ORD -->|"7. Publish event"| KFK
    KFK --> EMAIL
    KFK --> NOTIF
    CHK -->|"8. Return confirmation"| U
...
sequenceDiagram
    ...
    Customer->>Checkout: POST /checkout (cart_id, payment_token, address)

    Note over Checkout: Step 1: Validate Cart
    Checkout->>Cart: Get cart details
    Cart-->>Checkout: Cart items, totals

    Note over Checkout: Step 2: Verify Inventory
    Checkout->>Inv: Confirm reservations still valid
    Inv-->>Checkout: Reservations confirmed

    Note over Checkout: Step 3: Process Payment
    Checkout->>Pay: Authorize + Capture payment
    alt Payment Succeeds
        Pay-->>Checkout: Payment successful (payment_id)

        Note over Checkout: Step 4: Create Order
        Checkout->>Order: Create order
        Order->>DB: INSERT order record
        Order->>Kafka: Publish "order.created" event
        Order-->>Checkout: Order created (order_id)

        Note over Checkout: Step 5: Commit Inventory
        Checkout->>Inv: Commit reservations
        Inv-->>Checkout: Inventory decremented

        Checkout-->>Customer: Order confirmed!
    else Payment Fails
        Pay-->>Checkout: Payment declined
        Checkout->>Inv: Release reservations
        Checkout-->>Customer: Payment failed, please retry
    end
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        direction LR
        CU[Customers<br/>Web/Mobile]:::rose
        ME[Merchants<br/>Admin Panel]:::rose
    end

    subgraph Edge["Edge Layer"]
        CDN[CDN<br/>CloudFront/Fastly]:::green
        LB[Load Balancer]:::secondary
    ...
    subgraph Services["Application Layer"]
        direction TB
        SF[Storefront<br/>Service]:::primary
        PS[Product<br/>Service]:::primary
        INV[Inventory<br/>Service]:::primary
        CART[Cart<br/>Service]:::primary
        CHK[Checkout<br/>Service]:::primary
        ORD[Order<br/>Service]:::primary
    end
    ...
    subgraph Storage["Storage Layer"]
        direction LR
        PDB[(Product DB<br/>PostgreSQL)]:::purple
        IDB[(Inventory DB<br/>PostgreSQL)]:::purple
        ODB[(Order DB<br/>PostgreSQL)]:::purple
        S3[(Object Storage<br/>S3/GCS)]:::purple
    end
    ...
    subgraph Messaging["Async Processing"]
        KFK[Kafka]:::orange
        EMAIL[Email<br/>Service]:::lightblue
        NOTIF[Merchant<br/>Notifications]:::lightblue
        FULFILL[Fulfillment<br/>Trigger]:::lightblue
    end

    subgraph Background["Background Workers"]
        CLEANUP[Cleanup<br/>Service]:::lightblue
        RECON[Reconciliation<br/>Service]:::lightblue
    end
...
flowchart TB
    subgraph "Data Domains"
        P[Products<br/>Read-heavy, flexible schema]:::primary
        I[Inventory<br/>Strong consistency, atomic ops]:::orange
        O[Orders<br/>Durability, complex relations]:::green
        C[Carts<br/>Ephemeral, fast access]:::red
    end

    subgraph "Database Choices"
        PG[(PostgreSQL)]:::purple
        ES[(Elasticsearch)]:::secondary
        RD[(Redis)]:::orange
    end
...
flowchart TB
    subgraph "Shared Infrastructure"
        API[API Layer]:::primary
        SVC[Services]:::primary
        CACHE[Cache]:::orange
    end

    subgraph "Merchant A"
        A1[Store A<br/>Products, Orders]:::rose
    end

    subgraph "Merchant B"
        B1[Store B<br/>Products, Orders]:::rose
    end

    subgraph "Merchant C"
        C1[Store C<br/>Products, Orders]:::rose
    end

    A1 & B1 & C1 --> API --> SVC --> CACHE

    subgraph "Data Layer"
        DB[(Shared Database<br/>with store_id isolation)]:::purple
    end
...
flowchart LR
    subgraph "Hybrid Approach"
        direction TB
        SM[Shared Schema<br/>95% of stores]:::primary
        SS[Separate Schema<br/>4% of stores]:::orange
        DT[Dedicated DB<br/>1% of stores]:::red
    end

    SM --> |Traffic < 10K/day| SM
    SM --> |High traffic| SS
    SS --> |Enterprise tier| DT
...
sequenceDiagram
    ...
    U1->>App: Buy 1 item
    U2->>App: Buy 1 item
    App->>DB: SELECT stock WHERE product_id = 'xyz'
    App->>DB: SELECT stock WHERE product_id = 'xyz'
    DB-->>App: stock = 10
    DB-->>App: stock = 10

    Note over App: Both see 10 available

    App->>DB: UPDATE stock = 9
    App->>DB: UPDATE stock = 9

    Note over DB: Stock = 9 (should be 8!)
```
...
stateDiagram-v2
    ...
    Reserved --> Available: Cart Expires (15 min)
    Reserved --> Available: Remove from Cart
    Reserved --> Sold: Checkout Success
    Sold --> [*]: Order Delivered
```
...
```mermaid
flowchart LR
    subgraph "Hot Path (Flash Sales)"
        U[Users]:::rose --> R[(Redis)]:::red
        R --> |Success| CHK[Checkout]:::green
        R --> |Failure| ERR[Sold Out]:::orange
    end

    subgraph "Background Sync"
        R --> |Async| PG[(PostgreSQL)]:::purple
    end
...
```mermaid
flowchart LR
    subgraph "Before Flash Sale"
        N1[Normal Traffic<br/>100 QPS]:::primary
    end

    subgraph "During Flash Sale"
        N2[Spike Traffic<br/>10,000+ QPS]:::red
    end

    subgraph "After Flash Sale"

        N3[Return to Normal<br/>100 QPS]:::primary
    end

    N1 --> |"Sale starts"| N2 --> |"Items sell out"| N3
...
sequenceDiagram
    ...
    C->>API: Click "Buy Now"
    API->>Q: Enqueue checkout request
    Q-->>C: "You're in line, position #4,523"

    loop Process Queue
        Q->>W: Dequeue next request
        W->>INV: Check inventory
        alt In Stock
            W->>INV: Reserve item
...
```mermaid
timeline
    title Flash Sale Timeline
    T-24h : Announce sale
    T-2h : Pre-scale infrastructure
         : Warm caches with sale products
    T-1h : Pre-render product pages to CDN
         : Load inventory into Redis
    T-0 : Sale starts
        : CDN serves static content
        : Redis handles inventory
    T+1min : Sale ends (sold out)
    T+30min : Scale down resources
```
...
flowchart TB
    subgraph "Before Sale"
        P1[Pre-scale services]:::green
        P2[Warm caches]:::green
        P3[Pre-render to CDN]:::green
    end

    subgraph "During Sale"
        D1[CDN serves product pages]:::primary
        D2[Redis handles inventory]:::orange
        D3[Virtual queue for checkout]:::primary
        D4[Rate limiting as safety net]:::red
    end

    subgraph "Graceful Degradation"
        G1[Disable reviews]:::secondary
        G2[Disable recommendations]:::secondary
        G3[Simplify checkout flow]:::secondary
    end
...
sequenceDiagram
    ...
    C->>S: Click "Pay"
    S->>PG: Charge $50
    Note over S,PG: ❌ Network timeout here
    PG->>Bank: Debit card
    Bank-->>PG: Success
    PG-->>S: """ (timeout)

    Note over S: Did payment succeed"
    Note over S: If we retry, customer charged twice
    Note over S: If we don't, order never completes
```
...
sequenceDiagram
    ...
    C->>CHK: Submit order
    CHK->>PAY: Authorize $50
    PAY-->>CHK: Authorization ID (funds reserved)

    CHK->>INV: Confirm inventory
    alt Inventory Available
        CHK->>PAY: Capture payment
        PAY-->>CHK: Payment captured
        CHK-->>C: Order confirmed!
    else Out of Stock
        CHK->>PAY: Void authorization
        PAY-->>CHK: Authorization voided
        CHK-->>C: Sorry, item sold out (no charge)
    end
```
...
stateDiagram-v2
    ...
    Captured --> PartialRefund: Partial return
    Captured --> FullRefund: Full return
    Captured --> Disputed: Customer dispute

    PartialRefund --> FullRefund: Additional return
...
sequenceDiagram
    ...
    participant S as Login Process
    participant A as Account Cart

    Note over G: Session cart: 2x Blue Shirt
    Note over A: Account cart: 1x Blue Shirt, 1x Jeans

    G->>S: User logs in
    S->>A: Fetch account cart
    S->>S: Merge carts (sum quantities)
    Note over S: Result: 3x Blue Shirt, 1x Jeans
    S->>A: Save merged cart
    S->>G: Clear session cart
```
...
flowchart LR
    subgraph "Write Path"
        PS[Product Service]:::primary
        PDB[(PostgreSQL)]:::purple
        CDC[Change Data<br/>Capture]:::orange
    end
    ...
    subgraph "Read Path"
        SF[Storefront]:::primary
        U[Customer]:::rose
    end

    PS -->|Create/Update| PDB
    PDB -->|Stream changes| CDC
    CDC -->|Index| ES
    U -->|Search query| SF
    SF -->|Search| ES
...
flowchart LR
    subgraph "Search Results"
        direction TB
        R1[Blue Cotton T-Shirt]
        R2[Blue Denim Shirt]
        R3[Navy Polo Shirt]
    end

    subgraph "Facets (Filter Options)"
        direction TB
        F1[Category<br/>Tops: 45, Shirts: 23]:::orange
        F2[Color<br/>Blue: 15, Navy: 8]:::orange
        F3[Price<br/>$20-50: 12, $50-100: 8]:::orange
        F4[Size<br/>S: 5, M: 10, L: 8]:::orange
    end
