---
id: "hld-interviews-media-streaming-delivery-design-twitch"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Media Streaming Delivery"
subSection: ""
title: "Design Twitch"
slug: "hld-interviews-media-streaming-delivery-design-twitch"
summary: "In this chapter, we will explore the high-level design of a live streaming platform like Twitch."
eli10: "Imagine Design Twitch as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Twitch Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TB
    ...
    subgraph "Supporting Features"
        direction LR
        F4[VOD Recording<br/>Automatic archival]:::secondary
        F5[Follow/Subscribe<br/>Monetization]:::secondary
    end

    subgraph "Core Features"
        direction LR
        F1[Live Streaming<br/>Real-time broadcast]:::primary
        F2[Video Playback<br/>Adaptive quality]:::primary
        F3[Live Chat<br/>Sub-500ms delivery]:::primary
    end
...
```mermaid
flowchart LR
    subgraph "Ingest (600 Gbps)"
        I[100K Streams<br/>6 Mbps each]:::orange
    end

    subgraph "Platform"
        P[Transcode &<br/>Distribute]:::primary
    end

    subgraph "Egress (15 Tbps)"
        E[5M Viewers<br/>3 Mbps avg]:::green
    end

    I -->|600 Gbps| P -->|15 Tbps| E
...
flowchart LR
    ...
    subgraph "Chat APIs"
        F[POST /streams/id/chat<br/>Send message]:::secondary
        G[WS /streams/id/chat<br/>Subscribe to chat]:::secondary
    end

    subgraph "Viewing APIs"
        D[GET /streams/id/playback<br/>Get video URL]:::primary
        E[GET /vods/id/playback<br/>Watch VOD]:::primary
    end

    subgraph "Streaming APIs"
        A[POST /streams<br/>Start streaming]:::green
        B[PATCH /streams/id<br/>Update stream]:::orange
        C[DELETE /streams/id<br/>End stream]:::red
    end
...
sequenceDiagram
    ...
    Viewer->>Gateway: WebSocket connect
    Gateway->>Service: Validate connection
    Service-->>Gateway: Connection approved
    Gateway-->>Viewer: Connected to chat

    loop Message Stream
        Gateway-->>Viewer: New chat message
        Gateway-->>Viewer: New chat message
    end
```
...
flowchart LR
    subgraph "Streamer"
        OBS[Streaming Software<br/>OBS, Streamlabs]:::rose
    end

    subgraph "Ingest Layer"
        DNS[Stream Router<br/>GeoDNS]:::secondary
        I1[Ingest Server<br/>US-East]:::primary
        I2[Ingest Server<br/>EU-West]:::primary
        I3[Ingest Server<br/>Asia]:::primary
    end
...
sequenceDiagram
    ...
    participant DNS as Stream Router
    participant Ingest as Ingest Server
    participant Auth as Auth Service
    participant Pipeline as Video Pipeline
    participant DB as Stream Database

    OBS->>DNS: Connect to rtmp://live.twitch.example
    DNS-->>OBS: Nearest ingest: ingest-eu.twitch.example

    OBS->>Ingest: RTMP connect with stream_key
    Ingest->>Auth: Validate stream_key
    Auth->>DB: Look up user for stream_key
    DB-->>Auth: User: streamer_12345
    Auth-->>Ingest: Authenticated, user_id: streamer_12345

    Ingest->>DB: Create stream record (status: live)
    Ingest-->>OBS: Stream accepted, start sending video

    loop Every video frame
        OBS->>Ingest: Video frame
        Ingest->>Pipeline: Forward to transcoding
    end
```
...
flowchart TB
    subgraph "Input"
        I[Raw Stream<br/>6 Mbps 1080p]:::rose
    end

    subgraph "Transcoding"
        T[Transcoder Cluster]:::primary
    end

    subgraph "Output Qualities"
        Q1[1080p60<br/>6 Mbps]:::green
        Q2[720p60<br/>3 Mbps]:::green
        Q3[480p30<br/>1.5 Mbps]:::green
        Q4[360p30<br/>800 Kbps]:::green
    end
...
flowchart LR
    subgraph "Ingest"
        I1[Ingest Server 1]:::rose
        I2[Ingest Server 2]:::rose
    end

    subgraph "Queue"
        Q[Kafka/Kinesis<br/>Video Frames]:::secondary
    end

    subgraph "Transcoding Cluster"
        T1[Transcoder 1<br/>GPU]:::primary
        T2[Transcoder 2<br/>GPU]:::primary
        T3[Transcoder N<br/>GPU]:::primary
    end

    subgraph "Storage"
        S[Origin Storage<br/>Segments + Manifests]:::purple
    end
...
flowchart TB
    subgraph "Origin"
        OS[Origin Storage<br/>Source of Truth]:::purple
    end

    subgraph "CDN Edge Locations"
        E1[Edge: US-East<br/>NYC, DC, Atlanta]:::green
        E2[Edge: US-West<br/>LA, Seattle, SF]:::green
        E3[Edge: Europe<br/>London, Frankfurt, Paris]:::green
        E4[Edge: Asia<br/>Tokyo, Singapore, Seoul]:::green
    end

    subgraph "Viewers"
        V1[US Viewers]:::rose
        V2[EU Viewers]:::rose
        V3[Asia Viewers]:::rose
    end
...
sequenceDiagram
    ...
    API->>Play: Get playback info
    Play-->>Viewer: playback_url: cdn.example/abc123/master.m3u8

    Viewer->>CDN: GET master.m3u8 (manifest)
    alt Cache Hit
        CDN-->>Viewer: Return cached manifest
    else Cache Miss
        CDN->>Origin: Fetch manifest
        Origin-->>CDN: Return manifest
        CDN-->>Viewer: Return manifest (now cached)
    end

    Note over Viewer: Player parses manifest,<br/>selects 720p quality

    loop Every 4 seconds (segment duration)
        Viewer->>CDN: GET 720p/segment_042.ts
        CDN-->>Viewer: Return video segment
        Note over Viewer: Player buffers and plays
    end
```
...
flowchart LR
    subgraph "Cache TTLs"
        M[Manifest<br/>TTL: 2-4 sec]:::orange
        S[Segments<br/>TTL: 24 hours]:::green
    end

    M --> |"Short TTL so viewers<br/>discover new segments"| Note1[Viewers get fresh<br/>manifest often]:::secondary
    S --> |"Long TTL since segments<br/>are immutable"| Note2[Segments cached<br/>heavily at edge]:::secondary
...
```mermaid
flowchart TD
    subgraph "One Message In"
        M[Chat Message]:::rose
    end

    subgraph "100,000 Messages Out"
        V1[Viewer 1]:::green
        V2[Viewer 2]:::green
        V3[...]:::green
        V4[Viewer 100K]:::green
    end
...
```mermaid
sequenceDiagram
    participant VA as Viewer A (sender)
    participant GW1 as Chat Gateway 1
    participant CS as Chat Service
    participant PS as Pub/Sub (Redis)
    participant GW2 as Chat Gateway 2
    participant VB as Viewer B
    participant VC as Viewer C

    VA->>GW1: Send "Great play!"
    GW1->>CS: Process message
    Note over CS: Validate, moderate,<br/>check permissions

    CS->>PS: Publish to stream_abc123 channel
    PS-->>GW1: Message for stream_abc123
    PS-->>GW2: Message for stream_abc123

    GW1-->>VA: Receive "Great play!"
    GW1-->>VB: Receive "Great play!"
    GW2-->>VC: Receive "Great play!"
```
...
flowchart TB
    ...
    subgraph "Chat Gateways"
        GW1[Gateway 1<br/>10K connections]:::primary
        GW2[Gateway 2<br/>10K connections]:::primary
    end

    subgraph "Core Services"
        CS[Chat Service]:::orange
        PS[Redis Pub/Sub]:::green
    end

    subgraph "Storage"
        MS[(Message Store<br/>Cassandra)]:::purple
    end
...
flowchart TB
    subgraph "Streamers"
        S[Streamer<br/>OBS/Streamlabs]:::rose
    end

    subgraph "Ingest Layer"
        SR[Stream Router]:::secondary
        I1[Ingest Server]:::primary
        I2[Ingest Server]:::primary
    end

    subgraph "Processing Layer"
        Q[Message Queue<br/>Kafka]:::secondary
        T1[Transcoder]:::orange
        T2[Transcoder]:::orange
    end

    subgraph "Storage Layer"
        OS[Origin Storage]:::purple
        DB[(Metadata DB)]:::purple
        MS[(Chat Store)]:::purple
    end

    subgraph "Delivery Layer"
        CDN[CDN Edge Servers]:::green
    end

    subgraph "Chat System"
        GW1[Chat Gateway]:::primary
        GW2[Chat Gateway]:::primary
        CS[Chat Service]:::orange
        PS[Pub/Sub]:::green
    end
...
flowchart LR
    subgraph "Real-Time State"
		direction LR
        RT[Viewer Counts,<br/>Stream Status]:::red
        RT --> RD2[(Redis)]:::purple
    end

    subgraph "Large Binary Objects"
		direction LR
        BO[Video Segments]:::green
        BO --> S3[(S3/Object Storage)]:::purple
    end
	
    subgraph "High-Volume Time-Series"
		direction LR
        TS[Chat Messages]:::orange
        TS --> CS[(Cassandra)]:::purple
    end	
	
    subgraph "Relational Data"
		direction LR
        RD[Users, Streams,<br/>Subscriptions]:::primary
        RD --> PG[(PostgreSQL)]:::purple
    end	
...
flowchart TB
    subgraph "Protocol Comparison"
        HLS[HLS<br/>Segment-based<br/>10-30s latency]:::primary
        LLHLS[LL-HLS<br/>Partial segments<br/>2-5s latency]:::green
        WebRTC[WebRTC<br/>Frame-based<br/>sub-second latency]:::orange
    end

    subgraph "Trade-offs"
        T1[Latency vs Scalability]:::secondary
...
flowchart TD
    ...
    subgraph "LL-HLS"
        PA[Part A<br/>200ms]:::green --> PL[Player starts<br/>immediately]:::primary
        PB[Part B<br/>200ms]:::green --> PL
        PC[Part C<br/>200ms]:::green --> PL
    end
	
    subgraph "Traditional HLS"
        S1[Segment 1<br/>Complete 4s]:::orange --> P1[Player waits<br/>for full segment]:::secondary
    end	
...
flowchart TB
    subgraph "The Problem"
        V[500K Viewers]:::rose
        V -->|125K req/sec| C[CDN Edge]:::green
        C -->|Cache miss| O[Origin]:::orange
        O -->|Overloaded| X[Failure]:::red
    end
...
flowchart LR
    subgraph "CDN Edges"
        E1[Edge: NYC]:::green
        E2[Edge: LA]:::green
        E3[Edge: London]:::green
        E4[Edge: Tokyo]:::green
    end

    subgraph "Shield"
        SH[Shield Server<br/>Consolidates requests]:::orange
    end

    subgraph "Origin"
        O[Origin Server]:::purple
    end
...
```mermaid
flowchart LR
    subgraph "Prediction Signals"
        S1[Historical Data]:::primary
        S2[Follower Activity]:::primary
        S3[Social Media]:::primary
    end

    subgraph "Prediction Engine"
        PE[ML Model<br/>Predicts viewership]:::orange
    end

    subgraph "Proactive Actions"
        A1[Pre-warm CDN]:::green
        A2[Reserve Transcoders]:::green
        A3[Scale Chat]:::green
    end
...
```mermaid
flowchart TB
    M[Incoming Message]:::rose
    M --> CS[Chat Service]:::primary

    CS --> P1[Shard 1<br/>Gateway A, B<br/>25K viewers]:::green
    CS --> P2[Shard 2<br/>Gateway C, D<br/>25K viewers]:::green
    CS --> P3[Shard 3<br/>Gateway E, F<br/>25K viewers]:::green
    CS --> P4[Shard 4<br/>Gateway G, H<br/>25K viewers]:::green
...
flowchart LR
    subgraph "Live Stream"
        T[Transcoder]:::primary
        T --> OS[Origin Storage<br/>Live Segments]:::orange
    end

    subgraph "Recording"
        RS[Recording Service]:::primary
        RS --> |"Collect segments<br/>after stream ends"| OS
    end

    subgraph "VOD Storage"
        HS[Hot Storage<br/>Recent VODs]:::green
        WS[Warm Storage<br/>Older VODs]:::secondary
        CS[Cold Storage<br/>Archive]:::purple
    end

    RS --> HS
    HS --> |"Age out after 7 days"| WS
    WS --> |"Age out after 30 days"| CS
...
flowchart TB
    subgraph "US East (Primary)"
        USE[Full Stack<br/>Ingest, Transcode,<br/>Origin, DB Primary]:::primary
    end

    subgraph "US West"
        USW[Full Stack<br/>Ingest, Transcode,<br/>Origin, DB Replica]:::primary
    end

    subgraph "EU West"
        EU[Ingest + Origin<br/>Transcode, DB Replica]:::secondary
    end

    subgraph "Asia Pacific"
        APAC[Ingest + Origin<br/>Transcode, DB Replica]:::secondary
    end

    subgraph "CDN PoPs (Dozens)"
        CDN[Edge Servers<br/>Video Delivery]:::green
    end

    USE <-->|Replication| USW
    USE <-->|Replication| EU
    USE <-->|Replication| APAC
    USE --> CDN
    USW --> CDN
    EU --> CDN
    APAC --> CDN
...
```mermaid
flowchart TD
    subgraph "Latency Budget (10s total)"
        A[Capture + Encode<br/>1-2s]:::primary
        B[Ingest Upload<br/>0.5-1s]:::primary
        C[Transcoding<br/>1-2s]:::orange
        D[CDN Distribution<br/>0.2-0.5s]:::green
        E[Player Buffer<br/>2-4s]:::secondary
    end
...
flowchart LR
    OBS[Streamer]:::rose
    OBS --> I1[Ingest Primary]:::green
    OBS --> I2[Ingest Backup]:::secondary

    I1 -->|Active| Pipeline[Processing]:::primary
    I2 -.->|Standby| Pipeline
...
flowchart LR
    ...
    App --> PG1[(Primary<br/>PostgreSQL)]:::green
    PG1 -->|Sync Replication| PG2[(Standby<br/>PostgreSQL)]:::secondary

    PG1 -.->|Failure detected| Monitor[Health Monitor]:::orange
    Monitor -->|Promote| PG2
