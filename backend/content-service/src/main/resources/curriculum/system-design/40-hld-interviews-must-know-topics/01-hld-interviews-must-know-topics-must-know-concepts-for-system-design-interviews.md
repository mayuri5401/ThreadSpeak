---
id: "hld-interviews-must-know-topics-must-know-concepts-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Must Know Topics"
subSection: ""
title: "Must-Know Concepts for System Design Interviews"
slug: "hld-interviews-must-know-topics-must-know-concepts-for-system-design-interviews"
summary: "System design interviews test your knowledge of how large-scale systems work. Without a solid foundation in core concepts, you'll struggle to design systems or explain your decisions."
eli10: "Imagine Must-Know Concepts for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Must-Know Concepts for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Must Know Topics","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Must-Know Concepts for System Design Interviews"
  code: |
    # Good - uses nouns, plural, and hierarchy
...
flowchart LR
    ...
    subgraph Vertical["Vertical Scaling"]
        direction TB
        A1[Small Server]:::secondary --> A2[Medium Server]:::orange --> A3[Large Server]:::primary
    end
...
```mermaid
flowchart LR
    A[Clients]:::primary --> B[Load Balancer]:::orange
    B --> C[Server 1]:::green
    B --> D[Server 2]:::green
    B --> E[Server 3]:::green
...
[Embed: https://link.excalidraw.com/readonly/aUl2Y8RC0e7w6F58om67](https://link.excalidraw.com/readonly/aUl2Y8RC0e7w6F58om67)
...
[Embed: https://link.excalidraw.com/readonly/Vbr1QxNa1A7KQKymCJGa](https://link.excalidraw.com/readonly/Vbr1QxNa1A7KQKymCJGa)
...
```mermaid
flowchart TD
    A[Primary]:::primary --> B[Replica 1]:::secondary
    A --> C[Replica 2]:::secondary
    A --> D[Replica 3]:::secondary
...
```mermaid
flowchart LR
    A[Application]:::primary --> B[Shard Router]:::orange
    B --> C[(Shard 1<br/>Users A-H)]:::purple
    B --> D[(Shard 2<br/>Users I-P)]:::purple
    B --> E[(Shard 3<br/>Users Q-Z)]:::purple
...
```mermaid
flowchart LR
    A[Application]:::primary --> B{Cache Hit"}
    B -->|Yes| C[Return from Cache]:::green
    B -->|No| D[Read from DB]:::orange
    D --> E[Store in Cache]:::secondary
    E --> F[Return Data]:::green
...
```mermaid
flowchart TD
    A[User in Tokyo]:::primary --> B[Tokyo Edge Server]:::green
    C[User in NYC]:::primary --> D[NYC Edge Server]:::green
    E[User in London]:::primary --> F[London Edge Server]:::green

    B --> G[Origin Server]:::orange
    D --> G
    F --> G
...
```mermaid
flowchart LR
    A[Producer]:::primary --> B[Message Queue]:::orange
    B --> C[Consumer 1]:::green
    B --> D[Consumer 2]:::green
    B --> E[Consumer 3]:::green
...
```mermaid
flowchart TD
    A[Publisher]:::primary --> B[Topic: Orders]:::orange
    B --> C[Inventory Service]:::green
    B --> D[Notification Service]:::green
    B --> E[Analytics Service]:::green
...
```mermaid
flowchart LR
    P1[Producer]:::primary --> T[Topic Log]:::orange
    P2[Producer]:::primary --> T
    T --> C1[Consumer Group A]:::green
    T --> C2[Consumer Group B]:::green
    T --> C3[Consumer Group C]:::green
...
flowchart TD
    ...
    subgraph WebSocket["WebSocket"]
        direction LR
        W1[Client]:::primary <-->|"Persistent Connection"| W2[Server]:::orange
    end

    subgraph Polling["Polling"]
        direction LR
        P1[Client]:::primary -->|"Request"| P2[Server]:::secondary
        P2 -->|"Response"| P1
        P1 -->|"Request again..."| P2
    end
...
flowchart TD
    subgraph Active-Passive
        direction LR
        S4[Primary]:::green --> DB1[(Database)]:::purple
        S5[Standby]:::secondary -.->|"Takes over on failure"| DB1
    end

    subgraph Active-Active
        direction LR
        LB1[Load Balancer]:::orange --> S1[Server 1]:::green
        LB1 --> S2[Server 2]:::green
        LB1 --> S3[Server 3]:::green
    end
...
```mermaid
flowchart LR
    A[Client]:::primary --> B{Health Check}
    B -->|Primary Healthy| C[Primary Server]:::green
    B -->|Primary Failed| D[Backup Server]:::orange
...
```mermaid
flowchart LR
    A[Closed]:::green -->|Failures exceed </br>threshold| B[Open]:::red
    B -->|Timeout expires| C[Half-Open]:::orange
...
flowchart TD
    subgraph CAP["CAP Theorem"]
        ...
        B[Availability]:::orange
        C[Partition Tolerance]:::green
    end

    A --- D[CA: Traditional RDBMS]:::purple
    A --- E[CP: MongoDB, HBase]:::purple
    B --- D
    B --- F[AP: Cassandra, DynamoDB]:::purple
    C --- E
    C --- F
...
```shell
# Offset-based (simple but problematic for large datasets)
GET /posts"ffset=20&limit=10

# Cursor-based (more robust for live data)
...
Total daily requests = 100M × 10 = 1 billion
Average QPS = 1 billion / 100,000 seconds = 10,000 QPS
...
- Each post is 1 KB (text + metadata)
- Store for 5 years

Daily storage = 10M × 1 KB = 10 GB
Yearly storage = 10 GB × 365 = 3.65 TB
5-year storage = 3.65 TB × 5 ≈ 18 TB
```
