---
id: "hld-interviews-distributed-infrastructure-design-object-storage-like-s3"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design Object Storage (like S3)"
slug: "hld-interviews-distributed-infrastructure-design-object-storage-like-s3"
summary: "This system design problem touches on so many fundamental concepts: distributed storage, data durability, consistency models, replication strategies, and large-scale data management."
eli10: "Imagine Design Object Storage (like S3) as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Object Storage (like S3) Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TD
    subgraph "Scale"
        S[100 PB Total Storage]:::primary
        O[100 Billion Objects]:::primary
        A[1 MB Average Size]:::primary
    end
...
flowchart LR
    ...
    subgraph "Object APIs"
        O1[PUT /bucket/key<br/>Upload]:::green
        O2[GET /bucket/key<br/>Download]:::primary
        O3[DELETE /bucket/key<br/>Delete]:::red
    end

    subgraph "Bucket APIs"
        B1[PUT /bucket<br/>Create]:::green
        B2[DELETE /bucket<br/>Delete]:::red
        B3[GET /bucket<br/>List Objects]:::primary
    end
...
flowchart TB
    ...
    subgraph "Read Path"
        ...
        R2 --> R3[Lookup Metadata]:::orange
        R3 --> R4[Fetch Data]:::purple
    end

    subgraph "Write Path"
		direction TB
        W1[Client]:::rose --> W2[API Gateway]:::secondary
        W2 --> W3[Store Data First]:::purple
        W3 --> W4[Then Store Metadata]:::orange
    end
...
```mermaid
flowchart LR
    A[Client]:::rose --> B[API Gateway]:::secondary
    B --> D[Data Service]:::purple
    D --> E[Storage Node 1]:::green
    D --> F[Storage Node 2]:::green
    D --> G[Storage Node 3]:::green
    D --> C[Metadata Service]:::orange
    C --> H[(Metadata DB)]:::purple
...
> **Why store data before metadata"**
>
...
sequenceDiagram
    ...
    Gateway->>Meta: Lookup object location
    Meta-->>Gateway: Return storage node addresses
    Gateway->>Data: Fetch from storage
    Data->>Storage: Read object data
    Storage-->>Data: Return bytes
    Data-->>Gateway: Stream data
    Gateway-->>Client: Return object
```
...
flowchart TB
    ...
    subgraph "Prefix Queries"
        Q1["prefix='2024/vacation/'<br/>→ beach.jpg, sunset.jpg"]:::orange
        Q2["prefix='2024/'<br/>→ all 2024 objects"]:::orange
    end

    subgraph "Bucket: my-photos"
        O1[2024/vacation/beach.jpg]:::primary
        O2[2024/vacation/sunset.jpg]:::primary
        O3[2024/birthday/cake.jpg]:::primary
        O4[profile.jpg]:::primary
    end
...
```mermaid
flowchart TD
    A[Region: us-east-1]:::primary --> B[AZ: us-east-1a]:::orange
    A --> C[AZ: us-east-1b]:::orange
    A --> D[AZ: us-east-1c]:::orange

    B --> E[Rack 1]:::secondary
    B --> F[Rack 2]:::secondary
...
flowchart TB
    ...
    subgraph Control["Control Plane"]
        BS[Bucket Service]:::orange
        MS[Metadata Service]:::orange
        PS[Placement Service]:::orange
        RM[Replication Manager]:::orange
    end

    subgraph Data["Data Plane"]
        DS[Data Service]:::purple
    end

    subgraph Storage["Storage Layer (3 AZs)"]
        subgraph AZ1["AZ-1"]
            SN1[Node 1]:::green
            SN2[Node 2]:::green
        end
    ...
    subgraph MetaDB["Metadata Storage"]
        MDB[(Distributed<br/>Metadata DB)]:::purple
    end
    ...
    DS --> SN1 & SN2 & SN3 & SN4 & SN5 & SN6

    RM -.->|Health Monitoring| SN1 & SN3 & SN5
    PS -.->|Placement Decisions| DS
...
flowchart LR
    ...
    subgraph "NoSQL"
        N1[Horizontal scale]:::primary
        N2[Predictable latency]:::primary
        N3[Simple operations]:::primary
        N4[Schema flexibility]:::primary
    end

    subgraph "SQL"
        S1[Strong consistency]:::green
        S2[Rich queries]:::green
        S3[ACID transactions]:::green
        S4[Familiar tooling]:::green
    end
...
```mermaid
flowchart LR
    subgraph "Problems with One-File-Per-Object"
        P1[Small objects waste space<br/>100-byte file uses 4KB block]:::red
        P2[Billions of files<br/>File system metadata explodes]:::red
        P3[Large files are unwieldy<br/>5TB file hard to replicate]:::red
    end
...
```mermaid
flowchart TD
    subgraph "1 GB Video File"
        O[video.mp4]:::rose
    end

    subgraph "16 x 64MB Chunks"
        C1[Chunk 0]:::primary
        C2[Chunk 1]:::primary
        C3[Chunk 2]:::primary
        C4[...]:::secondary
...
```mermaid
flowchart LR
    subgraph "Data Blob (256 MB)"

        A["Object A<br/>100 bytes<br/>offset: 0"]:::primary
        B["Object B<br/>50 KB<br/>offset: 100"]:::primary
        C["Object C<br/>1 MB<br/>offset: 51,300"]:::primary
        D["...hundreds more..."]:::secondary
    end
...
```mermaid
flowchart TB
    subgraph "Chunk abc123"
        C[64 MB of data]:::orange
    end

    subgraph "Replicas"
        R1[Node 7<br/>AZ-1, Rack-2]:::green
        R2[Node 23<br/>AZ-2, Rack-5]:::green
        R3[Node 41<br/>AZ-3, Rack-8]:::green
    end
...
```mermaid
flowchart TB
    D[Data Chunk]:::orange --> N1[Node in AZ-1]:::green
    D --> N2[Node in AZ-2]:::green
    D --> N3[Node in AZ-3]:::green
...
```mermaid
flowchart TB
    subgraph "Original Data (640 MB)"
        D[10 data chunks<br/>64 MB each]:::primary
    end

    subgraph "Encoded (896 MB)"
        E1[D1]:::primary
        E2[D2]:::primary
        E3[...]:::secondary
        E4[D10]:::primary
...
flowchart LR
    subgraph "Storage Tiers"
        H[Hot Data<br/>Frequent Access]:::red
        W[Warm Data<br/>Occasional Access]:::orange
        C[Cold Data<br/>Rare Access]:::primary
    end

    subgraph "Strategy"
        H --> H1[3-way replication<br/>Lowest latency]:::red
        W --> W1[2-way + erasure<br/>Balanced]:::orange
        C --> C1[Erasure only<br/>Lowest cost]:::primary
    end
...
sequenceDiagram
    ...
    Client->>DataSvc: Upload object
    par Write to nodes
        DataSvc->>Node1: Write data
        DataSvc->>Node2: Write data
        DataSvc->>Node3: Write data
    end
    Node1-->>DataSvc: OK
    Node2-->>DataSvc: OK
    Note over DataSvc: Quorum (2/3) reached
    DataSvc->>MetaSvc: Record metadata
    MetaSvc->>DB: Insert record
    DB-->>MetaSvc: Committed
    MetaSvc-->>DataSvc: OK
    DataSvc-->>Client: Success
```
...
flowchart TB
    subgraph "Write Quorum (W=2)"
        W1[Node 1: OK]:::green
        W2[Node 2: OK]:::green
        W3[Node 3: Pending]:::orange
    end

    subgraph "Read Quorum (R=2)"
        R1[Node 1: Has data]:::green
        R2[Node 3: Has data]:::green
    end

    W1 --> Q[Quorum Overlap<br/>Guaranteed]:::purple
    R1 --> Q
...
```mermaid
flowchart LR
    I[1. Initiate<br/>Get upload ID]:::primary --> U[2. Upload Parts<br/>In parallel]:::orange --> C[3. Complete<br/>Assemble object]:::green
...
flowchart TB
    subgraph "5 GB File"
        F[large-video.mp4]:::rose
    end

    subgraph "Parts (100 MB each)"
        P1[Part 1<br/>0-100 MB]:::primary
        P2[Part 2<br/>100-200 MB]:::primary
        P3[Part 3<br/>200-300 MB]:::primary
        P4[...]:::secondary
        P5[Part 50<br/>4.9-5 GB]:::primary
    end
...
```mermaid
flowchart LR
    C[Client]:::rose --> G[Gateway]:::secondary
    G --> D[Data Service]:::purple
    D --> N[Storage Node]:::green
    N --> K[Disk]:::orange
...
```mermaid
flowchart TD
    S[Scrubber Service]:::orange --> R[Read chunk from disk]:::secondary
    R --> C[Recalculate checksum]:::secondary
    C --> V{Matches stored<br/>checksum"}:::purple
    V -->|Yes| M[Mark as verified<br/>Update timestamp]:::green
    V -->|No| T[Trigger repair<br/>from replica]:::red
...
```mermaid
flowchart LR
    A[User deletes<br/>object]:::rose --> B[Metadata marked<br/>as deleted]:::orange
    B --> C[Grace period<br/>24-48 hours]:::secondary
    C --> D[GC finds<br/>orphaned data]:::purple
    D --> E[Physical<br/>deletion]:::red
...
```mermaid
flowchart TD
    subgraph "Object History"
        V1[Version 1<br/>Original]:::primary
        V2[Version 2<br/>Updated]:::primary
        DM[Delete Marker<br/>Latest]:::red
    end
...
flowchart LR
    subgraph "Hot"
        S[Standard<br/>Milliseconds<br/>$$$]:::red
    end

    subgraph "Warm"
        IA[Infrequent Access<br/>Milliseconds<br/>$$]:::orange
    end

    subgraph "Cold"
        A[Archive<br/>Hours<br/>$]:::primary
    end
...
```mermaid
flowchart LR
    U[Upload]:::green --> S[Standard<br/>Day 0-30]:::red
    S --> IA[Infrequent Access<br/>Day 31-90]:::orange
    IA --> A[Archive<br/>Day 91-365]:::primary
    A --> D[Delete<br/>Day 366+]:::secondary
