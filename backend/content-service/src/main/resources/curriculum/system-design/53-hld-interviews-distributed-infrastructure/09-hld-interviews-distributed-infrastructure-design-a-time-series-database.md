---
id: "hld-interviews-distributed-infrastructure-design-a-time-series-database"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design a Time-Series Database"
slug: "hld-interviews-distributed-infrastructure-design-a-time-series-database"
summary: "What makes time-series data special\" Three characteristics shape everything about how we design for it:"
eli10: "Imagine Design a Time-Series Database as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design a Time-Series Database Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Data Points"
        direction LR
        A[Timestamp<br/>8 bytes]:::primary
        B[Metric Name<br/>~20 bytes]:::secondary
        C[Value<br/>8 bytes]:::orange
        D[Tags<br/>~64 bytes]:::purple
    end
...
flowchart TB
    ...
    subgraph "Read Path"
        direction LR
        R1[Dashboards]:::rose --> R2[Query Layer]:::primary
        R2 --> R3[(Partitions)]:::purple
    end
	
    subgraph "Write Path"
        direction LR
        W1[Collectors]:::rose --> W2[Ingest Layer]:::primary
        W2 --> W3[Storage Engine]:::purple
    end	
...
```mermaid
flowchart TB
    U[Metric Collectors]:::rose

    subgraph Gateway["Gateway Layer"]
        AG[Write API Gateway]:::secondary
    end

    subgraph Ingest["Ingest Layer"]
        I1[Ingest Node 1]:::primary
        I2[Ingest Node 2]:::primary
        I3[Ingest Node 3]:::primary
    end

    subgraph Durability["Durability Layer"]
        WAL[Write-Ahead Log]:::orange
    end

    subgraph Storage["Storage Layer"]
        SE[(Storage Engine)]:::purple
    end
...
sequenceDiagram
    ...
    Client->>Gateway: POST /write (batch of 1000 points)
    Note over Gateway: Validate, route to<br/>available ingest node
    Gateway->>Ingest: Forward batch
    Ingest->>WAL: Append to WAL
    WAL-->>Ingest: Confirmed durable
    Ingest-->>Gateway: Acknowledge
    Gateway-->>Client: 200 OK (points_written: 1000)
    Note over Ingest: Buffer accumulates<br/>more points...
    Ingest->>Storage: Flush buffer (every 2s or 64MB)
    Storage-->>Ingest: Confirmed
    Ingest->>WAL: Mark entries as persisted
```
...
```mermaid
flowchart TD
    subgraph "Query: 2pm to 3pm"
        Q[Time Range Query]:::rose
    end

    subgraph "Storage Partitions"
        P1[12:00-14:00]:::secondary
        P2[14:00-16:00]:::green
        P3[16:00-18:00]:::secondary
        P4[18:00-20:00]:::secondary
    end
...
```mermaid
flowchart TD
    I[Ingest Node]:::primary --> MB[Memory Buffer<br/>MemTable]:::orange

    MB --> |Flush| P1[Partition: 00:00-02:00<br/>Immutable file]:::primary
    MB --> |Flush| P2[Partition: 02:00-04:00<br/>Immutable file]:::primary
    MB --> |Flush| P3[Partition: 04:00-06:00<br/>Immutable file]:::primary

    subgraph Background["Background Processes"]
        C[Compactor<br/>Merge small files]:::secondary
        DS[Downsampler<br/>Create aggregates]:::secondary
    end

    P1 --> C
    P2 --> C
    C --> LP[(Large Partition Files<br/>Optimized for queries)]:::purple

    LP --> DS
    DS --> AGG[(Aggregated Data<br/>1-min, 1-hr averages)]:::purple

    M[(Metadata Store)]:::purple
...
```mermaid
flowchart TB
    U[Dashboard / User]:::rose

    subgraph Gateway["Gateway Layer"]
        QG[Query API Gateway]:::secondary
    end

    subgraph Cache["Cache Layer"]
        QC[Query Cache]:::orange
    end

    subgraph Query["Query Layer"]
        Q1[Query Node 1]:::primary
        Q2[Query Node 2]:::primary
    end

    subgraph Storage["Storage Layer"]
        P1[(Partition 1)]:::purple
        P2[(Partition 2)]:::purple
        P3[(Partition 3)]:::purple
        M[(Metadata)]:::purple
    end
...
sequenceDiagram
    participant Client
    participant Gateway as Query Gateway
    participant Cache as Query Cache
    participant QN1 as Query Node 1
    participant QN2 as Query Node 2
    participant Meta as Metadata Store
    participant Storage as Partition Files

    Client->>Gateway: POST /query (cpu.usage, 2pm-3pm, host=server-1)
    Gateway->>Cache: Check cache
    Cache-->>Gateway: Miss
    Gateway->>Meta: Which partitions cover 2pm-3pm"
    Meta-->>Gateway: Partitions P5, P6
    Gateway->>QN1: Scan P5 for cpu.usage, host=server-1
    Gateway->>QN2: Scan P6 for cpu.usage, host=server-1
    QN1->>Storage: Read P5, filter, aggregate
    QN2->>Storage: Read P6, filter, aggregate
    Storage-->>QN1: Partial result (60 points)
    Storage-->>QN2: Partial result (55 points)
    QN1-->>Gateway: Aggregated data for P5
    QN2-->>Gateway: Aggregated data for P6
    Gateway->>Gateway: Merge results
    Gateway->>Cache: Store result (TTL=30s)
    Gateway-->>Client: Final response (12 aggregated points)
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        direction LR
        C1[Metrics Collectors<br/>Telegraf, StatsD]:::rose
        C2[Applications<br/>Custom agents]:::rose
        C3[Query Dashboards<br/>Grafana, Custom]:::rose
    end

    subgraph WritePath["Write Path"]
        WG[Write API Gateway]:::secondary
        I1[Ingest Node]:::primary
        I2[Ingest Node]:::primary
        WAL[Write-Ahead Log]:::orange
    end
    ...
    subgraph ReadPath["Read Path"]
        QG[Query API Gateway]:::secondary
        QC[Query Cache]:::orange
        Q1[Query Node]:::primary
        Q2[Query Node]:::primary
    end

    subgraph Storage["Storage Layer"]
        M[(Metadata Store)]:::purple
        HOT[(Hot Partitions<br/>Last 24 hours)]:::primary
        WARM[(Warm Partitions<br/>Last 30 days)]:::secondary
        COLD[(Cold Storage<br/>S3 / GCS)]:::purple
    end

    subgraph Background["Background Workers"]
        COMP[Compactor]:::lightblue
        DS[Downsampler]:::lightblue
        RET[Retention Manager]:::lightblue
    end
...
flowchart LR
    subgraph "Column-Oriented"
        C1[Timestamps: t1, t2, t3, ...]:::primary
        C2[Values: v1, v2, v3, ...]:::primary
        C3[Tags: tag1, tag2, tag3, ...]:::primary
    end

    subgraph "Row-Oriented"
        R1[Row 1: ts, metric, value, tags]:::secondary
        R2[Row 2: ts, metric, value, tags]:::secondary
        R3[Row 3: ts, metric, value, tags]:::secondary
    end
...
```mermaid
flowchart TD
    subgraph "Metric: cpu.usage"
        S1[Series 1<br/>host=server-1]:::primary
        S2[Series 2<br/>host=server-2]:::primary
        S3[Series 3<br/>host=server-3]:::primary
    end

    S1 --> P1["(t1, 45.2), (t2, 46.1), (t3, 44.8), ..."]:::orange
    S2 --> P2["(t1, 72.3), (t2, 71.9), (t3, 73.1), ..."]:::orange
    S3 --> P3["(t1, 33.5), (t2, 34.2), (t3, 33.8), ..."]:::orange
...
flowchart TD
    subgraph "Parquet File Structure"
        direction TB
        H[File Header]:::secondary
        RG1[Row Group 1<br/>~128 MB of data]:::primary
        RG2[Row Group 2<br/>~128 MB of data]:::primary
        F[Footer<br/>Schema + Stats]:::secondary
    end

    subgraph "Row Group Contents"
        direction LR
        TS[Timestamps<br/>Compressed]:::orange
        VAL[Values<br/>Compressed]:::orange
        SID[Series IDs<br/>Compressed]:::orange
    end
...
```mermaid
flowchart TD
    subgraph "Fast Path (In-Memory)"
        W[Write Request]:::rose --> WAL[1. Append to WAL]:::orange
        WAL --> MT[2. Insert into MemTable]:::primary
        MT --> ACK[3. Acknowledge to Client]:::green
    end

    subgraph "Background Path (Async)"
        MT --> |When buffer full| FL[4. Flush to Disk]:::secondary
        FL --> COMP[5. Compaction]:::purple
    end
...
flowchart TD
    subgraph "Raw Data"
        R1[Timestamp: 1702900800000]:::primary
        R2[Value: 45.23]:::primary
        R3[Tags: host=server-1, region=us-east]:::primary
    end

    subgraph "Compressed (8-12 bytes/point)"
        C1[Delta: +1000]:::green
        C2[XOR: 3 bits changed]:::green
        C3[Dictionary ID: 42]:::green
    end

    R1 --> |Delta encoding| C1
    R2 --> |XOR encoding| C2
    R3 --> |Dictionary| C3
...
```mermaid
flowchart LR
    R[Raw Data<br/>100 bytes/point]:::rose --> T[1. Delta-of-Delta<br/>Timestamps]:::orange
    T --> V[2. XOR Encoding<br/>Values]:::orange
    V --> D[3. Dictionary<br/>Tags]:::orange
    D --> B[4. Block Compression<br/>LZ4]:::secondary
    B --> S[Compressed Data<br/>8-12 bytes/point]:::green
...
```mermaid
flowchart LR
    RAW[Raw Data<br/>1-second resolution]:::primary
    AGG1[1-Minute Aggregates<br/>avg, min, max, count]:::orange
    AGG2[1-Hour Aggregates]:::orange
    AGG3[1-Day Aggregates]:::orange

    RAW --> |Computed hourly| AGG1
    AGG1 --> |Computed daily| AGG2
    AGG2 --> |Computed weekly| AGG3
...
```mermaid
flowchart TD
    Q[Query arrives]:::rose --> PP[1. Time Range<br/>Prune partitions]:::orange
    PP --> TI[2. Tag Filter<br/>Use inverted index]:::orange
    TI --> PA[3. Check for<br/>Pre-aggregates]:::orange
    PA --> |Pre-agg exists| READ1[Read pre-computed data]:::green
    PA --> |No pre-agg| READ2[Read raw + aggregate on-the-fly]:::secondary
    READ1 --> MERGE[4. Merge results<br/>from all nodes]:::primary
    READ2 --> MERGE
    MERGE --> RESULT[Return to client]:::green
...
flowchart LR
    subgraph "Hot Tier (SSD)"
        HOT[Raw Data<br/>1-second resolution<br/>Last 7 days]:::primary
    end

    subgraph "Warm Tier (HDD)"
        WARM[1-minute aggregates<br/>Last 30 days]:::secondary
    end

    subgraph "Cold Tier (Object Storage)"
        COLD[1-hour aggregates<br/>Last 1 year]:::purple
    end

    subgraph "Archive Tier (Glacier)"
        ARCH[1-day aggregates<br/>3+ years]:::purple
    end

    HOT --> |After 7 days| WARM
    WARM --> |After 30 days| COLD
    COLD --> |After 1 year| ARCH
...
```mermaid
flowchart TB
    C[Client Write]:::rose --> COORD[Coordinator Node]:::secondary

    subgraph "Replica Set (N=3)"
        R1[Replica 1]:::primary
        R2[Replica 2]:::primary
        R3[Replica 3]:::primary
    end
...
```mermaid
flowchart TD
    subgraph P1["Partition 1 (metrics A–M)"]
        P1R1["Node 1"]:::primary
        P1R2["Node 2"]:::primary
        P1R3["Node 3"]:::primary
    end

    subgraph P2["Partition 2 (metrics N–Z)"]
        P2R1["Node 4"]:::secondary
        P2R2["Node 5"]:::secondary
        P2R3["Node 6"]:::secondary
    end

    W["Incoming Writes"]:::rose -->|"hash(metric)"| P1R1
    W -->|"hash(metric)"| P2R1
...
sequenceDiagram
    ...
    L->>F1: Heartbeat
    L->>F2: Heartbeat
    Note over L: Node crashes
    Note over F1,F2: Timeout (150ms)
    F1->>F2: RequestVote
    F2->>F1: VoteGranted
