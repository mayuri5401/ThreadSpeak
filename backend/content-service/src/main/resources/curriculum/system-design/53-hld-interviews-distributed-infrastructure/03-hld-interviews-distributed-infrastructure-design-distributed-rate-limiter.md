---
id: "hld-interviews-distributed-infrastructure-design-distributed-rate-limiter"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design Distributed Rate Limiter"
slug: "hld-interviews-distributed-infrastructure-design-distributed-rate-limiter"
summary: "In this article, we will dive into the system design of a distributed rate limiter, and explore the the 5 most commonly used rate limiting algorithms with examples, pros and cons."
eli10: "Imagine Design Distributed Rate Limiter as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Distributed Rate Limiter Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Distributed Infrastructure","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Design Distributed Rate Limiter"
  code: |
    ratelimit:{client_id}:{resource}:{window}
---
...
sequenceDiagram
    ...
    Gateway->>RLS: Check rate limit (client_id, resource)
    RLS->>RLS: Look up rules from cache
    RLS->>Redis: GET/INCR counter
    Redis-->>RLS: Current count
    RLS->>RLS: Compare count vs limit
    RLS-->>Gateway: {allowed: true/false, remaining: N}
...
flowchart LR
    ...
    subgraph Rate_Limiters[Rate Limiter Service]
        RL1[Rate Limiter 1]
        RL2[Rate Limiter 2]
    end
    ...
    class C1,C2 primary
    class GW1,GW2,GW3 secondary
    class RL1,RL2 orange
...
sequenceDiagram
    ...
    Admin->>RulesService: PUT /rules (new rate limit)
    RulesService->>RulesService: Validate rule
    RulesService->>DB: Save rule
    RulesService->>RLS: Invalidate cache / Push update
    RLS->>RLS: Refresh rules cache
    RulesService-->>Admin: 200 OK
```
...
flowchart TB
    ...
    class C,GW1,GW2 primary
    class LB,RL1,RL2 secondary
    class Redis,RulesDB purple
    class RS,Admin orange
    class API,RC green
```
...
Time 0s:  Bucket = 10 tokens
          10 requests arrive → All allowed, bucket = 0
Time 1s:  1 token refilled, bucket = 1
          2 requests arrive → 1 allowed, 1 rejected
Time 5s:  4 more tokens refilled, bucket = 5 (1 remaining + 4 refilled)
          5 requests arrive → All allowed
```

#### Implementation (Redis)

```shell
KEYS: ratelimit:{client}:{resource}
FIELDS: tokens (float), last_refill (timestamp)

On each request:
1. Calculate tokens to add based on time elapsed since last_refill
2. Set tokens = min(capacity, tokens + refilled_tokens)
3. If tokens >= 1: decrement tokens, allow request
...
```shell
Time 0s:  10 requests arrive
          5 added to queue, 5 rejected (queue full)
Time 0.5s: 1 request processed, queue = 4
Time 1s:  2 requests processed, queue = 2
          3 new requests arrive, queue = 5
```
...
```shell
KEY: ratelimit:{client}:{resource}:{window_start}
...
```shell
- 100 requests at 12:59:30 → All allowed (window 12:00-12:59)
- 100 requests at 13:00:30 → All allowed (window 13:00-13:59)
- Total: 200 requests in 1 minute (12:59:30 to 13:00:30)
```
...
```shell
Time 10s: Request arrives, log = [10], count = 1, allowed
Time 12s: Request arrives, log = [10, 12], count = 2, allowed
Time 15s: Request arrives, log = [10, 12, 15], count = 3, allowed
Time 18s: Request arrives, log = [10, 12, 15, 18], count = 4, allowed
Time 19s: Request arrives, log = [10, 12, 15, 18, 19], count = 5, allowed
Time 20s: Request arrives, log = [12, 15, 18, 19] (10s removed), count = 4, allowed
```

#### Implementation (Redis)

```shell
KEY: ratelimit:{client}:{resource}
TYPE: Sorted Set (ZSET)
SCORE: Request timestamp
VALUE: Unique request ID

ZREMRANGEBYSCORE key 0 {current_time - window}  # Remove old entries
ZCARD key                                        # Count remaining
If count < limit:
    ZADD key {current_time} {request_id}        # Add new entry
    Allow
Else:
    Reject
```
...
Where overlap_percentage = 1 - (time_into_current_window / window_size)
```
...
```shell
Previous window (12:00-12:59): 80 requests
Current window (13:00-13:59): 30 requests
Current time: 13:15 (25% into current window)

Overlap = 1 - 0.25 = 0.75 (75% of previous window still in sliding window)
Weighted count = (80 × 0.75) + 30 = 60 + 30 = 90

Since 90 < 100: Allow request
```
...
```shell
KEYS:
  ratelimit:{client}:{resource}:{prev_window}
  ratelimit:{client}:{resource}:{curr_window}

1. Get counts for both windows
2. Calculate overlap percentage
3. Compute weighted count
4. If weighted_count < limit:
...
if weighted < tonumber(ARGV[1]) then
    ...
    return {1, weighted + 1}  -- allowed, new count
else
    return {0, weighted}  -- rejected, current count
end
```
...
flowchart TB
    Client[Client]
    CDN[CDN/Edge<br/>Coarse IP-based limits]
    Gateway[API Gateway<br/>API key/User limits]
    App[Application<br/>Business-specific limits]
...
flowchart TB
    Request[Request]
    Primary{Redis Available"}
    Fallback{Local Cache<br/>Available"}
    Allow[Allow Request]

    Request --> Primary
    Primary -->|Yes| Redis[Redis Rate Limit]
    Primary -->|No| Fallback
    Fallback -->|Yes| Local[Local Rate Limit]
    Fallback -->|No| Allow

    Redis --> Decision1{Allowed"}
    Local --> Decision2{Allowed"}

    Decision1 -->|Yes| Forward[Forward to Backend]
    Decision1 -->|No| Reject[429 Reject]
    Decision2 -->|Yes| Forward
    Decision2 -->|No| Reject
    Allow --> Forward
    ...
    class Request,Forward primary
    class Allow,Redis,Local green
    class Reject red
    class Primary,Fallback,Decision1,Decision2 orange
```
