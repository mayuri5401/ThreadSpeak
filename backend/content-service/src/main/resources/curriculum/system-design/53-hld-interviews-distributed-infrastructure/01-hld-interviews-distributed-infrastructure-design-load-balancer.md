---
id: "hld-interviews-distributed-infrastructure-design-load-balancer"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design Load Balancer"
slug: "hld-interviews-distributed-infrastructure-design-load-balancer"
summary: "Load balancers appear in almost every distributed system architecture. Understanding how to design one from scratch demonstrates deep knowledge of networking, high availability, and system scalability."
eli10: "Imagine Design Load Balancer as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Load Balancer Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Traffic Profile"
        direction TB
        Peak["Peak Traffic<br/>1M RPS"]:::red
        Avg["Average Traffic<br/>~300K RPS"]:::orange
        Conn["Concurrent Connections<br/>~500K"]:::primary
    end
...
flowchart LR
    ...
    subgraph "Data Plane"
        E[Traffic Forwarding<br/>No explicit API]:::secondary
    end
...
flowchart TB
    ...
    subgraph "Backend Servers"
        B1[Backend 1<br/>Healthy ✓]:::green
        B2[Backend 2<br/>Unhealthy ✗]:::red
        B3[Backend 3<br/>Healthy ✓]:::green
    end

    subgraph "Routing"
        RE[Routing Engine]:::secondary
        BP[Backend Pool<br/>Active: B1, B3]:::purple
    end
...
flowchart LR
    ...
    subgraph "After Primary Fails"
        direction LR
        C2[Clients]:::rose --> VIP2[Virtual IP<br/>192.168.1.100]:::primary
        LB2A["LB Primary<br/>(Failed)"]:::red
        VIP2 --> LB2B["LB Standby<br/>(Now Active)"]:::green
        LB2B --> BP2[Backends]:::purple
    end

    subgraph "Normal Operation"
        direction LR
        C1[Clients]:::rose --> VIP1[Virtual IP<br/>192.168.1.100]:::primary
        VIP1 --> LB1A["LB Primary<br/>(Handling traffic)"]:::green
        LB1A --> BP1[Backends]:::purple
        LB1B["LB Standby<br/>(Idle)"]:::orange
        LB1A <-.->|"Heartbeat"| LB1B
    end
...
flowchart TB
    ...
    subgraph "After Node 1 Fails"
        direction TB
        DNS2[DNS updated or<br/>health check removes]:::secondary
        DNS2 -.->|"Removed"| LB1F[LB Node 1<br/>Failed]:::red
        DNS2 --> LB2F[LB Node 2<br/>Handling 100% traffic]:::green
        LB2F --> BP2[Backend Pool]:::purple
    end

    subgraph "Normal Operation"
        direction TB
        DNS[DNS resolves to<br/>multiple IPs]:::secondary
        DNS --> LB1[LB Node 1<br/>Handling 50% traffic]:::green
        DNS --> LB2[LB Node 2<br/>Handling 50% traffic]:::green
        LB1 --> BP[Backend Pool]:::purple
        LB2 --> BP
    end
...
flowchart TB
    ...
    subgraph LBCluster["Load Balancer Cluster"]
        VIP[Virtual IP / DNS<br/>Entry Point]:::primary
        LB1[LB Node 1]:::secondary
        LB2[LB Node 2]:::secondary
        SS[(Session Store<br/>Redis)]:::purple
    end

    subgraph ControlPlane["Control Plane"]
        HC[Health Checker]:::orange
        CM[Config Manager]:::orange
        CS[(Config Store)]:::purple
    end

    subgraph BackendPool["Backend Pool"]
        B1[Backend 1<br/>Healthy]:::green
        B2[Backend 2<br/>Healthy]:::green
        B3[Backend 3<br/>Unhealthy]:::red
    end
...
flowchart LR
    ...
    subgraph "Persistent Store (etcd/Consul)"
        E[Backend Configuration<br/>addresses, weights]:::purple
        F[Pool Configuration<br/>algorithms, settings]:::purple
    end

    subgraph "Shared Cache (Redis)"
        D[Session Mappings<br/>for sticky sessions]:::orange
    end

    subgraph "In-Memory (Per LB Node)"
        A[Active Connections<br/>500K entries]:::primary
        B[Backend Health Status<br/>~1000 entries]:::primary
        C[Connection Counters<br/>for least-conn algo]:::primary
    end
...
flowchart TB
    ...
    LB[Load Balancer<br/>counter++]:::primary

    subgraph "Backends"
        B1["Backend 1<br/>Gets: 1, 4"]:::orange
        B2["Backend 2<br/>Gets: 2, 5"]:::green
        B3["Backend 3<br/>Gets: 3, 6"]:::purple
    end
...
flowchart TB
    ...
    subgraph WR["Weighted Distribution"]
        B1["Backend 1<br/>weight: 3 → 50%"]:::orange
        B2["Backend 2<br/>weight: 1 → 17%"]:::green
        B3["Backend 3<br/>weight: 2 → 33%"]:::purple
    end
...
flowchart LR
    subgraph "Clients"
        C1["Client A<br/>192.168.1.10"]:::rose
        C2["Client B<br/>192.168.1.20"]:::rose
        C3["Client C<br/>192.168.1.30"]:::rose
    end

    LB["Load Balancer<br/>hash(IP) % 3"]:::primary
...
flowchart LR
    subgraph "Conceptual Ring"
        direction TB
        RING["Hash Ring<br/>0 → 2^32"]:::secondary
    end

    subgraph "Backends on Ring"
        BA["Backend A<br/>position: 1000"]:::orange
        BB["Backend B<br/>position: 3000"]:::green
        BC["Backend C<br/>position: 6000"]:::purple
    end

    subgraph "Request Mapping"
        R1["Request (hash=500)<br/>→ Backend A"]:::rose
        R2["Request (hash=2500)<br/>→ Backend B"]:::rose
        R3["Request (hash=5000)<br/>→ Backend C"]:::rose
    end
...
flowchart LR
    ...
    subgraph "Phase 2: Draining"
        DR[Stop New Traffic<br/>Allow Existing to Complete]:::orange
    end

    subgraph "Phase 3: Removed"
        RM[Fully Removed<br/>from Pool]:::secondary
    end

    subgraph "Phase 4: Recovery"
        REC[Health Checks Pass<br/>Backend Returns]:::green
    end
...
flowchart LR
    ...
    B1 & B2 & B3 --> SS[(Redis<br/>Session Store)]:::purple
...
```mermaid
flowchart TD
    C[Client<br/>192.168.1.100]:::rose --> L4[L4 Load Balancer]:::primary
    L4 --> B1[Backend 1]:::green
    L4 --> B2[Backend 2]:::green

    L4 -.- INFO["Sees only:<br/>• IP addresses<br/>• Port numbers<br/>• TCP/UDP"]:::secondary
...
flowchart TD
    ...
    L7 -->|"/admin/*"| Admin[Admin Servers]:::purple

    L7 -.- INFO["Sees everything:<br/>• URL, headers, cookies<br/>• HTTP method<br/>• Request body"]:::secondary
...
```mermaid
flowchart TD
    C[Client]:::rose -->|"HTTPS<br/>(encrypted)"| LB[Load Balancer<br/>SSL Termination]:::primary
    LB -->|"HTTP<br/>(plain)"| B1[Backend 1]:::green
    LB -->|"HTTP<br/>(plain)"| B2[Backend 2]:::green

    LB -.- CERT[Certificate stored<br/>only on LB]:::orange
...
flowchart LR
    subgraph "After Active Fails"
        ...
        LB2A["Former Active<br/>(failed)"]:::red
        VIP2 --> LB2B["Standby<br/>(claimed VIP)"]:::green
    end

    subgraph "Normal Operation"
        direction LR
        C1[Clients]:::rose --> VIP1[VIP: 10.0.0.1]:::primary
        VIP1 --> LB1A["Active LB<br/>(owns VIP)"]:::green
...
flowchart TB
    subgraph "Global Anycast"
        ...
        LB_AS[LB in Asia<br/>FAILED]:::red
