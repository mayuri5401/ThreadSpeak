---
id: "hld-interviews-distributed-infrastructure-design-api-gateway"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design API Gateway"
slug: "hld-interviews-distributed-infrastructure-design-api-gateway"
summary: "API gateways are fundamental to modern microservices architectures. Understanding how to design one demonstrates knowledge of distributed systems, security, performance optimization, and service orchestration."
eli10: "Imagine Design API Gateway as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design API Gateway Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    C[Client]:::rose --> GW[Gateway Core]:::primary
    GW <--> RR[Route Registry]:::purple
    GW <--> SD[Service Discovery]:::orange
    GW --> B1[user-service<br/>instance 1]:::green
    GW --> B2[user-service<br/>instance 2]:::green
    GW --> B3[user-service<br/>instance 3]:::green
...
sequenceDiagram
    ...
    C->>GW: GET /api/v1/users/123
    GW->>RR: Match path pattern
    RR-->>GW: Route: user-service
    GW->>SD: Get healthy instances
    SD-->>GW: [instance-1, instance-2, instance-3]
    GW->>US: GET /users/123
...
```mermaid
flowchart TD
    C[Client + Credentials]:::rose --> GW[Gateway Core]:::primary
    GW --> AH[Auth Handler]:::orange

    subgraph "Validation"
        AH --> JV{JWT"}:::secondary
        AH --> AK{API Key"}:::secondary
        AH --> OA{OAuth"}:::secondary
        JV --> |"Verify signature"| V1[Local Verification]:::green
        AK --> |"Lookup key"| CR[(Client Registry)]:::purple
        OA --> |"Introspect"| AS[Auth Server]:::purple
    end

    V1 & CR & AS --> PE[Policy Engine]:::orange
    PE --> |"Authorized"| BE[Backend Service]:::green
    PE --> |"Denied"| ERR[401/403 Error]:::red
...
```mermaid
flowchart TD
    C[Client Request]:::rose --> GW[Gateway Core]:::primary
    GW --> RL[Rate Limiter]:::orange
    RL --> |"Check limits"| RS[(Redis<br/>Rate State)]:::purple
    RS --> RL

    RL --> |"Over limit"| R429[429 Too Many<br/>Requests]:::red
    RL --> |"Under limit"| CACHE{Check<br/>Cache}:::secondary

    CACHE --> |"Hit"| RESP[Return Cached<br/>Response]:::green
    CACHE --> |"Miss"| TX[Transformer]:::orange
    TX --> BE[Backend Service]:::green
    BE --> CACHE_STORE[Store in Cache]:::orange
    CACHE_STORE --> RESP
...
stateDiagram-v2
    ...
    Open --> HalfOpen: After timeout (30s)
...
flowchart LR
    ...
    subgraph "Half-Open State"
		direction LR
        PROBE["5 probe requests"]:::orange --> BE2["Backend"]:::purple
        BE2 --> CHECK{"4/5 succeed""}:::secondary
        CHECK -->|"Yes"| CLOSED["Circuit CLOSES"]:::green
        CHECK -->|"No"| REOPEN["Circuit REOPENS"]:::red
    end

    subgraph "Open State (30 seconds)"
		direction LR
        REQ["New requests"]:::rose --> FAST["Fail immediately<br/>503 Service Unavailable"]:::red
        FAST -.->|"No backend call"| BE["Backend"]:::purple
    end

    subgraph "Closed State"
	  	direction LR
        R1["100 requests"]:::primary --> F1["60 failures<br/>60% error rate"]:::orange
        F1 --> T1{"Error rate<br/>> 50%""}:::secondary
        T1 -->|"Yes"| OPEN[Circuit OPENS]:::red
    end
...
flowchart TB
    ...
    subgraph Gateway["API Gateway Cluster"]
        GW1[Gateway Node 1]:::primary
        GW2[Gateway Node 2]:::primary
        GW3[Gateway Node N]:::primary
    end

    subgraph Pipeline["Request Pipeline"]
        TLS[TLS Termination]:::orange
        AUTH[Auth Handler]:::orange
        RL[Rate Limiter]:::orange
        ROUTE[Router]:::orange
        CB[Circuit Breaker]:::orange
        TX[Transformer]:::orange
    end

    subgraph State["Shared State"]
        REDIS[(Redis<br/>Cache + Rate Limits)]:::purple
        CONFIG[(Config Store<br/>etcd/Consul)]:::purple
    end

    subgraph Backends["Backend Services"]
        US[User Service]:::green
        OS[Order Service]:::green
    ...
    C1 & C2 & C3 --> LB
    LB --> GW1 & GW2 & GW3
    GW1 & GW2 & GW3 --> TLS --> AUTH --> RL --> ROUTE --> CB --> TX
    TX --> US & OS & PS
    GW1 & GW2 & GW3 <--> REDIS
    GW1 & GW2 & GW3 <--> CONFIG
    GW1 & GW2 & GW3 --> LOG & MET & TRACE
...
```mermaid
flowchart LR
    REQ[Request]:::rose --> TLS[TLS<br/>Termination]:::secondary
    TLS --> AUTH[Auth]:::orange
    AUTH --> RL[Rate<br/>Limit]:::orange
    RL --> ROUTE[Route<br/>Match]:::orange
    ROUTE --> CACHE{Cache<br/>Check}:::secondary

    CACHE -->|"Hit"| RESP[Response]:::green
    CACHE -->|"Miss"| CB[Circuit<br/>Breaker]:::orange
    CB --> TX[Transform]:::orange
    TX --> LB[Load<br/>Balance]:::orange
    LB --> BE[Backend]:::purple
    BE --> RESP
...
flowchart TD
    ...
    WILD1 -.->|"user-service"| US[User Service]:::purple
    WILD2 -.->|"order-service"| OS[Order Service]:::purple
    WILD3 -.->|"product-service"| PS[Product Service]:::purple
...
```mermaid
flowchart TD
    REQ["/api/v1/users/me"]:::rose --> MATCH{Find matching routes}:::secondary

    MATCH --> R1[Exact: /api/v1/users/me<br/>Priority 1]:::green
    MATCH --> R2[Param: /api/v1/users/id<br/>Priority 2]:::orange
    MATCH --> R3[Prefix: /api/v1/users/*<br/>Priority 3]:::yellow
    MATCH --> R4[Regex: /api/v.*/users/.*<br/>Priority 4]:::rose

    R1 --> |"Selected"| WIN[Route to profile service]:::green
...
sequenceDiagram
    ...
    C->>GW: Request + X-API-Key: sk_live_abc123
    GW->>GW: Hash the API key
    GW->>CR: Lookup by hash
    alt Key found and active
        CR-->>GW: Client info (tier, permissions)
        GW->>BE: Forward with X-Client-ID header
        BE-->>GW: Response
        GW-->>C: 200 OK
    else Key not found
        CR-->>GW: Not found
...
flowchart LR
    subgraph JWT["JWT Token Structure"]
        direction TB
        H["Header<br/>{alg: RS256, typ: JWT}"]:::orange
        P["Payload<br/>{sub: user_123,<br/>exp: 1699999999,<br/>roles: [admin, user]}"]:::green
        S["Signature<br/>RS256(header.payload, private_key)"]:::purple
    end
...
sequenceDiagram
    ...
    Note over C,GW: JWT is self-contained - no database lookup
    C->>GW: Request + Authorization: Bearer eyJhbG...
    GW->>GW: Decode JWT (base64)
    GW->>GW: Verify signature (using public key)
    GW->>GW: Check expiration (exp claim)
    alt Valid token
        GW->>GW: Extract claims (user_id, roles)
        GW->>BE: Forward + X-User-ID: user_123
        BE-->>GW: Response
        GW-->>C: 200 OK
...
sequenceDiagram
    ...
    Note over C,AS: Token obtained earlier via OAuth flow
    C->>GW: API request + access_token
    GW->>AS: Validate token (introspection)
    AS->>GW: Token valid + scopes + user_id
    GW->>GW: Check scopes match required permissions
    alt Authorized
...
```mermaid
flowchart TD
    subgraph "Boundary Spike"
        T1["12:00:59<br/>100 requests"]:::orange --> T2["12:01:00<br/>100 requests"]:::orange
        T2 --> T3["200 requests<br/>in 2 seconds!"]:::red
    end
...
flowchart TB
    subgraph "Token Bucket"
        REFILL["Refill Rate<br/>+10 tokens/sec"]:::green
        BUCKET["Bucket<br/>Capacity: 100<br/>Current: 75"]:::primary
    end

    REQ[Incoming Request]:::rose --> CHECK{Tokens > 0"}:::secondary

    REFILL --> BUCKET
    BUCKET --> CHECK
    CHECK --> |"Yes"| ALLOW["Consume 1 token<br/>ALLOW"]:::green
    CHECK --> |"No"| REJECT["REJECT<br/>429 Too Many Requests"]:::red
...
```mermaid
flowchart TB
    IN["Incoming Requests<br/>(Variable rate)"]:::rose --> BUCKET["Request Queue<br/>Capacity: 100<br/>Current: 45"]:::primary
    BUCKET --> OUT["Processed Requests<br/>(Constant 10/sec)"]:::green

    OVERFLOW{Bucket full"}:::secondary
    IN --> OVERFLOW
    OVERFLOW -->|"No"| BUCKET
    OVERFLOW -->|"Yes"| REJECT["REJECT"]:::red
...
```mermaid
flowchart TD
    GW1[Gateway 1]:::primary --> REDIS[(Redis)]:::purple
    GW2[Gateway 2]:::primary --> REDIS
    GW3[Gateway 3]:::primary --> REDIS
...
```mermaid
flowchart TD
    REQ[Request]:::rose --> KEY[Generate Cache Key]:::secondary
    KEY --> LOOKUP[Cache Lookup]:::orange

    LOOKUP -->|"Hit"| FRESH{Still Fresh"}:::secondary
    FRESH -->|"Yes"| RETURN[Return Cached]:::green
    FRESH -->|"No / Stale"| REVAL[Revalidate]:::orange

    LOOKUP -->|"Miss"| BACKEND[Call Backend]:::purple
    REVAL -->|"304 Not Modified"| RETURN
    REVAL -->|"200 New Content"| STORE[Update Cache]:::orange
...
```mermaid
flowchart TB
    subgraph "Closed State (Normal)"
        C_REQ["Requests flow through"]:::primary
        C_MON["Monitor success/failure rate"]:::secondary
        C_REQ --> C_MON
        C_MON --> C_CHECK{"Failure rate<br/>> threshold""}:::secondary
        C_CHECK -->|"No"| C_REQ
        C_CHECK -->|"Yes (e.g., > 50%)"| TRIP["CIRCUIT OPENS"]:::red
    end

    subgraph "Open State (30 seconds)"
        O_REQ["New requests"]:::rose
        O_REQ --> O_FAIL["Fail immediately<br/>503 Service Unavailable"]:::red
        O_FAIL -.->|"No backend call"| O_BE["Backend"]:::purple
        O_TIMER["Timer: 30 seconds"]:::orange
        O_TIMER --> HALF["Move to Half-Open"]:::orange
    end

    subgraph "Half-Open State"
        H_PROBE["Allow 5 test requests"]:::orange
        H_BE["Backend"]:::purple
        H_PROBE --> H_BE
        H_BE --> H_CHECK{"4/5 succeed""}:::secondary
        H_CHECK -->|"Yes"| CLOSE["CIRCUIT CLOSES"]:::green
        H_CHECK -->|"No"| REOPEN["CIRCUIT REOPENS"]:::red
    end
...
flowchart LR
    subgraph "Client"
        C[REST Client]:::rose
        JSON1[JSON Request]:::orange
    end

    subgraph "Gateway"
        PARSE[Parse JSON]:::secondary
        CONV1[Convert to Protobuf]:::secondary
        CONV2[Convert to JSON]:::secondary
    end

    subgraph "Backend"
        GRPC[gRPC Service]:::purple
        PROTO[Protobuf Response]:::green
    end

    C --> JSON1 --> PARSE --> CONV1 -->|"gRPC call"| GRPC
    GRPC --> PROTO --> CONV2 -->|"JSON Response"| C
...
flowchart LR
    subgraph "Backend Response"
        BR["user_id: 123<br/>email: user@example.com<br/>password_hash: $2b$12...<br/>ssn: 123-45-6789"]:::orange
    end

    subgraph "Gateway Filter"
        FILTER["Remove:<br/>• password_hash<br/>• ssn"]:::secondary
    end

    subgraph "Client Response"
        CR["user_id: 123<br/>email: user@example.com"]:::green
    end
...
```mermaid
flowchart TB
    C["Client: GET /dashboard"]:::rose --> GW["API Gateway"]:::primary

    GW -->|"Parallel"| US["User Service"]:::orange
    GW -->|"Parallel"| OS["Order Service"]:::orange
    GW -->|"Parallel"| NS["Notification Service"]:::orange

    US -->|"profile"| AGG["Aggregator"]:::secondary
    OS -->|"recent_orders"| AGG
    NS -->|"unread_count"| AGG

    AGG --> RESP["{profile, recent_orders, unread_count}"]:::green
    RESP --> C
...
```mermaid
flowchart TD
    subgraph "Trace: req_abc123"
        GW["Gateway<br/>15ms total"]:::primary
        AUTH["Auth<br/>2ms"]:::orange
        RL["Rate Limit<br/>1ms"]:::orange
        BE["user-service<br/>40ms"]:::green
        DB["database<br/>35ms"]:::purple
    end
...
```mermaid
flowchart TB
    DNS[DNS / Global Load Balancer]:::primary --> R1["Region 1"]
    DNS --> R2["Region 2"]
