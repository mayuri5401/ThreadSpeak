---
id: "hld-interviews-basic-questions-design-pastebin"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Basic Questions"
subSection: ""
title: "Design Pastebin"
slug: "hld-interviews-basic-questions-design-pastebin"
summary: "What makes Pastebin interesting from a system design perspective is the asymmetry between writes and reads. Most users create a few pastes but share them with many people."
eli10: "Imagine Design Pastebin as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Pastebin Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Basic Questions","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Design Pastebin"
  code: |
    Average write QPS = 1,000,000 pastes / 86,400 seconds ≈ 12 QPS
---
...
```shell
Write bandwidth = 12 QPS × 10.5 KB = 126 KB/s (average)
Read bandwidth = 115 QPS × 10.5 KB = 1.2 MB/s (average)
Peak read bandwidth = 350 QPS × 10.5 KB = 3.6 MB/s
```
...
flowchart LR
    subgraph "Core API Operations"
        POST[POST /pastes<br/>Create new paste]:::green
        GET[GET /pastes/key<br/>Retrieve paste]:::primary
        DELETE[DELETE /pastes/key<br/>Remove paste]:::red
    end

    POST --> |Returns| URL[Unique URL]:::orange
    GET --> |Returns| Content[Paste Content]:::orange
    DELETE --> |Returns| Confirm[Confirmation]:::orange
...
{
  "paste_key": "abc12345",
  "paste_url": "https://pastebin.com/abc12345",
...
[Embed: https://link.excalidraw.com/readonly/kEBcjac7LHwkLmY3FfCL](https://link.excalidraw.com/readonly/kEBcjac7LHwkLmY3FfCL)
...
sequenceDiagram
    ...
    Client->>API: POST /pastes (content, metadata)
    Note over API: Validate request,<br/>check rate limits
    API->>PS: Forward validated request
    PS->>KGS: Request unique key
    KGS-->>PS: Return key (abc12345)
    PS->>S3: Store content at key path
    S3-->>PS: Confirm storage
    PS->>DB: Insert metadata record
    DB-->>PS: Confirm insertion
    PS-->>API: Return paste URL
    API-->>Client: 201 Created with paste_url
```
...
> **Why store content before metadata"**
>
...
[Embed: https://link.excalidraw.com/readonly/KeWo7Y3qZ71BQpwLWwWH](https://link.excalidraw.com/readonly/KeWo7Y3qZ71BQpwLWwWH)
...
sequenceDiagram
    ...
    Client->>CDN: GET /pastes/abc12345
    alt CDN Cache Hit
        ...
        alt Cache Hit
            Cache-->>PS: Return content
        else Cache Miss
            PS->>DB: Get metadata
            DB-->>PS: Return metadata
            PS->>S3: Get content
            S3-->>PS: Return content
            PS->>Cache: Store in cache
        end
        PS-->>API: Return paste
        API-->>CDN: Return paste
        CDN-->>Client: Return paste (cache for future)
    end
```
...
```mermaid
flowchart LR
    CDN[CDN Cache<br/>TTL: 1 hour]:::green
    Redis[Redis Cache<br/>TTL: 24 hours]:::orange
    DB[(Database<br/>Source of Truth)]:::purple

    CDN -->|miss| Redis -->|miss| DB
...
Now that we have designed both the write and read paths, let's step back and see the complete architecture. We have also added one more component that we have not discussed yet: the Cleanup Service for handling expired pastes.
 ...
 CDN\n    Mobile --> CDN\n    API_Client --> CDN\n    CDN --> LB\n    LB --> AG\n    AG --> PS\n    PS --> KGS\n    PS --> Redis\n    PS --> DB\n    PS --> S3\n    Cleanup --> DB\n    Cleanup --> S3\n\n    style CDN fill:#00ceff stroke:#000\n    style LB fill:#38d9a9,stroke:#000\n    style AG fill:#38d9a9,stroke:#000\n    style PS fill:#00ceff,stroke:#000\n    style KGS fill:#ffa94d,stroke:#000\n    style Redis fill:#ff8787,stroke:#000\n    style DB fill:#9775fa,stroke:#000\n    style S3 fill:#9775fa,stroke:#000\n    style Cleanup fill:#69db7c,stroke:#000"} -->
```mermaid
flowchart TB
    subgraph Clients
        Web[Web Browser]
...
flowchart LR
    subgraph "Metadata (PostgreSQL)"
        MD[paste_key, user_id,<br/>timestamps, visibility,<br/>content_path]:::primary
    end

    subgraph "Content (Object Storage)"
        CT[Actual paste text<br/>up to 10 MB]:::orange
    end
...
> **Why both content_path and content_inline"**
>
...
```mermaid
flowchart TD
    A[Generate Random<br/>8-char String]:::primary --> B{Key exists<br/>in DB"}
    B -->|No| C[Store Paste]:::green
    B -->|Yes| D[Regenerate]:::orange
    D --> A
    C --> E[Return URL]:::green
...
flowchart TB
    subgraph "Background Process"
        KGS[Key Generation Service<br/>Runs periodically]:::purple
    end

    subgraph "Key Pool"
        KP[(Keys Table<br/>Millions of pre-generated keys)]:::orange
    end

    subgraph "Paste Creation"
        PS[Paste Service]:::primary
        R[Return key to user]:::green
    end

    KGS -->|Generate and store<br/>new keys when pool low| KP
    PS -->|Fetch one unused key<br/>and mark as used| KP
    KP --> PS
    PS --> R
...
```shell
| key_value | is_used | assigned_at |
|-----------|---------|-------------|
| abc12345  | false   | null        |
| def67890  | false   | null        |
| xyz98765  | true    | 2024-01-15  |
```
...
```mermaid
flowchart LR
    subgraph "Snowflake ID Structure"
        direction LR
        A[Sign Bit<br/>1 bit<br/>always 0]:::secondary
        B[Timestamp<br/>41 bits<br/>milliseconds since epoch]:::primary
        C[Worker ID<br/>10 bits<br/>identifies the server]:::orange
        D[Sequence<br/>12 bits<br/>counter per millisecond]:::green
    end
...
```mermaid
flowchart TD
    A[New Paste]:::rose --> B{Content Size}

    B -->|< 64 KB| C[Store Inline<br/>in Metadata DB]:::primary
    B -->|>= 64 KB| D[Store in<br/>Object Storage]:::purple

    C --> E[Single Read<br/>Low Latency]:::green
    D --> F[Two Reads<br/>Cost Effective]:::green

    subgraph "~95% of Pastes"
        C
        E
    end

    subgraph "~5% of Pastes"
        D
        F
    end
...
```shell
Pastes Table:
| paste_key | content_inline      | content_path          | content_size |
|-----------|---------------------|-----------------------|--------------|
| abc123    | "print('hello')"    | null                  | 16           |
| def456    | "function sum..."   | null                  | 892          |
| xyz789    | null                | s3://pastes/xyz789    | 5242880      |
```
...
```mermaid
flowchart LR
    A[Original: 100 KB<br/>of log text]:::rose --> B[GZIP Compression]:::primary
    B --> C[Compressed: ~25 KB]:::green
    C --> D[Store in S3]:::purple
...
```mermaid
flowchart LR
    A[User Request]:::rose --> B[Layer 1: CDN<br/>~10-50ms global]:::primary
    B -->|miss| C[Layer 2: Redis<br/>~1ms from origin]:::orange
    C -->|miss| D[Layer 3: Database<br/>~20ms]:::purple
    D -->|large paste| E[Object Storage<br/>~50ms]:::purple

    B -->|"hit (most common)"| R1[Return to user]:::green
    C -->|hit| R2[Return to user]:::green
    D --> R3[Return to user]:::green
    E --> R3
...
Vary: Accept-Encoding                // separate cache entries for gzip vs plain
...
```shell
Cache Key:    paste:{paste_key}
Cache Value:  JSON {metadata, content}
...
```mermaid
flowchart LR
    A[Request for paste]:::rose --> B[Fetch from cache/DB]:::primary
    B --> C{Has paste expired"}
    C -->|"expires_at < now"| D[Return 410 Gone]:::red
    C -->|Still valid| E[Return content]:::green
...
```mermaid
flowchart TB
    A[Cleanup Job Starts<br/>Every hour]:::orange --> B[Query: SELECT expired pastes<br/>LIMIT 1000]:::primary
    B --> C{Found any"}
    C -->|No| D[Sleep until next run]:::secondary
    C -->|Yes| E[Delete content from S3]:::purple
    E --> F[Delete metadata from DB]:::purple
    F --> G[Invalidate cache]:::orange
    G --> B
...
```shell
cache_ttl = min(paste_remaining_lifetime, default_max_ttl)

Example:
- Paste expires in 30 minutes
- Default max TTL is 24 hours
- Cache TTL = min(30 minutes, 24 hours) = 30 minutes
```
...
flowchart TB
    subgraph "Request Flow"
        R[Incoming Request]:::primary --> I{Identify User}
        I --> A[Anonymous<br/>IP-based]:::rose
        I --> B[Registered<br/>Account-based]:::orange
        I --> C[Premium<br/>Paid account]:::green
    end

    subgraph "Limits Applied"
        A --> A1[10 creates/hour<br/>100 reads/hour]:::rose
        B --> B1[100 creates/hour<br/>1000 reads/hour]:::orange
        C --> C1[1000 creates/hour<br/>Unlimited reads]:::green
    end
...
> **Why different limits for creates vs reads"**
>
...
```mermaid
flowchart LR
    A[New Paste]:::rose --> B[Size Check]:::primary
    B --> C[Spam Filter]:::primary
    C --> D[Link Scanner]:::primary
    D --> E{Pass all checks"}
    E -->|Yes| F[Store Paste]:::green
    E -->|No| G[Reject or Flag]:::red
...
```mermaid
flowchart TB
    LB[Load Balancer]:::purple --> PS1[Paste Service 1]:::primary
    LB --> PS2[Paste Service 2]:::primary
    LB --> PS3[Paste Service 3]:::primary
    LB --> PS4[Paste Service N...]:::primary

    PS1 & PS2 & PS3 & PS4 --> Redis[(Redis Cluster)]:::orange
    PS1 & PS2 & PS3 & PS4 --> DB[(Database)]:::purple
...
flowchart TB
    subgraph "Database Cluster"
        Primary[(Primary<br/>Handles all writes)]:::orange
        R1[(Replica 1<br/>Read traffic)]:::secondary
        R2[(Replica 2<br/>Read traffic)]:::secondary
        R3[(Replica 3<br/>Read traffic)]:::secondary
    end
    ...
    PS[Paste Service] -->|Writes| Primary
    PS -->|Reads| R1 & R2 & R3
...
flowchart LR
    subgraph "Redis Cluster"
        N1[Node 1<br/>Keys a-h]:::red
        N2[Node 2<br/>Keys i-p]:::red
        N3[Node 3<br/>Keys q-z]:::red
    end

    PS[Paste Service] --> N1 & N2 & N3
