---
id: "hld-interviews-concept-deep-dives-database-design-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Concept Deep Dives"
subSection: ""
title: "Database Design for System Design Interviews"
slug: "hld-interviews-concept-deep-dives-database-design-for-system-design-interviews"
summary: "In every system design interview, you'll face the question: \"How would you store this data\"\""
eli10: "Imagine Database Design for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Database Design for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TB
    subgraph NoSQL["NoSQL Database Types"]
        direction TB
        DOC["Document Stores<br/>(MongoDB, DynamoDB)"]
        KV["Key-Value Stores<br/>(Redis, DynamoDB)"]
        WC["Wide-Column Stores<br/>(Cassandra, HBase)"]
        GR["Graph Databases<br/>(Neo4j, Neptune)"]
    end

    DOC --> D1["Flexible JSON documents<br/>Nested data<br/>Schema evolution"]
    KV --> K1["Simple lookups<br/>Caching<br/>Session storage"]
    WC --> W1["Time-series data<br/>High write throughput<br/>Column families"]
    GR --> G1["Relationships<br/>Social networks<br/>Recommendations"]

    style NoSQL fill:#00ceff,stroke:#000,color:#000
    style DOC fill:#ffa94d,stroke:#000,color:#000
    style KV fill:#ffa94d,stroke:#000,color:#000
    style WC fill:#ffa94d,stroke:#000,color:#000
    style GR fill:#ffa94d,stroke:#000,color:#000
    style D1 fill:#69db7c,stroke:#000,color:#000
    style K1 fill:#69db7c,stroke:#000,color:#000
    style W1 fill:#69db7c,stroke:#000,color:#000
    style G1 fill:#69db7c,stroke:#000,color:#000
```
...
flowchart TD
    ...
    R --> L1
    L1 --> LL1
    LL1 --> |"Row 42"| RES["Result"]

    style R fill:#00ceff,stroke:#000,color:#000
    style L1 fill:#ffa94d,stroke:#000,color:#000
    style L2 fill:#ffa94d,stroke:#000,color:#000
    style LL1 fill:#69db7c,stroke:#000,color:#000
    style LL2 fill:#38d9a9,stroke:#000,color:#000
    style LL3 fill:#38d9a9,stroke:#000,color:#000
    style LL4 fill:#38d9a9,stroke:#000,color:#000
    style Q fill:#9775fa,stroke:#000,color:#000
    style RES fill:#69db7c,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    subgraph Server["Single Database Server"]
        T["orders table"]
        T --> P1["orders_2023"]
        T --> P2["orders_2024"]
        T --> P3["orders_2025"]
    ...
    Q["Query: WHERE created_at = '2024-06-15'"]
    Q --> P2

    style T fill:#ffa94d,stroke:#000,color:#000
    style P1 fill:#38d9a9,stroke:#000,color:#000
    style P2 fill:#69db7c,stroke:#000,color:#000
    style P3 fill:#38d9a9,stroke:#000,color:#000
    style Q fill:#9775fa,stroke:#000,color:#000
```
...
flowchart TD
    ...
    ROUTER --> S1["Shard 0<br/>Users A-H"]
    ROUTER --> S2["Shard 1<br/>Users I-P"]
    ROUTER --> S3["Shard 2<br/>Users Q-Z"]

    style APP fill:#9775fa,stroke:#000,color:#000
    style ROUTER fill:#ffa94d,stroke:#000,color:#000
...
flowchart TD
    W["Writes"] --> P["Primary"]
    P --> |"Replication<br/>Stream"| R1["Replica 1"]
    P --> |"Replication<br/>Stream"| R2["Replica 2"]

    C1["Read Client 1"] --> R1
    C2["Read Client 2"] --> R2
    C3["Read Client 3"] --> R1

    style P fill:#00ceff,stroke:#000,color:#000
    style R1 fill:#69db7c,stroke:#000,color:#000
    style R2 fill:#69db7c,stroke:#000,color:#000
    style W fill:#ffa94d,stroke:#000,color:#000
    style C1 fill:#9775fa,stroke:#000,color:#000
    style C2 fill:#9775fa,stroke:#000,color:#000
    style C3 fill:#9775fa,stroke:#000,color:#000
```
...
flowchart LR
    ...
    style L1 fill:#00ceff,stroke:#000,color:#000
    style L2 fill:#00ceff,stroke:#000,color:#000
    style C1 fill:#9775fa,stroke:#000,color:#000
    style C2 fill:#9775fa,stroke:#000,color:#000
```
...
flowchart TD
    ...
    style N1 fill:#00ceff,stroke:#000,color:#000
    style N2 fill:#00ceff,stroke:#000,color:#000
    style N3 fill:#ffa94d,stroke:#000,color:#000
```
...
flowchart LR
    ...
    S --> SP["Always latest data<br/>Higher latency<br/>Lower availability"]
    E --> EP["May read stale data<br/>Lower latency<br/>Higher availability"]
    ...
    style SP fill:#69db7c,stroke:#000,color:#000
    style EP fill:#69db7c,stroke:#000,color:#000
```
...
sequenceDiagram
    ...
    Note over Client,Replica2: Write Operation (Synchronous)
    Client->>Primary: Write X=1
    Primary->>Replica1: Replicate X=1
    Primary->>Replica2: Replicate X=1
    Replica1-->>Primary: ACK
    Replica2-->>Primary: ACK
    Primary-->>Client: Write Confirmed

    Note over Client,Replica2: Read Operation (Always Sees Latest)
    Client->>Replica1: Read X
    Replica1-->>Client: X=1 (latest value)
```
...
sequenceDiagram
    ...
    Note over Writer,Primary: Write Operation (async)
    Writer->>Primary: Write X=1
    Primary-->>Writer: Write Confirmed
    Note right of Primary: Replication happens<br/>in background

    Note over Reader,Replica: Reader May See Stale Data
    Reader->>Replica: Read X
    Replica-->>Reader: X=0 (stale!)

    Primary--)Replica: Replicate X=1 (async)

    Note over Reader,Replica: Eventually Consistent
    Reader->>Replica: Read X
    Replica-->>Reader: X=1 (now consistent)
```
...
sequenceDiagram
    ...
    Note over UserA,UserB: User A Writes and Reads
    UserA->>Primary: Write X=1
    Primary-->>UserA: Write Confirmed
    UserA->>Primary: Read X
    Primary-->>UserA: X=1 (sees own write)

    Note over UserA,UserB: User B Reads (May Be Stale)
    UserB->>Replica: Read X
    Note right of Replica: Replication not<br/>yet complete
    Replica-->>UserB: X=0 (stale data)

    Primary--)Replica: Replicate X=1 (async)

    Note over UserA,UserB: User B Eventually Sees Update
    UserB->>Replica: Read X
    Replica-->>UserB: X=1 (now consistent)
```
...
sequenceDiagram
    ...
    Note over Alice,Carol: Alice Posts a Message
    Alice->>Server: Post "Hello"
    Server-->>Alice: Post Confirmed

    Note over Alice,Carol: Bob Sees Post and Replies
    Bob->>Server: Read Posts
    Server-->>Bob: ["Hello"]
    Bob->>Server: Reply "Hi" (to "Hello")
    Server-->>Bob: Reply Confirmed

    Note over Alice,Carol: Carol Reads (Causal Order Preserved)
    Carol->>Server: Read Posts
    Server-->>Carol: ["Hello", "Hi"]
    Note right of Carol: Always sees "Hello"<br/>before "Hi"<br/>(never reply before post)
```
...
1. Primary key defined (UUID or Snowflake for distributed)
2. Foreign keys for relationships
3. Appropriate data types (smallest that fits)
4. NOT NULL where required
