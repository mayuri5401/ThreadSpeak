---
id: "hld-interviews-interview-patterns-generating-unique-ids"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Generating Unique IDs"
slug: "hld-interviews-interview-patterns-generating-unique-ids"
summary: "You're designing a URL shortener. Each shortened URL needs a unique identifier. Simple enough, you think. Just use an auto-incrementing database ID."
eli10: "Imagine Generating Unique IDs as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Generating Unique IDs Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
...
flowchart LR
    subgraph Problem
        A[Server A] -->|ID: 1001| DB[(Database)]
        B[Server B] -->|ID: 1001| DB
    end
    ...
    style X fill:#ff8787,stroke:#000,color:#000
```
...
flowchart TD
    subgraph Single Server
        C[Counter] --> N1[1] --> N2[2] --> N3[3] --> N4[4] --> N5[...]
    end

    style C fill:#00ceff,stroke:#000,color:#000
    style N1 fill:#69db7c,stroke:#000,color:#000
    style N2 fill:#69db7c,stroke:#000,color:#000
    style N3 fill:#69db7c,stroke:#000,color:#000
    style N4 fill:#69db7c,stroke:#000,color:#000
    style N5 fill:#69db7c,stroke:#000,color:#000
```
...
flowchart TD
    ...
    style CA fill:#00ceff,stroke:#000,color:#000
    style CB fill:#ffa94d,stroke:#000,color:#000
    style ID1 fill:#00ceff,stroke:#000,color:#000
    style ID2 fill:#ffa94d,stroke:#000,color:#000
    style COL fill:#ff8787,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    subgraph Coordination Approach
        S1[Server 1] --> CA[Central Authority]
        S2[Server 2] --> CA
        S3[Server 3] --> CA
    end

    CA --> P1[Latency]
    CA --> P2[Single Point of Failure]
    CA --> P3[Bottleneck]

    style CA fill:#ff8787,stroke:#000,color:#000
...
```mermaid
flowchart TD
    subgraph Server1[Server 1: Odd IDs]
        S1[1] --> S1a[3] --> S1b[5] --> S1c[7]
    end

    subgraph Server2[Server 2: Even IDs]
        S2[2] --> S2a[4] --> S2b[6] --> S2c[8]
    end

    style S1 fill:#00ceff,stroke:#000,color:#000
    style S1a fill:#00ceff,stroke:#000,color:#000
    style S1b fill:#00ceff,stroke:#000,color:#000
    style S1c fill:#00ceff,stroke:#000,color:#000
    style S2 fill:#ffa94d,stroke:#000,color:#000
    style S2a fill:#ffa94d,stroke:#000,color:#000
    style S2b fill:#ffa94d,stroke:#000,color:#000
    style S2c fill:#ffa94d,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    subgraph UUID Versions
        V1[UUID v1<br/>Timestamp + MAC]:::orange
        V4[UUID v4<br/>Random]:::primary
        V7[UUID v7<br/>Timestamp + Random]:::purple
    end

    V1 --> P1[Sortable by time<br/>Exposes MAC address]
    V4 --> P2[Most common<br/>Not sortable]
    V7 --> P3[Best of both worlds<br/>Recommended for new systems]
...
```mermaid
flowchart TD
    subgraph Sequential[Sequential IDs - Efficient]
        I1[Insert 1] --> E1[End of index]
        I2[Insert 2] --> E1
        I3[Insert 3] --> E1
    end

    subgraph Random[Random UUIDs - Inefficient]
        U1[Insert 550e...] --> M1[Middle of index]
        U2[Insert a29b...] --> B1[Beginning of index]
        U3[Insert 1d4a...] --> D1[Different location]
    end

    style E1 fill:#69db7c,stroke:#000,color:#000
    style M1 fill:#ff8787,stroke:#000,color:#000
    style B1 fill:#ff8787,stroke:#000,color:#000
...
```mermaid
flowchart LR
    subgraph Snowflake["Snowflake ID - 64 bits"]
        direction LR
        A["Sign<br/>1 bit<br/>(always 0)"]:::secondary
        B["Timestamp<br/>41 bits<br/>(ms since epoch)"]:::primary
        C["Machine ID<br/>10 bits<br/>(1,024 machines)"]:::orange
        D["Sequence<br/>12 bits<br/>(4,096 per ms)"]:::purple
    end
...
```mermaid
flowchart TD
    subgraph Capacity[Snowflake Capacity]
        M[Per Machine<br/>4,096 IDs/ms<br/>= 4M IDs/sec]:::primary
        C[Per Cluster<br/>1,024 machines<br/>= 4B IDs/sec]:::orange
        T[Time Range<br/>41 bits<br/>= 69 years]:::purple
    end
...
```mermaid
flowchart TD
    subgraph ClockIssues[Clock-Related Issues]
        B[Clock Moves<br/>Backward]:::red
        S[Clock Skew<br/>Between Servers]:::orange
    end

    B --> B1[Could generate<br/>duplicate IDs]
    B --> B2[Solution: Refuse to<br/>generate until clock catches up]

    S --> S1[Minor ordering<br/>imperfection]
    S --> S2[Solution: Use NTP,<br/>accept slight inaccuracy]
...
```mermaid
flowchart LR
    subgraph ULID["ULID - 128 bits"]
        direction LR
        T["Timestamp<br/>48 bits<br/>(ms since Unix epoch)"]:::primary
        R["Randomness<br/>80 bits<br/>(cryptographically random)"]:::orange
    end

    T --- R

    subgraph Encoding["Crockford's Base32 Encoding"]
        E["01ARZ3NDEKTSV4RRFFQ69G5FAV<br/>(26 characters)"]:::purple
    end
...
```mermaid
flowchart LR
    subgraph ObjectID["ObjectID - 12 bytes (96 bits)"]
        direction LR
        T["Timestamp<br/>4 bytes<br/>(Unix seconds)"]:::primary
        R["Random<br/>5 bytes<br/>(per process)"]:::orange
        C["Counter<br/>3 bytes<br/>(incrementing)"]:::purple
    end
...
sequenceDiagram
    ...
    participant TS as Ticket Server

    A1->>TS: Request ID block
    TS->>A1: IDs 1-1000

    A2->>TS: Request ID block
    TS->>A2: IDs 1001-2000

    Note over A1: Uses IDs 1-1000 locally<br/>No more coordination needed

    Note over A2: Uses IDs 1001-2000 locally<br/>No more coordination needed

    A1->>TS: Block exhausted, request more
    TS->>A1: IDs 2001-3000
```
...
```mermaid
flowchart TD
    Q1{Need coordination-free<br/>generation"}
    Q1 -->|No| A[Auto-Increment<br/>or Ticket Server]:::green
    Q1 -->|Yes| Q2{Need time-sortable<br/>IDs"}

    Q2 -->|No| B[UUID v4]:::orange
    Q2 -->|Yes| Q3{Need 64-bit IDs<br/>specifically"}

    Q3 -->|Yes| Q4{Can assign<br/>machine IDs"}
    Q3 -->|No| D[ULID or UUID v7]:::purple

    Q4 -->|Yes| C[Snowflake]:::primary
    Q4 -->|No| D
