---
id: "hld-interviews-interview-patterns-handling-high-write-traffic"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Handling High Write Traffic"
slug: "hld-interviews-interview-patterns-handling-high-write-traffic"
summary: "While most systems are read-heavy, some of the most challenging problems in distributed systems involve handling massive write loads. Every GPS ping from an Uber driver, every click tracked by analytics, every log line from a server, every IoT sen..."
eli10: "Imagine Handling High Write Traffic as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Handling High Write Traffic Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
...
flowchart LR
    subgraph Monitoring["Logging & Monitoring"]
        D[Datadog]:::primary
        S[Splunk]:::primary
        P[Prometheus]:::primary
    end

    subgraph Realtime["Real-time Data"]
        U[Uber Location]:::orange
        ST[Stock Trades]:::orange
        AD[Ad Clicks]:::orange
    end

    subgraph IoT["IoT & Sensors"]
        TL[Tesla Fleet]:::purple
        SM[Smart Meters]:::purple
        WR[Wearables]:::purple
    end

    HWT[High Write Pattern]:::secondary --> Monitoring
    HWT --> Realtime
    HWT --> IoT
...
flowchart TD
    ...
    subgraph writes["Writes: Single Point Bottleneck"]
    ...
    subgraph reads["Reads: Horizontally Scalable"]
        R[Read Request]:::primary
        C[Cache Layer]:::green
        R1[(Replica 1)]:::green
        R2[(Replica 2)]:::green
        R3[(Replica N)]:::green
...
```mermaid
flowchart LR
    subgraph devices["10 Million Devices"]
        D1[Device 1]:::primary
        D2[Device 2]:::primary
        D3[Device N]:::primary
    end
    ...
    subgraph calc["Traffic Calculation"]
        I[Interval: 5 seconds]:::secondary
        W[2 Million Writes/Sec]:::orange
        B[1 GB/Sec Ingestion]:::orange
        S[86 TB/Day Storage]:::red
    end

    subgraph db["Database Capacity"]
        DB[(PostgreSQL<br/>~50K writes/sec max)]:::red
    end
...
```mermaid
flowchart LR
    subgraph spectrum["Durability Spectrum"]
        M[Memory Only<br/>~1μs]:::green
        AD[Async Disk<br/>~100μs]:::green
        SD[Sync Disk<br/>~1-10ms]:::orange
        SR[Sync + Replica<br/>~10-50ms]:::orange
        MR[Multi-Region<br/>~50-200ms]:::red
    end

    M --> AD --> SD --> SR --> MR

    L1[Fast & Risky]:::green -.-> M
    L2[Slow & Safe]:::red -.-> MR
...
```mermaid
flowchart TD
    subgraph toolkit["Write Scaling Toolkit"]
        B[Batching & Buffering<br/>Combine writes, reduce overhead]:::primary
        A[Async Processing<br/>Decouple acceptance from processing]:::primary
        S[Sharding<br/>Distribute load across databases]:::primary
        WO[Write-Optimized Storage<br/>LSM trees, append-only logs]:::primary
        ES[Event Sourcing / CQRS<br/>Separate read and write models]:::primary
        BP[Backpressure & Rate Limiting<br/>Handle overload gracefully]:::primary
    end

    B --> |"First step"| A
    A --> |"Scale horizontally"| S
    S --> |"Optimize storage"| WO
    WO --> |"Extreme scale"| ES
    ES --> |"Protect system"| BP
...
```mermaid
flowchart TD
    subgraph individual["Individual Write"]
        N1[Network RTT<br/>2ms]:::orange
        P1[Parse Query<br/>0.1ms]:::orange
        T1[Begin Transaction<br/>0.1ms]:::orange
        W1[Write Data<br/>0.5ms]:::green
        C1[Commit + Fsync<br/>2ms]:::orange
        Total1[Total: ~5ms]:::red
    end

    N1 --> P1 --> T1 --> W1 --> C1 --> Total1
...
```mermaid
flowchart TD
    subgraph batch["Batched Write (1000 rows)"]
        N2[Network RTT<br/>2ms]:::orange
        P2[Parse Query<br/>0.2ms]:::orange
        T2[Begin Transaction<br/>0.1ms]:::orange
        W2[Write 1000 Rows<br/>50ms]:::green
        C2[Commit + Fsync<br/>2ms]:::orange
        Total2[Total: ~55ms]:::green
    end

    N2 --> P2 --> T2 --> W2 --> C2 --> Total2
...
flowchart TD
    subgraph client["Application"]
        ...
        B[Write Buffer<br/>In-Memory]:::orange
    ...
    subgraph triggers["Flush Triggers"]
        T1[Size Limit<br/>1000 items]:::secondary
        T2[Time Limit<br/>100ms]:::secondary
    end

    B --> T1
    B --> T2
    T1 --> F[Flush to DB]:::green
    T2 --> F
    F --> DB[(Database)]:::purple
...
```mermaid
flowchart TD
    subgraph methods["Bulk Write Methods"]
        I[Individual INSERTs<br/>1x speed]:::red
        M[Multi-Value INSERT<br/>60x speed]:::orange
        C[COPY Command<br/>300x speed]:::green
    end
...
```mermaid
flowchart LR
    subgraph sync["Synchronous Path (Fast)"]
        A[Application]:::primary
        R[(Redis<br/>In-Memory)]:::green
    end
...
```mermaid
flowchart LR
    subgraph producers["Producers (Accept Writes)"]
        P1[API Server 1]:::primary
        P2[API Server 2]:::primary
        P3[API Server N]:::primary
    end

    subgraph queue["Message Queue (Buffer)"]
        Q[Kafka / SQS / RabbitMQ<br/>Durable, Ordered]:::orange
    end

    subgraph consumers["Consumers (Process Writes)"]
        C1[Worker 1]:::green
        C2[Worker 2]:::green
        C3[Worker N]:::green
    end
...
```mermaid
flowchart LR
    subgraph comparison["Queue Selection"]
        K[Kafka<br/>1M+ msg/sec<br/>Streaming, Partitioned]:::green
        RMQ[RabbitMQ<br/>50K msg/sec<br/>Low Latency, Flexible]:::primary
        SQS[Amazon SQS<br/>100K+ msg/sec<br/>Managed, Serverless]:::primary
        RS[Redis Streams<br/>100K+ msg/sec<br/>Simple, In-Memory]:::orange
    end
...
```mermaid
flowchart LR
    Q[Queue]:::orange --> C1[Consumer 1]:::green
    Q --> C2[Consumer 2]:::green
    Q --> C3[Consumer N]:::green

    Note[Each message processed by<br/>exactly one consumer]:::secondary
...
```mermaid
flowchart LR
    subgraph queue["Kafka Topic"]
        P0[Partition 0<br/>user_id % 3 = 0]:::orange
        P1[Partition 1<br/>user_id % 3 = 1]:::orange
        P2[Partition 2<br/>user_id % 3 = 2]:::orange
    end

    P0 --> C0[Consumer 0]:::green
    P1 --> C1[Consumer 1]:::green
    P2 --> C2[Consumer 2]:::green

    Note[Order preserved within partition<br/>Scale by adding partitions]:::secondary
...
```mermaid
flowchart TD
    M[Message Arrives]:::primary --> C{Already<br/>Processed"}:::orange
    C -->|Yes| Skip[Skip - Return Success]:::green
    C -->|No| P[Process Message]:::secondary
    P --> Store[Store Result + Message ID]:::green
    Store --> Ack[Acknowledge Message]:::green
...
```mermaid
flowchart TD
    subgraph btree["B-Tree Write (Random I/O)"]
        W1[Write Request]:::primary
        F1[Find Page in Tree]:::secondary
        R1[Read Page from Disk]:::orange
        M1[Modify Page]:::secondary
        WD1[Write Page to Disk]:::red
        UI1[Update Index Pages]:::red
    end
...
flowchart TD
    subgraph lsm["LSM Tree Structure"]
        W[Write]:::primary --> M[Memtable<br/>In-Memory Buffer]:::green

        M -->|"When full"| L0[Level 0 SSTable<br/>On Disk, Sorted]:::orange
        L0 -->|"Background<br/>Compaction"| L1[Level 1 SSTable<br/>Merged & Sorted]:::orange
        L1 -->|"Background<br/>Compaction"| L2[Level 2 SSTable<br/>Largest Level]:::purple
    end

    Note[All writes are sequential!<br/>No random I/O]:::secondary
...
```mermaid
flowchart LR
    subgraph comparison["B-Tree vs LSM Tree"]
        subgraph btree["B-Tree (PostgreSQL)"]
            BW[Writes: Moderate]:::orange
            BR[Reads: Fast]:::green
            BS[Space: Efficient]:::green
        end

        subgraph lsm["LSM Tree (Cassandra)"]
            LW[Writes: Very Fast]:::green
            LR[Reads: Moderate]:::orange
            LS[Space: Less Efficient]:::orange
        end
    end
...
```mermaid
flowchart LR
    A[Application]:::primary --> R[Shard Router]:::secondary

    subgraph shards["Database Shards"]
        S0[(Shard 0<br/>Users 0-24%)]:::purple
        S1[(Shard 1<br/>Users 25-49%)]:::purple
        S2[(Shard 2<br/>Users 50-74%)]:::purple
        S3[(Shard 3<br/>Users 75-99%)]:::purple
    end
...
flowchart LR
    ...
    subgraph good["Good: Shard by User ID"]
        U1[User A]:::green --> SA[(Shard 1)]:::green
        U2[User B]:::green --> SB[(Shard 2)]:::green
        U3[User C]:::green --> SC[(Shard 3)]:::green
    end

    subgraph bad["Bad: Shard by Timestamp"]
        T[All Current Writes]:::red --> S1[(Shard: Today)]:::red
        S0[(Shard: Yesterday)]:::green
    end
...
```mermaid
flowchart TD
    subgraph naive["Naive Modulo: key % N"]
        N3[3 Shards]:::orange -->|"Add 1 shard"| N4[4 Shards]:::orange
        N4 --> R[~75% of keys<br/>must move]:::red
    end

    subgraph consistent["Consistent Hashing"]
        C3[3 Shards]:::green -->|"Add 1 shard"| C4[4 Shards]:::green
        C4 --> R2[~25% of keys<br/>must move]:::green
    end
...
```mermaid
flowchart LR
    subgraph problem["Cross-Shard Transaction"]
        T[Transfer $100]:::primary
        D[Debit User A<br/>Shard 1]:::orange
        C[Credit User B<br/>Shard 3]:::orange

        T --> D
        T --> C
    end

    Note[Must be atomic!<br/>Both succeed or both fail]:::red
...
flowchart LR
    subgraph traditional["Traditional: Mutable State"]
        U1[Update 1]:::primary --> R[(Balance: 100)]:::orange
        U2[Update 2]:::primary --> R
        U3[Update 3]:::primary --> R
        R --> F[(Balance: 150)]:::red

        Note[History lost!<br/>Only current state remains]:::red
    end
...
```mermaid
flowchart TD
    subgraph eventsource["Event Sourcing: Immutable Log"]
        E1[UserCreated<br/>balance: 0]:::green
        E2[DepositMade<br/>+100]:::green
        E3[WithdrawalMade<br/>-20]:::green
        E4[DepositMade<br/>+70]:::green

        E1 --> E2 --> E3 --> E4

        E4 --> R[Replay = Balance: 150]:::primary
    end

    Note[Complete history preserved!<br/>Audit trail built-in]:::green
...
flowchart TD
    subgraph write["Write Side (Commands)"]
        WR[Write Request]:::primary
        CS[Command Service<br/>Validates & Accepts]:::secondary
        ES[(Event Store<br/>Append-Only Log)]:::orange
    end

    subgraph projection["Projection Layer"]
        P[Event Processor<br/>Subscribes to Events]:::secondary
    end

    subgraph read["Read Side (Queries)"]
        RR[Read Request]:::primary
        QS[Query Service]:::secondary
        RM1[(Read Model 1<br/>User Profiles)]:::green
        RM2[(Read Model 2<br/>Analytics)]:::green
        RM3[(Read Model 3<br/>Search Index)]:::green
    end
...
```mermaid
flowchart TD
    subgraph cascade["Cascade Failure"]
        T[Traffic Spike<br/>10,000 writes/sec]:::primary
        S[Server<br/>Capacity: 5,000/sec]:::orange
        Q[Queue Builds Up]:::orange
        M[Memory Exhausted]:::red
        C[Crash]:::red
    end
...
```mermaid
flowchart LR
    R[Requests]:::primary --> RL{Rate<br/>Limiter}:::orange
    RL -->|"Under limit"| A[Accept]:::green
    RL -->|"Over limit"| D[429 Too Many<br/>Requests]:::red
...
```mermaid
flowchart LR
    P[Producer]:::primary -->|"Writes"| Q[Queue]:::orange
    Q -->|"Backpressure<br/>signal"| P
    Q --> C[Consumer]:::green
    C --> DB[(Database)]:::purple

    Note[When queue fills,<br/>producer slows down]:::secondary
...
```mermaid
flowchart TD
    R[Request]:::primary --> O{Server<br/>Overloaded"}:::orange
    O -->|"No"| P[Process Normally]:::green
    O -->|"Yes"| PR{Request<br/>Priority"}:::orange
    PR -->|"High"| P
    PR -->|"Low"| S[Shed/Drop]:::red
    S --> E[Return 503]:::red
...
flowchart LR
    subgraph normal["Normal Mode"]
        N1[Write to DB]:::green
        N2[Update Cache]:::green
        N3[Send to Analytics]:::green
        N4[Trigger Notifications]:::green
    end

    subgraph degraded["Degraded Mode"]
        D1[Write to DB]:::green
        D2[Update Cache]:::green
        D3[Queue Analytics]:::orange
        D4[Skip Notifications]:::red
    end
...
flowchart TD
    subgraph ingestion["1. Ingestion Layer"]
        ...
        A1[API Server]:::primary
        A2[API Server]:::primary
    end

    subgraph buffering["2. Buffering Layer"]
        K[Kafka<br/>Partitioned by Key<br/>Replication Factor: 3]:::green
    end

    subgraph processing["3. Processing Layer"]
        CG1[Consumer Group<br/>Real-Time]:::primary
        CG2[Consumer Group<br/>Batch Analytics]:::primary
    end

    subgraph storage["4. Storage Layer"]
        S1[(Shard 1<br/>Cassandra)]:::purple
        S2[(Shard 2<br/>Cassandra)]:::purple
        S3[(Shard N<br/>Cassandra)]:::purple
    end

    subgraph read["5. Read Layer (CQRS)"]
        ES[(Elasticsearch<br/>Search)]:::green
        RS[(Redis<br/>Real-Time)]:::green
        CH[(ClickHouse<br/>Analytics)]:::green
    end
...
flowchart LR
    subgraph flow["Data Flow"]
        I[1. INGEST<br/>Accept & validate]:::primary
        B[2. BUFFER<br/>Absorb spikes]:::orange
        P[3. PROCESS<br/>Batch & transform]:::secondary
        S[4. STORE<br/>Persist durably]:::green
        R[5. PROJECT<br/>Build read models]:::purple
    end
