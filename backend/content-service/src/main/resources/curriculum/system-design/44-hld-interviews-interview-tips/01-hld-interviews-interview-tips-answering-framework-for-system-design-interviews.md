---
id: "hld-interviews-interview-tips-answering-framework-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Tips"
subSection: ""
title: "Answering Framework for System Design Interviews"
slug: "hld-interviews-interview-tips-answering-framework-for-system-design-interviews"
summary: "You walk into a system design interview. The interviewer says, \"Design Instagram.\" Where do you even start\""
eli10: "Imagine Answering Framework for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Answering Framework for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    A[1. Requirements<br/>5-7 min]:::phase1 --> B[2. Estimation<br/>3-5 min]:::phase2
    B --> C[3. API Design<br/>3-5 min]:::phase3
    C --> D[4. High-Level<br/>Design<br/>8-10 min]:::phase4
    D --> E[5. Database<br/>Design<br/>5-7 min]:::phase5
    E --> F[6. Deep Dives<br/>12-18 min]:::phase6
    F --> G[7. Wrap-Up<br/>3-5 min]:::phase7

    classDef phase1 fill:#00ceff,stroke:#000,color:#000
    classDef phase2 fill:#ffa94d,stroke:#000,color:#000
    classDef phase3 fill:#ffd43b,stroke:#000,color:#000
    classDef phase4 fill:#69db7c,stroke:#000,color:#000
    classDef phase5 fill:#9775fa,stroke:#000,color:#000
    classDef phase6 fill:#3bc9db,stroke:#000,color:#000
    classDef phase7 fill:#f783ac,stroke:#000,color:#000
```
...
```mermaid
flowchart LR
    A[Client]:::primary --> B[Server]:::primary
    B --> C[(Database)]:::purple
...
```mermaid
flowchart LR
    A[Client]:::primary --> B[Load Balancer]:::secondary
    B --> C[Server 1]:::primary
    B --> D[Server 2]:::primary
    C --> E[(Database)]:::purple
    D --> E
...
flowchart TD
    ...
    subgraph Edge
        C[CDN]:::secondary
        D[Load Balancer]:::secondary
    end

    subgraph Application
        E[Tweet Service]:::primary
        F[Timeline Service]:::primary
        G[User Service]:::primary
    end

    subgraph Data
        H[(Tweet DB)]:::purple
        I[(User DB)]:::purple
        J[(Timeline Cache)]:::orange
        K[Message Queue]:::green
    end

    subgraph Background
        L[Fan-out Workers]:::teal
    end
...
```mermaid
flowchart LR
    A[User Posts<br/>Tweet]:::primary --> B[Fan-out<br/>Service]:::secondary
    B --> C[Follower 1<br/>Timeline]:::orange
    B --> D[Follower 2<br/>Timeline]:::orange
    B --> E[Follower N<br/>Timeline]:::orange
...
```mermaid
flowchart TD
    A[User Posts<br/>Tweet]:::primary --> B{Celebrity"}:::secondary
    B -->|No| C[Push to<br/>Followers]:::green
    B -->|Yes| D[Store in<br/>Tweet DB]:::orange

    E[User Requests<br/>Timeline]:::primary --> F[Merge pre-computed<br/>+ celebrity tweets]:::purple
