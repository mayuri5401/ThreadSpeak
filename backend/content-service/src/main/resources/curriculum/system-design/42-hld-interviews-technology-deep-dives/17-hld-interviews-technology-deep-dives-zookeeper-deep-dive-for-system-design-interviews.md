---
id: "hld-interviews-technology-deep-dives-zookeeper-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Zookeeper Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-zookeeper-deep-dive-for-system-design-interviews"
summary: "If you have spent any time working with distributed systems, you have probably run into Zookeeper. It powers coordination in Kafka, HBase, Solr, and countless internal systems at large companies. Yet in interviews, I often see candidates either tr..."
eli10: "Imagine Zookeeper Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Zookeeper Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Need coordination between services"]:::primary
    A -->|Yes| B[Need strong consistency"]:::primary
    A -->|No| C[Use direct communication]:::red
    B -->|Yes| D[Small data size"]:::primary
    B -->|No| E[Consider eventual consistency]:::orange
    D -->|Yes| F[Zookeeper is a good fit]:::green
    D -->|No| G[Use distributed database]:::orange
...
```mermaid
flowchart TD
    subgraph Ensemble["Zookeeper Ensemble (5 nodes)"]
        L[Leader]:::green
    ...
    subgraph Clients["Clients"]
        C1[Service A]:::primary
        C2[Service B]:::primary
        C3[Service C]:::primary
    end
...
```mermaid
flowchart LR
    C[Client]:::primary --> F[Follower]:::orange
    F -->|Forward| L[Leader]:::green
    L -->|Propose| F1[Follower 1]:::orange
    L -->|Propose| F2[Follower 2]:::orange
    F1 -->|ACK| L
    F2 -->|ACK| L
    L -->|Commit| F1
    L -->|Commit| F2
...
```mermaid
flowchart TD
    subgraph Types["Znode Types"]
        P[Persistent]:::primary
        E[Ephemeral]:::orange
        PS[Persistent Sequential]:::green
        ES[Ephemeral Sequential]:::purple
    end

    P --> P1["Survives client disconnect"]
    E --> E1["Deleted when session ends"]
    PS --> PS1["Name has auto-incremented suffix"]
    ES --> ES1["Ephemeral + Sequential"]
...
```mermaid
flowchart TD
    subgraph ZAB["ZAB Protocol Phases"]
        D[Discovery]:::primary --> S[Synchronization]:::orange
        S --> B[Broadcast]:::green
        B -->|Leader fails| D
    end
...
```mermaid
flowchart TD
    A[Server starts]:::primary --> B{Leader exists"}
    B -->|Yes| C[Connect as follower]:::green
    B -->|No| D[Enter LOOKING state]:::orange
    D --> E[Propose self]:::orange
    E --> F[Exchange votes]:::orange
    F --> G{Have quorum"}
    G -->|Yes| H[Winner becomes leader]:::green
    G -->|No| F
...
sequenceDiagram
    ...
    C->>L: write("/config", data)
    L->>L: Create proposal (zxid=0x100)
    L->>F1: PROPOSAL(zxid=0x100, data)
    L->>F2: PROPOSAL(zxid=0x100, data)
    F1->>F1: Write to disk
    F2->>F2: Write to disk
    F1->>L: ACK(zxid=0x100)
    F2->>L: ACK(zxid=0x100)
    L->>L: Quorum reached, commit
    L->>F1: COMMIT(zxid=0x100)
    L->>F2: COMMIT(zxid=0x100)
    L->>C: Success
```
...
```mermaid
flowchart LR
    C[Client]:::primary -->|1. getData + watch| S[Server]:::green
    S -->|2. Return data| C
    S -->|3. Data changes| S
    S -->|4. Notify| C
    C -->|5. Re-register watch| S
...
```mermaid
flowchart TD
    A[Client connects]:::primary --> B[Session created]:::green
    B --> C[Client sends heartbeats]:::green
    C --> D{Heartbeat received"}
    D -->|Yes| C
    D -->|No| E[Session timeout countdown]:::orange
    E --> F{Reconnected"}
    F -->|Yes| C
    F -->|No| G[Session expired]:::red
    G --> H[Ephemeral nodes deleted]:::red
...
```mermaid
flowchart LR
    A[NOT_CONNECTED]:::red --> B[CONNECTING]:::orange
    B --> C[CONNECTED]:::green
    C --> D[CLOSED]:::red
    B --> D
    C --> B
...
flowchart TD
    subgraph Election["Leader Election"]
        L["/election/candidate-0001"]:::green
        C1["/election/candidate-0002"]:::orange
        C2["/election/candidate-0003"]:::orange
        C3["/election/candidate-0004"]:::orange
    end

    L --> L1["Leader (lowest sequence)"]
    C1 --> C1W["Watches candidate-0001"]
    C2 --> C2W["Watches candidate-0002"]
    C3 --> C3W["Watches candidate-0003"]
...
sequenceDiagram
    ...
    C1->>ZK: create(/locks/resource/lock-, EPHEMERAL_SEQUENTIAL)
    ZK->>C1: /locks/resource/lock-0001
    C1->>ZK: getChildren(/locks/resource)
    ZK->>C1: [lock-0001] (I'm smallest, lock acquired)

    C2->>ZK: create(/locks/resource/lock-, EPHEMERAL_SEQUENTIAL)
    ZK->>C2: /locks/resource/lock-0002
    C2->>ZK: getChildren(/locks/resource)
    ...
    ZK->>C2: NodeDeleted event
    C2->>ZK: getChildren(/locks/resource)
    ZK->>C2: [lock-0002] (I'm smallest, lock acquired)
```
...
flowchart TD
    subgraph Services["Service Registration"]
        S1[API Instance 1]:::primary --> R1["/services/api/instance-1<br/>EPHEMERAL"]:::green
        S2[API Instance 2]:::primary --> R2["/services/api/instance-2<br/>EPHEMERAL"]:::green
        S3[API Instance 3]:::primary --> R3["/services/api/instance-3<br/>EPHEMERAL"]:::green
    end

    subgraph Discovery["Service Discovery"]
        C[Client]:::orange --> W["Watch /services/api"]:::orange
        W --> R1
        W --> R2
        W --> R3
    end
...
```mermaid
flowchart TD
    subgraph Ensemble["Voting Members"]
        L[Leader]:::green
        F1[Follower]:::orange
        F2[Follower]:::orange
    end

    subgraph Observers["Non-voting Observers"]
        O1[Observer 1]:::purple
        O2[Observer 2]:::purple
        O3[Observer 3]:::purple
    end
    ...
    C1[Read-heavy clients]:::primary --> O1
    C2[Read-heavy clients]:::primary --> O2
