---
id: "hld-interviews-technology-deep-dives-amazon-s3-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Amazon S3 Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-amazon-s3-deep-dive-for-system-design-interviews"
summary: "Walk into any system design interview involving file storage, and S3 will almost certainly come up."
eli10: "Imagine Amazon S3 Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Amazon S3 Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[What type of data"]:::primary
    A -->|Structured/Relational| B[Consider RDS/DynamoDB]:::red
    A -->|Unstructured/Files| C[Access pattern"]:::primary
    C -->|Multiple times/day| D[S3 Standard]:::green
    C -->|Once per month| E[S3 Standard-IA]:::orange
    C -->|Once per quarter| F[S3 Glacier Instant]:::purple
    C -->|Once per year or less| G[S3 Glacier Deep Archive]:::purple
...
flowchart LR
    subgraph Object["S3 Object"]
        direction TB
        K[Key: users/123/photo.jpg]:::primary
        V[Value: Binary Data up to 5TB]:::green
        M[Metadata: content-type, custom-tags]:::orange
        VID[Version ID: abc123...]:::purple
    end
...
flowchart TB
    subgraph Client["Client Layer"]
        A[Your Application]:::primary
    end

    subgraph Frontend["S3 Frontend"]
        B[Request Router]:::orange
        C[Authentication/<br/>Authorization]:::orange
    end

    subgraph Metadata["Metadata Layer"]
        D[Index Service]:::purple
        E[Namespace<br/>Management]:::purple
    end

    subgraph Storage["Storage Layer"]
        F[Storage Node<br/>AZ-1]:::green
        G[Storage Node<br/>AZ-2]:::green
        H[Storage Node<br/>AZ-3]:::green
    end
...
```mermaid
flowchart LR
    subgraph Keys["Object Keys"]
        K1[images/2024/01/photo1.jpg]:::primary
        K2[images/2024/01/photo2.jpg]:::primary
        K3[images/2024/02/photo3.jpg]:::orange
        K4[logs/2024/01/app.log]:::purple
    end

    subgraph Partitions["S3 Partitions"]
        P1[Partition A]:::primary
        P2[Partition B]:::orange
        P3[Partition C]:::purple
    end
...
```mermaid
flowchart LR
    A[1. Client<br/>Request]:::primary --> B[2. DNS<br/>Resolution]:::orange
    B --> C[3. S3<br/>Endpoint]:::purple
    C --> D{4. Auth<br/>Check}:::orange
    D -->|Valid| E[5. Fetch from<br/>Storage]:::green
    D -->|Invalid| F[403 Forbidden]:::red
    E --> G[6. Return<br/>Response]:::green
...
```mermaid
flowchart LR
    subgraph Hot["Hot Storage (Frequent Access)"]
        direction LR
        A[S3 Standard<br/>$0.023/GB]:::green
        B[Intelligent-Tiering<br/>Auto-optimized]:::green
    end

    subgraph Warm["Warm Storage (Infrequent Access)"]
        direction LR
        C[Standard-IA<br/>$0.0125/GB]:::orange
        D[One Zone-IA<br/>$0.01/GB]:::orange
    end

    subgraph Cold["Cold Storage (Archive)"]
        direction LR
        E[Glacier Instant<br/>$0.004/GB]:::purple
        F[Glacier Flexible<br/>$0.0036/GB]:::purple
        G[Glacier Deep Archive<br/>$0.00099/GB]:::purple
    end
...
```mermaid
flowchart LR
    A[Upload<br/>Day 0<br/>Standard]:::green --> B[Day 30<br/>Standard-IA]:::orange
    B --> C[Day 90<br/>Glacier Flexible]:::purple
    C --> D[Day 365<br/>Delete]:::red
...
```mermaid
flowchart TD
    A[Object Uploaded]:::primary --> B[Frequent Access Tier]:::green
    B -->|No access for 30 days| C[Infrequent Access Tier]:::orange
    C -->|Accessed| B
    C -->|No access for 90 days| D[Archive Instant Access<br/>Optional]:::purple
    D -->|Accessed| B
    D -->|No access for 180 days| E[Deep Archive Access<br/>Optional]:::purple
    E -->|Accessed| B
...
flowchart LR
    subgraph Write["Write Phase"]
        A[PUT Object]:::orange
        B[Write to<br/>AZ-1, AZ-2, AZ-3]:::purple
        C[Update Index]:::purple
        D[Return Success]:::green
    end

    subgraph Read["Read Phase"]
        E[GET Object]:::primary
        F[Read from Index]:::purple
        G[Fetch Data]:::green
    end
...
```mermaid
flowchart LR
    subgraph Timeline["Object: config.json"]
        V1[v1<br/>Jan 5<br/>1 KB]:::orange
        V2[v2<br/>Jan 10<br/>1 KB]:::orange
        V3[v3<br/>Jan 15<br/>2 KB<br/>Current]:::green
    end

    GET[GET config.json]:::primary --> V3
    GETV1[GET "versionId=v1]:::primary --> V1
...
flowchart TD
    ...
    subgraph Solution["Prefix Distribution"]
        C[data/a/]:::green
        D[data/b/]:::green
        E[data/c/]:::green
        F[data/d/]:::green
        C --> G[5,500 GET/s]:::green
        D --> H[5,500 GET/s]:::green
        E --> I[5,500 GET/s]:::green
        F --> J[5,500 GET/s]:::green
    end
	
    subgraph Problem["Single Prefix Bottleneck"]
        A[data/]:::red
        A --> B[5,500 GET/s limit]:::red
    end	

    K[Total: 22,000 GET/s]:::primary
...
```mermaid
flowchart LR
    A[1. Initiate<br/>Get Upload ID]:::primary --> B[2. Upload Parts<br/>In Parallel]:::orange
    B --> C[Part 1]:::orange
    B --> D[Part 2]:::orange
    B --> E[Part 3]:::orange
    B --> F[Part N]:::orange
    C --> G[3. Complete<br/>Combine Parts]:::green
    D --> G
    E --> G
    F --> G
...
flowchart TD
    ...
    subgraph With["With Transfer Acceleration"]
        A2[User in<br/>Sydney]:::primary --> B2[CloudFront<br/>Edge Sydney]:::orange --> C2[AWS<br/>Backbone]:::purple --> D2[S3 in<br/>US-East]:::green
    end

    subgraph Without["Without Transfer Acceleration"]
        A1[User in<br/>Sydney]:::primary --> B1[Public<br/>Internet]:::red --> C1[S3 in<br/>US-East]:::green
    end
...
```mermaid
flowchart TD
    A[S3 Request]:::primary --> B{IAM Policy<br/>Allow"}
    B -->|Explicit Deny| Z[403 Forbidden]:::red
    B -->|Allow or No Policy| C{Bucket Policy<br/>Allow"}
    C -->|Explicit Deny| Z
    C -->|Allow or No Policy| D{ACL<br/>Allow"}
    D -->|Deny| Z
    D -->|Allow or No Rule| E{Block Public<br/>Access"}
    E -->|Blocked| Z
    E -->|Not Blocked| F[Access Granted]:::green
...
```mermaid
flowchart TD
    A[Public Access<br/>Request]:::primary --> B{Block Public<br/>Access Enabled"}
    B -->|Yes| C[403 Forbidden]:::red
    B -->|No| D{Bucket Policy<br/>Allows"}
    D -->|Yes| E[Access Granted]:::green
    D -->|No| C
...
```mermaid
flowchart TD
    A[How should data be encrypted"]:::primary
    A --> B{Who manages keys"}

    B -->|AWS handles everything| C[SSE-S3]:::green
    B -->|I need audit trail & rotation| D[SSE-KMS]:::orange
    B -->|I provide my own keys| E[SSE-C]:::purple
    B -->|Zero trust in AWS| F[Client-Side<br/>Encryption]:::red
...
```mermaid
flowchart LR
    A[1. Client requests<br/>upload URL]:::primary --> B[2. Server generates<br/>pre-signed URL]:::orange
    B --> C[3. Client uploads<br/>directly to S3]:::green
    C --> D[4. S3 validates<br/>signature & expiry]:::purple
...
flowchart LR
    ...
    subgraph Users["AP-Southeast-1 (User Region)"]
        C[Read Replica]:::green
    end
...
```mermaid
flowchart TD
    A[Object Lock]:::primary
    A --> B[Governance Mode]:::orange
    A --> C[Compliance Mode]:::red

    B --> D[Special permissions<br/>can override]:::orange
    C --> E[No one can delete<br/>Not even root]:::red
...
```mermaid
flowchart LR
    A[Client]:::primary --> B[API Gateway]:::orange
    B --> C[Metadata Service<br/>DynamoDB]:::purple
    B --> D[S3<br/>File Storage]:::green
    D --> E[CloudFront<br/>CDN]:::orange
    E --> A
...
flowchart LR
    subgraph Upload["Upload Phase"]
        A[User]:::primary --> B[Multipart<br/>Upload]:::orange
        B --> C[S3<br/>Originals]:::green
    end

    subgraph Process["Processing Phase"]
        C --> D[Transcode<br/>Lambda/ECS]:::purple
        D --> E[S3<br/>Transcoded]:::green
    end

    subgraph Deliver["Delivery Phase"]
        E --> F[CloudFront<br/>CDN]:::orange
        F --> G[Viewers]:::primary
    end
