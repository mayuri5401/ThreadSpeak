---
id: "hld-interviews-technology-deep-dives-prometheus-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Prometheus Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-prometheus-deep-dive-for-system-design-interviews"
summary: "Prometheus is the monitoring system you reach for when you want clear visibility into a distributed system, not just dashboards, but an explainable model for how metrics are collected, stored, queried, and alerted on."
eli10: "Imagine Prometheus Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Prometheus Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Need monitoring"]:::primary
    A -->|Yes| B[Numeric time-series data"]:::primary
    A -->|No| C[No monitoring needed]:::red
    B -->|Yes| D[High cardinality traces needed"]:::primary
    B -->|No| E[Consider logs or traces]:::orange
    D -->|Yes| F[Add Jaeger/Tempo for traces]:::orange
    D -->|No| G[Dynamic environment"]:::primary
    G -->|Yes| H[Prometheus is a good fit]:::green
    G -->|No| H
...
```mermaid
flowchart LR
    subgraph Targets["Targets (Applications)"]
        T1[Service A\n:8080/metrics]:::primary
        T2[Service B\n:8080/metrics]:::primary
        T3[Service C\n:8080/metrics]:::primary
    end

    subgraph Prometheus["Prometheus Server"]
        SD[Service Discovery]:::orange
        SC[Scraper]:::orange
        TS[(TSDB)]:::purple
        QE[Query Engine]:::green
    end

    SD --> SC
    SC -->|HTTP GET /metrics| T1
    SC -->|HTTP GET /metrics| T2
    SC -->|HTTP GET /metrics| T3
    SC --> TS
    QE --> TS
...
### 2.2 Key Components
 ...
 RET\n    RET --> TSDB\n    TSDB --> RULES\n    RULES -->|Alerts| AM\n    AM -->|Notify| SLACK[Slack]:::pink\n    AM -->|Notify| PD[PagerDuty]:::pink\n    TSDB --> HTTP\n    HTTP --> GRAF\n    HTTP --> UI\n\n    classDef primary fill:#00ceff,stroke:#000,color:#000\n    classDef orange fill:#ffa94d,stroke:#000,color:#000\n    classDef green fill:#69db7c,stroke:#000,color:#000\n    classDef purple fill:#9775fa,stroke:#000,color:#000\n    classDef red fill:#ff8787,stroke:#000,color:#000\n    classDef pink fill:#da77f2,stroke:#000,color:#000"} -->
```mermaid
flowchart TD
    subgraph Discovery["Service Discovery"]
        K8S[Kubernetes]:::primary
        CONSUL[Consul]:::primary
        FILE[File SD]:::primary
        EC2[AWS EC2]:::primary
    end

    subgraph Prometheus["Prometheus Server"]
        RET[Retrieval]:::orange
        TSDB[(TSDB\nTime Series DB)]:::purple
        HTTP[HTTP Server]:::green
        RULES[Rule Evaluation]:::orange
    end
...
```mermaid
flowchart TD
    SD[Service Discovery]:::orange --> SC[Scraper]:::primary
    SC -->|HTTP GET /metrics| T[Targets]:::green
    T -->|Metrics text| SC
    SC --> TSDB[(TSDB)]:::purple
    TSDB --> QE[Query Engine]:::primary
    TSDB --> RE[Rule Engine]:::primary
    RE -->|Alerts| AM[Alertmanager]:::red
    QE --> G[Grafana/API]:::green
    AM --> N[Notifications]:::red
...
```mermaid
flowchart TD
    subgraph TimeSeries["Time Series: http_requests_total{method='GET', status='200'}"]
        S1["t1: 100"]:::primary
        S2["t2: 105"]:::primary
        S3["t3: 112"]:::primary
        S4["t4: 120"]:::primary
...
```mermaid
flowchart LR
    subgraph Histogram["Request Duration Histogram"]
        B1["le=0.1\n24054"]:::green
        B2["le=0.5\n33444"]:::orange
        B3["le=1.0\n34001"]:::orange
        B4["le=+Inf\n34012"]:::red
...
```mermaid
flowchart LR
    subgraph Systems["Third-Party Systems"]
        MYSQL[(MySQL)]:::purple
        REDIS[(Redis)]:::purple
        LINUX[Linux]:::purple
        K8S[Kubernetes]:::purple
    end

    subgraph Exporters["Exporters"]
        ME[MySQL Exporter]:::orange
        RE[Redis Exporter]:::orange
        NE[Node Exporter]:::orange
        KSM[kube-state-metrics]:::orange
    end

    subgraph Prom["Prometheus"]
        P[Prometheus Server]:::primary
    end
...
flowchart LR
    subgraph Jobs["Batch Jobs"]
        J1[Backup Job]:::primary
        J2[ETL Job]:::primary
        J3[Report Job]:::primary
    end

    PG[Pushgateway]:::orange

    P[Prometheus]:::green

    J1 -->|Push on completion| PG
    J2 -->|Push on completion| PG
    J3 -->|Push on completion| PG
    P -->|Scrape| PG
...
```mermaid
flowchart TD
    subgraph Storage["Prometheus Storage"]
        subgraph Head["Head Block (in memory)"]
            WAL[Write-Ahead Log]:::orange
            MEM[In-Memory Chunks]:::primary
        end
        ...
        subgraph Compact["Compacted Blocks"]
            C1[Compacted\n6h data]:::green
        end
    end
...
flowchart LR
    subgraph Prom["Prometheus"]
        RULES[Alert Rules]:::primary
        EVAL[Rule Evaluation]:::primary
    end

    subgraph AM["Alertmanager"]
        DEDUP[Deduplication]:::orange
        GROUP[Grouping]:::orange
        ROUTE[Routing]:::orange
        SILENCE[Silencing]:::orange
        INHIBIT[Inhibition]:::orange
    end
...
```mermaid
stateDiagram-v2
    [*] --> Inactive: Expression false
    Inactive --> Pending: Expression true
    Pending --> Firing: True for 'for' duration
    Pending --> Inactive: Expression false
    Firing --> Inactive: Expression false
    Firing --> Firing: Expression true
```
...
flowchart TD
    subgraph DC1["Datacenter 1"]
        T1[Targets]:::primary --> P1[Prometheus DC1]:::orange
    end

    subgraph DC2["Datacenter 2"]
        T2[Targets]:::primary --> P2[Prometheus DC2]:::orange
    end

    subgraph Global["Global View"]
        PG[Global Prometheus]:::green
    end
...
flowchart TD
    subgraph Cluster1["Cluster 1"]
        P1[Prometheus]:::primary
        S1[Thanos Sidecar]:::orange
    end

    subgraph Cluster2["Cluster 2"]
        P2[Prometheus]:::primary
        S2[Thanos Sidecar]:::orange
    end

    subgraph Thanos["Thanos Components"]
        Q[Thanos Query]:::green
        ST[Thanos Store]:::purple
        C[Thanos Compactor]:::purple
    end
...
```mermaid
flowchart TD
    A[Need to scale"]:::primary
    A --> B{Cardinality too high"}
    B -->|Yes| C[Reduce cardinality first]:::orange
    B -->|No| D{Single cluster"}
    D -->|Yes| E{Need long retention"}
    D -->|No| F[Use Thanos/Cortex]:::green
    E -->|Yes| F
    E -->|No| G{Query performance OK"}
    G -->|Yes| H[Current setup sufficient]:::green
    G -->|No| I[Add recording rules]:::orange
    I --> J{Still slow"}
    J -->|Yes| K[Shard by job/team]:::orange
    J -->|No| H
...
flowchart LR
    subgraph OTel["OpenTelemetry"]
        SDK[OTel SDK]:::primary
        COL[OTel Collector]:::orange
    end
