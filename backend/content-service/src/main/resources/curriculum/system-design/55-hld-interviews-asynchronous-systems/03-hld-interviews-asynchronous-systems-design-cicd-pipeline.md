---
id: "hld-interviews-asynchronous-systems-design-cicd-pipeline"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Asynchronous Systems"
subSection: ""
title: "Design CI/CD Pipeline"
slug: "hld-interviews-asynchronous-systems-design-cicd-pipeline"
summary: "This system design problem touches on distributed systems, job scheduling, artifact management, and deployment strategies, concepts that are fundamental to modern software engineering."
eli10: "Imagine Design CI/CD Pipeline as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design CI/CD Pipeline Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph "What Users Do"
        A[Define Pipeline<br/>in YAML]:::rose
        B[Push Code<br/>to Git]:::rose
    end

    subgraph "What the System Does"
        C[Parse Pipeline<br/>Definition]:::primary
        D[Execute Stages<br/>in Isolated Containers]:::primary
        E[Store Artifacts<br/>and Logs]:::orange
        F[Deploy to<br/>Environments]:::green
        G[Send<br/>Notifications]:::secondary
    end
...
```mermaid
flowchart TD
    subgraph "Storage Per Build"
        A[Build Logs<br/>~500 KB]:::primary
        B[Artifacts<br/>~50 MB avg]:::orange
        C[Metadata<br/>~5 KB]:::secondary
    end

    subgraph "Total Per Build"
        T[~50.5 MB]:::purple
    end
...
flowchart TB
    ...
    subgraph "Orchestration"
        direction LR
        O1[API Gateway]:::secondary
        O2[Pipeline Service]:::primary
        O3[Job Queue]:::orange
    end

    subgraph "Execution"
        direction LR
        E1[Worker Pool]:::primary
        E2[Container Runtime]:::lightblue
    end

    subgraph "Storage & Delivery"
        direction LR
        S1[Artifact Service]:::primary
        S2[(Object Storage)]:::purple
        S3[Deployment Service]:::green
    end
...
flowchart LR
    ...
    subgraph Application["Application Layer"]
        PS[Pipeline Service]:::primary
        YAML[YAML Parser]:::primary
    end

    subgraph Storage["Storage Layer"]
        DB[(Pipeline DB)]:::purple
    end
    ...
    U -->|"1. POST /pipelines/.../runs"| AG
    AG -->|"2. Authenticate & forward"| PS
    PS -->|"3. Fetch & parse config"| YAML
    PS -->|"4. Create run record"| DB
    PS -->|"5. Queue first stages"| JQ
    AG -->|"6. Return run_id"| U
...
sequenceDiagram
    ...
    Dev->>GW: POST /pipelines/{id}/runs
    Note over GW: Authenticate request,<br/>check rate limits
    GW->>PS: Forward validated request
    PS->>Git: Fetch pipeline.yaml from repo
    Git-->>PS: Return config file
    PS->>PS: Parse YAML, build stage DAG
    PS->>DB: Create run + stage records
    DB-->>PS: Confirm insertion
    PS->>Q: Enqueue stages with no dependencies
    PS-->>GW: Return run_id + status
    GW-->>Dev: 201 Created with run details
```
...
> **Why queue stages instead of running them directly"**
>
...
flowchart TB
    ...
    subgraph "Isolation"
        C1[Container]:::lightblue
        C2[Container]:::lightblue
        C3[Container]:::lightblue
    end

    subgraph "Supporting Services"
        LS[Log Service]:::secondary
        SM[Secret Manager]:::secondary
    end
...
sequenceDiagram
    ...
    W->>Q: Poll for jobs
    Q->>W: Return job (stage details, repo info)
    W->>CR: Pull Docker image (node:18, python:3.11, etc.)
    CR-->>W: Image ready
    W->>C: Create container with resource limits
    W->>Git: Clone source code at commit SHA
    W->>SM: Fetch secrets for this build
    SM-->>W: Return encrypted secrets
    W->>C: Inject env vars and secrets
    W->>C: Execute build commands
    loop During execution
        C->>LS: Stream logs in real-time
    end
    alt Build succeeds
        W->>PS: Report success + artifacts location
        PS->>Q: Enqueue dependent stages
    else Build fails
        W->>PS: Report failure + error details
        PS->>PS: Mark run as failed (if required stage)
    end
...
> **What happens when a worker crashes mid-build"**
>
...
flowchart LR
    ...
    subgraph "Storage Phase"
        AS[Artifact Service]:::primary
        OS[(Object Storage<br/>S3/GCS)]:::purple
    end

    subgraph "Deploy Phase"
        DS[Deployment Service]:::green
    end

    subgraph "Targets"
        STG[Staging]:::secondary
        PROD[Production]:::secondary
    end

    W -->|"Upload binary,<br/>Docker image,<br/>test reports"| AS
    AS --> OS
    OS --> DS
    DS --> STG
    DS --> PROD
...
sequenceDiagram
    ...
    W->>AS: POST /runs/{run_id}/artifacts (multipart upload)
    Note over AS: Validate size, check quotas
    AS->>OS: Stream file to storage bucket
    Note over OS: Store at path:<br/>artifacts/{run_id}/{stage}/{filename}
    OS-->>AS: Return storage key + checksum
    AS->>DB: Insert artifact record
    Note over DB: Store: artifact_id, run_id,<br/>storage_path, size, expires_at
    AS-->>W: Return artifact_id + signed download_url
```
...
sequenceDiagram
    ...
    PS->>DS: Trigger deploy (run_id, environment: staging)
    DS->>AS: Get artifact URL for run
    AS-->>DS: Return signed download URL
    DS->>SM: Fetch deployment secrets
    SM-->>DS: Return credentials
    DS->>K8s: Apply deployment manifest
    K8s-->>DS: Deployment started

    loop Until healthy or timeout
        DS->>HM: Check deployment health
        HM->>K8s: Query pod status, run health checks
        HM-->>DS: Health status
    end

    alt Deployment healthy
        DS-->>PS: Report success
    else Deployment unhealthy
        DS->>K8s: Rollback to previous version
        DS-->>PS: Report failure + rollback
    end
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        direction LR
        Dev[Developer]:::rose
        Git[Git Webhook]:::rose
        Schedule[Scheduler]:::rose
    end

    subgraph Gateway["Gateway Layer"]
        GW[API Gateway]:::secondary
    end
    ...
    subgraph Storage["Storage Layer"]
        PDB[(Pipeline DB)]:::purple
        OS[(Object Storage)]:::purple
        LogStore[(Log Storage)]:::purple
    end
    ...
    WM -.->|"scale & health check"| W1 & W2 & W3
...
flowchart LR
    ...
    subgraph "Not Good For"
        E[Large blob storage<br/>use object storage]:::red
        F[Time-series logs<br/>use log aggregator]:::red
    end
	
    subgraph "PostgreSQL Advantages"
        A[ACID transactions<br/>for status updates]:::green
        B[Flexible querying<br/>by multiple fields]:::green
        C[Foreign key relationships<br/>between tables]:::green
        D[Mature tooling<br/>and operations]:::green
    end	
...
```mermaid
flowchart LR
    A[Build]:::primary --> B[Unit Tests]:::primary
    A --> C[Lint]:::primary
    B --> D[Integration Tests]:::primary
    C --> D
    D --> E[Deploy Staging]:::green
    E --> F[Deploy Production]:::green
...
sequenceDiagram
    ...
    Note over PS: Build stage has no dependencies
    PS->>Q: Queue "build" stage
    W->>Q: Pick up job
    W->>W: Execute build commands
    W->>PS: StageCompleted(build, success)

    PS->>DB: Update build.status = success
    PS->>DB: Find stages depending on build
    Note over PS: Unit Tests and Lint both depend only on Build
    PS->>Q: Queue "unit-tests" stage
    PS->>Q: Queue "lint" stage

    par Unit Tests and Lint run in parallel
        W->>PS: StageCompleted(unit-tests, success)
        PS->>DB: Update unit-tests.status = success
    and
        W->>PS: StageCompleted(lint, success)
        PS->>DB: Update lint.status = success
    end

    PS->>DB: Check integration-tests dependencies
    Note over PS: Both unit-tests and lint are now success
    PS->>Q: Queue "integration-tests" stage
```
...
stateDiagram-v2
    ...
    Queued --> Running: First stage starts
...
stateDiagram-v2
    ...
    Pending --> Skipped: Skip condition met

    Running --> Success: Commands exit 0
    Running --> Failed: Commands exit non-zero
...
flowchart TB
    subgraph "Worker Node"
        subgraph "Container (Isolated)"
            FS[Fresh Filesystem]:::primary
            ENV[Environment Variables]:::primary
            SEC[Injected Secrets]:::orange
            SRC[Source Code<br/>read-only mount]:::secondary
            WS[Workspace<br/>read-write mount]:::secondary
        end

        LIMITS["Resource Limits<br/>CPU: 2 cores<br/>Memory: 4GB<br/>Disk: 10GB"]:::red

        CR[Container Runtime<br/>Docker / containerd]:::purple
    end

    subgraph "External Services"
        REG[(Container Registry)]:::purple
        SM[Secret Manager]:::orange
        GIT[(Git Provider)]:::purple
    end
...
flowchart TB
    ...
    subgraph "Priority Queues"
        Q1[Critical<br/>Production hotfixes]:::red
        Q2[High<br/>Main branch builds]:::orange
        Q3[Normal<br/>Feature branches]:::primary
        Q4[Low<br/>Scheduled jobs]:::secondary
    end
...
sequenceDiagram
    ...
    Note over Q: Job available
    W1->>Q: Fetch job
    Q->>W1: Job + 30 min visibility timeout
    Note over Q: Job now invisible

    loop Every 5 minutes
        W1->>Q: Heartbeat (extend timeout)
    end

    Note over W1: Worker crashes!

    Note over Q: 30 min passes, no heartbeat
    Note over Q: Job becomes visible again

    W2->>Q: Fetch job
    Q->>W2: Same job (retry #1)
    W2->>Q: Job complete
    Note over Q: Job deleted
```
...
```mermaid
flowchart TB
    START[Job Failed]:::red --> CHECK{Failure Type"}

    CHECK -->|"Infrastructure<br/>(network, crash)"| INFRA[Infrastructure Failure]:::orange
    CHECK -->|"Code Error<br/>(compile, test)"| CODE[Code Failure]:::red

    INFRA --> RETRY{Retry Count<br/>< Max"}

    RETRY -->|"Yes"| BACKOFF[Apply Exponential<br/>Backoff]:::primary
    RETRY -->|"No"| DLQ[Move to Dead<br/>Letter Queue]:::red

    BACKOFF --> REQUEUE[Requeue Job]:::primary
    REQUEUE --> EXECUTE[Execute Job]:::green

    EXECUTE --> SUCCESS{Success"}
    SUCCESS -->|"Yes"| DONE[Mark Complete]:::green
    SUCCESS -->|"No"| CHECK

    DLQ --> ALERT[Alert Operator]:::orange

    CODE --> FAIL[Mark as Failed]:::red
    FAIL --> NOTIFY[Notify Developer]:::primary
...
flowchart TB
    subgraph "Monitoring"
        QM[Queue Monitor]:::primary
        SM[Schedule Manager]:::primary
    end

    subgraph "Auto-Scaler"
        AS[Auto-Scaling Controller]:::primary
        DECIDE{Scale Decision}:::orange
    end

    subgraph "Worker Pool"
        BASE["Base Pool<br/>(always on)"]:::green
        DYN["Dynamic Pool<br/>(scales)"]:::primary
        SPOT["Spot Instances<br/>(cost-saving)"]:::secondary
    end

    subgraph "Signals"
        SIG1["Queue Depth > Threshold"]:::orange
        SIG2["Scheduled Peak Hours"]:::orange
        SIG3["Predictive ML Model"]:::purple
    end
    ...
    DECIDE -->|"Scale Up"| DYN
    DECIDE -->|"Scale Up (cheap)"| SPOT
    DECIDE -->|"Scale Down"| DYN

    BASE -.->|"min capacity"| AS
    DYN -.->|"current count"| AS
...
flowchart TB
    ...
    subgraph "Warm Start (<5 sec)"
		direction TB
        W1[Receive Job]:::primary --> W2[Image Cached]:::green
        W2 --> W3[Create Container]:::green
        W3 --> W4[Clone Source]:::orange
        W4 --> W5[Execute Build]:::green
    end

    subgraph "Cold Start (30+ sec)"
		direction TB
        C1[Receive Job]:::primary --> C2[Pull Image]:::red
        C2 --> C3[Create Container]:::orange
        C3 --> C4[Clone Source]:::orange
        C4 --> C5[Execute Build]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Image Analytics"
        USAGE[Usage Tracker]:::primary
        TOP10["Top 10 Images<br/>node:18, python:3.11,<br/>golang:1.21, etc."]:::orange
    end

    subgraph "Warm Pool Manager"
        WPM[Pool Manager]:::primary
    end

    subgraph "Workers"
        direction LR
        W1["Worker 1<br/>✓ node:18<br/>✓ python:3.11"]:::green
        W2["Worker 2<br/>✓ golang:1.21<br/>✓ java:17"]:::green
        W3["Worker N<br/>✓ ruby:3.2<br/>✓ rust:latest"]:::green
    end

    USAGE -->|"analyze"| TOP10
    TOP10 -->|"distribute"| WPM
    WPM -->|"pre-pull"| W1
    WPM -->|"pre-pull"| W2
    WPM -->|"pre-pull"| W3
