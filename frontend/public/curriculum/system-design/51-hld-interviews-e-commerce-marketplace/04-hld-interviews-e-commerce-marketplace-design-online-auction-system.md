---
id: "hld-interviews-e-commerce-marketplace-design-online-auction-system"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - E commerce Marketplace"
subSection: ""
title: "Design Online Auction System"
slug: "hld-interviews-e-commerce-marketplace-design-online-auction-system"
summary: "This system design problem tests your understanding of concurrency control, real-time communication, time-sensitive distributed operations, and consistency guarantees."
eli10: "Imagine Design Online Auction System as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Online Auction System Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph "Bid Traffic Pattern"
        direction LR
        A[Steady State<br/>580 QPS]:::primary --> B[Peak Hours<br/>1,740 QPS]:::orange
        B --> C[Hot Auction Ending<br/>10,000+ QPS burst]:::red
    end
...
```mermaid
flowchart TB
    subgraph "Real-time Scale"
        A[1M Active Auctions]:::primary
        B[50 Watchers per Auction<br/>Average]:::orange
        C[50M WebSocket<br/>Connections]:::purple
    end
...
flowchart LR
    subgraph "Core API Operations"
        POST1[POST /auctions<br/>Create listing]:::green
        POST2[POST /auctions/id/bids<br/>Place bid]:::primary
        GET1[GET /auctions/id<br/>View auction]:::orange
        GET2[GET /auctions/id/bids<br/>Bid history]:::orange
    end
...
```mermaid
flowchart TB
    subgraph "Write Path (Bid Heavy)"
        W1[Place Bid]:::orange --> W2[Strong Consistency<br/>Low Latency<br/>Concurrency Control]:::orange
    end

    subgraph "Read Path (View Heavy)"
        R1[View Auction]:::primary --> R2[Eventually Consistent OK<br/>Heavily Cached<br/>CDN Friendly]:::primary
    end
...
```mermaid
flowchart LR
    A[Seller]:::rose --> B[API Gateway]:::secondary
    B --> C[Auction Service]:::primary
    C --> D[(Auction DB)]:::purple
    C --> E[Search Index]:::orange
    C --> F[Event Bus]:::green
...
sequenceDiagram
    ...
    Seller->>API: POST /auctions (item details)
    Note over API: Validate auth,<br/>check rate limits
    API->>AS: Forward request
    AS->>AS: Validate inputs,<br/>generate auction_id
    AS->>DB: Insert auction record
    DB-->>AS: Confirm insertion
    AS->>Search: Publish auction_created event
    AS-->>API: Return auction details
    API-->>Seller: 201 Created with auction_url
```
...
```mermaid
flowchart TB
    subgraph "Bid Processing"
        A[Bidder]:::rose --> B[API Gateway]:::secondary
        B --> C[Bid Service]:::primary
        C --> D{Validate}:::orange
        D -->|Valid| E[Apply Bid<br/>Atomic Transaction]:::green
        D -->|Invalid| F[Reject<br/>400/409/410]:::red
        E --> G[(Database)]:::purple
        E --> H[Event Bus]:::green
    end
...
sequenceDiagram
    ...
    Bidder->>API: POST /auctions/123/bids
    API->>BS: Forward bid request
    BS->>DB: Fetch auction state (with lock)
    DB-->>BS: current_price: $150, status: active
    Note over BS: Validate: bid ($160) ><br/>current ($150) + increment ($5)"
    BS->>DB: Insert bid, update auction.current_price
    DB-->>BS: Confirm
    BS->>Events: Publish bid_placed event
    BS-->>API: Return success
    API-->>Bidder: 200 OK (you are winning)
```
...
flowchart TB
    subgraph "The Problem"
        U1[User A<br/>watching auction 123]:::rose
        U2[User B<br/>watching auction 123]:::rose
        U3[User C<br/>watching auction 123]:::rose

        WS1[WebSocket Server 1]:::primary
        WS2[WebSocket Server 2]:::primary
...
```mermaid
flowchart LR
    A[Bid Service]:::primary --> B[Message Broker<br/>Redis/Kafka]:::orange
    B --> C[WS Server 1]:::secondary
    B --> D[WS Server 2]:::secondary
    B --> E[WS Server N]:::secondary
    C --> F[Clients<br/>watching auctions]:::green
    D --> G[Clients<br/>watching auctions]:::green
    E --> H[Clients<br/>watching auctions]:::green
...
sequenceDiagram
    ...
    BS->>Broker: Publish bid_placed (auction: 123)
    Broker->>WS1: Event for auction 123
    Broker->>WS2: Event for auction 123
    par Push to all watchers
        WS1->>Clients: Update: new price $165
        WS2->>Clients: Update: new price $165
    end
```
...
sequenceDiagram
    ...
    Scheduler->>AS: Trigger closure (auction 123)
    AS->>DB: Lock auction, check status
    Note over AS: Verify still active,<br/>handle any in-flight bids
    AS->>DB: Update status = "ended" or "sold"
    AS->>AS: Determine winner
    AS->>Notify: Publish auction_closed event
    Notify->>Notify: Send winner notification
    Notify->>Notify: Send seller notification
    Notify->>Notify: Notify other bidders
```
...
```mermaid
flowchart TD
    A[Scheduler detects<br/>end time passed]:::primary --> B[Auction Service<br/>acquires lock]:::secondary
    B --> C{Already closed"}:::orange
    C -->|Yes| D[Return early]:::secondary
    C -->|No| E{Reserve price met"}:::orange
    E -->|Yes| F[Mark as SOLD<br/>Record winner]:::green
    E -->|No| G[Mark as ENDED<br/>No sale]:::red
    F --> H[Notification Service]:::purple
    G --> H

    H --> I[Winner Notification<br/>Payment instructions]:::green
    H --> J[Seller Notification<br/>Sale complete/failed]:::orange
    H --> K[Other Bidders<br/>Auction ended]:::secondary
...
flowchart TB
    ...
    subgraph Gateway["Gateway Layer"]
        AG[API Gateway]:::primary
        WSG[WebSocket Gateway]:::primary
    end

    subgraph Services["Application Layer"]
        AS[Auction Service]:::orange
        BS[Bid Service]:::orange
        SS[Search Service]:::orange
        NS[Notification Service]:::orange
        Sched[Scheduler Service]:::orange
    end
    ...
    subgraph Messaging["Message Layer"]
        Broker[Message Broker<br/>Kafka/Redis]:::purple
    end

    subgraph Storage["Storage Layer"]
        DB[(PostgreSQL)]:::purple
        ES[(Elasticsearch)]:::purple
        S3[(Object Storage)]:::purple
    end
...
flowchart TD
    ...
    subgraph "Elasticsearch (Search)"
        D[Auction Index]:::orange
    end

    subgraph "Redis (Cache)"
        E[Hot Auction Data]:::green
    end
...
flowchart LR
    subgraph "The Problem"
        A[Auction at $100]:::primary
        B[Alice bids $110]:::rose
        C[Bob bids $110]:::rose
        D[Who wins"]:::orange
    end
...
sequenceDiagram
    ...
    Alice->>Server: Bid $110
    Server->>DB: SELECT current_price, version<br/>WHERE auction_id = 123
    DB-->>Server: price=$100, version=5
    Note over Server: Validate: $110 > $100 + $5" Yes
    Server->>DB: UPDATE SET price=$110, version=6<br/>WHERE id=123 AND version=5
    alt Version unchanged
        DB-->>Server: 1 row updated
        Server-->>Alice: Bid accepted!
    else Version changed (conflict)
        DB-->>Server: 0 rows updated
        Server->>Server: Retry from start
    end
```
...
sequenceDiagram
    ...
    Note over DB: Row locked for Alice

    Bob->>DB: BEGIN
    Bob->>DB: SELECT ... FOR UPDATE
    Note over Bob,DB: Bob waits...

    Alice->>DB: UPDATE price
    Alice->>DB: INSERT bid
    Alice->>DB: COMMIT
    Note over DB: Lock released

    DB-->>Bob: Row available
    Bob->>DB: SELECT ... FOR UPDATE (continues)
    Note over Bob: Sees Alice's update
```
...
flowchart TB
    subgraph "Bid Router"
        R[Route by hash<br/>of auction_id]:::primary
    end

    subgraph "Bid Processors"
        P1[Processor 1<br/>Auctions A-F]:::orange
        P2[Processor 2<br/>Auctions G-M]:::orange
        P3[Processor 3<br/>Auctions N-Z]:::orange
    end
    ...
    P1 --> Q1[Single-threaded<br/>queue per auction]:::green
    P2 --> Q2[Single-threaded<br/>queue per auction]:::green
    P3 --> Q3[Single-threaded<br/>queue per auction]:::green
...
```mermaid
flowchart TB
    subgraph "Proxy Bidding Example"
        A[Auction starts at $20]:::primary
        B[Alice sets max = $100<br/>System bids $25]:::green
        C[Bob bids $30<br/>System bids $35 for Alice]:::orange
        D[Bob sets max = $50<br/>System compares: Alice $100 > Bob $50]:::orange
        E[Price becomes $55<br/>Alice leads at Bob's max + increment]:::green
    end
...
```mermaid
flowchart TD
    subgraph "Classic Snipe"
        A[Auction ends at 3:00:00 PM]:::primary
        B[At 2:59:58 PM<br/>Sniper bids $105]:::red
        C[Other bidders have<br/>no time to respond]:::orange
        D[Item sells for less<br/>than it might have]:::orange
    end
...
```mermaid
flowchart TB
    A[Bid placed]:::primary --> B{Less than 5 min<br/>remaining"}
    B -->|No| C[Process normally]:::green
    B -->|Yes| D{Extensions < max"}
    D -->|Yes| E[Extend by 5 minutes]:::orange
    D -->|No| F[Process normally<br/>at limit]:::green
    E --> G[Notify all watchers]:::secondary
...
flowchart LR
    subgraph "Redis Sorted Set"
        A[auction_endings]:::purple
        B["Score = end_timestamp<br/>Member = auction_id"]:::primary
    end
    ...
    A --> W1
    A --> W2
    W1 --> C[Close auction]:::green
    W2 --> D[Close auction]:::green
...
```mermaid
flowchart TB
    subgraph "Bid Placed on Auction 123"
        BS[Bid Service]:::primary
        PUB[PUBLISH auction:123<br/>new price: $150]:::orange
    end

    subgraph "Message Broker"
        CH[Channel: auction:123]:::purple
    end

    subgraph "WebSocket Servers"
        WS1[Server 1<br/>Subscribed to: 123, 456]:::secondary
        WS2[Server 2<br/>Subscribed to: 789]:::secondary
        WS3[Server 3<br/>Subscribed to: 123, 999]:::secondary
    end

    subgraph "Clients"
        C1[User A watching 123]:::green
        C2[User B watching 123]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Per WebSocket Server"
        A[Connection Registry]:::primary
        B[connection_id → list of auction_ids]:::orange
        C[auction_id → list of connection_ids]:::orange
    end
...
```mermaid
flowchart TB
    subgraph "Normal Auction"
        P1[Processor 1<br/>50 auctions<br/>100 bids/min]:::green
        P2[Processor 2<br/>50 auctions<br/>100 bids/min]:::green
        P3[Processor 3<br/>50 auctions<br/>100 bids/min]:::green
    end

    subgraph "Hot Auction Problem"
        H1[Processor 1<br/>50 auctions<br/>100 bids/min]:::green
        H2[Processor 2<br/>1 HOT auction<br/>5000 bids/min]:::red
        H3[Processor 3<br/>50 auctions<br/>100 bids/min]:::green
    end
