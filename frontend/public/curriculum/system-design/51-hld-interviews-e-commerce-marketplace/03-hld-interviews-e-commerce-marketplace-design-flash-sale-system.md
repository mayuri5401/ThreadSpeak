---
id: "hld-interviews-e-commerce-marketplace-design-flash-sale-system"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - E commerce Marketplace"
subSection: ""
title: "Design Flash Sale System"
slug: "hld-interviews-e-commerce-marketplace-design-flash-sale-system"
summary: "What makes flash sales particularly interesting is the asymmetry of outcomes. Out of 5 million purchase attempts, only 10,000 might succeed. That means 99.8% of requests result in failure, either due to sold-out inventory or system protection mech..."
eli10: "Imagine Design Flash Sale System as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Flash Sale System Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TD
    subgraph "Traffic Timeline"
        T1["T-60s<br/>100 QPS<br/>Users browsing"]:::secondary
        T2["T-10s<br/>10,000 QPS<br/>Users refreshing"]:::orange
        T3["T+0s<br/>500,000 QPS<br/>THUNDERING HERD"]:::red
        T4["T+10s<br/>400,000 QPS<br/>Still buying"]:::red
        T5["T+30s<br/>100,000 QPS<br/>Sold out rejections"]:::orange
        T6["T+5min<br/>1,000 QPS<br/>Stragglers leaving"]:::secondary
    end
...
flowchart LR
    subgraph "Request Outcomes"
        A[5 Million<br/>Purchase Attempts]:::primary --> B[10,000<br/>Successful]:::green
        A --> C[4.99 Million<br/>Sold Out]:::red
    end
...
flowchart LR
    subgraph "Core API Operations"
        GET[GET /flash-sales/id<br/>View sale details]:::primary
        POST1[POST /flash-sales/id/orders<br/>Reserve inventory]:::orange
        POST2[POST /orders/id/pay<br/>Complete payment]:::green
    end

    GET -->|"Sale active""| POST1
    POST1 -->|"Reserved""| POST2
    POST2 -->|"Paid""| DONE[Order Confirmed]:::green
...
flowchart TB
    subgraph "Request Flow "
        direction LR
        A[5M Requests]:::rose --> B[CDN<br/>Static Content]:::secondary
        B --> C[Rate Limiter<br/>Block Abuse]:::orange
        C --> D[Sold-Out Cache<br/>Fast Rejection]:::red
        D --> E[Inventory Check<br/>Redis Atomic]:::primary
        E --> F[10K Orders<br/>Database]:::purple
    end
...
sequenceDiagram
    ...
    User->>CDN: Load flash sale page
    CDN-->>User: Return cached static content (fast)

    User->>CDN: POST /orders (purchase request)
    ...
    alt Rate limit exceeded
        RL-->>API: Reject (429)
        API-->>User: Too Many Requests
    else Within limit
        RL->>Queue: Enqueue order request
        Queue-->>User: Request queued (202 Accepted)
        Note over User: User sees "Processing..."
        Queue->>OS: Dequeue at controlled rate
        OS->>OS: Process order
    end
```
...
flowchart TB
    subgraph "Race Condition"
        direction LR
        R1[Request 1<br/>Check: 1 left]:::primary
        R2[Request 2<br/>Check: 1 left]:::primary
        R3[Request 3<br/>Check: 1 left]:::primary

        D1[Decrement<br/>to 0]:::orange
        D2[Decrement<br/>to -1]:::red
        D3[Decrement<br/>to -2]:::red
...
flowchart LR
    ...
    subgraph Services["Processing Layer"]
        OS[Order Service]:::primary
        IS[Inventory Service]:::primary
    end

    subgraph Data["Data Layer"]
        Redis[(Redis<br/>Inventory Counter)]:::red
        DB[(PostgreSQL<br/>Order Records)]:::purple
    end
...
sequenceDiagram
    ...
    OS->>IS: Reserve item (item_id, quantity, user_id)
    IS->>Redis: Execute Lua script<br/>(atomic check-and-decrement)

    alt Inventory Available
        Redis-->>IS: Return remaining count (>= 0)
        IS->>DB: Create reservation record
        DB-->>IS: Reservation confirmed
        IS-->>OS: Success (reservation_id, expires_at)
        Note over OS: User has 10 minutes to pay
    else Sold Out
        Redis-->>IS: Return -1 (no inventory)
        IS-->>OS: Sold Out (409 Conflict)
        Note over OS: No database hit needed!
    end
```
...
flowchart LR
    ...
    subgraph Background["Background Workers"]
        TH[Timeout Handler]:::lightblue
    end

    subgraph External["External Services"]
        PG[Payment Gateway<br/>Stripe/Adyen]:::pink
    end
...
sequenceDiagram
    ...
    User->>OS: POST /orders/{id}/pay
    OS->>DB: Check reservation status
    Note over OS: Verify reservation exists<br/>and has not expired

    OS->>PS: Process payment (amount, payment_token)
    PS->>PG: Charge payment method

    alt Payment Successful
        PG-->>PS: Payment confirmed
        PS->>DB: Update order status to "confirmed"
        PS->>NS: Trigger confirmation
        NS-->>User: Order confirmed! (email + push)
        PS-->>User: 200 OK (confirmation_number)
    else Payment Failed
        PG-->>PS: Payment declined
        PS->>Redis: INCR inventory (release item)
        PS->>DB: Update order status to "payment_failed"
        PS-->>User: 402 Payment Failed (reason)
        Note over User: User can retry with<br/>different payment method
    end

    Note over TH: Background: Every 30 seconds
    TH->>DB: SELECT * FROM orders<br/>WHERE status='reserved'<br/>AND expires_at < NOW()
    loop For each expired reservation
        TH->>Redis: INCR inventory (release)
        TH->>DB: UPDATE status = 'expired'
    end
```
...
```mermaid
flowchart LR
    subgraph "Hot Path (During Sale)"
        A[Inventory Checks]:::red --> B[(Redis)]:::red
        C[Reservations]:::orange --> B
    end

    subgraph "Durable Storage"
        D[Order Records]:::primary --> E[(PostgreSQL)]:::purple
        F[Analytics]:::primary --> E
    end
...
flowchart TB
    subgraph "The Challenge"
        direction LR
        A[10,000 Requests]:::primary --> B[Single Counter<br/>inventory = 10000]:::orange
        B --> C[Need Exactly<br/>10,000 Successes]:::green
    end
...
```mermaid
flowchart LR
    subgraph "Database Locking"
        R1[Request 1<br/>Acquires Lock]:::green --> DB[(Database Row)]:::purple
        R2[Request 2<br/>Waits...]:::orange -.-> DB
        R3[Request 3<br/>Waits...]:::orange -.-> DB
    end
...
flowchart LR
    subgraph "Redis Atomic Operations"
        R1[Request 1]:::primary --> LUA[Lua Script<br/>Check + Decrement]:::orange
        R2[Request 2]:::primary --> LUA
        R3[Request 3]:::primary --> LUA
        LUA --> REDIS[(Redis<br/>inventory: 9997)]:::red
    end
...
```mermaid
flowchart LR
    subgraph "Phase 1: Fast (Synchronous)"
        A[Purchase Request]:::rose --> B[Redis Lua Script<br/>Atomic Decrement]:::red
        B --> C{Success"}
        C -->|Yes| D[Return Reservation ID]:::green
        C -->|No| E[Return Sold Out]:::orange
    end

    subgraph "Phase 2: Durable (Async)"
        D --> F[Write to PostgreSQL<br/>Async]:::purple
    end
...
```mermaid
flowchart TD
    subgraph "The Thundering Herd"
        A[Normal Traffic<br/>1,000 QPS]:::secondary --> B[Sale Starts<br/>T+0]:::red
        B --> C[Peak Traffic<br/>500,000 QPS]:::red
        C --> D[Sold Out<br/>T+10s]:::orange
        D --> E[Decline<br/>100,000 QPS]:::orange
    end
...
```mermaid
flowchart LR
    A[500K req/sec]:::red --> Q[Message Queue<br/>Capacity: 1M]:::orange
    Q --> S[Order Service<br/>Processing: 50K/sec]:::primary
...
```mermaid
flowchart LR
    A[Request]:::rose --> B{Sold Out<br/>Cache"}
    B -->|Yes| C[Reject Immediately<br/>at Edge]:::red
    B -->|No| D[Continue to<br/>Order Service]:::green
...
flowchart LR
    subgraph "Before Sale"
        A[Registration<br/>Opens T-5min]:::primary --> B[Users Register<br/>Interest]:::orange
    end

    subgraph "At Sale Start"
        B --> C[Random Selection<br/>of Winners]:::green
        C --> D[Winners Get<br/>Purchase Token]:::green
        C --> E[Non-Winners Get<br/>Notification]:::red
    end
...
```mermaid
flowchart LR
    A[User Arrives]:::rose --> B{Load<br/>Threshold"}
    B -->|Below| C[Direct to<br/>Sale Page]:::green
    B -->|Above| D[Waiting Room<br/>Position #4,521]:::orange
    D --> E[Admitted When<br/>Capacity Available]:::green
...
```mermaid
flowchart LR
    A[Incoming Traffic]:::rose --> B[Virtual Waiting Room<br/>Controls Admission]:::secondary
    B --> C[API Gateway<br/>Rate Limiting]:::orange
    C --> D[Request Queue<br/>Buffers Spikes]:::orange
    D --> E[Sold-Out Cache<br/>Early Rejection]:::red
    E --> F[Order Service<br/>Inventory Check]:::primary
...
flowchart TD
    subgraph "Human User"
        H1[See Countdown]:::rose --> H2[Click Button<br/>~200ms]:::rose --> H3[1 Request]:::secondary
    end

    subgraph "Bot"
        B1[Detect Countdown]:::red --> B2[Auto-Submit<br/>~1ms]:::red --> B3[500 Requests]:::red
    end
...
flowchart TD
    subgraph "Timeline"
        T1["10:00:00<br/>User A reserves<br/>last item"]:::green
        T2["10:05:00<br/>User B wants item<br/>sees 'sold out'"]:::red
        T3["10:08:00<br/>User A abandons<br/>(no payment)"]:::orange
        T4["10:10:00<br/>Reservation expires<br/>item released"]:::secondary
        T5["10:10:01<br/>User B already<br/>gave up"]:::red
    end
...
```mermaid
flowchart LR
    A[Create<br/>Reservation]:::green --> B[Enqueue Delayed<br/>Message]:::orange
    B --> C[Message Queue<br/>Holds Until expires_at]:::primary
    C --> D[Message Delivered<br/>at T+10min]:::orange
    D --> E{Still<br/>Reserved"}
    E -->|Yes| F[Release<br/>Inventory]:::secondary
    E -->|No| G[Ignore<br/>Already Paid]:::secondary
...
flowchart LR
    subgraph "Request Routing"
        A[Incoming<br/>Requests]:::rose --> LB[Load Balancer<br/>Hash by item_id]:::primary
        LB --> S1[Order Service 1<br/>Items A-M]:::orange
        LB --> S2[Order Service 2<br/>Items N-Z]:::orange
    end

    S1 --> R[(Redis Cluster)]:::red
    S2 --> R
...
flowchart LR
    subgraph "Redis Cluster"
        direction TB
        M1[Primary 1<br/>Slots 0-5460]:::red
        M2[Primary 2<br/>Slots 5461-10922]:::red
        M3[Primary 3<br/>Slots 10923-16383]:::red
        R1[Replica 1]:::orange
        R2[Replica 2]:::orange
        R3[Replica 3]:::orange
        M1 --> R1
