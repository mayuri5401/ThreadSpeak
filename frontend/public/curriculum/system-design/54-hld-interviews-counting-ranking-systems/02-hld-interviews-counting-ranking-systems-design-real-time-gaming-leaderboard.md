---
id: "hld-interviews-counting-ranking-systems-design-real-time-gaming-leaderboard"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Counting Ranking Systems"
subSection: ""
title: "Design Real-Time Gaming Leaderboard"
slug: "hld-interviews-counting-ranking-systems-design-real-time-gaming-leaderboard"
summary: "What makes this problem interesting from a system design perspective is the fan-out challenge."
eli10: "Imagine Design Real-Time Gaming Leaderboard as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Real-Time Gaming Leaderboard Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph Update[Score Update]
        U[1 score change<br/>in top 100]:::orange
    end

    subgraph Fanout[Fan-out Challenge]
        F[Must notify<br/>8M viewers]:::red
    end

    subgraph Result[Result]
        R[800M messages/sec<br/>at 100 updates/sec]:::red
    end
...
```mermaid
flowchart TB
    subgraph "HTTP APIs (Request/Response)"
        POST[POST /scores<br/>Submit score]:::green
        GET1[GET /rankings<br/>Get leaderboard]:::primary
        GET2[GET /participants/id<br/>Get rank]:::primary
    end

    subgraph "WebSocket API (Persistent)"
        WS[ws://subscribe<br/>Live updates]:::orange
    end

    POST --> |"Returns"| R1[New rank + delta]:::secondary
    GET1 --> |"Returns"| R2[Top N rankings]:::secondary
    GET2 --> |"Returns"| R3[Participant position]:::secondary
    WS --> |"Pushes"| R4[Rank changes]:::secondary
...
```mermaid
flowchart TB
    subgraph "Write Path (10K updates/sec)"

        W1[Participant]:::rose --> W2[API Gateway]:::secondary
        W2 --> W3[Score Service]:::primary
        W3 --> W4[(Redis)]:::purple
        W3 --> W5[Event Bus]:::orange
    end
...
```mermaid
flowchart TB
    subgraph "Read Path (10M concurrent connections)"

        R1[Event Bus]:::orange --> R2[Fan-out Layer]:::primary
        R2 --> R3[WebSocket Gateways]:::secondary
        R3 --> R4[Viewers]:::green
    end
...
flowchart TB
    ...
    subgraph Application["Application Layer"]
        SS[Score Service]:::primary
    end

    subgraph Storage["Storage Layer"]
        RD[(Redis Sorted Set)]:::purple
    end
    ...
    U -->|"1. POST /scores"| AG
    AG -->|"2. Forward request"| SS
    SS -->|"3. Get current rank"| RD
    RD -->|"4. Current rank: 5"| SS
    SS -->|"5. Update score"| RD
    SS -->|"6. Get new rank"| RD
    RD -->|"7. New rank: 3"| SS
    SS -->|"8. Publish event"| EB
    AG -->|"9. Return new rank"| U
...
sequenceDiagram
    ...
    EB->>FO: RankChangeEvent<br/>(leaderboard:123, ranks 3-5)

    FO->>SM: Who subscribes to top 100<br/>of leaderboard:123"
    SM-->>FO: [connection_ids...]

    FO->>SM: Who tracks user_456"
    SM-->>FO: [connection_ids...]

    FO->>WS: Send update to these connections
    WS->>V: Push rank change message
```
...
```mermaid
flowchart TB
    subgraph Event[Score Update at Rank 5]
        E[User moved to<br/>rank 5]:::orange
    end

    subgraph Subs[Subscription Types]
        S1[Top 100 watchers<br/>Must notify]:::red
        S2[Participant trackers<br/>Must notify if tracking<br/>affected users]:::green
        S3[Rank 500-600 watchers<br/>Skip notification]:::secondary
    end
...
flowchart TB
    ...
    subgraph L1[Level 1: Fan-out Services]
        F1[Fan-out 1]:::primary
        F2[Fan-out 2]:::primary
        F3[Fan-out 3]:::primary
    end

    subgraph L2[Level 2: WebSocket Gateways]
        W1[WS Gateway 1<br/>~1M connections]:::secondary
        W2[WS Gateway 2<br/>~1M connections]:::secondary
        W3[WS Gateway 3<br/>~1M connections]:::secondary
        W4[WS Gateway 4<br/>~1M connections]:::secondary
        W5[WS Gateway 5<br/>~1M connections]:::secondary
        W6[WS Gateway 6<br/>~1M connections]:::secondary
    end

    subgraph Viewers[10M Viewers]
        V[Connected Viewers]:::green
    end
...
```mermaid
flowchart LR
    V[New Viewer<br/>Location: Tokyo]:::rose --> CR[Connection Router]:::primary

    CR --> |"Least loaded<br/>+ Geographic"| W1[WS Gateway<br/>Tokyo Region]:::green
    CR -.-> W2[WS Gateway<br/>US West]:::secondary
    CR -.-> W3[WS Gateway<br/>EU]:::secondary
...
flowchart TB
    subgraph Clients[Client Layer]
        P[Participants<br/>Submit Scores]:::rose
        V[Viewers<br/>Watch Rankings]:::green
    end

    subgraph Edge[Edge Layer]
        AG[API Gateway]:::secondary
        CR[Connection Router]:::secondary
    end

    subgraph Services[Application Layer]
        SS[Score Service]:::primary
        LS[Leaderboard Service]:::primary
        SM[Subscription Manager]:::primary
    end

    subgraph Messaging[Event Layer]
        EB[Event Bus<br/>Kafka]:::orange
    end

    subgraph Fanout[Fan-out Layer]
        FO1[Fan-out 1]:::primary
        FO2[Fan-out 2]:::primary
        FO3[Fan-out 3]:::primary
    end

    subgraph WebSocket[WebSocket Layer]
        WS1[WS Gateway 1]:::secondary
        WS2[WS Gateway 2]:::secondary
        WS3[WS Gateway 3]:::secondary
    end

    subgraph Storage[Storage Layer]
        RD[(Redis Cluster)]:::purple
...
flowchart TB
    subgraph Hot[Hot Storage - Redis]
        R1[Live Rankings<br/>Sorted Sets]:::red
        R2[Subscriptions<br/>Sets]:::red
        R3[Connection State<br/>Hashes]:::red
    end

    subgraph Warm[Warm Storage - PostgreSQL]
        P1[Final Rankings]:::orange
        P2[Score Events<br/>Audit Log]:::orange
        P3[Leaderboard<br/>Config]:::orange
    end

    subgraph Access[Access Patterns]
        A1[Real-time reads<br/>< 1ms latency]:::green
        A2[Historical queries<br/>< 100ms latency]:::secondary
    end
    ...
    Hot -->|"Event ends:<br/>Snapshot to PostgreSQL"| Warm
...
flowchart TB
    ...
    subgraph SSE[Server-Sent Events]
		direction TB
        SSE1[Client]:::primary -->|"Connect once"| SSE2[Server]:::secondary
        SSE2 -->|"Stream updates"| SSE1
    end

    subgraph WebSocket[WebSocket]
		direction TB
        WS1[Client]:::primary <-->|"Persistent connection"| WS2[Server]:::secondary
    end

    subgraph LongPolling[Long Polling]
		direction TB
        LP1[Client]:::primary -->|"Request & wait"| LP2[Server]:::secondary
        LP2 -->|"Response when ready"| LP1
    end

    subgraph Polling[Polling]
		direction TB
        P1[Client]:::primary -->|"Request every 1s"| P2[Server]:::secondary
        P2 -->|"Full response"| P1
    end
...
```mermaid
flowchart TB
    subgraph SortedSet[Redis Sorted Set Internal Structure]
        subgraph SkipList[Skip List - O log N ordering]
            L3[Level 3]:::purple --> N1_3[15000]:::primary
            L2[Level 2]:::purple --> N1_2[15000]:::primary --> N2_2[14800]:::primary
            L1[Level 1]:::purple --> N1_1[15000]:::primary --> N2_1[14800]:::primary --> N3_1[14500]:::primary --> N4_1[14200]:::primary
        end

        subgraph HashMap[Hash Map - O 1 lookup]
            H1[user_001 → 15000]:::orange
            H2[user_002 → 14800]:::orange
            H3[user_003 → 14500]:::orange
            H4[user_004 → 14200]:::orange
        end
    end
...
```mermaid
flowchart LR
    subgraph Problem[The Tie-Breaking Problem]
        A[User A<br/>Score: 1000<br/>Time: 10:00:05]:::primary
        B[User B<br/>Score: 1000<br/>Time: 10:00:03]:::primary
        Q[Who ranks<br/>higher"]:::orange
    end

    subgraph Solution[Composite Score Solution]
        S1[Base Score]:::green
        S2[+]:::secondary
        S3[Inverted<br/>Timestamp]:::green
        S4[= Single<br/>Sortable Value]:::purple
    end
...
```mermaid
flowchart TB
    subgraph Updates[Score Update]
        U[Rank 5 changed]:::orange
    end

    subgraph Segments[Subscription Segments]
        S1[Top 10 watchers<br/>500K viewers]:::primary
        S2[Top 100 watchers<br/>7.5M viewers]:::primary
        S3[Rank 100-1000<br/>1M viewers]:::secondary
        S4[Specific participant<br/>trackers]:::secondary
    end
...
sequenceDiagram
    ...
    Note over S1,S2: t=0: Both servers process updates
    S1->>S1: User A scores 100
    S2->>S2: User B scores 150

    Note over S1,S2: t=1: Both broadcast (race condition)

    S1-->>V: A is rank 1 (score 100)
    Note over V: Viewer sees A at rank 1

    S2-->>V: B is rank 1 (score 150)
    Note over V: Wait... B should be rank 1!

    rect rgb(255, 135, 135)
        Note over V: Inconsistent state!<br/>Viewer briefly saw wrong ranking
    end
```
...
```mermaid
flowchart TB
    subgraph Viewers[10 Milon Viewers]
        V1[Viewers<br/>Regi A]:::primary
        V2[Viewers<br/>Regi B]:::primary
        V3[Viewers<br/>Regi C]:::primary
    end

    subgraph Router[Connectn Router]
        CR[Load Balancer<brGeographic + Load-based]:::orange
    end

    subgraph Gateways[WebSoet Gateways]
        G1[Gateway 1<br/>1Monnections]:::secondary
        G2[Gateway 2<br/>1Monnections]:::secondary
        G3[Gateway 3<br/>1M connections]:::secondary
        G4[Gateway 4<br/>1M connections]:::secondary
        G5[Gateway 5<br/>1M connections]:::secondary
        G6[...<br/>+ redundancy]:::secondary
    end
...
flowchart TB
    subgraph Normal[Normal Operation]
        N1[Connected]:::green -->|"Gateway crash"| D[Disconnected]:::red
    end

    subgraph Recovery[Recovery Process]
        D -->|"Immediate"| R1[Reconnect attempt 1<br/>delay: 1s]:::orange
        R1 -->|"Fail"| R2[Reconnect attempt 2<br/>delay: 2s]:::orange
        R2 -->|"Fail"| R3[Reconnect attempt 3<br/>delay: 4s]:::orange
        R3 -->|"..."| RN[Attempt N<br/>max delay: 30s]:::orange
    end

    subgraph Resync[Resync After Connect]
        RN -->|"Success"| S1[Send last known<br/>version number]:::primary
        R1 -->|"Success"| S1
        R2 -->|"Success"| S1
        R3 -->|"Success"| S1
        S1 --> S2{Gap too large"}
        S2 -->|"No"| S3[Receive missed<br/>delta updates]:::green
        S2 -->|"Yes"| S4[Fetch full<br/>leaderboard]:::primary
        S3 --> N1
        S4 --> N1
    end
...
```mermaid
flowchart TB
    subgraph Failures[Potential Failure Points]
        F1[Redis Failure]:::red
        F2[WebSocket Gateway<br/>Failure]:::red
        F3[Event Bus<br/>Failure]:::red
    end

    subgraph Mitigations[Mitigation Strategies]
        M1[Redis Sentinel<br/>Auto-failover]:::green
        M2[Client Reconnection<br/>Exponential Backoff]:::green
        M3[Direct Push Fallback<br/>+ Client Polling]:::green
    end

    subgraph Patterns[Resilience Patterns]
        P1[Circuit Breaker]:::orange
        P2[Queue Buffering]:::orange
        P3[Graceful Degradation]:::orange
    end
