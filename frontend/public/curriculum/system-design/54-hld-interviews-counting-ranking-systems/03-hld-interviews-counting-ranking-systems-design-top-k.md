---
id: "hld-interviews-counting-ranking-systems-design-top-k"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Counting Ranking Systems"
subSection: ""
title: "Design Top K"
slug: "hld-interviews-counting-ranking-systems-design-top-k"
summary: "This problem tests your understanding of streaming algorithms, distributed counting, and trade-offs between precision and scalability."
eli10: "Imagine Design Top K as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Top K Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Event Ingestion"
        direction TB
        EI1[Average: 50K/sec]:::primary
        EI2[Peak: 100K/sec]:::orange
        EI3[Daily: 4.3B events]:::primary
    end

    subgraph "Query Traffic"
        direction TB
        QT1[~1K queries/sec]:::secondary
        QT2[Mostly Top 10/100]:::secondary
        QT3[< 100ms latency]:::secondary
    end
...
```mermaid
flowchart LR
    subgraph "Traffic Distribution (Zipf's Law)"
        direction TB
        H1[Top 1% queries<br/>50% of traffic]:::green
        H2[Top 10% queries<br/>80% of traffic]:::orange
        H3[Long tail 90%<br/>20% of traffic]:::rose
    end

    H1 --> A[Must track accurately]:::primary
    H2 --> B[Track with good precision]:::primary
    H3 --> C[Can approximate or skip]:::secondary
...
flowchart LR
    subgraph "API Operations"
        POST[POST /events<br/>Ingest events]:::orange
        GET[GET /top-k<br/>Query results]:::primary
    end

    POST --> |Async acknowledgment| ACK[Event accepted]:::green
    GET --> |Cached response| RES[Top K items]:::green
...
```mermaid
flowchart TB
    subgraph "Event Ingestion Pipeline"
        C[Clients]:::rose
        AG[API Gateway]:::secondary
        K[Kafka]:::orange
        IS1[Ingestion Service 1]:::primary
        IS2[Ingestion Service 2]:::primary
        IS3[Ingestion Service 3]:::primary
        CS[(Count Store)]:::purple
    end

    C -->|"1. POST /events"| AG
    AG -->|"2. Publish"| K
    K -->|"3. Consume"| IS1 & IS2 & IS3
    IS1 & IS2 & IS3 -->|"4. Update counts"| CS
...
sequenceDiagram
    ...
    participant Store as Count Store

    Client->>Gateway: POST /events (batch of 100)
    Note over Gateway: Validate request format
    Gateway->>Kafka: Publish events to partitions
    Kafka-->>Gateway: Ack
    Gateway-->>Client: 202 Accepted

    Note over Kafka,IS: Async processing begins

    loop Every few milliseconds
        IS->>Kafka: Poll for events
        Kafka-->>IS: Return event batch
        Note over IS: Increment local counters
    end

    loop Every 5 seconds
        IS->>Store: Flush local counts
        Note over Store: Merge with existing counts
    end
```
...
```mermaid
flowchart TB
    subgraph "Top K Computation"
        CS[(Count Store)]:::purple
        AS[Aggregation Service]:::orange
        RC[(Result Cache)]:::green
        SCH[Scheduler]:::secondary
    end

    SCH -->|"Trigger every N seconds"| AS
    AS -->|"Read counts"| CS
    AS -->|"Compute Top K"| AS
    AS -->|"Store results"| RC
...
```mermaid
flowchart LR
    C[Client]:::rose --> QS[Query Service]:::primary
    QS --> RC[(Result Cache)]:::green
    RC --> QS
    QS --> C
...
sequenceDiagram
    ...
    Client->>QS: GET /top-k"k=10&window=1h
    QS->>Cache: Get "topk:search:1h"
    Cache-->>QS: Return pre-computed list
    QS-->>Client: 200 OK with top 10 items
    Note over Client,QS: Total latency: ~10ms
```
...
```mermaid
flowchart TB
    subgraph "Bucketing Strategy"
        direction TB
        B1[1-second buckets<br/>for 1-minute window<br/>60 buckets]:::primary
        B2[1-minute buckets<br/>for 1-hour window<br/>60 buckets]:::primary
        B3[1-hour buckets<br/>for 24-hour window<br/>24 buckets]:::primary
        B4[1-day buckets<br/>for 7-day window<br/>7 buckets]:::primary
    end

    B1 & B2 & B3 & B4 --> AGG[Sum relevant buckets<br/>to compute window total]:::orange
...
```mermaid
flowchart TD
    subgraph "Hopping Windows (1-hour with 1-minute hop)"
        W1[10:00 - 11:00]:::primary
        W2[10:01 - 11:01]:::primary
        W3[10:02 - 11:02]:::primary
    end
...
flowchart TB
    ...
    subgraph Ingestion["Ingestion Layer"]
        K[Kafka]:::orange
        IS1[Ingestion<br/>Service]:::primary
        IS2[Ingestion<br/>Service]:::primary
        IS3[Ingestion<br/>Service]:::primary
    end

    subgraph Storage["Storage Layer"]
        Redis[(Redis Cluster<br/>Counters + Results)]:::purple
        TSDB[(Time-Series DB<br/>Historical)]:::purple
    end

    subgraph Computation["Computation Layer"]
        AGG[Aggregation<br/>Service]:::green
        SCH[Scheduler]:::secondary
    end
...
flowchart TD
    subgraph "Hot Storage"
        H1[Real-time counters]:::orange
        H2[Pre-computed Top K]:::orange
        H3[Time-bucketed data]:::orange
    end

    subgraph "Cold Storage"
        C1[Historical counts]:::purple
        C2[Long-term trends]:::purple
        C3[Analytics data]:::purple
    end

    H1 & H2 & H3 -->|"Sub-ms access"| R[(Redis)]:::red
    C1 & C2 & C3 -->|"Analytical queries"| T[(Time-Series DB)]:::purple
...
```mermaid
flowchart LR
    subgraph "Redis Hash Structure"
        K1[Key: counts:search:2024011510]:::primary
        F1["iphone 16: 45230"]:::orange
        F2["taylor swift: 32100"]:::orange
        F3["super bowl: 28900"]:::orange
        F4["...more items..."]:::secondary
    end
...
```mermaid
flowchart LR
    E[Event: iphone 16]:::rose --> HM[Hash Map]:::primary
    HM --> |"Lookup + Increment"| C["iphone 16: 145231"]:::green
...
flowchart TB
    subgraph "Count-Min Sketch Structure"
        direction TB
        R1[Row 1: hash₁]:::primary
        R2[Row 2: hash₂]:::primary
        R3[Row 3: hash₃]:::primary
        R4[Row 4: hash₄]:::primary
    end

    subgraph "Counter Array"
        C1["[5][12][3][8][2][7]..."]:::orange
        C2["[7][4][15][2][9][1]..."]:::orange
        C3["[3][9][6][11][4][8]..."]:::orange
        C4["[8][2][7][5][12][3]..."]:::orange
    end
...
```mermaid
flowchart TD
    E[New Event]:::rose --> C{Item in table"}
    C -->|Yes| I[Increment count]:::green
    C -->|No| F{Table full"}
    F -->|No| A[Add item with count=1]:::primary
    F -->|Yes| R[Replace min-count item<br/>Inherit its count + 1]:::orange
...
```mermaid
flowchart TB
    E[Event]:::rose --> C{Is item in<br/>Hot Tier"}
    C -->|Yes| H[Update exact count<br/>in Hot Tier]:::green
    C -->|No| CMS[Update Count-Min Sketch]:::primary
    CMS --> P{Estimated count ><br/>promotion threshold"}
    P -->|Yes| PR[Promote to Hot Tier]:::orange
    P -->|No| D[Done]:::secondary

    subgraph "Hot Tier (Redis)"
        HT[Top 10,000 items<br/>Exact counts]:::green
    end

    subgraph "Warm Tier (CMS)"
        WT[All other items<br/>Approximate counts]:::primary
    end
...
```mermaid
flowchart TB
    E[Events]:::rose --> P[Partitioner<br/>hash item mod N]:::primary
    P --> P0[Partition 0<br/>items 0-33%]:::orange
    P --> P1[Partition 1<br/>items 34-66%]:::orange
    P --> P2[Partition 2<br/>items 67-100%]:::orange

    P0 & P1 & P2 --> M[Merge for Global Top K]:::green
...
flowchart TB
    subgraph "Ingestion Nodes"
        N1[Node 1<br/>Local counts]:::primary
        N2[Node 2<br/>Local counts]:::primary
        N3[Node 3<br/>Local counts]:::primary
    end

    subgraph "Central Store"
        R[(Redis<br/>Global counts)]:::purple
    end

    N1 & N2 & N3 -->|"Flush every 5 sec<br/>HINCRBY"| R
...
```mermaid
flowchart TD
    K[Kafka Topic<br/>Partitioned by item]:::orange --> KS1[Kafka Streams<br/>Processor 1]:::primary
    K --> KS2[Kafka Streams<br/>Processor 2]:::primary
    K --> KS3[Kafka Streams<br/>Processor 3]:::primary

    KS1 --> R1[(RocksDB<br/>Local State)]:::purple
    KS2 --> R2[(RocksDB<br/>Local State)]:::purple
    KS3 --> R3[(RocksDB<br/>Local State)]:::purple
...
```mermaid
flowchart TD
    S[Scan items]:::rose --> C{Heap size < K"}
    C -->|Yes| P[Push item to heap]:::green
    C -->|No| CM{Item count ><br/>heap min"}
    CM -->|Yes| R[Pop min, push item]:::orange
    CM -->|No| SK[Skip item]:::secondary
...
```mermaid
flowchart LR
    U[Count Update<br/>item X: +1]:::rose --> C{X in Top K"}
    C -->|Yes| UP[Update position]:::green
    C -->|No| CK{X.count ><br/>K-th item"}
    CK -->|Yes| I[Insert X, evict K-th]:::orange
    CK -->|No| D[No change needed]:::secondary
...
flowchart TB
    ...
    subgraph "Sliding Windows (continuous)"
        S1["[now-1h, now)"]:::green
        S2["Updates every event"]:::secondary
    end

    subgraph "Hopping Windows (30min hop)"
        H1["[10:00-11:00)"]:::orange
        H2["[10:30-11:30)"]:::orange
        H3["[11:00-12:00)"]:::orange
    end
	
    subgraph "Tumbling Windows"
        T1["[10:00-11:00)"]:::primary
        T2["[11:00-12:00)"]:::primary
        T3["[12:00-13:00)"]:::primary
        T1 --- T2 --- T3
    end	
...
flowchart LR
    subgraph "Traffic Distribution"
        TS[Taylor Swift<br/>50,000/sec]:::red
        IS[iPhone 16<br/>5,000/sec]:::orange
        OT[Other items<br/>< 100/sec each]:::secondary
    end
...
flowchart TB
    subgraph "Failure Scenarios"
        K[Kafka Failure]:::orange
        R[Redis Failure]:::orange
        I[Ingestion Crash]:::orange
        A[Aggregation Crash]:::orange
    end

    K --> KB[Buffer at gateway<br/>Replay when recovered]:::green
    R --> RB[Redis Cluster failover<br/>Lose last flush interval]:::green
    I --> IB[Kafka rebalances<br/>Lose local buffer]:::green
    A --> AB[Serve stale cache<br/>Recover and recompute]:::green
