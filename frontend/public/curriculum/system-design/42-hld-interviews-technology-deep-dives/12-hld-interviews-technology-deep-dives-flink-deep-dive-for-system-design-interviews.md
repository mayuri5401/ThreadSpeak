---
id: "hld-interviews-technology-deep-dives-flink-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Flink Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-flink-deep-dive-for-system-design-interviews"
summary: "Imagine you are building a fraud detection system that needs to analyze millions of credit card transactions per second, correlate them with user behavior patterns, and flag suspicious activity before the transaction completes."
eli10: "Imagine Flink Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Flink Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Need real-time processing"]:::primary
    A -->|Yes| B[Need stateful computations"]:::primary
    A -->|No| C[Consider batch processing]:::red
    B -->|Yes| D[Need exactly-once guarantees"]:::primary
    B -->|No| E[Consider Kafka Streams]:::orange
    D -->|Yes| F[Need complex windowing"]:::primary
    D -->|No| G[Flink or Kafka Streams]:::orange
    F -->|Yes| H[Flink is a good fit]:::green
    F -->|No| H
...
```mermaid
flowchart LR
    S1[Source Stream]:::primary --> T1[Filter]:::orange
    T1 --> T2[Map]:::orange
    T2 --> T3[KeyBy]:::orange
    T3 --> T4[Window]:::orange
    T4 --> T5[Aggregate]:::orange
    T5 --> Sink[Sink]:::green
...
```mermaid
flowchart TD
    subgraph Source["Source (Parallelism 2)"]
        S1[Source 1]:::primary
        S2[Source 2]:::primary
    end

    subgraph Map["Map (Parallelism 4)"]
        M1[Map 1]:::orange
        M2[Map 2]:::orange
        M3[Map 3]:::orange
        M4[Map 4]:::orange
    ...
    subgraph KeyBy["KeyBy + Aggregate (Parallelism 2)"]
        K1[Keyed 1]:::green
        K2[Keyed 2]:::green
    end
...
```mermaid
flowchart TB
    subgraph Forward["Forward (Same parallelism, chained)"]
        F1[Task 0] --> F2[Task 0]
        F3[Task 1] --> F4[Task 1]
    end

    subgraph Hash["Hash Partitioning (keyBy)"]
        H1[Source] --> H2[Task 0: keys A,C]
        H1 --> H3[Task 1: keys B,D]
    end

    subgraph Broadcast["Broadcast (to all)"]
        B1[Config] --> B2[Task 0]
        B1 --> B3[Task 1]
        B1 --> B4[Task 2]
    end
    ...
    class F1,F3,H1,B1 primary
    class F2,F4,H2,H3,B2,B3,B4 orange
```
...
flowchart TD
    subgraph Client
        C[Flink Client]:::primary
    end

    subgraph JobManager["JobManager (Master)"]
        JM[JobManager]:::purple
        D[Dispatcher]:::purple
        RM[ResourceManager]:::purple
    end

    subgraph TaskManagers["TaskManagers (Workers)"]
        TM1[TaskManager 1]:::green
        TM2[TaskManager 2]:::green
        TM3[TaskManager 3]:::green
    end

    subgraph Storage
        CP[(Checkpoint<br/>Storage)]:::orange
        HS[(State<br/>Backend)]:::orange
    end
...
flowchart TD
    subgraph TM1["TaskManager 1 (4 slots)"]
        S1[Slot 1<br/>Source → Map]:::primary
        S2[Slot 2<br/>Source → Map]:::primary
        S3[Slot 3<br/>KeyedAgg]:::orange
        S4[Slot 4<br/>Sink]:::green
    end

    subgraph TM2["TaskManager 2 (4 slots)"]
        S5[Slot 1<br/>Source → Map]:::primary
        S6[Slot 2<br/>Source → Map]:::primary
        S7[Slot 3<br/>KeyedAgg]:::orange
        S8[Slot 4<br/>Sink]:::green
    end
...
```mermaid
flowchart TB
    subgraph JobGraph["Job Graph (Logical)"]
        direction LR
        JG1[Source]:::primary --> JG2[Map]:::primary --> JG3[KeyBy]:::primary --> JG4[Aggregate]:::primary --> JG5[Sink]:::primary
    end

    subgraph ExecutionGraph["Execution Graph (Physical, parallelism 2)"]
        direction TB
        S0[Source 0]:::orange --> M0[Map 0]:::orange --> A0[Aggregate 0]:::green --> SK0[Sink 0]:::purple
        S1[Source 1]:::orange --> M1[Map 1]:::orange --> A1[Aggregate 1]:::green --> SK1[Sink 1]:::purple
        M0 --> A1
        M1 --> A0
    end
...
flowchart TD
    subgraph HA["High Availability Setup"]
        JM1[JobManager 1<br/>Leader]:::green
        JM2[JobManager 2<br/>Standby]:::orange
        JM3[JobManager 3<br/>Standby]:::orange
    end

    ZK[ZooKeeper<br/>Leader Election]:::purple
    ...
    subgraph Storage
        S3[(S3/HDFS<br/>Checkpoint Storage)]:::primary
    end
...
```mermaid
flowchart TD
    S[State Types]:::primary --> KS[Keyed State]:::orange
    S --> OS[Operator State]:::orange

    KS --> VS[ValueState]:::green
    KS --> LS[ListState]:::green
    KS --> MS[MapState]:::green
    KS --> RS[ReducingState]:::green
    KS --> AS[AggregatingState]:::green

    OS --> LOS[ListState]:::purple
    OS --> BOS[BroadcastState]:::purple
...
```mermaid
flowchart LR
    subgraph HashMapBackend["HashMapStateBackend"]
        HM[In-Memory<br/>HashMap]:::primary
        HMC[Fast<br/>Small State]:::green
    end

    subgraph RocksDBBackend["EmbeddedRocksDBStateBackend"]
        RDB[(RocksDB<br/>On Disk)]:::purple
        RDBC[Scalable<br/>Large State]:::orange
    end
...
flowchart LR
    subgraph EventTime["Event Time"]
        ET[When event<br/>occurred]:::green
        ETE[Embedded in<br/>event data]:::green
    end

    subgraph ProcessingTime["Processing Time"]
        PT[When event<br/>processed]:::orange
        PTE[System clock<br/>of operator]:::orange
    end

    subgraph IngestionTime["Ingestion Time"]
        IT[When event<br/>entered Flink]:::purple
        ITE[Source operator<br/>timestamp]:::purple
    end
...
```mermaid
flowchart LR
    subgraph Stream["Event Stream with Watermarks"]
        E1[Event<br/>10:01]:::primary
        W1[Watermark<br/>10:00]:::orange
        E2[Event<br/>10:05]:::primary
        E3[Event<br/>10:02]:::primary
        W2[Watermark<br/>10:02]:::orange
        E4[Event<br/>10:08]:::primary
        W3[Watermark<br/>10:05]:::orange
    end

    E1 --> W1 --> E2 --> E3 --> W2 --> E4 --> W3
...
flowchart TD
    subgraph Tumbling["Tumbling Window (5 min)"]
        TW1[00:00-00:05]:::primary
        TW2[00:05-00:10]:::primary
        TW3[00:10-00:15]:::primary
    end

    subgraph Sliding["Sliding Window (10 min, slide 5 min)"]
        SW1[00:00-00:10]:::orange
        SW2[00:05-00:15]:::orange
        SW3[00:10-00:20]:::orange
    end

    subgraph Session["Session Window (5 min gap)"]
        SE1[User activity]:::green
        SE2[5 min gap]:::red
        SE3[New session]:::green
    end
...
```mermaid
flowchart TD
    JM[JobManager]:::purple -->|Trigger| B[Checkpoint Barrier]:::orange
    B --> TM1[TaskManager 1]:::primary
    B --> TM2[TaskManager 2]:::primary
    B --> TM3[TaskManager 3]:::primary

    TM1 -->|State| S[(Checkpoint<br/>Storage)]:::green
    TM2 -->|State| S
    TM3 -->|State| S
...
```mermaid
flowchart LR
    subgraph Input1["Input Stream 1"]
        I1A[Data]:::primary
        I1B[Barrier n]:::red
        I1C[Data]:::primary
    end

    subgraph Input2["Input Stream 2"]
        I2A[Data]:::primary
        I2B[Data]:::primary
        I2C[Barrier n]:::red
    end
    ...
    subgraph Operator
        OP[Align barriers<br/>Snapshot state]:::orange
    end
...
```mermaid
flowchart TD
    F[Task Failure]:::red --> D[JobManager detects failure]:::orange
    D --> R[Restart strategy triggers]:::orange
    R --> L[Load last checkpoint]:::primary
    L --> RS[Restore state to operators]:::primary
    RS --> RO[Reset source offsets]:::primary
    RO --> RES[Resume processing]:::green
...
```mermaid
flowchart LR
    K1[Kafka Source]:::primary --> F[Flink Job]:::green --> K2[Kafka Sink]:::primary

    K1 -->|Checkpointed offsets| F
    F -->|Two-phase commit| K2
...
flowchart TD
    subgraph Session["Session Mode"]
        SC[Long-running cluster]:::primary
        SJ1[Job 1]:::orange
        SJ2[Job 2]:::orange
        SJ3[Job 3]:::orange
        SC --> SJ1
    ...
    subgraph PerJob["Per-Job Mode"]
        PJ1[Cluster 1<br/>Job 1]:::green
        PJ2[Cluster 2<br/>Job 2]:::green
    end

    subgraph Application["Application Mode"]
        AM1["Cluster 1<br/>main() in cluster"]:::purple
    end
