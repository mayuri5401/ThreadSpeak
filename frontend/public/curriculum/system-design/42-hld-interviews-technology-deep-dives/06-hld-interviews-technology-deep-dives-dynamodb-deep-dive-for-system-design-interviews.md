---
id: "hld-interviews-technology-deep-dives-dynamodb-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "DynamoDB Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-dynamodb-deep-dive-for-system-design-interviews"
summary: "When Amazon needed a database that could handle Prime Day, they built DynamoDB. In 2023, DynamoDB processed over 89 million requests per second during peak traffic, powering the shopping carts, inventory checks, and order processing that make the ..."
eli10: "Imagine DynamoDB Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "DynamoDB Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TD
    ...
    subgraph Cache["Optional Caching"]
        DAX[DAX Cluster]:::orange
    end

    subgraph DDB["DynamoDB Service"]
        RR[Request Router]:::green
        subgraph Partitions["Storage Partitions"]
            P1[Partition 1]:::purple
            P2[Partition 2]:::purple
            P3[Partition N]:::purple
        end
        subgraph Replicas["Replication"]
            R1[AZ-1 Replica]:::teal
            R2[AZ-2 Replica]:::teal
            R3[AZ-3 Replica]:::teal
        end
    end
...
```mermaid
flowchart TD
    A[Do you know your access patterns"]:::primary
    A -->|Yes| B[Need low latency at scale"]:::primary
    A -->|No| C[Consider SQL Database]:::red
    B -->|Yes| D[Simple queries enough"]:::primary
    B -->|No| E[Consider Redis/ElastiCache]:::orange
    D -->|Yes| F[DynamoDB is a good fit]:::green
    D -->|No| G[Need joins/aggregations"]:::primary
    G -->|Yes| H[Consider SQL or DocumentDB]:::orange
    G -->|No| F
...
flowchart LR
    ...
    subgraph Bad["Bad: Hot Partition"]
        A1[All writes to<br/>status='active']:::red
    end	
...
```mermaid
flowchart TD
    subgraph SingleTable["Single-Table Design"]
        direction TB
        U["USER#123"]:::primary
        UP["PROFILE"]:::orange
        UO1["ORDER#001"]:::green
...
flowchart TD
    subgraph BaseTable["Base Table"]
        BT[PK: USER#123<br/>SK: ORDER#001]:::primary
    end

    subgraph GSI1["GSI1 - Inverted"]
        G1[PK: ORDER#001<br/>SK: USER#123]:::orange
    end

    subgraph GSI2["GSI2 - By Status"]
        G2[PK: PENDING<br/>SK: 2024-01-15]:::purple
    end
...
```mermaid
flowchart TD
    A[What is your traffic pattern"]:::primary
    A --> B{Predictable"}
    B -->|Yes| C[Provisioned Mode]:::green
    B -->|No| D{High baseline"}
    D -->|Yes| E[Provisioned + Auto Scaling]:::orange
    D -->|No| F[On-Demand Mode]:::purple
...
```mermaid
flowchart LR
    A[Application]:::primary --> B[DAX Cache]:::orange
    B -->|Cache Miss| C[DynamoDB]:::purple
    B -->|Cache Hit| A
...
```mermaid
flowchart LR
    A[Item Expires]:::orange --> B[TTL Deletes Item]:::primary
    B --> C[Stream Record Created]:::purple
    C --> D[Lambda Archives to S3]:::green
