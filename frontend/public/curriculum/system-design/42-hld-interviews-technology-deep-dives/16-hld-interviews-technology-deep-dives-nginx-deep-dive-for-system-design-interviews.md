---
id: "hld-interviews-technology-deep-dives-nginx-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Nginx Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-nginx-deep-dive-for-system-design-interviews"
summary: "Draw any system design diagram with multiple backend servers, and you'll need something in front of them."
eli10: "Imagine Nginx Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Nginx Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    ...
    subgraph Edge["Edge Layer"]
        N[Nginx<br/>SSL, Cache, Load Balance]:::orange
    end

    subgraph Backend["Private Network"]
        S1[App Server 1]:::green
        S2[App Server 2]:::green
        S3[App Server 3]:::green
    end
...
flowchart TD
    ...
    subgraph EventDriven["Event-Driven (Nginx)"]
        direction LR
        R5[Request 1]:::primary --> W1[Worker 1<br/>Event Loop]:::green
        R6[Request 2]:::primary --> W1
        R7[Request 3]:::primary --> W1
        R8[Request 4]:::primary --> W1
    end

    subgraph Traditional["Thread-Per-Connection (Apache)"]
        direction LR
        R1[Request 1]:::primary --> P1[Thread 1<br/>Waiting on I/O]:::red
        R2[Request 2]:::primary --> P2[Thread 2<br/>Waiting on I/O]:::red
        R3[Request 3]:::primary --> P3[Thread 3<br/>Waiting on I/O]:::red
        R4[Request 4]:::primary --> P4[Thread 4<br/>Waiting on I/O]:::red
    end
...
```mermaid
flowchart TD
    M[Master Process<br/>Configuration, Signals, Port Binding]:::purple

    subgraph Workers["Worker Processes"]
        W1[Worker 1<br/>Event Loop]:::primary
        W2[Worker 2<br/>Event Loop]:::primary
        W3[Worker 3<br/>Event Loop]:::primary
        W4[Worker 4<br/>Event Loop]:::primary
    end
    ...
    C1[Connections]:::orange --> W1
    C2[Connections]:::orange --> W2
    C3[Connections]:::orange --> W3
    C4[Connections]:::orange --> W4
...
```mermaid
flowchart TD
    A[Client Request<br/>Arrives]:::primary --> B[Accept<br/>Connection]:::orange
    B --> C[Read Headers<br/>Parse Request]:::orange
    C --> D[Match<br/>Server Block]:::purple
    D --> E[Match<br/>Location Block]:::purple
    E --> F{Request<br/>Type"}
    F -->|Static File| G[Serve from<br/>Disk/Cache]:::green
    F -->|Proxy| H[Forward to<br/>Backend]:::orange
    H --> I[Receive<br/>Response]:::orange
    I --> J{Cacheable"}
    J -->|Yes| K[Store in Cache]:::green
    J -->|No| L[Skip Cache]:::orange
    K --> M[Send to Client]:::green
    L --> M
    G --> M
    M --> N[Write Log]:::purple
...
flowchart LR
    ...
    subgraph Edge["DMZ"]
        RP[Nginx<br/>SSL, Auth, Rate Limit]:::orange
    end
...
flowchart TD
    ...
    subgraph With["With Buffering"]
        B2[Backend]:::green -->|Fast: 100ms| N[Nginx<br/>Buffer]:::orange
        N -->|Slow: 30s| C2[3G Client]:::primary
    end

    subgraph Without["Without Buffering"]
        B1[Backend]:::red -->|Slow: tied for 30s| C1[3G Client]:::primary
    end
...
flowchart LR
    N[Nginx]:::orange
    N -->|Req 1, 4, 7...| S1[Server 1]:::primary
    N -->|Req 2, 5, 8...| S2[Server 2]:::primary
    N -->|Req 3, 6, 9...| S3[Server 3]:::primary
...
flowchart LR
    N[Nginx]:::orange
    N -->|New request goes here| S2

    S1[Server 1<br/>5 active connections]:::red
    S2[Server 2<br/>2 active connections]:::green
    S3[Server 3<br/>4 active connections]:::orange
...
```mermaid
flowchart TD
    A[Do you need session stickiness"]:::primary
    A -->|Yes| B{Based on what"}
    B -->|Cookie/Header| C[Generic Hash<br/>with consistent]:::green
    B -->|Client IP| D[IP Hash<br/>Warning: NAT issues]:::orange
    A -->|No| E{Are request times variable"}
    E -->|Yes| F[Least Connections]:::green
    E -->|No| G{Servers identical"}
    G -->|Yes| H[Round Robin]:::green
    G -->|No| I[Weighted Round Robin]:::orange
...
```mermaid
flowchart LR
    C[Client]:::primary --> N[Nginx]
    N --> D{Cache"}
    D -->|HIT| R[Cached Response]:::green
    D -->|MISS| B[Backend]:::orange
    B --> N
    N --> C
...
```mermaid
flowchart TD
    A[Request]:::primary --> B{In Cache"}
    B -->|No| C[MISS<br/>Fetch from Backend]:::red
    B -->|Yes| D{Fresh"}
    D -->|Yes| E[HIT<br/>Serve Immediately]:::green
    D -->|No, Expired| F{Backend OK"}
    F -->|Yes| G[EXPIRED<br/>Refresh Cache]:::orange
    F -->|No| H[STALE<br/>Serve Old Content]:::orange
...
```mermaid
flowchart LR
    C[Client]:::primary -->|HTTPS<br/>Encrypted| N[Nginx<br/>SSL Termination]:::orange
    N -->|HTTP<br/>Unencrypted| S1[Server 1]:::green
    N -->|HTTP<br/>Unencrypted| S2[Server 2]:::green
    N -->|HTTP<br/>Unencrypted| S3[Server 3]:::green
...
```mermaid
flowchart TD
    subgraph Full["Full Handshake"]
        C1[Client]:::primary -->|1. ClientHello| S1[Server]:::orange
        S1 -->|2. ServerHello + Cert| C1
        C1 -->|3. Key Exchange| S1
        S1 -->|4. Finished| C1
    end

    subgraph Resumed["Session Resumption"]
        C2[Client]:::primary -->|1. ClientHello + Session ID| S2[Server]:::green
        S2 -->|2. ServerHello + Finished| C2
    end
...
```mermaid
flowchart TD
    IN[Incoming Requests<br/>15/second]:::primary --> B[Bucket<br/>burst=20]:::orange
    B -->|Drain at 10/s| OUT[Backend]:::green
    B -->|Overflow when full| REJ[429 Rejected]:::red
...
> **Important caveat about distributed systems**
>
...
```mermaid
flowchart TD
    VIP[Virtual IP: 10.0.0.100]:::purple

    subgraph Active
        N1[Nginx Primary]:::green
        K1[Keepalived]:::orange
    end

    subgraph Passive
        N2[Nginx Backup]:::red
        K2[Keepalived]:::orange
    end
...
```mermaid
flowchart TD
    CLB[Cloud Load Balancer<br/>AWS ALB / GCP LB]:::purple

    N1[Nginx 1]:::primary
    N2[Nginx 2]:::primary
    N3[Nginx 3]:::primary

    S1[Backend Pool]:::green
...
flowchart LR
    subgraph Before["Before Reload"]
        M1[Master]:::purple --> W1[Old Workers]:::orange
    end

    subgraph During["During Reload"]
        M2[Master]:::purple --> W2[Old Workers<br/>Finishing requests]:::orange
        M2 --> W3[New Workers<br/>Accepting new]:::green
    end

    subgraph After["After Reload"]
        M3[Master]:::purple --> W4[New Workers]:::green
    end
