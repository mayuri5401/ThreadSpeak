---
id: "hld-interviews-technology-deep-dives-aws-lambda-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "AWS Lambda Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-aws-lambda-deep-dive-for-system-design-interviews"
summary: "AWS Lambda is the most widely used serverless compute service. When you need to run code without managing servers, handle variable workloads, or build event-driven architectures, Lambda is often the go-to solution."
eli10: "Imagine AWS Lambda Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "AWS Lambda Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Need compute"]:::primary
    A -->|Yes| B[Predictable, steady load"]:::primary
    B -->|Yes| C[Consider EC2/ECS]:::orange
    B -->|No| D[Execution time >15 min"]:::primary
    D -->|Yes| E[Consider ECS/Batch]:::orange
    D -->|No| F[Need sub-100ms latency"]:::primary
    F -->|Always| G[Consider EC2/ECS with ALB]:::orange
    F -->|Occasionally OK| H[Lambda is a good fit]:::green
...
flowchart TD
    subgraph Sandbox["Execution Environment"]
        A[Your Code]:::primary
        B[Runtime<br/>Node.js/Python/Java/etc]:::orange
        C[Lambda Runtime API]:::purple
        D[Extensions]:::green
    end
...
```mermaid
flowchart LR
    A[INIT Phase]:::orange --> B[INVOKE Phase]:::green
    B --> C{Another request"}
    C -->|Yes, within timeout| B
    C -->|No| D[SHUTDOWN Phase]:::red
...
```mermaid
flowchart LR
    subgraph FirstInvocation["First Invocation (Cold)"]
        A1[Download Code]:::orange --> A2[Init Runtime]:::orange
        A2 --> A3[Run Init Code]:::orange
        A3 --> A4[Execute Handler]:::green
    end

    subgraph SubsequentInvocations["Subsequent Invocations (Warm)"]
        B1[Execute Handler]:::green
    end
...
```mermaid
flowchart TD
    A[Lambda Invoked]:::primary --> B{Warm environment<br/>available"}
    B -->|Yes| C[Warm Start<br/>~1-10ms overhead]:::green
    B -->|No| D[Cold Start<br/>100ms - 10s+ overhead]:::red
    D --> E[Download Code]:::orange
    E --> F[Create Environment]:::orange
    F --> G[Initialize Runtime]:::orange
    G --> H[Run Init Code]:::orange
    H --> I[Execute Handler]:::green
    C --> I
...
flowchart TD
    ...
    subgraph Provisioned["Provisioned Concurrency"]
        E[Request]:::primary --> F[Pre-warmed<br/>Environment]:::green
        F --> G[Execute]:::green
    end

    subgraph Standard["Standard Lambda"]
        A[Request]:::primary --> B{Cold Start"}
        B -->|Yes| C[Wait 100ms-10s]:::red
        B -->|No| D[Execute]:::green
    end
...
flowchart TD
    subgraph Requests["Incoming Requests"]
        R1[Request 1]:::primary
        R2[Request 2]:::primary
        R3[Request 3]:::primary
        R4[Request 4]:::primary
    end

    subgraph Environments["Execution Environments"]
        E1[Env 1<br/>Processing R1]:::green
        E2[Env 2<br/>Processing R2]:::green
        E3[Env 3<br/>Processing R3]:::green
        E4[Env 4<br/>Processing R4]:::green
    end
...
```mermaid
flowchart TD
    subgraph Scaling["Lambda Scaling"]
        A[0: Burst to 3,000]:::green --> B[+1 min: +500]:::orange
        B --> C[+2 min: +500]:::orange
        C --> D[+N min: +500]:::orange
        D --> E[Account Limit]:::red
    end
...
flowchart TD
    subgraph Account["Account Limit: 1,000"]
        A[Function A<br/>Reserved: 200]:::green
        B[Function B<br/>Reserved: 100]:::green
        C[Unreserved Pool<br/>700]:::orange
    end

    D[Other Functions]:::primary --> C
...
```mermaid
flowchart TD
    A[100 Requests]:::primary --> B{Within Provisioned<br/>Concurrency"}
    B -->|Yes| C[Pre-warmed<br/>No Cold Start]:::green
    B -->|No| D[Standard Scaling<br/>May Cold Start]:::orange
...
flowchart TD
    subgraph Sync["Synchronous"]
        A[API Gateway]:::primary --> B[Lambda]:::green
        B --> C[Response]:::primary
    end

    subgraph Async["Asynchronous"]
        D[S3 Event]:::orange --> E[Lambda]:::green
        E --> F[Success/Retry]:::orange
    end

    subgraph Poll["Poll-Based"]
        G[SQS Queue]:::purple --> H[Lambda<br/>Polls Queue]:::green
    end
...
flowchart LR
    ...
    subgraph ESM["Event Source Mapping"]
        B[Poller]:::purple
    end
...
```mermaid
flowchart LR
    A[Upload]:::primary --> B[S3 Bucket]:::orange
    B -->|Event| C[Lambda]:::green
    C --> D[Process]:::green
    D --> E[Output Bucket]:::purple
...
```mermaid
flowchart TD
    A[Event Publisher]:::primary --> B[SNS Topic]:::orange
    B --> C[Lambda 1<br/>Process Orders]:::green
    B --> D[Lambda 2<br/>Update Analytics]:::green
    B --> E[Lambda 3<br/>Send Notifications]:::green
...
```mermaid
flowchart TD
    A[Start]:::primary --> B[Validate Order]:::green
    B --> C{Valid"}
    C -->|Yes| D[Process Payment]:::green
    C -->|No| E[Reject Order]:::red
    D --> F{Payment OK"}
    F -->|Yes| G[Ship Order]:::green
    F -->|No| H[Handle Failure]:::orange
    G --> I[Complete]:::primary
    H --> I
    E --> I
...
```mermaid
flowchart TD
    A[Event Sources]:::primary --> B[EventBridge]:::orange
    B -->|Rule: order.created| C[Order Lambda]:::green
    B -->|Rule: user.signup| D[User Lambda]:::green
    B -->|Rule: payment.*| E[Payment Lambda]:::green
...
```mermaid
flowchart TD
    A[Low Traffic<br/><100K req/month]:::green --> B[Lambda Cheaper]:::green
    C[Medium Traffic<br/>100K-10M req/month]:::orange --> D[Depends on Pattern]:::orange
    E[High Steady Traffic<br/>>10M req/month]:::red --> F[EC2/ECS Often Cheaper]:::red
...
```mermaid
flowchart TD
    A[Need serverless compute"]:::primary
    A -->|Yes| B{Execution time"}
    B -->|<15 min| C{Traffic pattern"}
    B -->|>15 min| D[ECS Fargate]:::orange
    C -->|Spiky/Variable| E[Lambda]:::green
    C -->|Steady| F{Latency critical"}
    F -->|Yes| G[ECS Fargate]:::orange
    F -->|No| H[Lambda or Fargate]:::green
...
flowchart LR
    subgraph Frontend["User-Facing"]
        A[API Gateway]:::primary --> B[Lambda<br/>Auth, Simple Logic]:::green
    end

    subgraph Backend["Heavy Processing"]
        B --> C[ECS Fargate<br/>Complex Processing]:::orange
        B --> D[SQS]:::purple
        D --> C
    end
