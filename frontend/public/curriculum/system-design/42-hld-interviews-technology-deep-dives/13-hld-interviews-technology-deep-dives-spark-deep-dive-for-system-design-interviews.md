---
id: "hld-interviews-technology-deep-dives-spark-deep-dive-for-system-design-interviews"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Technology Deep Dives"
subSection: ""
title: "Spark Deep Dive for System Design Interviews"
slug: "hld-interviews-technology-deep-dives-spark-deep-dive-for-system-design-interviews"
summary: "Imagine you are building an analytics platform for an e-commerce company. Every day, you need to process 50 million user events, join them with product catalogs and user profiles, compute aggregated metrics, train recommendation models, and popula..."
eli10: "Imagine Spark Deep Dive for System Design Interviews as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Spark Deep Dive for System Design Interviews Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Need to process large data"]:::primary
    A -->|Yes| B[Batch or streaming"]:::primary
    A -->|No| C[Consider simpler tools]:::red
    B -->|Batch| D[Need complex analytics"]:::primary
    B -->|Streaming| E[Latency requirements"]:::primary
    D -->|Yes| F[Spark is a good fit]:::green
    D -->|No| G[Consider Hive or Presto]:::orange
    E -->|Seconds OK| F
    E -->|Milliseconds| H[Consider Flink]:::orange
...
```mermaid
flowchart LR
    subgraph RDD["RDD (2011)"]
        R1[Low-level API]:::primary
        R2[Type-safe]:::primary
        R3[No optimization]:::red
    end

    subgraph DF["DataFrame (2015)"]
        D1[High-level API]:::green
        D2[Schema-aware]:::green
        D3[Catalyst optimizer]:::green
    end

    subgraph DS["Dataset (2016)"]
        S1[Type-safe]:::green
        S2[Object-oriented]:::green
        S3[Best of both]:::green
    end
...
flowchart LR
    ...
    subgraph Actions["Actions (Trigger)"]
        A1[count]:::green
        A2[collect]:::green
        A3[save]:::green
        A4[show]:::green
...
flowchart TD
    ...
    subgraph Wide["Wide Transformations (Shuffle)"]
        W1[Partition 1]:::primary --> S[Shuffle]:::red
        W2[Partition 2]:::primary --> S
        W3[Partition 3]:::primary --> S
        S --> WO1[Output 1]:::orange
        S --> WO2[Output 2]:::orange
    end

    subgraph Narrow["Narrow Transformations"]
        N1[Partition 1]:::primary --> N1O[Output 1]:::green
        N2[Partition 2]:::primary --> N2O[Output 2]:::green
        N3[Partition 3]:::primary --> N3O[Output 3]:::green
    end
...
```mermaid
flowchart TD
    subgraph Job["Job (triggered by action)"]
...
flowchart TD
    subgraph Driver["Driver Program"]
        SC[SparkContext]:::purple
        DAG[DAG Scheduler]:::purple
        TS[Task Scheduler]:::purple
    end

    subgraph ClusterManager["Cluster Manager"]
        CM[YARN / Kubernetes / Standalone]:::orange
    end
...
```mermaid
flowchart LR
    Code[User Code]:::primary --> DAG[Build DAG]:::orange
    DAG --> Stages[Split into Stages]:::orange
    Stages --> Tasks[Create Tasks]:::orange
    Tasks --> Schedule[Schedule to Executors]:::purple
    Schedule --> Execute[Execute in Parallel]:::green
    Execute --> Results[Collect Results]:::green
...
```mermaid
flowchart LR
    subgraph Locality["Data Locality Levels"]
        L1[PROCESS_LOCAL]:::green
        L2[NODE_LOCAL]:::green
        L3[RACK_LOCAL]:::orange
        L4[ANY]:::red
    end

    L1 -->|"Data in executor memory"| L2
    L2 -->|"Data on same node disk"| L3
    L3 -->|"Data in same rack"| L4
    L4 -->|"Data anywhere"| Done[Execute Task]:::primary
...
```mermaid
flowchart LR
    SQL[SQL / DataFrame API]:::primary --> LP[Logical Plan]:::orange
    LP --> ALP[Analyzed Logical Plan]:::orange
    ALP --> OLP[Optimized Logical Plan]:::green
    OLP --> PP[Physical Plans]:::purple
    PP --> SPP[Selected Physical Plan]:::purple
    SPP --> RDD[RDD Execution]:::primary
...
```mermaid
flowchart TD
    subgraph After["With Pushdown"]
        A1[Read Filtered Data]:::green
    end

    subgraph Before["Without Pushdown"]
        B1[Read All Data]:::red --> B2[Filter]:::orange
    end
...
```mermaid
flowchart TD
    J[Join]:::primary --> Size{Table Size"}
    Size -->|Small < 10MB| BHJ[Broadcast Hash Join]:::green
    Size -->|Medium| SMJ[Sort Merge Join]:::orange
    Size -->|Large| SHJ[Shuffle Hash Join]:::orange

    BHJ --> Exec1[Broadcast small table<br/>No shuffle]:::green
    SMJ --> Exec2[Shuffle both<br/>Sort and merge]:::orange
    SHJ --> Exec3[Shuffle both<br/>Hash join]:::orange
...
flowchart TD
    ...
    subgraph With["With AQE"]
        A1[200 initial partitions]:::orange
        A2[Coalesced to 50]:::green
    end

    subgraph Without["Without AQE"]
        W1[Fixed 200 partitions]:::red
        W2[Many small partitions]:::red
    end
...
flowchart TD
    subgraph Input["Input Stream"]
        I1[New Data]:::primary
        I2[Arriving Continuously]:::primary
    end

    subgraph Table["Unbounded Table"]
        T1[Existing Rows]:::orange
        T2[New Rows Appended]:::green
    end

    subgraph Query["Query"]
        Q1[Same DataFrame API]:::purple
    end

    subgraph Output["Output"]
        O1[Results Table]:::green
        O2[Updated Incrementally]:::green
    end

    Input --> Table --> Query --> Output
...
```mermaid
flowchart TD
    subgraph Tumbling["Tumbling Window (10 min)"]
        TW1[00:00-00:10]:::primary
        TW2[00:10-00:20]:::primary
        TW3[00:20-00:30]:::primary
    end
...
flowchart LR
    subgraph Events
        E1[Event t=10:00]:::primary
        E2[Event t=10:05]:::primary
        E3[Event t=09:55<br/>Late]:::orange
        E4[Event t=09:45<br/>Dropped]:::red
    end

    subgraph Watermark
        W[Watermark = 10:05 - 10min = 09:55]:::purple
    end

    E3 -->|Within watermark| Processed[Processed]:::green
    E4 -->|Before watermark| Dropped[Dropped]:::red
...
flowchart TD
    subgraph Executor["Executor Memory"]
        subgraph OnHeap["On-Heap Memory"]
            Reserved[Reserved Memory<br/>300MB]:::red
            subgraph UsableMemory["Usable Memory"]
                subgraph SparkMemory["Spark Memory (60%)"]
                    Storage[Storage Memory<br/>Cached data]:::green
                    Execution[Execution Memory<br/>Shuffles, joins, sorts]:::orange
                end
                User["User Memory (40%)<br/>User data structures"]:::purple
            end
        end

        OffHeap[Off-Heap Memory<br/>Optional]:::primary
    end
...
flowchart TD
    ...
    subgraph Shuffle["Shuffle"]
        S[Sort + Write to Disk]:::red
        N[Network Transfer]:::red
        R[Read + Merge]:::red
    end
...
flowchart TD
    ...
    subgraph Bucket["With Bucketing"]
        BR1[Read Bucketed A]:::primary --> BJ[Join]:::green
        BR2[Read Bucketed B]:::primary --> BJ
    end

    subgraph NoBucket["Without Bucketing"]
        R1[Read Table A]:::primary --> S1[Shuffle A]:::red
        R2[Read Table B]:::primary --> S2[Shuffle B]:::red
        S1 --> J1[Join]:::orange
        S2 --> J1
    end
...
flowchart TD
    ...
    subgraph Solutions["Solutions"]
        S1[Salting]:::orange
        S2[AQE Skew Join]:::orange
        S3[Two-Phase Aggregation]:::orange
    end

    subgraph Skewed["Skewed Data"]
        SK1[Key A: 1M rows]:::red
        SK2[Key B: 1K rows]:::green
        SK3[Key C: 1K rows]:::green
    end
