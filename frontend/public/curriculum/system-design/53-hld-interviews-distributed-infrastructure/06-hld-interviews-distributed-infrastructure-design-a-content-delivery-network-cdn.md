---
id: "hld-interviews-distributed-infrastructure-design-a-content-delivery-network-cdn"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design a Content Delivery Network (CDN)"
slug: "hld-interviews-distributed-infrastructure-design-a-content-delivery-network-cdn"
summary: "This system design problem tests your understanding of caching, distributed systems, DNS, networking, and global-scale infrastructure."
eli10: "Imagine Design a Content Delivery Network (CDN) as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design a Content Delivery Network (CDN) Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Distributed Infrastructure","System Design","Architecture"]
...
flowchart LR
    subgraph Global["Global Traffic"]
        NA[North America<br/>30% = 3M req/sec]:::primary
        EU[Europe<br/>25% = 2.5M req/sec]:::primary
        AP[Asia Pacific<br/>35% = 3.5M req/sec]:::primary
        OT[Other Regions<br/>10% = 1M req/sec]:::secondary
    end
...
```mermaid
flowchart LR
    subgraph Traffic["Incoming: 10M req/sec"]
        R[Requests]:::rose
    end

    subgraph Edge["Edge Cache Layer"]
        HIT[Cache Hits<br/>9.5M req/sec<br/>95%]:::green
        MISS[Cache Misses<br/>500K req/sec<br/>5%]:::orange
    end

    subgraph Shield["Shield Layer"]
        SH[Shield Hits<br/>475K req/sec]:::green
        SM[Shield Misses<br/>25K req/sec]:::red
    end
...
When a user's browser requests [`https://cdn.example.com/image.jpg`](https://cdn.example.com/image.jpg), the first step is DNS resolution. The browser needs to convert `cdn.example.com` into an IP address. This is our opportunity to direct the user to an optimal edge server.

```mermaid
flowchart TB
    U[User in Tokyo]:::rose

    subgraph DNS["DNS Resolution"]
        direction LR
        R[Local DNS Resolver]:::secondary
        AUTH[CDN Authoritative DNS]:::purple
    end

    subgraph Edge["Edge Servers"]
        direction LR
        TK[Tokyo PoP<br/>203.0.113.1]:::green
        NY[New York PoP<br/>203.0.113.2]:::orange
        LN[London PoP<br/>203.0.113.3]:::orange
    end

    U -->|"1. Resolve cdn.example.com"| R
    R -->|"2. Query authoritative DNS"| AUTH
    AUTH -->|"3. Return Tokyo IP<br/>(user is in Japan)"| R
    R -->|"4. Return 203.0.113.1"| U
    U -->|"5. Connect to Tokyo"| TK
...
flowchart TB
    ...
    subgraph Internet["Internet Routing (BGP)"]
        direction TB
        R1[Router]:::secondary
        R2[Router]:::secondary
    end

    subgraph Edge["Same IP: 203.0.113.1 everywhere"]
        direction LR
        TK[Tokyo PoP]:::green
        NY[New York PoP]:::orange
        LN[London PoP]:::orange
    end

    U --> R1
    R1 --> R2
    R2 -->|"Shortest BGP path"| TK
    R2 -.->|"Further BGP paths"| NY
    R2 -.->|"Further BGP paths"| LN
...
```mermaid
flowchart LR
    REQ[Incoming Request]:::rose

    subgraph Server["Edge Server"]
        direction LR
        L1["L1: Memory Cache<br/>64-128 GB<br/>< 1ms access"]:::primary
        L2["L2: SSD Cache<br/>2-4 TB<br/>< 5ms access"]:::orange
        L3["L3: HDD Cache<br/>20+ TB<br/>< 20ms access"]:::secondary
    end

    REQ --> L1
    L1 -->|"Miss"| L2
    L2 -->|"Miss"| L3
    L3 -->|"Miss"| UP[Upstream Fetch]:::purple
...
sequenceDiagram
    ...
    participant L2 as SSD Cache

    User->>Edge: GET /images/logo.png
    Note over Edge: Compute cache key
    Edge->>L1: Check memory cache
    L1-->>Edge: HIT! Return content
    Note over Edge: Add X-Cache: HIT header
    Edge-->>User: 200 OK (12ms)
```
...
flowchart TB
    subgraph Asia["Asia Region"]
        TK[Tokyo PoP]:::secondary
        OS[Osaka PoP]:::secondary
        SE[Seoul PoP]:::secondary
        AS[Asia Shield<br/>Singapore]:::orange
    end

    subgraph Europe["Europe Region"]
        LN[London PoP]:::secondary
        FR[Frankfurt PoP]:::secondary
        PA[Paris PoP]:::secondary
        ES[Europe Shield<br/>Amsterdam]:::orange
    end

    subgraph Origin
        O[Customer Origin Server]:::purple
    end
...
sequenceDiagram
    ...
    participant Shield as Asia Shield
    participant Origin

    User->>Edge: GET /video/intro.mp4
    Note over Edge: Cache miss (all tiers)

    Edge->>Shield: GET /video/intro.mp4
    Note over Shield: Check shield cache

    alt Shield Hit
        Shield-->>Edge: Return cached content
    else Shield Miss
        Shield->>Origin: GET /video/intro.mp4
        Origin-->>Shield: Return content + cache headers
        Note over Shield: Cache for future requests
        Shield-->>Edge: Return content
    end

    Note over Edge: Cache locally
    Edge-->>User: 200 OK with X-Cache: MISS
```
...
flowchart TB
    subgraph Users["Users Worldwide"]
        U1[User Asia]:::rose
        U2[User Europe]:::rose
        U3[User Americas]:::rose
    end

    subgraph DNS["DNS Layer"]
        ADNS[Authoritative DNS<br/>GeoDNS + Health Checks]:::purple
    end

    subgraph Edge["Edge Layer (200 PoPs)"]
        E1[Tokyo PoP<br/>L1/L2/L3 Cache]:::secondary
        E2[London PoP<br/>L1/L2/L3 Cache]:::secondary
        E3[New York PoP<br/>L1/L2/L3 Cache]:::secondary
    end

    subgraph Shield["Shield Layer (Regional)"]
        S1[Asia Shield<br/>Singapore]:::orange
        S2[Europe Shield<br/>Amsterdam]:::orange
        S3[Americas Shield<br/>Virginia]:::orange
    end

    subgraph Origin["Origin Layer"]
        O1[Customer Origin<br/>Server]:::green
    end

    subgraph Control["Control Plane"]
        CP[Configuration<br/>Distribution]:::pink
        INV[Cache Invalidation<br/>Service]:::pink
        MON[Health Monitoring<br/>+ Metrics]:::pink
    end
...
flowchart TB
    ...
    subgraph Ops["Operational"]
        direction LR
        O1[Invalidation Log<br/>Purge tracking]:::secondary
        O2[Access Logs<br/>Request records]:::secondary
        O3[Metrics<br/>Performance data]:::secondary
    end

    subgraph Config["Persistent"]
        direction LR
        P1[Customer Settings<br/>Domains, origins]:::orange
        P2[SSL Certificates<br/>Private keys]:::orange
        P3[Routing Rules<br/>Custom behavior]:::orange
    end
	
    subgraph Cache["Ephemeral"]
        direction LR
        C1[Cached Content<br/>Actual files]:::primary
        C2[Cache Metadata<br/>TTLs, ETags]:::primary
    end	
...
sequenceDiagram
    ...
    participant Auth as CDN Authoritative DNS
    participant Geo as Geolocation DB
    participant Edge as Tokyo Edge Server

    Browser->>Resolver: Resolve cdn.example.com
    Resolver->>Auth: Query cdn.example.com
    Auth->>Geo: Lookup resolver IP location
    Geo-->>Auth: Tokyo, Japan
    Note over Auth: Select Tokyo PoP<br/>(closest healthy server)
    Auth-->>Resolver: 203.0.113.1 (Tokyo), TTL=60s
    Resolver-->>Browser: 203.0.113.1
    Note over Resolver: Cache for 60 seconds
...
flowchart TB
    ...
    subgraph Internet["Internet Backbone"]
        R1[ISP Router]:::secondary
        R2[Tier-1 Router]:::secondary
        R3[Tier-1 Router]:::secondary
    end

    subgraph CDN["All Advertise 203.0.113.1"]
        TK[Tokyo PoP]:::green
        NY[New York PoP]:::orange
        LN[London PoP]:::orange
    end
...
sequenceDiagram
    ...
    Note over Browser: DNS resolves to GLB
    Browser->>GLB: GET /image.jpg<br/>Host: cdn.example.com
    Note over GLB: Analyze: client IP,<br/>Accept headers,<br/>server load
    GLB-->>Browser: 302 Redirect<br/>Location: tokyo.cdn.example.com/image.jpg
    Browser->>Edge: GET /image.jpg
    Edge-->>Browser: 200 OK + content
```
...
flowchart TB
    ...
    subgraph Anycast["Anycast Edge Layer"]
        E1[Tokyo]:::secondary
        E2[Seoul]:::secondary
        E3[Singapore]:::secondary
    end

    subgraph DNS["DNS-Routed Shield Layer"]
        S1[Asia Shield]:::orange
    end

    Origin[Origin]:::purple

    U -->|"Anycast: nearest edge"| E1
    E1 -->|"DNS: regional shield"| S1
    S1 --> Origin
...
```mermaid
flowchart LR
    API[Purge API]:::primary --> Queue[Message Queue<br/>Kafka/SQS]:::orange

    Queue --> P1[PoP 1]:::secondary
    Queue --> P2[PoP 2]:::secondary
    Queue --> P3[PoP ...]:::secondary
    Queue --> PN[PoP 200]:::secondary

    P1 --> ACK[(Ack Store)]:::green
    P2 --> ACK
    P3 --> ACK
    PN --> ACK

    ACK --> Status[Status API]:::primary
...
```mermaid
flowchart LR
    API[Purge API]:::primary --> Log[(Invalidation Log)]:::purple

    P1[PoP 1]:::secondary -->|"Poll every 5s"| Log
    P2[PoP 2]:::secondary -->|"Poll every 5s"| Log
    PN[PoP N]:::secondary -->|"Poll every 5s"| Log

    Log -->|"New entries since v42"| P1
    Log -->|"New entries since v41"| P2
...
```mermaid
flowchart TB
    subgraph Origin["Origin Response Headers"]
        OH["Cache-Tag: product-123, category-electronics"]:::purple
    end

    subgraph Cache["Cache Storage"]
        URL1["/products/123/main.jpg"]:::secondary
        URL2["/products/123/thumb.jpg"]:::secondary
        URL3["/products/456/main.jpg"]:::orange
    end

    subgraph Index["Tag Index"]
        T1["product-123"]:::primary --> URL1
        T1 --> URL2
        T2["product-456"]:::primary --> URL3
        T3["category-electronics"]:::primary --> URL1
        T3 --> URL3
    end

    Purge["Purge: product-123"]:::red -->|"Instantly finds"| URL1
    Purge -->|"Instantly finds"| URL2
...
```mermaid
flowchart LR
    subgraph Edge["200 Edge PoPs"]
        E1[PoP 1<br/>Cache Miss]:::orange
        E2[PoP 2<br/>Cache Miss]:::orange
        EN[PoP 200<br/>Cache Miss]:::orange
    end

    subgraph Origin
        O[Origin Server<br/>Overwhelmed!]:::red
    end
...
```mermaid
flowchart TB
    R1[Request 1]:::primary --> Check{In Cache"}
    R2[Request 2]:::primary --> Check
    R3[Request 3]:::primary --> Check
    RN[Request N]:::primary --> Check

    Check -->|"No"| InFlight{Being<br/>Fetched"}
    Check -->|"Yes"| Serve[Serve from Cache]:::green

    InFlight -->|"No (first request)"| Fetch[Fetch from Origin]:::orange
    InFlight -->|"Yes"| Wait[Wait Queue]:::secondary

    Fetch --> Cache[Cache Response]:::green
    Cache --> Wait
    Wait --> ServeAll[Serve All Waiters]:::green
...
sequenceDiagram
    ...
    Note over Edge: Cache entry expired<br/>(content is stale)

    U1->>Edge: GET /image.jpg
    Note over Edge: Serve stale immediately,<br/>trigger background refresh
    Edge-->>U1: 200 OK (stale content, instant)

    Edge->>Origin: GET /image.jpg (background)
    Origin-->>Edge: Fresh content
    Note over Edge: Cache updated

    U2->>Edge: GET /image.jpg
    Edge-->>U2: 200 OK (fresh content)
```
...
flowchart TD
    subgraph With["With Jitter"]
        T2["Server A: 54:32"]:::green
        T3["Server B: 57:15"]:::green
        T4["Server C: 59:48"]:::green
        T2 --> Spread["Spread over 6 min"]:::secondary
        T3 --> Spread
        T4 --> Spread
    end

    subgraph Without["Without Jitter"]
        T1["All expire at 60:00"]:::red --> Spike["Traffic Spike"]:::red
    end
...
flowchart TB
    subgraph Edge["200 Edge PoPs"]
        ...
        E2[PoP 2]:::secondary
        E3[PoP ...]:::secondary
        EN[PoP 200]:::secondary
    end

    Origin[Origin Server]:::red
    ...
    Note1["Every PoP fetches independently"]:::orange
...
```mermaid
flowchart TB
    subgraph Asia["Asia Region: 50 PoPs"]
        TK[Tokyo]:::secondary
        SE[Seoul]:::secondary
        SG1[Singapore]:::secondary
        AN[...]:::secondary
    end

    subgraph Shield["Shields (5-10 globally)"]
        AS[Asia Shield<br/>Request Coalescing<br/>Large Cache]:::orange
    end
    ...
    Note1["50 edge misses → 1 shield request"]:::primary
...
flowchart TD
    ...
    subgraph Approach2["Pass-Through"]
		direction TB
        U2[User]:::rose -->|"HTTPS"| E2[Edge Server]:::orange
        E2 -->|"HTTPS tunnel"| O2[Origin]:::purple
        Note2["TLS ends at origin<br/>Cannot cache"]:::red -.-> E2
    end

    subgraph Approach1["Edge Termination"]
		direction TB
        U1[User]:::rose -->|"HTTPS"| E1[Edge Server]:::secondary
        E1 -->|"HTTP/HTTPS"| O1[Origin]:::purple
        Note1["TLS ends at edge<br/>Lower latency"]:::green -.-> E1
    end
...
sequenceDiagram
    ...
    Customer->>CDN: Add domain cdn.example.com
    CDN->>CDN: Validate domain ownership (DNS/HTTP challenge)
    CDN->>LE: Request certificate for cdn.example.com
    LE-->>CDN: Issue certificate (valid 90 days)
    CDN->>Edge: Distribute certificate to all PoPs
    Note over CDN: Auto-renew at 60 days
```
...
```mermaid
flowchart LR
    User[User]:::rose -->|"TLS Hello"| Edge[Edge Server]:::secondary
    Edge -->|"Sign this data"| KS[Key Server<br/>in secure DC]:::green
    KS -->|"Signature"| Edge
    Edge -->|"TLS Complete"| User
...
```mermaid
flowchart TB
    Attack["DDoS Attack<br/>1 Tbps distributed"]:::red

    subgraph CDN["CDN: 100+ Tbps capacity"]
        E1[Tokyo PoP]:::secondary
        E2[London PoP]:::secondary
        E3[New York PoP]:::secondary
        EN[...200 PoPs]:::secondary
    end

    Origin[Origin Server<br/>Protected]:::green

    Attack --> E1
    Attack --> E2
    Attack --> E3

    E1 & E2 & E3 -->|"Clean traffic"| Origin
...
```mermaid
flowchart LR
    Internet[Internet Traffic<br/>Mixed: legit + malicious]:::rose

    subgraph Security["CDN Security Layers"]
        L1["Layer 1: Anycast<br/>Distribute load"]:::secondary
        L2["Layer 2: Rate Limiting<br/>Throttle abuse"]:::orange
        L3["Layer 3: WAF<br/>Block exploits"]:::purple
        L4["Layer 4: Bot Detection<br/>Filter bots"]:::pink
        L1 --> L2 --> L3 --> L4
    end

    Origin[Origin<br/>Clean traffic only]:::green
