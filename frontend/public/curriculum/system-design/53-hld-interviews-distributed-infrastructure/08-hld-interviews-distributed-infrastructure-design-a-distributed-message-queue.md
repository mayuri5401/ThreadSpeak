---
id: "hld-interviews-distributed-infrastructure-design-a-distributed-message-queue"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design a Distributed Message Queue"
slug: "hld-interviews-distributed-infrastructure-design-a-distributed-message-queue"
summary: "This problem touches on many distributed systems fundamentals: partitioning for scale, replication for fault tolerance, ordering guarantees, and the subtle differences between delivery semantics."
eli10: "Imagine Design a Distributed Message Queue as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design a Distributed Message Queue Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Daily Volume"
        D[43 Billion<br/>messages/day]:::primary
    end

    subgraph "Per Second"
        P[500K avg<br/>1M peak]:::orange
    end

    subgraph "Weekly"
        W[300 Billion<br/>messages/week]:::green
    end
...
flowchart TB
    ...
    subgraph "Read I/O"
        R1[3 Consumer Groups<br/>each 2.1 GB/sec]:::primary
        R2[Total Read I/O<br/>6.3 GB/sec]:::green
    end

    subgraph "Write I/O"
        W1[Producer Writes<br/>2.1 GB/sec]:::primary
        W2[Replication<br/>×3]:::orange
        W3[Total Write I/O<br/>6.3 GB/sec]:::green
    end
...
flowchart TB
    subgraph "Write Path"
        direction LR
        W1[Producers]:::rose --> W2[Brokers]:::primary
        W2 --> W3[Partitioned<br/>Storage]:::purple
    end

    subgraph "Read Path"
        direction LR
        R1[Consumers]:::green --> R2[Coordinator]:::orange
        R2 --> R3[Partition<br/>Assignment]:::primary
    end
...
flowchart TB
    subgraph "Topic: orders"
        P0[Partition 0<br/>offset: 0,1,2,3...]:::primary
        P1[Partition 1<br/>offset: 0,1,2,3...]:::primary
        P2[Partition 2<br/>offset: 0,1,2,3...]:::primary
        P3[Partition 3<br/>offset: 0,1,2,3...]:::primary
    end
...
```mermaid
flowchart LR
    M[Message arrives]:::rose --> C{Has partition key"}
    C -->|Yes| H[hash key mod num_partitions]:::primary
    C -->|No| R[Round-robin<br/>to next partition]:::primary
    H --> P[Write to partition]:::green
    R --> P
...
flowchart TB
    ...
    subgraph Application["Broker Cluster"]
        B1[Broker 1<br/>Partition 0 Leader]:::primary
        B2[Broker 2<br/>Partition 1 Leader]:::primary
        B3[Broker 3<br/>Partition 2 Leader]:::primary
    end

    subgraph Storage["Partition Logs"]
        D1[(Partition 0<br/>Log)]:::purple
        D2[(Partition 1<br/>Log)]:::purple
        D3[(Partition 2<br/>Log)]:::purple
    end

    P -->|"1. POST /topics/orders/messages"| LB
    LB -->|"2. Route to partition leader"| B1
    B1 -->|"3. Append to log"| D1
    D1 -->|"4. Return offset"| B1
    B1 -->|"5. ACK with offset"| P
...
flowchart LR
    subgraph "Partition 0 Log"
        ...
        M3[Msg 3]:::primary
        M4[...]:::secondary
        NEW[New messages<br/>append here →]:::green
    end
...
flowchart TB
    subgraph "Partition 0 Replicas"
        L[Broker 1<br/>LEADER]:::purple
        F1[Broker 2<br/>Follower]:::orange
        F2[Broker 3<br/>Follower]:::orange
    end

    P[Producer]:::rose -->|writes| L
    L -.->|replicates| F1
    L -.->|replicates| F2

    C[Consumer]:::green -->|reads| L
...
flowchart TB
    subgraph "Coordinator Responsibilities"
        M[Track Group<br/>Membership]:::primary
        A[Assign<br/>Partitions]:::primary
        R[Trigger<br/>Rebalance]:::primary
    end

    subgraph "Consumer Group"
        C1[Consumer 1<br/>♥ heartbeat]:::green
        C2[Consumer 2<br/>♥ heartbeat]:::green
    end
...
sequenceDiagram
    ...
    C->>CO: 1. Join group "order-processor"
    CO->>C: 2. Assigned partitions [0, 1]
    C->>OS: 3. Get last committed offset for P0, P1
    OS-->>C: 4. P0: offset 100, P1: offset 250

    loop Poll messages
        C->>B: 5. Fetch from P0 starting at offset 100
        B-->>C: 6. Messages 100-199
        Note over C: Process messages
        C->>OS: 7. Commit offset 200 for P0
    end

    Note over C,CO: If consumer crashes...
    CO->>CO: 8. Detect missed heartbeat
    CO->>CO: 9. Reassign P0, P1 to other consumers
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        ...
        P1[Producer 1]:::rose
        P2[Producer 2]:::rose
        C1[Consumer<br/>Group 1]:::green
        C2[Consumer<br/>Group 2]:::green
    end
    ...
    subgraph Cluster["Broker Cluster"]
        subgraph Broker1[Broker 1]
            B1P0[(P0 Leader)]:::purple
            B1P1[(P1 Follower)]:::orange
        end

        subgraph Broker2[Broker 2]
            B2P0[(P0 Follower)]:::orange
            B2P1[(P1 Leader)]:::purple
        end

        subgraph Broker3[Broker 3]
            B3P0[(P0 Follower)]:::orange
            B3P1[(P1 Follower)]:::orange
        end
    end

    subgraph Coordination["Coordination Layer"]
        CO[Coordinator<br/>Service]:::primary
        META[(Metadata<br/>Store)]:::primary
    end

    P1 & P2 --> LB
    LB --> Broker1 & Broker2 & Broker3
    Broker1 & Broker2 --> C1 & C2

    CO -.-> Broker1 & Broker2 & Broker3
    CO <--> META
...
flowchart LR
    ...
    subgraph "Message Queue Storage"
        MQ1[Sequential I/O]:::green
        MQ2[Append-Only]:::green
        MQ3[Simple Reads by Offset]:::green
    end

    subgraph "Traditional Database"
        DB1[Random I/O]:::red
        DB2[ACID Overhead]:::red
        DB3[Index Maintenance]:::red
    end
...
flowchart TB
    ...
    subgraph "Metadata Storage"
        direction LR
        M1[Topic Configs]:::orange
        M2[Consumer Offsets]:::orange
        M3[Partition Assignments]:::orange
    end

    subgraph "Message Storage"
        direction TB
        L1[Topic: orders]:::primary
        L2[Topic: payments]:::primary
        L1 --> P0[(Partition 0<br/>Segment Files)]:::purple
        L1 --> P1[(Partition 1<br/>Segment Files)]:::purple
        L2 --> P2[(Partition 0<br/>Segment Files)]:::purple
    end
...
```mermaid
flowchart LR
    subgraph "Message Record"
        direction LR
        A[Offset<br/>8 bytes]:::primary
        B[Timestamp<br/>8 bytes]:::primary
        C[Key Length<br/>4 bytes]:::secondary
        D[Key<br/>variable]:::secondary
        E[Value Length<br/>4 bytes]:::orange
        F[Value<br/>variable]:::orange
        G[CRC<br/>4 bytes]:::green
    end
...
flowchart TB
    subgraph "Delivery Semantics Spectrum"
        direction LR
        AMO[At-Most-Once<br/>May lose, never duplicate]:::orange
        ALO[At-Least-Once<br/>Never lose, may duplicate]:::primary
        EO[Exactly-Once<br/>Never lose, never duplicate]:::green
    end
...
sequenceDiagram
    ...
    P->>B: Send message
    Note over P: Continue immediately<br/>(don't wait for ACK)
    Note over B: May crash before<br/>persisting = message lost
```
...
sequenceDiagram
    ...
    P->>B: Send message
    B->>B: Persist to disk
    Note over B: ACK lost in network
    P->>B: Retry (timeout)
    Note over B: Message stored twice
    B-->>P: ACK
```
...
flowchart LR
    subgraph "Idempotent Producer"
        P[Producer ID: 42]:::primary
    end

    subgraph "Broker Tracking"
        B[Last seq for P42: 99]:::orange
    end

    P -->|"seq=100"| B
    B -->|"Accept (new)"| R1[Stored]:::green
    P -->|"seq=100 (retry)"| B
    B -->|"Reject (duplicate)"| R2[Ignored]:::red
...
flowchart LR
    subgraph "Trade-off"
        direction LR
        GO[Global Ordering<br/>1 partition = 1 consumer]:::red
        PO[Partition Ordering<br/>N partitions = N consumers]:::primary
        NO[No Ordering<br/>Maximum parallelism]:::green
    end
...
flowchart TB
    subgraph "User Events"
        U1[user-123: login]:::primary
        U2[user-123: purchase]:::primary
        U3[user-123: logout]:::primary
        U4[user-456: login]:::orange
        U5[user-456: browse]:::orange
    end

    subgraph "Partitions"
        P0[Partition 0]:::purple
        P1[Partition 1]:::purple
    end

    U1 & U2 & U3 -->|"hash(user-123) = 0"| P0
    U4 & U5 -->|"hash(user-456) = 1"| P1

    subgraph "Consumers"
        C0[Consumer 0<br/>sees user-123 in order]:::green
        C1[Consumer 1<br/>sees user-456 in order]:::green
    end
...
flowchart TB
    subgraph "Partition 0"
        L[Leader<br/>offset: 1000]:::purple
        F1[Follower 1<br/>offset: 1000<br/>IN SYNC]:::green
        F2[Follower 2<br/>offset: 950<br/>LAGGING]:::red
    end

    subgraph "ISR Set"
        ISR[Leader + Follower 1]:::green
    end
...
flowchart LR
    ...
    subgraph "acks=all (All ISR)"
		direction LR
        A2[Producer sends]:::rose --> B2[Leader persists]:::primary
        B2 --> C2[Followers persist]:::primary
        C2 --> D2[ACK to producer]:::green
        D2 --> E2[Strongest guarantee]:::green
    end

    subgraph "acks=1 (Leader Only)"
		direction LR
        A1[Producer sends]:::rose --> B1[Leader persists]:::primary
        B1 --> C1[ACK to producer]:::green
        C1 -.->|"May lose if leader crashes"| D1[before replication]:::orange
    end

    subgraph "acks=0 (Fire and Forget)"
		direction LR
        A0[Producer sends]:::rose --> B0[Returns immediately]:::green
        B0 -.->|"May lose data"| C0[No guarantee]:::red
    end
...
sequenceDiagram
    ...
    CO->>CO: Detect missed heartbeat
    CO->>CO: Select new leader from ISR
    CO->>F1: You are the new leader
    F1->>F1: Promote to leader role
    CO->>F2: New leader is F1
    Note over F1: Resume accepting writes
```
...
```mermaid
flowchart TB
    subgraph "Rebalance Triggers"
        T1[Consumer joins]:::green
        T2[Consumer leaves]:::orange
        T3[Consumer crashes]:::red
        T4[Partitions added]:::primary
        T5[Subscription changes]:::primary
    end

    T1 & T2 & T3 & T4 & T5 --> R[REBALANCE]:::purple
...
sequenceDiagram
    ...
    Note over C3: C3 joins group
    CO->>C1: Stop processing, revoke partitions
    CO->>C2: Stop processing, revoke partitions
    Note over C1,C2: All consumers stopped
    CO->>CO: Calculate new assignment
    CO->>C1: New assignment: P0, P1
    CO->>C2: New assignment: P2, P3
    CO->>C3: New assignment: P4, P5
    Note over C1,C3: All resume processing
```
...
sequenceDiagram
    ...
    Note over C3: C3 joins group
    CO->>CO: Calculate new assignment
    Note over CO: Only C2 needs to give up P4, P5
    CO->>C2: Revoke P4, P5 only
    Note over C1: Continues processing P0, P1, P2
    Note over C2: Continues processing P3
    CO->>C3: Assign P4, P5
    Note over C1,C3: Minimal disruption
```
...
flowchart TB
    ...
    subgraph "Sticky Assignment"
        S1[Keep previous<br/>assignment when<br/>possible]:::green
    end

    subgraph "Round-Robin Assignment"
        RR1[C1: P0, P3]:::orange
        RR2[C2: P1, P4]:::orange
        RR3[C3: P2, P5]:::orange
    end
...
flowchart TB
    ...
    subgraph "With Batching"
        B1[Msg 1]:::primary --> Batch[Batch<br/>1000 msgs]:::green
        B2[Msg 2]:::primary --> Batch
        B3[...]:::primary --> Batch
        Batch --> N[1 Network call]:::green
    end

    subgraph "Without Batching"
        M1[Msg 1]:::rose --> N1[Network call]:::red
        M2[Msg 2]:::rose --> N2[Network call]:::red
        M3[Msg 3]:::rose --> N3[Network call]:::red
    end
...
flowchart LR
    subgraph "Retention Policies"
        T[Time-Based<br/>Delete after N days]:::primary
        S[Size-Based<br/>Delete when > N GB]:::orange
        C[Compaction<br/>Keep latest per key]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Partition 0 Segments"
        S1[Segment 1<br/>Jan 1-7<br/>EXPIRED]:::red
        S2[Segment 2<br/>Jan 8-14<br/>EXPIRED]:::red
        S3[Segment 3<br/>Jan 15-21<br/>Keep]:::green
        S4[Segment 4<br/>Jan 22-now<br/>Active]:::primary
    end

    S1 -->|delete| D[Deleted]:::red
    S2 -->|delete| D
