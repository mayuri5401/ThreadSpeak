---
id: "hld-interviews-distributed-infrastructure-design-distributed-cache"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Distributed Infrastructure"
subSection: ""
title: "Design Distributed Cache"
slug: "hld-interviews-distributed-infrastructure-design-distributed-cache"
summary: "This problem touches on many fundamental system design concepts: data partitioning, consistency vs availability trade-offs, failure handling, and cache invalidation (famously one of the two hard problems in computer science)."
eli10: "Imagine Design Distributed Cache as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Distributed Cache Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TB
    subgraph "Storage Math"
        A[100 TB Total Data]:::primary --> B[64 GB per Node]:::orange
        B --> C[~1,600 Nodes Required]:::green
    end
...
```mermaid
flowchart LR
    subgraph "Core Cache Operations"
        PUT[PUT /cache/key<br/>Store value]:::green
        GET[GET /cache/key<br/>Retrieve value]:::primary
        DEL[DELETE /cache/key<br/>Remove entry]:::red
        BATCH[POST /cache/batch<br/>Bulk operations]:::orange
    end
...
```mermaid
flowchart LR
    A[Application]:::rose --> B[Cache Client Library]:::secondary
    B --> C[Cache Node]:::primary
    C --> D[In-Memory<br/>Hash Table]:::purple
...
sequenceDiagram
    ...
    App->>Client: cache.put("user:123", data)
    Client->>Node: PUT request
    Node->>HT: Insert at hash(key)
    HT-->>Node: Done
    Node-->>Client: Success
    Client-->>App: OK
```
...
```mermaid
flowchart TB
    subgraph "Keys Map to Nodes"
        K1[key:123 → hash:2500<br/>Maps to Node B]:::orange
        K2[key:456 → hash:5500<br/>Maps to Node C]:::orange
        K3[key:789 → hash:900<br/>Maps to Node A]:::orange
    end

    subgraph "Nodes on Ring"
        N1[Node A<br/>position: 1000]:::primary
        N2[Node B<br/>position: 4000]:::primary
        N3[Node C<br/>position: 7000]:::primary
    end
	
    subgraph "Consistent Hashing Ring"
        direction TB
        R[Hash Ring<br/>0 to 2³² - 1]:::secondary
    end	
...
```mermaid
flowchart LR
    A[Application]:::rose --> B[Cache Client]:::secondary
    B --> C{Router<br/>Consistent Hash}:::orange
    C --> D[Node A]:::primary
    C --> E[Node B]:::primary
    C --> F[Node C]:::primary
    C --> G[Node D]:::primary
...
```mermaid
flowchart LR
    subgraph "Key 'user:123' Replicas"
        direction TB
        K[Key Position<br/>hash = 2500]:::orange
    end

    subgraph "Node Positions"
        N1[Node A: 1000]:::secondary
        N2[Node B: 4000<br/>PRIMARY]:::primary
        N3[Node C: 7000<br/>Replica 2]:::purple
        N4[Node D: 9000<br/>Replica 3]:::purple
    end

    K -.->|"Clockwise lookup"| N2
    N2 -.->|"Next node"| N3
    N3 -.->|"Next node"| N4
...
flowchart TB
    ...
    subgraph "Cache Clients"
        C1[Client 1]:::secondary
        C2[Client 2]:::secondary
        C3[Client 3]:::secondary
    end
    ...
    CM -->|"Topology updates"| C1
    CM -->|"Topology updates"| C2
    CM -->|"Topology updates"| C3
...
sequenceDiagram
    ...
    participant N_B as Node B (fails)
    participant Clients as Cache Clients
    participant N_C as Node C (takes over)

    CM->>N_B: Heartbeat
    Note over N_B: No response
    CM->>N_B: Heartbeat (retry)
    Note over N_B: No response
    CM->>N_B: Heartbeat (retry)
    Note over N_B: No response (3 failures)
    Note over CM: Mark Node B as DOWN
    CM->>Clients: Topology update: Node B removed
    Note over Clients: Update routing table
    Note over N_C: Keys from B now served by replicas
    Note over Clients: Requests for B's keys go to C
```
...
flowchart TB
    subgraph "Application Servers"
        A1[App Server 1]:::rose
        A2[App Server 2]:::rose
        A3[App Server 3]:::rose
    end

    subgraph "Cache Client Libraries"
        C1[Client Library<br/>+ Router]:::secondary
        C2[Client Library<br/>+ Router]:::secondary
        C3[Client Library<br/>+ Router]:::secondary
    end

    subgraph "Cache Cluster (1,600 nodes)"
        N1[Node 1]:::primary
        N2[Node 2]:::primary
        N3[Node 3]:::primary
        N4[Node N...]:::primary
    ...
    CM[Cluster Manager<br/>Heartbeats + Topology]:::orange
    ...
    CM -.->|"Health monitoring"| N1
    CM -.->|"Health monitoring"| N2
    CM -.->|"Health monitoring"| N3
    CM -.->|"Health monitoring"| N4
...
flowchart TB
    subgraph "Cache Memory"
        direction TB
        HT[Primary Hash Table<br/>Key → Entry Pointer]:::primary
        EI[Expiration Index<br/>Sorted by expire_time]:::orange
        LRU[LRU List<br/>Access order tracking]:::purple
        DATA[Entry Data<br/>Keys + Values + Metadata]:::secondary
    end
...
> **Why not a tree-based structure"**
>
...
```mermaid
flowchart LR
    HEAD[HEAD<br/>Most Recent]:::green --> A[Entry A]:::primary
    A --> B[Entry B]:::primary
    B --> C[Entry C]:::primary
    C --> TAIL[TAIL<br/>Least Recent]:::red
...
flowchart TB
    ...
    subgraph "After: 11 Nodes"
        K4["key_A → hash=42 → 42%11 = Node 9 ❌"]:::red
        K5["key_B → hash=73 → 73%11 = Node 7 ❌"]:::red
        K6["key_C → hash=89 → 89%11 = Node 1 ❌"]:::red
    end

    subgraph "Before: 10 Nodes"
        K1["key_A → hash=42 → 42%10 = Node 2"]:::primary
        K2["key_B → hash=73 → 73%10 = Node 3"]:::primary
        K3["key_C → hash=89 → 89%10 = Node 9"]:::primary
    end
...
flowchart LR
    subgraph "Mapping Rules"
        M1[1. Hash each node's ID to get its position]:::primary
        M2[2. Hash each key to get its position]:::primary
        M3[3. Walk clockwise to find owning node]:::primary
    end

    subgraph "The Ring Concept"
        direction TB
        R[Circular Hash Space<br/>0 → 2³² - 1 → 0]:::secondary
    end
...
flowchart TB
    ...
    subgraph "Solution: Virtual Nodes"
        S1[Node A → 100 virtual nodes<br/>scattered across ring]:::primary
        S2[Node B → 100 virtual nodes<br/>scattered across ring]:::primary
        S3[Node C → 100 virtual nodes<br/>scattered across ring]:::primary
    end

    subgraph "Problem: Uneven Distribution"
        P1[Node A owns 40% of ring]:::red
        P2[Node B owns 35% of ring]:::orange
        P3[Node C owns 25% of ring]:::green
    end
...
flowchart LR
    subgraph "LRU Data Structure"
        direction LR
        HEAD[HEAD<br/>Most Recent]:::green --> A[Entry A<br/>accessed 1s ago]:::primary
        A --> B[Entry B<br/>accessed 5s ago]:::primary
        B --> C[Entry C<br/>accessed 10s ago]:::primary
        C --> TAIL[TAIL<br/>Evict First]:::red
    end
...
```mermaid
flowchart TB
    A[Memory Full<br/>Need to Evict]:::red --> B[Sample 5 Random Keys]:::primary
    B --> C[Check last_accessed<br/>for each sample]:::primary
    C --> D[Evict the Oldest<br/>among samples]:::orange
    D --> E{Need more space"}
    E -->|Yes| B
    E -->|No| F[Done]:::green
...
```mermaid
flowchart TB
    E[Entry Lifecycle]:::primary --> T{TTL expired"}
    T -->|Yes| R[Remove Entry]:::red
    T -->|No| M{Memory pressure"}
    M -->|Yes| L[LRU Eviction]:::orange
    M -->|No| K[Keep Entry]:::green
...
flowchart LR
    subgraph "Consistency Spectrum"
        E[Eventual Consistency<br/>Fast, simple, may be stale]:::green
        S[Strong Consistency<br/>Slow, complex, always fresh]:::red
    end

    E --> |"TTL-based"| M1[...]:::secondary
    M1 --> |"Cache-aside"| M2[...]:::secondary
    M2 --> |"Write-through"| S
...
```mermaid
flowchart LR
    A[Application]:::rose --> C[Cache]:::primary
    C --> D[(Database)]:::purple

    A -->|"1. Write to cache"| C
    C -->|"2. Sync write to DB"| D
    D -->|"3. Confirm"| C
...
flowchart LR
    A[Application]:::rose --> C[Cache]:::primary
    C -.-> Q[Async Queue]:::orange
    Q -.-> D[(Database)]:::purple

    A -->|"1. Write to cache"| C
    C -->|"2. Return immediately"| A
    Q -->|"3. Batch write later"| D
...
```mermaid
flowchart TB
    APP[App Server]:::rose -->|"Publish: invalidate user:123"| BROKER[Message Broker<br/>Redis Pub/Sub, Kafka]:::orange
    BROKER --> N1[Cache Node 1]:::primary
    BROKER --> N2[Cache Node 2]:::primary
    BROKER --> N3[Cache Node 3]:::primary
...
```mermaid
flowchart TB
    subgraph "Key: user:123"
        W[Writes]:::orange --> P[Primary<br/>Node A]:::primary
        P -.->|Async replicate| R1[Replica<br/>Node B]:::purple
        P -.->|Async replicate| R2[Replica<br/>Node C]:::purple
    end
...
flowchart LR
    ...
    W[Write: 2 acks required]:::orange --> N1
    W --> N2

    R[Read: 2 responses required]:::green --> N2
    R --> N3

    N2 -.->|"At least one node<br/>has latest value"| OVERLAP[Overlap!]:::secondary
...
sequenceDiagram
    ...
    CM->>N: Heartbeat (t=3s)
    Note over CM: No response (1/3)
    CM->>N: Heartbeat (t=4s)
    Note over CM: No response (2/3)
    CM->>N: Heartbeat (t=5s)
    Note over CM: No response (3/3)
    Note over CM: Mark Node A as FAILED
```
...
flowchart LR
    subgraph "The Problem"
        N1[Node A<br/>100K req/s<br/>Hot key here!]:::red
        N2[Node B<br/>600 req/s]:::green
        N3[Node C<br/>600 req/s]:::green
        N4[Node D<br/>600 req/s]:::green
    end
...
```mermaid
flowchart LR
    APP[Application Server]:::rose --> L1{L1 Cache<br/>In-process memory}:::primary
    L1 -->|Hit| RETURN[Return immediately]:::green
    L1 -->|Miss| L2[L2: Distributed Cache]:::orange
    L2 --> RETURN
...
```mermaid
flowchart TB
    HK[Hot Key<br/>taylor:profile]:::orange --> D[Detect: > 1000 req/s]:::primary
    D --> REP[Replicate to<br/>5 additional nodes]:::purple
    REP --> N1[Node A]:::green
    REP --> N2[Node B]:::green
    REP --> N3[Node C]:::green
    REP --> N4[Node D]:::green
...
flowchart TB
    subgraph "Security Layers"
        N[Network Isolation<br/>Private VPC, no internet access]:::primary
        A[Authentication<br/>Username/password or certificates]:::orange
        E[Encryption in Transit<br/>TLS for all connections]:::purple
        AC[Access Control<br/>Read/write permissions]:::green
    end

    EXT[External Attacker]:::red -.->|"Blocked"| N
    INT[Internal Service]:::secondary --> N --> A --> E --> AC --> CACHE[(Cache)]:::primary
...
```mermaid
flowchart TB
    E[Key Expires]:::red --> R1[Request 1]:::primary
    E --> R2[Request 2]:::primary
    E --> R3[Request 3]:::primary

    subgraph "Without Lock"
        R1 --> DB1[Hit DB]:::purple
        R2 --> DB2[Hit DB]:::purple
        R3 --> DB3[Hit DB]:::purple
    end

    subgraph "With Lock"
        R1 --> L[Acquire Lock]:::orange
        L --> DB[Hit DB once]:::purple
        R2 -.->|Wait| L
        R3 -.->|Wait| L
    end
