---
id: "hld-interviews-real-time-communication-design-video-conferencing-system-like-zoom"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Real Time Communication"
subSection: ""
title: "Design Video Conferencing System like Zoom"
slug: "hld-interviews-real-time-communication-design-video-conferencing-system-like-zoom"
summary: "This system design problem tests your understanding of real-time communication protocols, media streaming architectures, and scaling strategies for latency-sensitive systems."
eli10: "Imagine Design Video Conferencing System like Zoom as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Video Conferencing System like Zoom Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Real Time Communication","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Design Video Conferencing System like Zoom"
  code: |
    50 million participants × 7.5 Mbps = 375 Petabits/second
---
...
```mermaid
flowchart LR
    subgraph Scale["System Scale"]
        M["10M Concurrent<br/>Meetings"]:::primary
        P["50M Concurrent<br/>Participants"]:::orange
    end

    subgraph Bandwidth["Bandwidth Challenge"]
        BW["375 Petabits/sec<br/>Raw Requirement"]:::red
    end

    subgraph Solutions["Optimization Strategies"]
        S1["Distributed<br/>Media Servers"]:::green
        S2["Selective<br/>Forwarding"]:::green
        S3["Adaptive<br/>Bitrate"]:::green
    end
...
```shell
Servers needed = 50 million participants / 500 per server = 100,000 media servers
```
...
```shell
Daily storage = 10M meetings × 10% recorded × 0.75 GB = 750 TB/day
```
...
```json
{
  ...
  "join_url": "https://meet.example.com/j/123456789",
  "host_key": "abc123xyz",
  "password": "meeting123",
...
```json
{
  ...
  "signaling_server_url": "wss://signal-us-east.example.com/ws",
  "ice_servers": [
...
sequenceDiagram
    ...
    API->>MS: Create meeting request
    MS->>MS: Generate meeting ID<br/>and password hash
    MS->>DB: Store meeting metadata
    DB-->>MS: Stored successfully
    MS-->>API: Meeting details
    API-->>Host: meeting_id, join_url, host_key
```
...
flowchart TB
    ...
    subgraph Signaling["Signaling Layer"]
        SIG["Signaling Server<br/>(WebSocket)"]:::secondary
    end

    subgraph Media["Media Layer"]
        SFU["Media Server<br/>(SFU)"]:::orange
        TURN["TURN Server<br/>(Fallback)"]:::red
    end

    C1 & C2 & C3 <-.->|"Signaling<br/>(WebSocket)"| SIG
    C1 & C2 & C3 ==>|"Media<br/>(UDP/RTP)"| SFU
    C1 -.->|"Fallback"| TURN
    TURN --> SFU
...
```mermaid
flowchart TB
    subgraph WebRTC["WebRTC Protocol Stack"]
        direction TB
        APP["Application<br/>(Your Code)"]:::primary

        subgraph Media["Media Layer"]
            SRTP["SRTP<br/>(Encrypted Media)"]:::orange
            RTP["RTP/RTCP<br/>(Media Transport)"]:::orange
        end

        subgraph Security["Security Layer"]
            DTLS["DTLS<br/>(Key Exchange)"]:::red
        end

        subgraph Transport["Transport Layer"]
            ICE["ICE<br/>(NAT Traversal)"]:::secondary
            STUN["STUN<br/>(Discover Public IP)"]:::purple
            TURN["TURN<br/>(Relay Fallback)"]:::purple
        end

        UDP["UDP (preferred) / TCP (fallback)"]:::green
    end
...
sequenceDiagram
    ...
    Note over C,TURN: Phase 1: Connect to Signaling
    C->>SS: Connect (WebSocket)
    SS-->>C: Connected + participant list

    Note over C,TURN: Phase 2: Media Negotiation (SDP Exchange)
    C->>C: Create WebRTC PeerConnection
    C->>C: Generate SDP Offer<br/>(codecs, capabilities)
    C->>SS: Send SDP Offer
    SS->>SFU: Forward offer
    SFU->>SFU: Generate SDP Answer
    SFU-->>SS: SDP Answer
    SS-->>C: Receive SDP Answer

    Note over C,TURN: Phase 3: ICE Connectivity
    C->>SS: Send ICE Candidates
    SS->>SFU: Forward ICE candidates
    SFU-->>SS: SFU ICE candidates
    SS-->>C: Receive ICE candidates

    Note over C,TURN: Phase 4: Media Flow
    alt Direct connection succeeds
        C->>SFU: Media streams (UDP/RTP)
        SFU->>C: Other participants' streams
    else Direct connection fails (firewall)
        C->>TURN: Media streams (relayed)
        TURN->>SFU: Forward media
        SFU->>TURN: Other participants' streams
        TURN->>C: Forward streams
    end
```
...
sequenceDiagram
    ...
    P->>P: User clicks "Share Screen"
    P->>P: Browser shows screen picker
    P->>P: getDisplayMedia() captures screen

    P->>SFU: Signal: "Starting screen share"
    P->>SFU: Add screen share media track
    SFU-->>P: Track accepted

    loop Screen content streaming
        P->>SFU: Stream screen frames (RTP)
        SFU->>V1: Forward screen stream
        SFU->>V2: Forward screen stream
    end

    Note over V1,V2: Display screen share<br/>in main view

    P->>SFU: User clicks "Stop Sharing"
    P->>SFU: Signal: "Screen share ended"
    SFU->>V1: Screen share ended
    SFU->>V2: Screen share ended
```
...
flowchart TB
    subgraph Clients["Client Applications"]
        Web["Web Client<br/>(Browser)"]:::primary
        Mobile["Mobile App<br/>(iOS/Android)"]:::primary
        Desktop["Desktop App<br/>(Electron)"]:::primary
    end
    ...
    subgraph SignalingLayer["Signaling Layer"]
        SS1["Signaling<br/>Server 1"]:::signaling
        SS2["Signaling<br/>Server 2"]:::signaling
        Redis[("Redis<br/>Pub/Sub")]:::signaling
    end

    subgraph MediaLayer["Media Layer"]
        SFU1["SFU 1<br/>(US-East)"]:::media
        SFU2["SFU 2<br/>(US-West)"]:::media
        SFU3["SFU 3<br/>(EU)"]:::media
        TURN["TURN Server<br/>(Relay)"]:::red
    end

    subgraph Services["Application Services"]
        MS["Meeting<br/>Service"]:::service
        PS["Presence<br/>Service"]:::service
        RS["Recording<br/>Service"]:::service
    end

    subgraph Storage["Data Storage"]
        MeetingDB[("PostgreSQL<br/>(Meetings)")]:::storage
        ChatDB[("Cassandra<br/>(Chat)")]:::storage
        S3[("S3<br/>(Recordings)")]:::storage
    end
    ...
    Web -.->|"WebSocket"| SS1
    Mobile -.->|"WebSocket"| SS2
    Desktop -.->|"WebSocket"| SS1
    SS1 <--> Redis
    SS2 <--> Redis

    Web ==>|"Media (UDP)"| SFU1
    Mobile ==>|"Media (UDP)"| SFU2
    Desktop ==>|"Media (UDP)"| SFU3

    Web -.->|"Firewall blocked""| TURN
    TURN --> SFU1
    ...
    SFU1 -.->|"Recording stream"| RS

    classDef primary fill:#00ceff,stroke:#000,color:#000
    classDef signaling fill:#38d9a9,stroke:#000,color:#000
    classDef media fill:#ffa94d,stroke:#000,color:#000
    classDef service fill:#9775fa,stroke:#000,color:#000
    classDef storage fill:#da77f2,stroke:#000,color:#000
    classDef red fill:#ff8787,stroke:#000,color:#000
```
...
flowchart LR
    subgraph PostgreSQL["PostgreSQL (Relational)"]
        U[("Users")]:::purple
        M[("Meetings")]:::purple
        P[("Participants")]:::purple
        R[("Recordings<br/>Metadata")]:::purple
    end

    subgraph Cassandra["Cassandra (Time-series)"]
        C[("Chat<br/>Messages")]:::orange
    end

    subgraph Redis["Redis (Ephemeral)"]
        PR[("Live Presence<br/>& State")]:::red
    end

    subgraph S3["Object Storage"]
        REC[("Recording<br/>Files (MP4)")]:::secondary
    end
...
```mermaid
flowchart TB
    subgraph Mesh["Mesh Topology (4 participants)"]
        A["User A"]:::primary
        B["User B"]:::primary
        C["User C"]:::primary
        D["User D"]:::primary
...
```mermaid
flowchart TB
    subgraph MCU["MCU Topology"]
        A["User A"]:::primary
        B["User B"]:::primary
        C["User C"]:::primary
        D["User D"]:::primary

        M["MCU Server<br/>(Decode → Mix → Encode)"]:::orange
        ...
        M -->|"Download 1 mixed stream"| A
        M -->|"Download 1 mixed stream"| B
        M -->|"Download 1 mixed stream"| C
        M -->|"Download 1 mixed stream"| D
    end
...
```mermaid
flowchart TB
    subgraph SFU["SFU Topology"]
        A["User A"]:::primary
        B["User B"]:::primary
        C["User C"]:::primary
        D["User D"]:::primary

        S["SFU Server<br/>(Forward packets, no transcoding)"]:::green
        ...
        S -->|"Streams from B,C,D"| A
        S -->|"Streams from A,C,D"| B
        S -->|"Streams from A,B,D"| C
        S -->|"Streams from A,B,C"| D
    end
...
flowchart TB
    subgraph US["US Region"]
        U1["US Users<br/>(150 people)"]:::primary
        SFU_US["SFU-US"]:::orange
        U1 --> SFU_US
    end

    subgraph EU["EU Region"]
        E1["EU Users<br/>(200 people)"]:::primary
        SFU_EU["SFU-EU"]:::orange
        E1 --> SFU_EU
    end

    subgraph Asia["Asia Region"]
        A1["Asia Users<br/>(150 people)"]:::primary
        SFU_ASIA["SFU-Asia"]:::orange
        A1 --> SFU_ASIA
    end

    SFU_US <-->|"Exchange active<br/>speaker streams"| SFU_EU
    SFU_EU <-->|"Exchange active<br/>speaker streams"| SFU_ASIA
    SFU_US <-->|"Exchange active<br/>speaker streams"| SFU_ASIA
...
```mermaid
flowchart LR
    subgraph Meeting["100-Person Meeting"]
        ALL["100 Participants"]:::primary
    end

    subgraph Optimization["What Each Client Actually Receives"]
        S1["Active Speaker<br/>1 stream @ 1080p"]:::green
        S2["Visible in Gallery<br/>24 streams @ 360p"]:::orange
        S3["Off-screen<br/>75 streams @ nothing"]:::red
    end

    subgraph Result["Result"]
        R["25 video streams<br/>instead of 100<br/><br/>~90% bandwidth saved"]:::secondary
    end
...
```mermaid
flowchart LR
    subgraph Sender["Sender Encodes 3 Layers"]
        H["High: 1080p<br/>2.5 Mbps"]:::green
        M["Medium: 720p<br/>1 Mbps"]:::orange
        L["Low: 360p<br/>300 Kbps"]:::red
    end

    subgraph SFU["SFU Decision"]
        S["Pick the right<br/>layer for each<br/>recipient"]:::purple
    end

    subgraph Receivers["Different Recipients"]
        R1["Alice (good network,<br/>viewing speaker)<br/>→ High layer"]:::green
        R2["Bob (average network)<br/>→ Medium layer"]:::orange
        R3["Carol (on mobile,<br/>limited bandwidth)<br/>→ Low layer"]:::red
    end
...
```mermaid
flowchart LR
    subgraph Network["Network (Variable Timing)"]
        N1["P1"]:::primary
        N2["P2"]:::primary
        N3["...gap..."]:::red
        N4["P3 P4 P5"]:::primary
    ...
    subgraph Buffer["Jitter Buffer (50ms)"]
        B["Queue packets<br/>Smooth timing"]:::orange
    end

    subgraph Playback["Playback (Consistent)"]
        P1["P1"]:::green
        P2["P2"]:::green
        P3["P3"]:::green
        P4["P4"]:::green
...
```mermaid
flowchart TB
    MONITOR["Monitor Network<br/>Loss, RTT, Bandwidth"]:::primary

    MONITOR --> CHECK{Network<br/>Quality"}:::orange

    CHECK -->|"Loss > 5%<br/>RTT > 200ms"| DEGRADE["Degrade Quality"]:::red
    CHECK -->|"Loss < 1%<br/>RTT < 100ms"| IMPROVE["Improve Quality"]:::green
    CHECK -->|"Stable"| MAINTAIN["Maintain Current"]:::secondary

    DEGRADE --> D1["1080p → 720p"]:::red
    DEGRADE --> D2["720p → 480p"]:::red
    DEGRADE --> D3["Reduce framerate"]:::red

    IMPROVE --> I1["480p → 720p"]:::green
    IMPROVE --> I2["720p → 1080p"]:::green

    D1 & D2 & D3 & I1 & I2 & MAINTAIN --> MONITOR
...
sequenceDiagram
    ...
    Note over R: Detects gap<br/>(missing Packet 3)

    R->>S: NACK: Packet 3 missing
    S->>R: Retransmit Packet 3

    Note over R: All packets received ✓
```
...
flowchart LR
    ...
    classDef primary fill:#00ceff,stroke:#000,color:#000
    classDef server fill:#ffa94d,stroke:#000,color:#000
    classDef broker fill:#9775fa,stroke:#000,color:#000
...
sequenceDiagram
    participant Host
    participant SFU
    participant Recording Bot
    participant Processing
    participant S3

    Host->>SFU: Start recording
    SFU->>Recording Bot: Join meeting
    Recording Bot->>SFU: Subscribe to all streams

    loop During meeting
        SFU->>Recording Bot: Forward media streams
        Recording Bot->>Recording Bot: Write to local disk
    end

    Host->>SFU: Stop recording
    SFU->>Recording Bot: Leave meeting
    Recording Bot->>Processing: Upload raw recording
    Processing->>Processing: Transcode, generate layouts
    Processing->>S3: Store final recording
    Processing->>Host: Recording ready notification
```
...
sequenceDiagram
    ...
    P->>S: Request to join meeting
    S->>S: Verify password ✓
    S->>S: Waiting room enabled"
    S-->>P: Placed in waiting room

    S->>H: "John wants to join"
    Note over H: Review participant

    alt Host admits
        H->>S: Admit participant
        S->>P: Admitted to meeting ✓
    else Host denies
        H->>S: Deny participant
        S->>P: Entry denied ❌
    end
```
...
flowchart LR
    subgraph Sender["Sender"]
        S["Encrypt with<br/>session key"]:::primary
    end

    subgraph SFU["SFU Server"]
        F["Forward encrypted<br/>packets (cannot decrypt)"]:::orange
    end

    subgraph Receiver["Receiver"]
        R["Decrypt with<br/>session key"]:::primary
    end

    S -->|"🔒 Encrypted"| F -->|"🔒 Encrypted"| R
