---
id: "hld-interviews-real-time-communication-design-live-comments"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Real Time Communication"
subSection: ""
title: "Design Live Comments"
slug: "hld-interviews-real-time-communication-design-live-comments"
summary: "In this chapter, we will aim to design a low-latency, scalable live comment system that allows thousands or even millions of users to exchange messages in real time during a live event."
eli10: "Imagine Design Live Comments as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Live Comments Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Real Time Communication","System Design","Architecture"]
codeSnippet:
  language: "json"
  explanation: "Production implementation for Design Live Comments"
  code: |
    {
      "text": "GOAL!!! What a shot!",
      "client_ts": "2025-10-17T15:00:01.012Z",
      "meta": { "lang": "en", "device": "android" }
    }
---
...
<details>
<summary>Approach 1 (Bad): HTTP Polling</summary>

This is the simplest method imaginable. The client repeatedly asks the server, "Are there any new comments yet"".

The client sends a `GET` request every few seconds (e.g., 2 seconds). The server returns any new comments that have appeared since the last poll.

**Pros:**

- Extremely simple to implement.
- Works on all browsers and networks without issue.

**Cons:**

- **High Latency:** There's always a delay. If a comment is posted right after a poll, the client won't see it until the next poll (up to 2 seconds later in our example).
- **Inefficient:** Most requests will return an empty response, wasting server resources and client battery. At our scale of 5 million viewers, this would mean millions of useless requests per second, instantly overwhelming our backend.

**Verdict:** Unsuitable for a large-scale, low-latency system.
</details>

<details>
<summary>Approach 2 (Good): WebSockets</summary>

WebSockets provide a persistent, **bi-directional** (full-duplex) communication channel over a single TCP connection.

The client establishes a single, persistent `ws://` or `wss://` connection. The server can then push comments to the client instantly, and the client can also send data (like new comments) back on the same connection.

**Pros:**

- **Extremely Low Latency:** The connection is always open, so data can be pushed instantly.
- **Bi-directional:** The client can both send and receive data on the same connection.

**Cons:**

- **Complexity:** Managing stateful connections for millions of users is complex. You need dedicated servers and logic to handle reconnects, heartbeats, etc.
- **Proxy/Firewall Issues:** Some corporate firewalls or older network proxies may block WebSocket connections.

**Verdict:** A very strong contender and often the default choice for chat. It offers the best performance but it may be a overkill since we only need to stream in once direction (from server to client).
</details>

<details>
<summary>Approach 3 (Great): Server-Sent Events (SSE)</summary>

SSE provides a persistent, **uni-directional** connection where the server can push data to the client over standard HTTP.

The client makes a single `GET` request to an SSE endpoint. The server keeps this connection open and sends a stream of events to the client as new comments arrive.

**Pros:**

- **Simple & Standard:** It's just HTTP. No special protocols are needed, making it firewall-friendly.
- **Automatic Reconnection:** The browser's native `EventSource` API automatically handles dropped connections, which is a huge advantage for resilience, especially on mobile networks.
- **Lightweight:** It's simpler than WebSockets, as there's no bi-directional protocol to manage.

**Cons:**

- **Uni-directional:** The client cannot send data back to the server over the SSE connection.

**Verdict:** A great choice for our specific one-directional messaging architecture.
</details>
...
```shell
event: new_comment
data: {"comment_id":"c1","user_id":"u1","username":"Alice","comment_text":"First comment!"}

event: new_comment
data: {"comment_id":"c2","user_id":"u2","username":"Bob","comment_text":"Hello everyone!"}
...
<!-- payload:embedBlock:START {"id":"68f26d507710e26b72bd6019","url":"https://link.excalidraw.com/readonly/FFOeFU8emrYEZZXoFjhp","mermaidCode":"sequenceDiagram\n    autonumber\n    participant U as User (Client)\n    participant G as API Gateway\n    participant CS as Comment Service\n    participant DB as NoSQL Database\n    participant MB as Message Broker (Pub/Sub)\n\n    U->>G: POST /comment {\"text\":\"GOAL!!!\", \"event_id\":\"1234\"}\n    G->>CS: Forward request (after auth & rate limiting)\n    CS->>DB: Store comment (persistent record)\n    CS->>MB: Publish comment event (for fanout)\n    MB-->>CS: Ack\n    CS-->>G: 200 OK (Comment accepted)\n    G-->>U: Response {\"status\":\"success\",\"timestamp\":...}"} -->
```mermaid
sequenceDiagram
    autonumber
    participant U as User (Client)
...
[Embed: https://link.excalidraw.com/readonly/hk4T3cGHzyHtQMOHS9n7](https://link.excalidraw.com/readonly/hk4T3cGHzyHtQMOHS9n7)
...
<!-- payload:embedBlock:START {"id":"68f271a67710e26b72bd601b","url":"https://link.excalidraw.com/readonly/CyIxVbGQXLtlIQBxlCkv","mermaidCode":"sequenceDiagram\n    autonumber\n    participant CS as Comment Service\n    participant MB as Message Broker\n    participant GW as Connection Gateway\n    participant CL as Clients (Viewers)\n\n    CS->>MB: Publish comment to topic \"event_1234\"\n    MB-->>GW: Broadcast comment to subscribed gateways\n    GW-->>CL: Push comment to connected WebSocket clients"} -->
```mermaid
sequenceDiagram
    autonumber
    participant CS as Comment Service
...
| --- | --- | --- | --- |
| **Redis Pub/Sub** |  |  | Ephemeral data (live chats) |
| **Apache Kafka** |  |  | Mission-critical event pipelines |
| **Managed Pub/Sub (GCP/AWS)** |  |  | Moderate-scale, general use cases |
...
```shell
comments:stream:123  → subscribed by gateways serving viewers of Stream 123
comments:stream:999  → subscribed by gateways serving viewers of Stream 999
```
...
For the largest events on Earth (e.g., a World Cup final, a new product launch) even a regional Redis instance can become overwhelmed. This is where we need a multi-layered approach to soften the blow.
 ...
 CentralBus\n    CentralBus -- 2. Replicates to --> AggregatorUS\n    CentralBus -- 2. Replicates to --> AggregatorEU\n\n    AggregatorUS -- 3. Re-publishes to --> RedisUS\n    AggregatorEU -- 3. Re-publishes to --> RedisEU\n\n    RedisUS -- 4. Fans out to --> GW_US1 & GW_US2\n    RedisEU -- 4. Fans out to --> GW_EU1 & GW_EU2"} -->
```mermaid
flowchart TD
    CommentSvc[Comment Service]

    subgraph Central Message Bus
        CentralBus[Kafka / Google Pub/Sub]
    end

    subgraph Region: US-East
        AggregatorUS[Regional Aggregator]
        RedisUS[Regional Redis Pub/Sub]
        GW_US1[SSE Gateway] & GW_US2[SSE Gateway]
    end

    subgraph Region: EU-West
        AggregatorEU[Regional Aggregator]
        RedisEU[Regional Redis Pub/Sub]
        GW_EU1[SSE Gateway] & GW_EU2[SSE Gateway]
    end

    CommentSvc -- 1. Publish ONCE --> CentralBus
    CentralBus -- 2. Replicates to --> AggregatorUS
    CentralBus -- 2. Replicates to --> AggregatorEU
...
```json
{
  "comments": [
    {
      "comment_id": "c123",
      "timestamp": "2025-10-18T12:15:32.112Z",
      "username": "Ravi",
      "comment_text": "That was an incredible play!"
    },
    {
      "comment_id": "c124",
      "timestamp": "2025-10-18T12:15:33.456Z",
      "username": "Priya",
      "comment_text": "I can't believe it!"
    }
  ],
  "next_start_time": 1080
}
```
...
- **On Seek (Jumping on the Timeline):** This is a critical event. If a user jumps from the 15-minute mark to the 58-minute mark:
   1. The client immediately **discards its entire comment buffer**. The comments for the 15-18 minute window are now irrelevant.
   2. It instantly fires a new API request to fetch the chunk of comments corresponding to the new timestamp (e.g., comments for the 58-61 minute window).
   3. As soon as the new chunk arrives, it populates the buffer, and the render loop resumes, displaying the correct comments for that moment in the event.
