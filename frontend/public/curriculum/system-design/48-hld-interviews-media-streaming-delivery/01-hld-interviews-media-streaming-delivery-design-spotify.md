---
id: "hld-interviews-media-streaming-delivery-design-spotify"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Media Streaming Delivery"
subSection: ""
title: "Design Spotify"
slug: "hld-interviews-media-streaming-delivery-design-spotify"
summary: "This system design problem touches on several fundamental concepts: content delivery at global scale, real-time streaming protocols, search infrastructure, recommendation systems, and high availability architecture."
eli10: "Imagine Design Spotify as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Spotify Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    subgraph "Daily Traffic"
        direction TB
        W[Streams<br/>1B per day<br/>11,500 QPS avg]:::orange
        R[Metadata<br/>50K-100K QPS<br/>searches, browses]:::primary
    end

    subgraph "Peak Traffic (3x)"
        direction TB
        WP[35,000 QPS<br/>peak streams]:::orange
        RP[~300K QPS<br/>peak metadata]:::primary
    end
...
flowchart LR
    subgraph "Storage Components"
        direction TB
        A[Audio Files<br/>1 PB total]:::purple
        B[Metadata<br/>200 GB]:::primary
        C[User Data<br/>7.5 TB]:::orange
    end
...
flowchart LR
    subgraph "Core API Operations"
        ...
        Stream[GET /songs/.../stream<br/>Get audio URL]:::green
        Playlist[POST /playlists<br/>Manage playlists]:::orange
        Recs[GET /recommendations<br/>Get personalized picks]:::purple
    end
...
flowchart LR
    subgraph "Stream Path (most traffic)"
        ...
        S5 -.->|cache miss| S6[(Object Storage)]:::purple
    end
...
flowchart TB
    ...
    subgraph Edge["Edge Layer"]
        CDN[CDN Edge Servers<br/>Distributed Globally]:::green
    end

    subgraph Storage["Storage Layer"]
        S3[(Object Storage<br/>Audio Files)]:::purple
    end
...
sequenceDiagram
    ...
    Note over PS: Check subscription,<br/>region, entitlements
    PS->>PS: Generate signed URL<br/>(15 min expiry)
...
flowchart LR
    ...
    subgraph Search["Search Infrastructure"]
        ES[(Elasticsearch Cluster)]:::purple
    end
...
sequenceDiagram
    ...
    alt Cache Hit
        ...
        Note over ES: Fuzzy match,<br/>rank by relevance<br/>and popularity
...
sequenceDiagram
    ...
    Client->>API: POST /playlists/pl_123/songs<br/>{song_ids: ["song_456"]}
    API->>PLS: Forward with user context
    PLS->>Cache: Get playlist metadata
    Note over PLS: Verify user owns<br/>this playlist
...
flowchart TB
    subgraph "Batch Path (runs daily/weekly)"
        ...
        Pipeline --> Features[(Feature Store)]:::purple
        Pipeline --> Recs[(Pre-computed<br/>Recommendations)]:::purple
    end
...
sequenceDiagram
    ...
    alt Cache Hit (common case)
        ...
        Note over RS: Rank candidates<br/>by relevance
...
flowchart TB
    ...
    subgraph Edge["Edge Layer"]
        CDN[CDN<br/>Audio Delivery]:::green
    ...
    subgraph Services["Application Layer"]
        direction LR
        PS[Playback<br/>Service]:::primary
        SS[Search<br/>Service]:::primary
        PLS[Playlist<br/>Service]:::primary
        RS[Recommendation<br/>Service]:::primary
        US[User<br/>Service]:::primary
    end
    ...
    subgraph Data["Data Layer"]
        ...
        MetaDB[(Metadata DB)]:::purple
        PlaylistDB[(Playlist DB)]:::purple
        UserDB[(User DB)]:::purple
    end
...
flowchart LR
    subgraph "Data Types and Their Databases"
        Songs[Song Metadata<br/>PostgreSQL]:::primary
        Search[Search Index<br/>Elasticsearch]:::purple
        Playlists[Playlists<br/>Cassandra]:::orange
        Sessions[User Sessions<br/>Redis]:::green
        History[Listening History<br/>Cassandra]:::orange
        Recs[Recommendations<br/>Redis]:::green
    end
...
```mermaid
flowchart LR
    A[Original Audio<br/>WAV/FLAC]:::rose --> B[Transcoding<br/>Service]:::primary
    B --> C[Quality Variants]:::orange
    B --> D[Chunking]:::orange
    B --> E[Encryption<br/>DRM]:::orange
    C & D & E --> F[(Object Storage)]:::purple
...
flowchart TB
    subgraph "Tiered Caching Strategy"
        Edge[Edge Cache<br/>Top 1% songs<br/>~99% of traffic]:::green
        Regional[Regional Cache<br/>Top 10% songs<br/>Remaining hot content]:::orange
        Origin[Origin - S3<br/>All 100M songs<br/>Long tail content]:::purple
    end
...
flowchart LR
    subgraph "Data Flow"
        DB[(PostgreSQL<br/>Source of Truth)]:::purple --> Sync[Change Data<br/>Capture]:::secondary
        Sync --> ES[(Elasticsearch<br/>Search Index)]:::primary
    end
...
flowchart TB
    subgraph "Ranking Signals"
        Text[Text Relevance<br/>How well query matches]:::primary
        Pop[Popularity<br/>Stream count, listeners]:::orange
        Pers[Personalization<br/>User's listening history]:::green
        Fresh[Recency<br/>New releases boost]:::purple
    end

    Text --> Score[Final<br/>Score]:::primary
    Pop --> Score
    Pers --> Score
    Fresh --> Score
...
flowchart TB
    subgraph "Explicit Signals (Strong)"
        Like[Liked Songs<br/>User explicitly saved]:::green
        Playlist[Added to Playlist<br/>Deliberate action]:::green
    end

    subgraph "Implicit Signals (Volume)"
        History[Listening History<br/>What they play]:::primary
        Skip[Skip Behavior<br/>Skipped = dislike]:::orange
        Duration[Time Spent<br/>Full song vs partial]:::primary
    end

    subgraph "Context Signals"
        Time[Time of Day<br/>Morning vs night]:::secondary
        Device[Device Type<br/>Phone, speaker, car]:::secondary
        Activity[Activity<br/>Workout, focus, sleep]:::secondary
    end

    Like & Playlist & History & Skip & Duration & Time & Device & Activity --> Model[User Taste<br/>Model]:::purple
...
flowchart LR
    subgraph "User-Song Matrix"
        M[Millions of users<br/>× Millions of songs]:::rose
    end

    subgraph "Matrix Factorization"
        U[User Embeddings<br/>100-dim vectors]:::primary
        S[Song Embeddings<br/>100-dim vectors]:::primary
    end

    M --> |ALS/SVD| U
    M --> |ALS/SVD| S
    U --> |dot product| P[Predicted<br/>Preferences]:::green
    S --> P
...
flowchart TB
    subgraph "Input Signals"
        ...
        CB[Content-Based<br/>Filtering]:::orange
        Context[Context<br/>Model]:::secondary
    end

    subgraph "Ensemble"
        Combine[Ensemble Layer<br/>Weighted combination]:::purple
    end

    subgraph "Output"
        Recs[Personalized<br/>Recommendations]:::green
    end
...
flowchart TB
    subgraph "Data Collection"
        Events[User Events<br/>plays, skips, likes]:::rose --> Kafka[Kafka]:::secondary
    end

    subgraph "Processing"
        Kafka --> Stream[Stream Processing<br/>Flink/Spark Streaming]:::primary
        Stream --> Features[(Feature Store)]:::purple
        Batch[Batch Pipeline<br/>Spark]:::green --> Features
        Batch --> Models[Train Models]:::green
    end

    subgraph "Serving"
        Models --> ModelServing[Model Serving<br/>TensorFlow Serving]:::orange
        Features --> ModelServing
        ModelServing --> API[Recommendation API]:::primary
    end

    subgraph "Storage"
        API --> Cache[(Redis<br/>Pre-computed recs)]:::orange
    end
...
flowchart TB
    ...
    PS1 & PS2 & PS3 & PS4 --> DB[(Databases)]:::purple
...
flowchart TB
    subgraph "Cassandra Cluster"
        N1[Node 1<br/>Users A-F]:::primary
        N2[Node 2<br/>Users G-M]:::primary
        N3[Node 3<br/>Users N-S]:::primary
        N4[Node 4<br/>Users T-Z]:::primary
    end
...
```mermaid
flowchart LR
    Client[Client]:::rose --> ClientCache[Client Cache<br/>Local Storage]:::green
    ClientCache --> CDN[CDN Cache<br/>Edge Locations]:::green
    CDN --> AppCache[Redis Cache<br/>Data Center]:::orange
    AppCache --> DB[(Database)]:::purple
...
sequenceDiagram
    ...
    Note over API: Verify premium subscription,<br/>region, device count
...
flowchart LR
    subgraph "Protection Layers"
        Encrypt[Audio Encryption<br/>Widevine/FairPlay]:::primary
        License[License Keys<br/>User + Device bound]:::orange
        Check[Periodic License Check<br/>Every 30 days]:::green
        Limits[Device Limits<br/>Max 5 devices]:::purple
    end
...
flowchart TB
    subgraph "Event Collection"
        Client[Client Apps]:::rose --> Events[Playback Events<br/>start, pause, seek, complete]:::primary
        Events --> Collector[Event Collector<br/>API]:::primary
    end

    subgraph "Stream Processing"
        Collector --> Kafka[(Kafka<br/>Durable queue)]:::orange
        Kafka --> Processor[Stream Processor<br/>Flink/Spark Streaming]:::primary
        Processor --> Fraud[Fraud Detection<br/>ML Models]:::red
    end

    subgraph "Storage & Analytics"
        Processor --> Analytics[(Analytics DB<br/>ClickHouse)]:::purple
        Processor --> Royalty[Royalty<br/>Aggregator]:::green
        Royalty --> Payments[(Payment<br/>System)]:::green
    end
...
flowchart LR
    subgraph "Fraud Signals"
        Bot[Bot-like Patterns<br/>Same song on loop]:::red
        Geo[Geographic Anomalies<br/>Impossible location changes]:::red
        Device[Device Fingerprinting<br/>Emulators, VMs]:::red
        Behavior[Account Behavior<br/>No variety, no playlists]:::red
    end

    Bot & Geo & Device & Behavior --> ML[ML Fraud<br/>Classifier]:::primary
    ML --> Decision{Fraud Score}
