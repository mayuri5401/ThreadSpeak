---
id: "hld-interviews-media-streaming-delivery-design-netflix"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Media Streaming Delivery"
subSection: ""
title: "Design Netflix"
slug: "hld-interviews-media-streaming-delivery-design-netflix"
summary: "What makes Netflix particularly interesting from a system design perspective is the combination of challenges it must solve simultaneously. Unlike a text-based service where a request returns kilobytes of data, a single Netflix stream delivers gig..."
eli10: "Imagine Design Netflix as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Netflix Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Media Streaming Delivery","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Design Netflix"
  code: |
    Storage per title (all formats): ~50 GB average
    Total video storage: 15,000 titles × 50 GB = 750 TB
---
...
flowchart TD
    subgraph "User Funnel"
        Reg[200M<br/>Registered]:::secondary
        DAU[100M<br/>Daily Active]:::orange
        Peak[10M<br/>Peak Concurrent]:::primary
    end
...
flowchart LR
    subgraph "Discovery APIs"
        Catalog[GET /catalog<br/>Browse content]:::primary
        Search[GET /search<br/>Find content]:::primary
    end

    subgraph "Playback APIs"
        Play[GET /playback<br/>Get stream URLs]:::orange
        Progress[POST /progress<br/>Update position]:::orange
    end

    subgraph "User APIs"
        Profile[GET /profiles<br/>List profiles]:::green
        History[GET /history<br/>Watch history]:::green
    end
...
{
  "content_id": "movie_123",
  "manifest_url": "https://cdn.netflix.com/manifest/movie_123.mpd",
  "cdn_servers": [
    "https://edge-east.netflix.com",
    "https://edge-west.netflix.com"
  ],
  "drm": {
    "license_url": "https://license.netflix.com/widevine",
    "certificate_url": "https://license.netflix.com/cert"
  },
  "resume_position_seconds": 2400,
  "audio_tracks": ["en", "es", "fr", "de"],
...
flowchart LR
    subgraph "Read Path (Streaming)"
        direction LR
        R1[User]:::rose --> R2[CDN Edge]:::green
        R2 -.->|cache miss| R3[(Origin Storage)]:::purple
    end
	
    subgraph "Metadata Path (Discovery)"
        direction LR
        M1[User]:::rose --> M2[API Gateway]:::secondary
        M2 --> M3[Application Services]:::primary
        M3 --> M4[(Metadata DB)]:::purple
    end
	
	
    subgraph "Write Path (Content Ingestion)"
        direction LR
        W1[Content Partner]:::rose --> W2[Upload Service]:::secondary
        W2 --> W3[Transcoding Pipeline]:::orange
        W3 --> W4[(Video Storage)]:::purple
    end	
...
```mermaid
flowchart TB
    CP[Content Partner]:::rose

    subgraph Upload["Upload Layer"]
        US[Upload Service]:::secondary
        Raw[(Raw Storage<br/>Temporary)]:::purple
    end

    subgraph Processing["Processing Pipeline"]
        Queue[Job Queue]:::orange
        Trans[Transcoding Workers<br/>Parallel encoding]:::primary
        QA[Quality Assurance<br/>Automated checks]:::secondary
    end

    subgraph Distribution["Distribution Layer"]
        Encoded[(Encoded Storage<br/>Permanent)]:::purple
        CDN[CDN Origin<br/>Global distribution]:::green
    end

    subgraph Metadata["Metadata"]
        MDB[(Content Database)]:::purple
    end

    CP -->|"Upload raw video"| US
    US -->|"Store temporarily"| Raw
    US -->|"Enqueue job"| Queue
    Queue -->|"Process"| Trans
    Trans -->|"Read raw"| Raw
    Trans -->|"Write encoded"| Encoded
    Trans -->|"Verify"| QA
    QA -->|"Update status"| MDB
    Encoded -->|"Replicate"| CDN
...
```mermaid
sequenceDiagram
    participant Partner as Content Partner
    ...
    Partner->>Upload: Upload raw video (100 GB)
    Upload->>Storage: Store in raw bucket
    Storage-->>Upload: Confirm stored
    Upload->>Queue: Enqueue transcoding job
    Upload-->>Partner: Upload accepted

    Note over Trans: Async processing begins

    Queue->>Trans: Dequeue job
    Trans->>Storage: Fetch raw video
    Trans->>Trans: Split into segments
    Trans->>Trans: Encode all variants in parallel
    Trans->>Storage: Store encoded segments
    Trans->>DB: Update content status: processing → ready
    Trans->>CDN: Trigger replication to edge locations
```
...
flowchart TB
    ...
    subgraph Services["Application Services"]
        CS[Catalog Service<br/>Content metadata]:::primary
        SS[Search Service<br/>Full-text search]:::primary
        RS[Recommendation Service<br/>Personalization]:::primary
        US[User Service<br/>Profiles & preferences]:::primary
    end

    subgraph Cache["Cache Layer"]
        Redis[(Redis Cluster<br/>Hot data)]:::orange
    end

    subgraph Data["Data Layer"]
        CMD[(Content Metadata<br/>PostgreSQL)]:::purple
        UDB[(User Data<br/>PostgreSQL)]:::purple
        VH[(Viewing History<br/>Cassandra)]:::purple
        ES[(Search Index<br/>Elasticsearch)]:::purple
    end
...
sequenceDiagram
    ...
    participant History as Viewing History
    participant Cache as Redis Cache
    participant DB as Content DB

    Client->>Gateway: GET /catalog"profile_id=123
    Gateway->>Gateway: Validate auth token
    Gateway->>Catalog: Get home page for profile

    par Parallel data fetching
        Catalog->>Cache: Get cached metadata
        Catalog->>Rec: Get personalized rows
        Rec->>History: Get recent viewing history
        History-->>Rec: Last 100 watched items
        Rec->>Rec: Generate recommendations
    end

    Rec-->>Catalog: Recommended content IDs
    Cache-->>Catalog: Content metadata

    alt Cache miss
        Catalog->>DB: Fetch missing metadata
        DB-->>Catalog: Content details
        Catalog->>Cache: Populate cache
    end

    Catalog-->>Gateway: Assembled home page
...
```mermaid
flowchart TB
    User[User Device]:::rose

    subgraph Application["Application Layer"]
        AG[API Gateway]:::secondary
        PS[Playback Service<br/>Authorization & routing]:::primary
        DRM[DRM Service<br/>License management]:::orange
    end

    subgraph CDN["CDN Layer (Global)"]
        Edge1[CDN Edge<br/>US East]:::green
        Edge2[CDN Edge<br/>US West]:::green
        Edge3[CDN Edge<br/>Europe]:::green
        Edge4[CDN Edge<br/>Asia]:::green
    end

    subgraph Origin["Origin Layer"]
        Origin1[(Origin Storage<br/>Primary)]:::purple
        Origin2[(Origin Storage<br/>Backup)]:::purple
    end

    User -->|"1. Request playback"| AG --> PS
    PS -->|"2. Check entitlements"| PS
    PS -->|"3. Select CDN servers"| PS
    PS -->|"4. Return manifest + CDN URLs"| User
    User -->|"5. Request DRM license"| DRM
    DRM -->|"6. Issue license"| User
    User -->|"7. Fetch video segments"| Edge1
    Edge1 -.->|cache miss| Origin1

    Edge2 & Edge3 & Edge4 -.-> Origin1
...
sequenceDiagram
    ...
    Client->>PS: GET /playback/movie_123
    PS->>PS: Verify subscription & availability
    PS->>PS: Check concurrent stream limit
    PS->>PS: Select optimal CDN servers
    PS-->>Client: Manifest URL + CDN servers + DRM info

    Client->>Edge: GET /manifest/movie_123.mpd
    Edge-->>Client: Streaming manifest

    Client->>DRM: Request license
    DRM->>DRM: Validate entitlement
    DRM-->>Client: Encrypted license key

    loop For each segment during playback
        Client->>Client: Measure bandwidth
        Client->>Client: Select appropriate quality
        Client->>Edge: GET /segment_001_1080p.ts
        alt Cache hit
            Edge-->>Client: Video segment
        else Cache miss
            Edge->>Origin: Fetch segment
            Origin-->>Edge: Video segment
            Edge->>Edge: Cache for future requests
            Edge-->>Client: Video segment
        end
        Client->>Client: Decrypt and play
    end
```
...
flowchart TB
    subgraph Clients
        Web[Web Browser]:::primary
        Mobile[Mobile App]:::primary
        TV[Smart TV]:::primary
    end

    subgraph Edge["CDN Edge Layer"]
        CDN1[CDN Edge 1]:::green
        CDN2[CDN Edge 2]:::green
        CDN3[CDN Edge 3]:::green
    end
    ...
    subgraph Services["Application Services"]
        CS[Catalog Service]:::orange
        PS[Playback Service]:::orange
        RS[Recommendation Service]:::orange
        US[User Service]:::orange
        SS[Search Service]:::orange
    end

    subgraph Processing["Content Processing"]
        Upload[Upload Service]:::orange
        Trans[Transcoding Service]:::orange
    end

    subgraph Data["Data Stores"]
        CMD[(Content Metadata)]:::purple
        UDB[(User Database)]:::purple
        VH[(Viewing History)]:::purple
        Cache[(Redis Cache)]:::purple
    end

    subgraph Storage["Video Storage"]
        Origin[(Origin Storage)]:::purple
    end
...
```mermaid
flowchart TB
    subgraph "Different Data, Different Databases"
        direction TB

        subgraph Caching["Cache (Redis)"]
            Sessions[Active Sessions]:::red
            HotData[Hot Metadata]:::red
        end

        subgraph Search["Search (Elasticsearch)"]
            Index[Content Search Index]:::green
        end
		
        subgraph TimeSeries["Time-Series (Cassandra)"]
            History[Viewing History]:::orange
            Progress[Watch Progress]:::orange
        end		
		
        subgraph Relational["Relational (PostgreSQL)"]
            Users[Users & Profiles]:::primary
            Content[Content Metadata]:::primary
            Subscriptions[Subscriptions]:::primary
        end		
    end
...
Fields:
  {device_id}: {
    ...
    "last_heartbeat": "2024-01-15T11:45:00Z"
  }
```
...
```mermaid
flowchart TB
    Raw[Raw Video File<br/>100 GB, single file]:::rose

    subgraph "Segmentation"
        Seg[Split into 4-second segments<br/>~1,800 segments for 2hr movie]:::primary
    end

    subgraph "Multi-Quality Encoding"
        Q1[480p @ 800 kbps]:::secondary
        Q2[720p @ 1.5 Mbps]:::secondary
        Q3[1080p @ 5 Mbps]:::orange
        Q4[4K @ 15 Mbps]:::orange
    end

    subgraph "Output"
        Man[Streaming Manifest<br/>Describes all variants]:::green
        Segs[7,200+ segment files<br/>Each quality × each segment]:::purple
    end

    Raw --> Seg
    Seg --> Q1 & Q2 & Q3 & Q4
    Q1 & Q2 & Q3 & Q4 --> Man & Segs
...
├── manifest.mpd (describes everything below)
├── 480p/
│   ├── segment_001.ts
│   ├── segment_002.ts
│   └── ... (1,800 segments)
...
```mermaid
flowchart TD
    Start[Start Playback]:::primary --> Init[Request Manifest]:::secondary
    Init --> Select[Select Conservative<br/>Initial Quality]:::secondary
    Select --> Download[Download Next Segment]:::green

    Download --> Measure[Measure Download Speed<br/>& Buffer Level]:::orange

    Measure --> Decide{Adaptation Decision}:::orange

    Decide -->|Buffer critically low| Emergency[Drop to Lowest Quality<br/>Avoid buffering!]:::red
    Decide -->|Buffer low OR<br/>bandwidth decreased| Down[Decrease Quality<br/>One level]:::orange
    Decide -->|Buffer healthy AND<br/>bandwidth can support higher| Up[Increase Quality<br/>One level]:::green
    Decide -->|Stable| Same[Keep Current Quality]:::secondary

    Emergency & Down & Up & Same --> Download
...
flowchart TB
    ...
    subgraph "With CDN"
        U2[User in Tokyo]:::rose -->|"20ms latency"| Edge[Tokyo<br/>Edge Server]:::green
        Edge -.->|"Cache miss only"| Origin2[(Virginia<br/>Origin)]:::purple
    end

    subgraph "Without CDN"
        U1[User in Tokyo]:::rose -->|"200ms latency"| Origin1[(Virginia<br/>Data Center)]:::purple
    end
...
flowchart LR
    ...
    subgraph ISP["ISP Network"]
        OCA[Open Connect Appliance]:::green
    end

    subgraph Netflix["Netflix Data Center"]
        Origin[(Origin Storage)]:::purple
        Control[Control Plane]:::orange
    end

    User --> OCA
    OCA -->|Cache Miss| Origin
    Control -->|Push Popular Content <br/> before users request| OCA
    Control -->|Steer users to <br/> best server| User
...
```mermaid
flowchart TD
    Request[Playback Request<br/>from Tokyo user]:::rose --> Factors{Consider Factors}:::orange

    Factors --> Geo[Geographic Distance<br/>Prefer nearby servers]:::secondary
    Factors --> Load[Server Load<br/>Avoid overloaded servers]:::secondary
    Factors --> ISP[ISP Topology<br/>Prefer same network]:::secondary
    Factors --> Content[Content Availability<br/>Server has content cached]:::secondary

    Geo & Load & ISP & Content --> Score[Score Each Server]:::primary

    Score --> Select[Select Best Server<br/>Return to client]:::green
...
flowchart LR
    subgraph "Similar Users"
        User1[User A<br/>Watched: 1,2,3,4]:::primary
        User2[User B<br/>Watched: 1,2,3,5]:::orange
    end

    subgraph "Recommendation"
        Rec[User B might like<br/>content 4]:::green
    end

    User1 -->|"Similar taste"| User2
    User2 -->|"Hasn't seen 4"| Rec
...
flowchart LR
    subgraph "User History"
        Watched[User watched<br/>Sci-fi thrillers with<br/>strong female leads]:::primary
    end

    subgraph "Content Matching"
        Match[Find content with<br/>similar attributes]:::orange
    end

    subgraph "Recommendation"
        Rec[Recommend: Arrival,<br/>Ex Machina, Annihilation]:::green
    end
...
flowchart TB
    subgraph "Input Signals"
        VH[Viewing History]:::primary
        Search[Search Queries]:::primary
        Browse[Browse Behavior]:::primary
        Time[Time of Day]:::primary
        Device[Device Type]:::primary
    end

    subgraph "Feature Processing"
        UserEmb[User Embedding<br/>Dense vector representing<br/>user preferences]:::orange
        ContentEmb[Content Embedding<br/>Dense vector representing<br/>each title]:::orange
    end

    subgraph "Models"
        CF[Collaborative Model]:::secondary
        CB[Content Model]:::secondary
        Seq[Sequential Model<br/>What you watch next]:::secondary
    end

    subgraph "Ranking"
        Combine[Combine Scores]:::purple
        Diversify[Diversify Results]:::purple
        Row[Generate Rows<br/>for Home Page]:::green
    end

    VH & Search & Browse --> UserEmb
    Time & Device --> Models
    UserEmb --> CF & CB & Seq
    ContentEmb --> CF & CB

    CF & CB & Seq --> Combine --> Diversify --> Row
...
```mermaid
flowchart TD
    Start[Playback Request]:::primary --> Check{Active Sessions < Limit"}:::orange
    Check -->|Yes| Create[Create Session]:::green
    Check -->|No| Deny[Deny Playback]:::red
    Create --> Play[Start Streaming]:::secondary
    Play --> HB[Send Heartbeat]:::secondary
    HB --> Update[Update Session]:::green
    Update --> HB
    Play --> Stop[Stop Playback]:::secondary
    Stop --> Delete[Delete Session]:::red
...
```shell
Approach: Use Redis transactions or Lua scripts

1. WATCH sessions:{user_id}
2. GET current session count
3. If count < limit:
   MULTI
   HSET sessions:{user_id} {device_id} {session_data}
   EXEC
4. If EXEC fails (another client modified), retry

Or use a Lua script for atomicity:
if redis.call('HLEN', key) < limit then
  redis.call('HSET', key, device_id, session_data)
  return 'OK'
else
...
```mermaid
flowchart LR
    Upload[Raw Upload]:::primary --> Validate[Validation]:::secondary
    Validate --> Chunk[Chunking]:::orange
    Chunk --> Encode[Parallel Encoding]:::green
    Encode --> Package[Packaging]:::orange
    Package --> QA[Quality Check]:::secondary
    QA --> Publish[Publish to CDN]:::green
...
```mermaid
flowchart TB
    Chunk[Single 4s Chunk]:::primary

    subgraph "Parallel Encoding Jobs"
        E1[480p H.264]:::secondary
        E2[720p H.264]:::secondary
        E3[1080p H.264]:::orange
        E4[1080p H.265]:::orange
        E5[4K H.265]:::orange
        E6[4K HDR]:::orange
    end

    Chunk --> E1 & E2 & E3 & E4 & E5 & E6

    E1 & E2 & E3 & E4 & E5 & E6 --> Combine[Combine Results]:::green
