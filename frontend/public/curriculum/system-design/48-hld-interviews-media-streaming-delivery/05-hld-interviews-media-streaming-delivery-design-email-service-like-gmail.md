---
id: "hld-interviews-media-streaming-delivery-design-email-service-like-gmail"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Media Streaming Delivery"
subSection: ""
title: "Design Email Service (like Gmail)"
slug: "hld-interviews-media-streaming-delivery-design-email-service-like-gmail"
summary: "In this chapter, we will explore the high-level design of an email service like Gmail."
eli10: "Imagine Design Email Service (like Gmail) as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Email Service (like Gmail) Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph "Storage Per Email"
        direction LR
        A[Metadata<br/>~2 KB]:::secondary
        B[Body + Headers<br/>~50 KB avg]:::orange
        C[Attachments<br/>~100 KB avg]:::primary
    end
...
```mermaid
flowchart TB
    subgraph "Core Email Operations"
        POST[POST /emails/send<br/>Send an email]:::green
        GET1[GET /emails<br/>List inbox/folder]:::primary
        GET2[GET /emails/id<br/>Get email details]:::primary
        SEARCH[GET /emails/search<br/>Search emails]:::orange
        UPLOAD[POST /attachments<br/>Upload attachment]:::purple
    end

    POST --> |Returns| MID[Message ID + Status]:::secondary
    GET1 --> |Returns| LIST[Email List + Pagination]:::secondary
    GET2 --> |Returns| FULL[Full Email + Body]:::secondary
    SEARCH --> |Returns| RESULTS[Search Results]:::secondary
    UPLOAD --> |Returns| AID[Attachment ID]:::secondary
...
flowchart LR
    ...
    subgraph "Receive Path"
        direction LR
        R1[External Server]:::pink --> R2[SMTP Receiver]:::primary
        R2 --> R3[Spam/Virus Filter]:::yellow
        R3 --> R4[Store in Mailbox]:::purple
    end

    subgraph "Send Path"
        direction LR
        S1[User Composes]:::rose --> S2[API Server]:::secondary
        S2 --> S3[Message Queue]:::orange
        S3 --> S4[SMTP Delivery]:::primary
        S4 --> S5[Recipient Server]:::pink
    end
...
flowchart TB
    ...
    subgraph Queue["Message Queue"]
        MQ[Kafka/RabbitMQ]:::orange
    end

    subgraph Delivery["Delivery Layer"]
        SMTP[Outbound SMTP Service]:::primary
        DNS[DNS Resolver]:::purple
    end

    subgraph Storage["Storage Layer"]
        Meta[(Metadata DB)]:::purple
        Blob[(Blob Storage)]:::purple
    end

    subgraph External["External"]
        ExtMail[External Mail Servers]:::pink
    end

    U -->|"1. POST /emails/send"| AG
    AG -->|"2. Forward"| API
    API -->|"3. Store in Sent"| Meta
    API -->|"3. Store body"| Blob
    API -->|"4. Queue for delivery"| MQ
    MQ -->|"5. Process"| SMTP
    SMTP -->|"6. Lookup MX"| DNS
    SMTP -->|"7. Deliver"| ExtMail
...
To send an email to [`bob@gmail.com`](mailto:bob@gmail.com), we need to find Gmail's mail servers. This is done by querying DNS for MX (Mail Exchanger) records. For `gmail.com`, this returns servers like `gmail-smtp-in.l.google.com`.
...
sequenceDiagram
    ...
    participant SMTP as Outbound SMTP
    participant DNS as DNS
    participant Ext as External Mail Server

    Client->>API: POST /emails/send
    Note over API: Validate request,<br/>check rate limits
    API->>Blob: Store email body
    Blob-->>API: Confirm storage
    API->>DB: Insert sent email record
    DB-->>API: Confirm insertion
    API->>Queue: Publish delivery job
    API-->>Client: 202 Accepted (queued)

    Note over Queue,Ext: Async Delivery Process
    Queue->>SMTP: Delivery job
    SMTP->>DNS: Query MX records
    DNS-->>SMTP: gmail-smtp-in.l.google.com
    SMTP->>Ext: SMTP connection + deliver
    alt Delivery Success
        Ext-->>SMTP: 250 OK
        SMTP->>DB: Update status: delivered
    else Temporary Failure
        Ext-->>SMTP: 450 Try again later
        SMTP->>Queue: Re-queue with backoff
    else Permanent Failure
        Ext-->>SMTP: 550 User not found
        SMTP->>DB: Update status: bounced
        SMTP->>Queue: Generate bounce notification
    end
```
...
flowchart TB
    subgraph External["External Systems"]
        ExtMail[External Mail Server]:::pink
    end

    subgraph Receiving["Receiving Layer"]
        SMTP[Inbound SMTP Service]:::primary
    end

    subgraph Security["Security Pipeline"]
        Auth[SPF/DKIM/DMARC<br/>Verification]:::orange
        Spam[Spam Filter]:::yellow
        Virus[Virus Scanner]:::red
    end

    subgraph Storage["Storage Layer"]
        Queue[Processing Queue]:::orange
        Meta[(Metadata DB)]:::purple
        Blob[(Blob Storage)]:::purple
    end
    ...
    subgraph Notification["User Notification"]
        ...
        WS[WebSocket Gateway]:::secondary
    end

    ExtMail -->|"1. SMTP connection"| SMTP
    SMTP -->|"2. Verify sender"| Auth
    Auth -->|"3. Check spam"| Spam
    Spam -->|"4. Scan attachments"| Virus
    Virus -->|"5. Queue for storage"| Queue
    Queue -->|"6. Store metadata"| Meta
    Queue -->|"6. Store body"| Blob
    Queue -->|"7. Notify user"| Sync
    Sync --> WS
...
sequenceDiagram
    participant Ext as External Mail Server
    participant SMTP as Inbound SMTP
    ...
    Ext->>SMTP: EHLO + MAIL FROM + RCPT TO
    SMTP->>SMTP: Verify recipient exists
    alt Recipient Unknown
        SMTP-->>Ext: 550 User not found
    else Recipient Valid
        SMTP-->>Ext: 250 OK, send DATA
        Ext->>SMTP: Email content (DATA)
        SMTP->>Auth: Verify SPF/DKIM/DMARC
        Auth-->>SMTP: Authentication result
        SMTP->>Spam: Analyze for spam
        Spam-->>SMTP: Spam score: 0.15
        SMTP->>Virus: Scan attachments
        Virus-->>SMTP: Clean
        SMTP->>Blob: Store email body
        SMTP->>DB: Insert inbox record
        SMTP-->>Ext: 250 Message accepted
        SMTP->>Sync: Notify user
        Sync->>User: Push notification
    end
```
...
```mermaid
flowchart TB
    subgraph "Email Storage Architecture"
        direction TB

        subgraph Metadata["Metadata (Frequent Access)"]
            Meta[(Distributed SQL<br/>CockroachDB/Spanner)]:::primary
        end

        subgraph Content["Content (Infrequent Access)"]
            HotBlob[(Hot Blob Storage<br/>Recent emails)]:::orange
            ColdBlob[(Cold Blob Storage<br/>Old emails)]:::purple
        end

        subgraph Attachments["Attachments"]
            AttachBlob[(CDN-backed<br/>Object Storage)]:::secondary
        end
    end

    API[API Server]:::rose --> Cache[(Redis Cache)]:::red
    Cache --> Meta
    Meta --> HotBlob
    Meta --> ColdBlob
    API --> AttachBlob
...
sequenceDiagram
    ...
    Client->>API: GET /emails"folder=inbox
    API->>Cache: Check recent inbox cache
    alt Cache Hit
        Cache-->>API: Return cached metadata
    else Cache Miss
        API->>DB: Query inbox emails
        DB-->>API: Return metadata (no body)
        API->>Cache: Populate cache
    end
    API-->>Client: Email list (summaries only)

    Note over Client: User opens specific email
    Client->>API: GET /emails/{email_id}
    API->>Cache: Check email cache
    alt Cache Hit
        Cache-->>API: Return full email
    else Cache Miss
        API->>DB: Get metadata + body_ref
        API->>Blob: Fetch email body
        Blob-->>API: Return body content
        API->>Cache: Cache full email
    end
    API-->>Client: Full email with body
```
...
```mermaid
flowchart TB
    subgraph "Write Path (Indexing)"
        direction LR
        NewEmail[New Email]:::rose --> Queue[Index Queue]:::orange
        Queue --> Indexer[Indexing Service]:::primary
        Indexer --> ES[(Elasticsearch<br/>Cluster)]:::purple
    end

    subgraph "Read Path (Searching)"
        direction LR
        Query[Search Query]:::rose --> API[API Server]:::primary
        API --> ES
        ES --> Results[Ranked Results]:::green
    end
...
sequenceDiagram
    ...
    Client->>API: GET /emails/search"query=from:alice+invoice
    API->>API: Parse query into ES query DSL
    API->>ES: Execute search (user_id filter + query)
    ES-->>API: Return matching email_ids + scores
    API->>DB: Fetch current metadata for results
    Note over API: Merge ES scores with fresh metadata
...
sequenceDiagram
    ...
    Phone->>API: Mark email as read
    API->>API: Update database
    API->>Events: Publish "email_read" event
    API-->>Phone: 200 OK

    Events->>Sync: Consume event
    Sync->>Registry: Get user's connected devices
    Registry-->>Sync: [Phone, Laptop, Desktop]
    Note over Sync: Skip Phone (source of change)
    Sync->>WS: Send update to Laptop, Desktop
    WS->>Laptop: WebSocket: email_read notification
    Note over Laptop: Update UI immediately
```
...
flowchart TB
    ...
    subgraph Services["Application Services"]
        ...
        OutSMTP[Outbound SMTP]:::primary
        InSMTP[Inbound SMTP]:::primary
        SyncSvc[Sync Service]:::primary
        IndexSvc[Indexing Service]:::primary
    end

    subgraph Processing["Email Processing"]
        Queue[Message Queue<br/>Kafka]:::orange
        Spam[Spam Filter]:::yellow
        Virus[Virus Scanner]:::red
    end

    subgraph Storage["Data Storage"]
        Meta[(Metadata DB<br/>CockroachDB)]:::purple
        Blob[(Blob Storage<br/>S3/GCS)]:::purple
        Cache[(Redis Cache)]:::orange
        ES[(Elasticsearch)]:::green
    end
...
```mermaid
flowchart LR
    subgraph "Data Type → Storage Choice"
        direction TB
        A[Email Metadata<br/>Structured, queryable]:::primary --> A1[Distributed SQL<br/>CockroachDB/Spanner]:::purple
        B[Email Bodies<br/>Large blobs, simple access]:::primary --> B1[Object Storage<br/>S3/GCS]:::purple
        C[Attachments<br/>Large files, CDN delivery]:::primary --> C1[Object Storage + CDN<br/>S3 + CloudFront]:::purple
        D[Search Index<br/>Full-text queries]:::primary --> D1[Elasticsearch]:::purple
    end
...
flowchart LR
    subgraph "Sender Side"
        MUA[MUA<br/>Mail User Agent<br/>Your email app]:::rose
        MSA[MSA<br/>Mail Submission Agent<br/>Accepts from clients]:::primary
        MTA1[MTA<br/>Mail Transfer Agent<br/>Routes email]:::primary
    end

    subgraph "Network"
        DNS[DNS<br/>MX Lookup]:::purple
    end

    subgraph "Receiver Side"
        MTA2[MTA<br/>Mail Transfer Agent<br/>Accepts incoming]:::primary
        MDA[MDA<br/>Mail Delivery Agent<br/>Stores in mailbox]:::primary
        Mailbox[(User Mailbox)]:::green
    end

    MUA -->|"Port 587<br/>SMTP"| MSA
    MSA --> MTA1
    MTA1 -->|"Query"| DNS
    DNS -->|"MX records"| MTA1
    MTA1 -->|"Port 25<br/>SMTP"| MTA2
    MTA2 --> MDA
    MDA --> Mailbox
...
```mermaid
flowchart TD
    A[Delivery Attempt]:::primary --> B{Result"}
    B -->|250 OK| C[Success<br/>Update status]:::green
    B -->|4xx Temp Failure| D[Retry Queue<br/>Exponential backoff]:::orange
    B -->|5xx Perm Failure| E[Bounce<br/>Notify sender]:::red
...
```mermaid
flowchart TB
    Email[Incoming Email]:::rose --> L1[Layer 1: Connection<br/>IP reputation, rate limits]:::primary
    L1 --> L2[Layer 2: Authentication<br/>SPF, DKIM, DMARC]:::orange
    L2 --> L3[Layer 3: Content<br/>Keywords, URLs, patterns]:::yellow
    L3 --> L4[Layer 4: ML Model<br/>Behavioral patterns]:::purple
    L4 --> Decision{Final Decision}

    Decision -->|Clean| Inbox[Inbox]:::green
    Decision -->|Spam| SpamFolder[Spam Folder]:::red
    Decision -->|Phishing| Reject[Reject]:::red
...
flowchart LR
    subgraph "Email Authentication"
        SPF[SPF Check<br/>Is IP authorized"]:::primary
        DKIM[DKIM Check<br/>Is signature valid"]:::orange
        DMARC[DMARC Policy<br/>What to do on failure"]:::yellow
    end

    Email[Incoming Email]:::rose --> SPF
    SPF --> DKIM
    DKIM --> DMARC
    DMARC --> Result{Result}
    Result -->|Pass| Accept[Accept]:::green
    Result -->|Fail + policy=reject| Reject[Reject]:::red
    Result -->|Fail + policy=quarantine| Spam[Spam Folder]:::red
...
flowchart LR
    ...
    subgraph "Search Flow"
        Query[User Search]:::rose --> API[API Server]:::primary
        API --> ES[(Elasticsearch)]:::purple
        ES --> Results[Return email_ids + scores]:::green
        Results --> API
        API --> DB[(Metadata DB)]:::purple
        DB --> Fresh[Fresh metadata]:::green
    end

    subgraph "New Email Flow"
		direction LR
        Email[New Email Stored]:::rose --> Event[Publish to Event Queue]:::orange
        Event --> Worker[Indexing Worker]:::primary
        Worker --> Extract[Extract searchable content]:::primary
        Extract --> Index[Write to Elasticsearch]:::purple
    end
...
```mermaid
flowchart TB
    Upload[User Uploads Attachment]:::rose --> API[API Server]:::primary
    API --> Virus[Virus Scanner]:::red
    Virus -->|Clean| Dedupe{Already stored"<br/>Check SHA-256}
    Dedupe -->|Yes| Reference[Reference existing blob]:::green
    Dedupe -->|No| Store[Store new blob]:::purple

    Virus -->|Infected| Reject[Reject Upload]:::red

    Store --> S3[(Object Storage)]:::purple
    Reference --> Meta[(Update Metadata)]:::orange
...
sequenceDiagram
    ...
    Client->>API: Request attachment download
    API->>API: Verify user has access
    API->>S3: Generate signed URL (expires in 1 hour)
    API-->>Client: Return signed URL
    Client->>CDN: Download via signed URL
    CDN-->>Client: Stream file content
```
