---
id: "hld-interviews-interview-tips-choosing-the-right-database-in-a-system-design-interview"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Tips"
subSection: ""
title: "Choosing the Right Database in a System Design Interview"
slug: "hld-interviews-interview-tips-choosing-the-right-database-in-a-system-design-interview"
summary: "\"What database should we use\"\""
eli10: "Imagine Choosing the Right Database in a System Design Interview as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Choosing the Right Database in a System Design Interview Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart TD
    A[Analyze Data Requirements]:::primary --> B[What is the data structure"]:::secondary
    A --> C[What are the access patterns"]:::secondary
    A --> D[What scale do we need"]:::secondary
    A --> E[What consistency is required"]:::secondary
    A --> F[What are the relationships"]:::secondary
...
flowchart TD
    subgraph "Relational Database"
        ...
        B[Orders Table]:::primary
        C[Products Table]:::primary
        A -->|Foreign Key| B
        B -->|Foreign Key| C
    end
...
flowchart LR
    subgraph "Document Database"
        A["{ user_id: 1,<br/>name: 'Alice',<br/>orders: [...],<br/>preferences: {...} }"]:::primary
        B["{ user_id: 2,<br/>name: 'Bob',<br/>orders: [...] }"]:::primary
    end
...
flowchart LR
    subgraph "Key-Value Store"
        A["session:abc123 → { user_id: 1, ... }"]:::primary
        B["cache:user:1 → { name: 'Alice', ... }"]:::orange
        C["rate:ip:192.168.1.1 → 42"]:::green
    end
...
flowchart TD
    subgraph "Wide-Column Store"
        A["Row Key: user1<br/>columns: {email: '...', name: '...', last_login: '...'}"]:::primary
        B["Row Key: user2<br/>columns: {email: '...', name: '...', phone: '...'}"]:::primary
    end
...
```mermaid
flowchart LR
    A((Alice)):::primary -->|FOLLOWS| B((Bob)):::primary
    B -->|FOLLOWS| C((Carol)):::primary
    C -->|FOLLOWS| A
    A -->|LIKES| D[Post 1]:::orange
    B -->|LIKES| D
...
```mermaid
flowchart TD
    A[What data are we storing"]:::primary --> B{Need ACID<br/>transactions"}
    B -->|Yes| C{Complex queries<br/>with joins"}
    B -->|No| D{Simple key-value<br/>access"}

    C -->|Yes| E[Relational DB<br/>PostgreSQL/MySQL]:::green
    C -->|No| F{Need flexibility"}

    D -->|Yes| G[Key-Value Store<br/>Redis/DynamoDB]:::orange
    D -->|No| H{Write-heavy<br/>at scale"}

    F -->|Yes| I[Document DB<br/>MongoDB]:::purple
    F -->|No| E

    H -->|Yes| J[Wide-Column<br/>Cassandra]:::pink
    H -->|No| K{Graph<br/>traversals"}

    K -->|Yes| L[Graph DB<br/>Neo4j]:::teal
    K -->|No| I
...
```mermaid
flowchart TD
    A[Application]:::primary --> B[(PostgreSQL<br/>Transactions)]:::green
    A --> C[(Redis<br/>Cache)]:::orange
    A --> D[(Elasticsearch<br/>Search)]:::purple
    A --> E[(Cassandra<br/>Time-series)]:::pink
