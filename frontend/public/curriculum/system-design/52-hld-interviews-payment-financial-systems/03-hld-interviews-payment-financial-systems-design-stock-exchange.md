---
id: "hld-interviews-payment-financial-systems-design-stock-exchange"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Payment Financial Systems"
subSection: ""
title: "Design Stock Exchange"
slug: "hld-interviews-payment-financial-systems-design-stock-exchange"
summary: "What makes stock exchange design fascinating from a systems perspective is the extreme demands it places on every component."
eli10: "Imagine Design Stock Exchange as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Stock Exchange Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph Buyers["Buyers (Want to Buy)"]
        B1[Buyer 1<br/>Buy 100 @ $150]:::green
        B2[Buyer 2<br/>Buy 200 @ $149]:::green
        B3[Buyer 3<br/>Buy 150 @ $148]:::green
    end

    subgraph Exchange["Stock Exchange"]
        OB[Order Book<br/>Tracks All Orders]:::primary
        ME[Matching Engine<br/>Finds Matches]:::orange
    end

    subgraph Sellers["Sellers (Want to Sell)"]
        S1[Seller 1<br/>Sell 100 @ $150]:::red
        S2[Seller 2<br/>Sell 300 @ $151]:::red
        S3[Seller 3<br/>Sell 200 @ $152]:::red
    end
    ...
    ME --> T[Trade Executed<br/>100 shares @ $150]:::purple
...
flowchart TB
    subgraph Core["Core Functions"]
        direction TB
        OP[Order Placement<br/>Submit buy/sell orders]:::primary
        OM[Order Matching<br/>Match buyers with sellers]:::orange
        TE[Trade Execution<br/>Transfer shares and cash]:::green
    end

    subgraph Supporting["Supporting Functions"]
        direction TB
        OC[Order Cancellation<br/>Remove pending orders]:::secondary
        MD[Market Data<br/>Real-time price feeds]:::purple
    end
...
```mermaid
flowchart LR
    subgraph "Order Volume"
        direction TB
        P[Peak: 100K orders/sec]:::red
        A[Average: ~25K orders/sec]:::orange
        D[Daily: ~500M orders]:::primary
    end
...
flowchart LR
    subgraph Source["Source Events"]
        ME[Matching Engine<br/>30K trades/sec]:::orange
    end

    subgraph Fanout["Fan-out Challenge"]
        EV[120K events/sec]:::primary
        SUB[× 10K subscribers]:::purple
        TOTAL[= 1.2B deliveries/sec]:::red
    end
...
flowchart TD
    subgraph "Order Management APIs"
        POST[POST /orders<br/>Place new order]:::green
        DEL[DELETE /orders/id<br/>Cancel order]:::red
        GET[GET /orderbook/symbol<br/>Order book snapshot]:::primary
    end

    subgraph "Market Data APIs"
        WS[WS /market-data<br/>Real-time streaming]:::purple
    end

    POST --> |Returns| RES1[Order confirmation<br/>with fill details]:::orange
    DEL --> |Returns| RES2[Cancellation status]:::orange
    WS --> |Streams| RES3[Trades, quotes,<br/>book updates]:::orange
...
flowchart TB
    subgraph "Critical Path (Latency Sensitive)"
        direction LR
        O[Order Arrives]:::rose --> V[Validate]:::primary
        V --> M[Match]:::orange
        M --> T[Trade Executed]:::green
    end

    subgraph "Data Path (Throughput Sensitive)"
        direction LR
        T2[Trade Event]:::green --> P[Publish]:::purple
        P --> S1[Subscriber 1]:::secondary
        P --> S2[Subscriber 2]:::secondary
        P --> SN[Subscriber N...]:::secondary
    end
...
flowchart LR
    ...
    subgraph Gateway["Entry Layer"]
        AG[API Gateway<br/>Auth, Rate Limiting]:::secondary
    end

    subgraph Processing["Processing Layer"]
        OMS[Order Management Service<br/>Validation, Lifecycle]:::primary
        RS[Risk Service<br/>Balance Checks]:::orange
    end

    subgraph Storage["Persistence Layer"]
        DB[(Order Database)]:::purple
    end

    subgraph Matching["Matching Layer"]
        ME[Matching Engine]:::green
    end
...
sequenceDiagram
    ...
    C->>AG: POST /orders (buy 100 AAPL @ $150)
    Note over AG: Authenticate trader,<br/>check rate limits
    AG->>OMS: Forward validated request
    OMS->>OMS: Validate parameters
    OMS->>RS: Check buying power
    RS->>RS: Reserve $15,000
    RS-->>OMS: Approved (funds reserved)
    OMS->>DB: Insert order (status: PENDING)
    DB-->>OMS: Order ID assigned
    OMS->>ME: Submit for matching
    OMS-->>C: Order accepted (ID: ord-123)
```
...
flowchart LR
    subgraph BIDS["Bids (Buy Orders)"]
        direction LR
        B1["$150.00 x 100<br/>Best Bid"]:::green
        B2["$149.50 x 300"]:::green
        B3["$149.00 x 200"]:::green
    end

    subgraph SPREAD[" "]
        SP["Spread<br/>$0.50"]:::yellow
    end

    subgraph ASKS["Asks (Sell Orders)"]
        direction LR
        A1["$150.50 x 200<br/>Best Ask"]:::red
        A2["$151.00 x 150"]:::red
        A3["$152.00 x 500"]:::red
    end
...
```mermaid
flowchart TB
    subgraph Step1["Step 1: Match Against Best Ask"]
        O1["Incoming: Market Buy 250 shares"]:::primary --> A1["Best Ask: $150.50 x 200"]:::red
        A1 --> F1["Trade: 200 shares @ $150.50<br/>Remaining to fill: 50"]:::orange
    end

    subgraph Step2["Step 2: Move to Next Price Level"]
        O2["Still need: 50 shares"]:::primary --> A2["Next Ask: $151.00 x 150"]:::red
        A2 --> F2["Trade: 50 shares @ $151.00<br/>Remaining to fill: 0"]:::orange
    end

    subgraph Result["Order Complete"]
        R["Total: 250 shares<br/>Average Price: $150.70"]:::green
    end
...
sequenceDiagram
    ...
    participant ME as Matching Engine
    participant TB as Trade Database
    participant MQ as Message Queue

    OMS->>SEQ: New order (buy 100 AAPL @ $150)
    Note over SEQ: Assign sequence #1,000,042
    SEQ->>ME: Order with seq #1,000,042
    ME->>ME: Check order book for matches

    alt Matching orders exist
        ME->>ME: Execute trade(s)
        ME->>TB: Persist trade record
        ME->>MQ: Publish TRADE event
        ME->>MQ: Publish BOOK_UPDATE event
    else No match (limit order)
        ME->>ME: Add to order book
        ME->>MQ: Publish BOOK_UPDATE event
    end

    Note over MQ: Events fan out to<br/>market data subscribers
```
...
flowchart TB
    subgraph Source["Event Source"]
        ME[Matching Engine<br/>120K events/sec]:::orange
    end

    subgraph Distribution["Distribution Layer"]
        MQ[Message Queue<br/>Kafka/Kinesis]:::purple
        MDP[Market Data Publisher<br/>Format & Aggregate]:::primary
    end

    subgraph Delivery["Delivery Layer"]
        MDG1[Gateway 1<br/>2,500 clients]:::secondary
        MDG2[Gateway 2<br/>2,500 clients]:::secondary
        MDG3[Gateway 3<br/>2,500 clients]:::secondary
        MDG4[Gateway N<br/>2,500 clients]:::secondary
    end
...
sequenceDiagram
    ...
    participant C as Trading Clients

    ME->>MQ: Trade: 100 AAPL @ $150.02
    ME->>MQ: Book Update: Ask $150.05 qty +300
    MQ->>MDP: Consume events
    Note over MDP: Combine into single<br/>market data message
    MDP->>MDG: Push aggregated update
    MDG->>C: WebSocket push to<br/>all AAPL subscribers
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        TC[Trading Clients<br/>Submit orders]:::rose
        MDC[Market Data Consumers<br/>Receive updates]:::rose
    end

    subgraph Entry["Entry Layer"]
        AG[API Gateway<br/>Auth, Rate Limiting]:::secondary
        MDG[Market Data Gateway<br/>WebSocket Connections]:::secondary
    end

    subgraph Processing["Order Processing Layer"]
        OMS[Order Management<br/>Validation, Lifecycle]:::primary
        RS[Risk Service<br/>Balance Checks]:::orange
        SEQ[Sequencer<br/>Ordering]:::orange
    end

    subgraph Core["Matching Layer"]
        ME[Matching Engine<br/>Price-Time Priority]:::green
    end

    subgraph Storage["Data Layer"]
        ODB[(Order Database)]:::purple
        TDB[(Trade Database)]:::purple
        MQ[Message Queue<br/>Kafka]:::purple
    end

    subgraph Distribution["Market Data Layer"]
        MDP[Market Data Publisher<br/>Format & Aggregate]:::primary
    end
...
flowchart LR
    ...
    subgraph "Supporting Data Stores"
        E[Redis: Real-time Caches]:::orange
        F[Kafka: Event Streaming]:::orange
        G[ClickHouse: Analytics]:::orange
    end
	
    subgraph "Why SQL for Trading Data"
        A[Strong ACID Guarantees]:::green
        B[Complex Joins for Reporting]:::green
        C[Transaction Support]:::green
        D[Mature Operations Tooling]:::green
    end	
...
```mermaid
flowchart TB
    subgraph Loop["Single-Threaded Event Loop"]
        direction TB
        S1["1. Receive Order<br/>from Input Queue"]:::primary
        S2["2. Match Against<br/>Order Book"]:::orange
        S3{"Match<br/>Found""}:::yellow
        S4["3a. Generate Trade<br/>Update Book"]:::green
        S5["3b. Add Order<br/>to Book"]:::secondary
        S6["4. Publish Events<br/>to Output Queue"]:::purple
    ...
    IQ[Input Queue<br/>Sequenced Orders]:::rose --> S1
    S6 --> OQ[Output Queue<br/>Trades & Updates]:::rose
...
flowchart TB
    subgraph OB["Order Book for AAPL"]
        ...
        subgraph BIDS["Bids (Buy Side)"]
            direction TB
            BP1["$150.00"]:::green
            BP2["$149.50"]:::green
            BP3["$149.00"]:::green

            BQ1["Order1 → Order2 → Order3<br/>(FIFO queue)"]:::secondary
            BQ2["Order4 → Order5"]:::secondary
            BQ3["Order6"]:::secondary
        ...
        subgraph ASKS["Asks (Sell Side)"]
            direction TB
            AP1["$150.50"]:::red
            AP2["$151.00"]:::red
            AP3["$152.00"]:::red

            AQ1["Order7 → Order8"]:::rose
            AQ2["Order9"]:::rose
            AQ3["Order10 → Order11"]:::rose
    ...
    BB["Best Bid: $150.00"]:::yellow -.-> BP1
    BA["Best Ask: $150.50"]:::yellow -.-> AP1
...
```mermaid
flowchart TB
    subgraph Router["Order Router"]
        R[Consistent Hash<br/>by Symbol]:::primary
    end

    subgraph Engines["Matching Engines"]
        ME1[Engine 1<br/>AAPL, GOOGL, MSFT]:::green
        ME2[Engine 2<br/>JPM, BAC, GS]:::orange
        ME3[Engine 3<br/>XOM, CVX, COP]:::purple
        ME4[Engine 4<br/>AMZN, META, NFLX]:::secondary
    end

    O1[Order: AAPL]:::yellow --> R
    O2[Order: JPM]:::yellow --> R
    O3[Order: XOM]:::yellow --> R
...
flowchart LR
    subgraph Input["Input Stage"]
        P1[Producer 1<br/>Network IO]:::primary
        P2[Producer 2<br/>Network IO]:::primary
    end

    subgraph Disruptor["LMAX Disruptor"]
        SEQ[Sequencer<br/>Claims slots]:::orange
        RB[Ring Buffer<br/>Pre-allocated array]:::purple
    end

    subgraph Consumers["Consumer Pipeline"]
        ML[Matcher<br/>Core logic]:::green
        JL[Journaler<br/>Write to disk]:::secondary
        RP[Publisher<br/>Market data]:::secondary
    end
...
```mermaid
sequenceDiagram
    participant A as Trader A<br/>(Slow Network)
    participant B as Trader B<br/>(Fast Network)
    participant GW as Gateway
    participant ME as Matching Engine

    Note over A,ME: Without Sequencing - UNFAIR

    A->>GW: Send Order (T=0)
    B->>GW: Send Order (T=2)
    Note over GW: B arrives first (T=3)
    Note over GW: A arrives later (T=5)
    GW->>ME: Process B's Order
    GW->>ME: Process A's Order
    Note over ME: B matched first!<br/>(Even though A sent first)
```
...
flowchart LR
    ...
    subgraph Sequencer["Central Sequencer"]
        SEQ["Assign Sequence<br/>next_seq++"]
    end

    subgraph Output["Sequenced Orders"]
        S1["Order A<br/>seq: 1"]
        S2["Order B<br/>seq: 2"]
        S3["Order C<br/>seq: 3"]
    end
    ...
    SEQ --> S2
    SEQ --> S3
    S1 --> ME[Matching Engine]
    S2 --> ME
    S3 --> ME

    style O1 fill:#00ceff,stroke:#000,color:#000
    style O2 fill:#00ceff,stroke:#000,color:#000
    style O3 fill:#00ceff,stroke:#000,color:#000
    style SEQ fill:#ffa94d,stroke:#000,color:#000
    style S1 fill:#69db7c,stroke:#000,color:#000
    style S2 fill:#69db7c,stroke:#000,color:#000
    style S3 fill:#69db7c,stroke:#000,color:#000
    style ME fill:#9775fa,stroke:#000,color:#000
```
...
```mermaid
flowchart TB
    ME[Matching<br/>Engine] --> MQ[Message<br/>Queue]

    MQ --> WS1[WebSocket Server 1<br/>2,000 clients]
    MQ --> WS2[WebSocket Server 2<br/>2,000 clients]
    MQ --> WS3[WebSocket Server 3<br/>2,000 clients]
    MQ --> WSN[WebSocket Server N<br/>2,000 clients]
    ...
    style ME fill:#ffa94d,stroke:#000,color:#000
    style MQ fill:#9775fa,stroke:#000,color:#000
    style WS1 fill:#00ceff,stroke:#000,color:#000
    style WS2 fill:#00ceff,stroke:#000,color:#000
    style WS3 fill:#00ceff,stroke:#000,color:#000
    style WSN fill:#00ceff,stroke:#000,color:#000
    style C1 fill:#38d9a9,stroke:#000,color:#000
    style C2 fill:#38d9a9,stroke:#000,color:#000
    style C3 fill:#38d9a9,stroke:#000,color:#000
    style CN fill:#38d9a9,stroke:#000,color:#000
```
...
```mermaid
flowchart TB
    ME[Matching Engine] --> MQ[Message Queue<br/>Kafka/Kinesis]

    MQ --> FO[Fan-out Layer<br/>Distribution Servers]

    FO --> E1[Edge Server 1<br/>US-East]
    FO --> E2[Edge Server 2<br/>US-West]
    FO --> E3[Edge Server 3<br/>Europe]
    FO --> E4[Edge Server 4<br/>Asia]
    ...
    style ME fill:#ffa94d,stroke:#000,color:#000
    style MQ fill:#9775fa,stroke:#000,color:#000
    style FO fill:#00ceff,stroke:#000,color:#000
    style E1 fill:#38d9a9,stroke:#000,color:#000
    style E2 fill:#38d9a9,stroke:#000,color:#000
    style E3 fill:#38d9a9,stroke:#000,color:#000
    style E4 fill:#38d9a9,stroke:#000,color:#000
...
```mermaid
flowchart TB
    O[Order Received] --> C1{Sufficient<br/>Funds"}

    C1 -->|No| R1[Reject:<br/>Insufficient Balance]
    C1 -->|Yes| C2{Order Size<br/>Valid"}

    C2 -->|No| R2[Reject:<br/>Order Too Large]
    C2 -->|Yes| C3{Price Within<br/>Bands"}

    C3 -->|No| R3[Reject:<br/>Price Out of Range]
    C3 -->|Yes| C4{Position<br/>Limit OK"}

    C4 -->|No| R4[Reject:<br/>Position Limit Exceeded]
    C4 -->|Yes| RS[Reserve Funds/Shares]

    RS --> FWD[Forward to<br/>Matching Engine]

    style O fill:#00ceff,stroke:#000,color:#000
    style C1 fill:#ffd43b,stroke:#000,color:#000
    style C2 fill:#ffd43b,stroke:#000,color:#000
    style C3 fill:#ffd43b,stroke:#000,color:#000
    style C4 fill:#ffd43b,stroke:#000,color:#000
    style R1 fill:#ff8787,stroke:#000,color:#000
    style R2 fill:#ff8787,stroke:#000,color:#000
    style R3 fill:#ff8787,stroke:#000,color:#000
    style R4 fill:#ff8787,stroke:#000,color:#000
    style RS fill:#ffa94d,stroke:#000,color:#000
    style FWD fill:#69db7c,stroke:#000,color:#000
```
...
```mermaid
flowchart TB
    subgraph CB["Circuit Breaker Levels"]
        direction TB

        PC["Previous Close: $100"]

        L1["Level 1: -7%<br/>Price hits $93"]
        L2["Level 2: -13%<br/>Price hits $87"]
        L3["Level 3: -20%<br/>Price hits $80"]
    ...
    style PC fill:#00ceff,stroke:#000,color:#000
    style L1 fill:#ffd43b,stroke:#000,color:#000
    ...
    style H3 fill:#9775fa,stroke:#000,color:#000
```
...
```mermaid
flowchart LR
    subgraph Recovery["Matching Engine Recovery"]
        direction LR
        S[Load Snapshot<br/>Seq #1,000,000] --> R[Replay Events<br/>1,000,001 to Current]
        R --> P[Resume<br/>Processing]
    end
    ...
    style R fill:#ffa94d,stroke:#000,color:#000
    style P fill:#69db7c,stroke:#000,color:#000
    style SNAP fill:#00ceff,stroke:#000,color:#000
    style LOG fill:#00ceff,stroke:#000,color:#000
```
...
flowchart TB
    ...
    subgraph OMS_Layer["Order Management Layer"]
        OMS1[OMS 1]
        OMS2[OMS 2]
        OMS3[OMS 3]
    end

    subgraph Sequencer_Layer["Sequencer Layer"]
        SEQ_P[Sequencer<br/>Primary]
        SEQ_S[Sequencer<br/>Standby]
    end
    ...
    style OMS1 fill:#38d9a9,stroke:#000,color:#000
    style OMS2 fill:#38d9a9,stroke:#000,color:#000
    style OMS3 fill:#38d9a9,stroke:#000,color:#000
    style SEQ_P fill:#ffa94d,stroke:#000,color:#000
    style SEQ_S fill:#ffd43b,stroke:#000,color:#000
    style ME fill:#ff8787,stroke:#000,color:#000
    style K1 fill:#9775fa,stroke:#000,color:#000
    style K2 fill:#9775fa,stroke:#000,color:#000
    style K3 fill:#9775fa,stroke:#000,color:#000
```
