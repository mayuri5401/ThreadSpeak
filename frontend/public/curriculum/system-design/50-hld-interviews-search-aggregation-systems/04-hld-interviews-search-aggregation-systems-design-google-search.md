---
id: "hld-interviews-search-aggregation-systems-design-google-search"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Search Aggregation Systems"
subSection: ""
title: "Design Google Search"
slug: "hld-interviews-search-aggregation-systems-design-google-search"
summary: "What makes this problem fascinating from a system design perspective is its breadth. It is arguably one of the most comprehensive system design problems you can encounter."
eli10: "Imagine Design Google Search as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Google Search Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TB
    subgraph "Storage Breakdown"
        A[100B Web Pages]:::rose --> B[10 PB Raw Data]:::purple
        B --> C[2-3 PB Index]:::primary
        C --> D[10+ PB with Replication]:::green
    end
...
flowchart LR
    subgraph "Infrastructure Scale"
        C[Crawlers<br/>Hundreds of machines<br/>11.5K pages/sec]:::orange
        I[Index Storage<br/>Thousands of machines<br/>10+ PB data]:::purple
        S[Serving Nodes<br/>100+ per region<br/>100K QPS capacity]:::green
    end
...
flowchart TB
    subgraph "Core API Operations"
        SEARCH[GET /search<br/>Main search query]:::primary
        SUGGEST[GET /suggest<br/>Autocomplete suggestions]:::orange
        SPELL[GET /spellcheck<br/>Spell correction]:::green
    end

    SEARCH --> R1[Ranked results with snippets]:::secondary
    SUGGEST --> R2[Query suggestions]:::secondary
    SPELL --> R3[Corrected query]:::secondary
...
```mermaid
flowchart TB
    subgraph "Three Subsystems"
        C[Crawling<br/>Discover and download pages]:::orange
        I[Indexing<br/>Process and build searchable data]:::primary
        S[Serving<br/>Handle queries and return results]:::green
    end

    C -->|feeds| I -->|provides data to| S
...
flowchart TB
    subgraph "Crawl Flow"
        direction LR
        UF[URL Frontier<br/>Prioritized queue]:::primary
        F[Fetcher<br/>HTTP requests]:::secondary
        DNS[DNS Resolver<br/>Cached lookups]:::orange
        WEB[Web Server]:::rose
        CS[Content Store<br/>Raw pages]:::purple
        LE[Link Extractor<br/>Find new URLs]:::green
    end

    UF -->|"1. Next URL"| F
    F -->|"2. Resolve hostname"| DNS
    DNS -->|"3. IP address"| F
    F -->|"4. HTTP request"| WEB
    WEB -->|"5. HTML response"| F
    F -->|"6. Store content"| CS
    CS -->|"7. Extract links"| LE
    LE -->|"8. New URLs"| UF
...
```mermaid
flowchart LR
    subgraph "Politeness Rules"
        R1[Respect robots.txt<br/>Follow crawl restrictions]:::red
        R2[Rate limit per domain<br/>Max 1 req/sec per host]:::orange
        R3[Identify yourself<br/>Use proper User-Agent]:::primary
        R4[Honor Crawl-delay<br/>Respect explicit delays]:::green
    end
...
flowchart TB
    subgraph "Indexing Flow"
        CS[Content Store<br/>Raw HTML]:::purple
        DP[Document Processor<br/>Extract text]:::secondary
        AN[Analyzer<br/>Tokenize & normalize]:::orange
        IB[Index Builder<br/>Build inverted index]:::primary
        LA[Link Analyzer<br/>Build link graph]:::rose
    end

    subgraph "Outputs"
        II[(Inverted Index)]:::green
        LG[(Link Graph)]:::green
        DS[(Document Store)]:::green
    end
...
flowchart TB
    ...
    QP[Query Processor<br/>Parse & expand query]:::orange
    CA[Cache<br/>Hot query results]:::green

    subgraph "Index Shards"
        IS1[Index Server 1]:::purple
        IS2[Index Server 2]:::purple
        IS3[Index Server 3]:::purple
        ISN[Index Server N]:::purple
    end

    AGG[Aggregator<br/>Merge & rank results]:::primary
    SG[Snippet Generator]:::secondary
...
```mermaid
flowchart TB
    subgraph "Crawling Subsystem"
        Internet((Internet)):::rose
        UF[URL Frontier]:::orange
        FET[Fetchers]:::secondary
        CS[Content Store]:::purple
    end

    subgraph "Indexing Subsystem"
        DP[Document Processor]:::orange
        IB[Index Builder]:::secondary
        LA[Link Analyzer]:::secondary
    end

    subgraph "Storage Layer"
        II[(Inverted Index)]:::purple
        LG[(Link Graph)]:::purple
        DS[(Document Store)]:::purple
    end

    subgraph "Serving Subsystem"
        Users((Users)):::rose
        QP[Query Processor]:::orange
        CACHE[Cache]:::green
        AGG[Aggregator]:::primary
        IS[Index Servers]:::secondary
    end
...
```mermaid
flowchart LR
    subgraph "Traditional DB Limitations"
        L1[100B documents cannot fit<br/>in a single database]:::red
        L2[Need sub-millisecond lookups<br/>from memory]:::red
        L3[Inverted indices need<br/>specialized structures]:::red
        L4[Read-heavy, append-mostly<br/>workload]:::red
    end
...
flowchart TB
    subgraph "Inverted Index Structure"
        TD[Term Dictionary<br/>Maps terms to posting lists]:::primary
        PL[Posting Lists<br/>Documents containing each term]:::orange
        TF[Term Statistics<br/>Frequencies for scoring]:::green
    end
...
flowchart TB
    subgraph "Document Partitioning"
        Q[Query]:::rose --> ALL[Send to ALL shards]:::orange
        ALL --> S1[Shard 1<br/>Docs 1-10B]:::primary
        ALL --> S2[Shard 2<br/>Docs 11-20B]:::primary
        ALL --> S3[Shard N<br/>...]:::primary
        S1 & S2 & S3 --> M[Merge top-K results]:::green
    end
...
```mermaid
flowchart TB
    subgraph "URL Frontier Architecture"
        PQ[Priority Queue<br/>Selects which host to crawl]:::primary

        subgraph "Per-Host Queues"
            H1[example.com<br/>url1, url2, url3]:::secondary
            H2[news.com<br/>url4, url5]:::secondary
            H3[blog.org<br/>url6, url7]:::secondary
        end

        DT[Delay Tracker<br/>Last request time per host]:::orange
    end
...
```mermaid
flowchart LR
    subgraph "Crawl Frequencies"
        direction TB
        F1[Breaking News<br/>Every 15 minutes]:::red
        F2[Popular Content<br/>Daily]:::orange
        F3[Static Pages<br/>Weekly]:::primary
        F4[Archive Content<br/>Monthly]:::secondary
    end
...
- [`http://example.com/page`](http://example.com/page) vs [`https://example.com/page`](https://example.com/page)
- [`www.example.com/page`](https://www.example.com/page) vs `example.com/page`
...
flowchart TB
    subgraph "Index Structure"
        TD[Term Dictionary<br/>Compact trie or FST]:::primary

        subgraph "Posting Lists"
            PL1[common term<br/>millions of docs]:::orange
            PL2[medium term<br/>thousands of docs]:::orange
            PL3[rare term<br/>dozens of docs]:::orange
        end

        subgraph "Per-Posting Entry"
            E1[doc_id]:::green
            E2[term_frequency]:::green
            E3[positions list]:::green
            E4[field flags]:::green
        end
    end
...
flowchart LR
    ...
    subgraph "Tier 0: Hot Cache"
        T0[Memory<br/>Most frequent terms<br/>Top docs by PageRank<br/>~5% of index]:::green
    end

    subgraph "Tier 1: Warm Storage"
        T1[SSD<br/>Medium frequency terms<br/>~20% of index]:::orange
    end

    subgraph "Tier 2: Cold Storage"
        T2[HDD<br/>Rare terms, long tail<br/>~75% of index]:::red
    end
...
```mermaid
flowchart LR
    subgraph "PageRank Intuition"
        A[Page A<br/>PR: 0.4]:::primary -->|link| D[Page D<br/>PR: 0.7]:::green
        B[Page B<br/>PR: 0.3]:::primary -->|link| D
        C[Page C<br/>PR: 0.2]:::primary -->|link| D

        E[Page E<br/>PR: 0.1]:::orange -->|link| A
    end
...
flowchart LR
    subgraph "Learning to Rank"
        F[Feature<br/>Extraction]:::secondary --> M[ML Model<br/>Gradient Boosted Trees]:::primary
        M --> S[Relevance<br/>Score]:::green
    end

    D[Document + Query]:::rose --> F
    T[Training Data<br/>Human ratings]:::orange --> M
...
```mermaid
flowchart LR
    subgraph Q["Query Intent Types"]
        N["Navigational<br/>facebook login"]:::primary
        I["Informational<br/>how tall is everest"]:::orange
        T["Transactional<br/>buy iphone 15"]:::green
    end

    N --> N1["Show facebook.com first"]:::secondary
    I --> I1["Show knowledge panel"]:::secondary
    T --> T1["Show shopping results"]:::secondary
...
```mermaid
flowchart LR
    Q[Query]:::rose --> L1[L1: Query Cache<br/>Full result pages<br/>30-50% hit rate]:::green
    L1 -->|miss| L2[L2: Posting Cache<br/>Frequent terms<br/>]:::orange
    L2 -->|miss| IDX[Index Servers]:::purple
...
flowchart TB
    subgraph "Quality Signals"
        direction TB
        C1[Content Length<br/>Very short = suspicious]:::orange
        C2[Unique Content Ratio<br/>High duplicate = scraper]:::orange
        C3[Ad Density<br/>Too many ads = low quality]:::orange
        L1[Link Velocity<br/>Sudden spike = purchased]:::primary
        L2[Link Neighborhood<br/>Links from spam sites]:::primary
        L3[Anchor Text Distribution<br/>Unnatural = manipulated]:::primary
    end
