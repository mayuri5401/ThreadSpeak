---
id: "hld-interviews-search-aggregation-systems-design-ad-click-aggregator"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Search Aggregation Systems"
subSection: ""
title: "Design Ad Click Aggregator"
slug: "hld-interviews-search-aggregation-systems-design-ad-click-aggregator"
...
eli10: "Imagine Design Ad Click Aggregator as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Ad Click Aggregator Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Search Aggregation Systems","System Design","Architecture"]
...
flowchart LR
    subgraph Sources
        W[Web Clicks]:::rose
        M[Mobile Clicks]:::rose
        S[SDK Events]:::rose
    end

    subgraph Processing
        AGG[Ad Click<br/>Aggregator]:::primary
    end

    subgraph Outputs
        DASH[Real-time<br/>Dashboards]:::green
        BILL[Billing<br/>Reports]:::orange
        ANAL[Analytics<br/>APIs]:::secondary
    end
...
flowchart LR
    ...
    subgraph "Read Path (Light)"
		direction LR
        R1[1M queries/day]:::primary --> R2[12 QPS avg]:::primary --> R3[120 QPS peak]:::primary
    end

    subgraph "Write Path (Dominant)"
		direction LR
        W1[10B clicks/day]:::orange --> W2[115K QPS avg]:::orange --> W3[350K QPS peak]:::red
    end
...
flowchart TB
    subgraph "Storage Growth"
        RAW[Raw Clicks<br/>1.5 TB/day<br/>45 TB/month]:::purple
        AGG[Aggregated Data<br/>140 GB/day<br/>4 TB/month]:::green
    end
...
flowchart LR
    subgraph "Ingestion APIs"
        POST1[POST /clicks<br/>Single click]:::orange
        POST2[POST /clicks/batch<br/>Batch clicks]:::orange
    end

    subgraph "Query APIs"
        GET1[GET /aggregations<br/>Flexible queries]:::primary
        GET2[GET /aggregations/top<br/>Leaderboards]:::primary
    end

    POST1 & POST2 --> |High volume| PROC[Processing<br/>Pipeline]:::secondary
    PROC --> STORE[(Aggregation<br/>Store)]:::purple
    STORE --> GET1 & GET2
...
flowchart LR
    ...
    subgraph "Write Path (115K QPS)"
        direction LR
        W1[Click Sources]:::rose --> W2[Ingestion Layer]:::primary
        W2 --> W3[Processing Pipeline]:::orange
        W3 --> W4[(Storage)]:::purple
    end
...
```mermaid
flowchart TB
    subgraph Clients["Click Sources"]
        WEB[Web Pixels]:::rose
        MOB[Mobile SDKs]:::rose
        SRV[Server-to-Server]:::rose
    end
    ...
    subgraph Receivers["Click Receiver Cluster"]
        CR1[Receiver 1]:::primary
        CR2[Receiver 2]:::primary
        CR3[Receiver N...]:::primary
    end

    subgraph Queue["Message Queue"]
        KF[Kafka Cluster]:::orange
    end
...
sequenceDiagram
    ...
    SDK->>LB: POST /clicks (click data)
    LB->>CR: Forward request
    Note over CR: Validate required fields<br/>Add server timestamp
    CR->>KF: Publish to clicks topic
    KF-->>CR: Acknowledge receipt
    CR-->>LB: 202 Accepted
    LB-->>SDK: 202 Accepted
    Note over SDK: Client continues<br/>(async processing begins)
    KF->>DL: Batch write to storage
```
...
```mermaid
flowchart TB
    RAW[Raw Clicks]:::orange

    subgraph Speed["Speed Layer (Real-time)"]
        STREAM[Stream Processing]:::primary
        RTV[Real-time View<br/>Approximate]:::green
    end

    subgraph Batch["Batch Layer"]
        BATCH[Batch Processing]:::primary
        BV[Batch View<br/>Accurate]:::green
    end

    subgraph Serving["Serving Layer"]
        MERGE[Merge Views]:::purple
    end
...
```mermaid
flowchart LR
    RAW[Raw Clicks]:::orange --> KF[Kafka<br/>Retained Log]:::orange
    KF --> SP[Stream Processor]:::primary
    SP --> AS[(Aggregation Store)]:::purple

    KF -.->|Replay if needed| SP
...
```mermaid
flowchart LR
    subgraph Input["From Kafka"]
        KF[Kafka Topic:<br/>raw_clicks]:::orange
    end

    subgraph Processing["Flink Processing"]
        CONSUME[Consume Events]:::primary
        DEDUP[Deduplication<br/>Check]:::green
        WINDOW[Time Windowing<br/>1-min tumbling]:::primary
        AGG[Aggregate by<br/>Dimensions]:::primary
    end

    subgraph External["External Stores"]
        REDIS[(Redis<br/>Dedup Store)]:::red
        CH[(ClickHouse<br/>Aggregations)]:::purple
    end
...
sequenceDiagram
    ...
    KF->>FL: Batch of click events
    loop For each click
        FL->>RD: SET click_id NX EX 86400
        alt New click (SET returned OK)
            RD-->>FL: OK (proceed)
            Note over FL: Add to window state
        else Duplicate (SET returned nil)
            RD-->>FL: nil (skip)
            Note over FL: Discard duplicate
        end
    end
    Note over FL: Window closes (1 minute elapsed)
    FL->>CH: INSERT aggregated counts
    CH-->>FL: Acknowledge
    Note over FL: Commit Kafka offset
```
...
```mermaid
flowchart LR
    DASH[Advertiser Dashboard]:::rose

    subgraph QueryLayer["Query Layer"]
        QS[Query Service]:::primary
    end

    subgraph Caching["Cache Layer"]
        RC[(Redis Cache)]:::orange
    end

    subgraph Storage["Aggregation Storage"]
        CH[(ClickHouse)]:::purple
    end
...
sequenceDiagram
    ...
    UI->>QS: GET /aggregations"...
    Note over QS: Validate permissions
    QS->>RC: Check cache (query hash)
    alt Cache hit
        RC-->>QS: Cached results
        QS-->>UI: Return results (fast)
    else Cache miss
        RC-->>QS: Miss
        QS->>CH: Execute query
        CH-->>QS: Query results
        QS->>RC: Cache results (TTL: 5min)
        QS-->>UI: Return results
    end
```
...
flowchart TB
    subgraph Sources["Click Sources"]
        WEB[Web Tracking]:::rose
        MOB[Mobile SDKs]:::rose
        API[API Integrations]:::rose
    end

    subgraph Ingestion["Ingestion Layer"]
        LB[Load Balancer]:::secondary
        CR[Click Receiver<br/>Cluster]:::primary
    end

    subgraph Streaming["Streaming Layer"]
        KF[Apache Kafka]:::orange
    end

    subgraph Processing["Processing Layer"]
        FL[Apache Flink]:::primary
        REDIS_D[(Redis<br/>Dedup)]:::red
    end

    subgraph Storage["Storage Layer"]
        DL[(Data Lake<br/>Raw Events)]:::purple
        CH[(ClickHouse<br/>Aggregations)]:::purple
    end

    subgraph Serving["Query Layer"]
        QS[Query Service]:::primary
        REDIS_C[(Redis<br/>Cache)]:::orange
    end

    subgraph Consumers["Consumers"]
        DASH[Dashboards]:::rose
        BILL[Billing]:::rose
        REPORT[Reports]:::rose
    end
...
flowchart TD
    subgraph "Storage Requirements"
        RAW[Raw Clicks<br/>115K writes/sec<br/>Append-only]:::orange
        DEDUP[Dedup Store<br/>350K ops/sec<br/>Low latency]:::red
        AGG[Aggregations<br/>Billions of rows<br/>Fast analytics]:::purple
    end

    RAW --> S3[(S3/Parquet)]:::orange
    DEDUP --> RD[(Redis)]:::red
    AGG --> CH[(ClickHouse)]:::purple
...
flowchart LR
    subgraph "Parquet File Structure"
        direction TB
        P1[Partition: date=2024-01-15]:::purple
        P2[Partition: hour=10]:::purple
        F1[File: clicks_001.parquet]:::orange
        F2[File: clicks_002.parquet]:::orange
    end
...
```mermaid
flowchart LR
    Q[Query Request]:::rose --> R{Time Range"}
    R -->|"< 24 hours"| M[(Minute Table)]:::primary
    R -->|"> 24 hours"| H[(Hourly Table)]:::orange
...
```mermaid
flowchart LR
    subgraph "Duplicate Sources"
        D1[Double-click]:::rose
        D2[Network retry]:::rose
        D3[SDK bug]:::rose
        D4[LB retry]:::rose
    end

    subgraph "Dedup System"
        DS[Deduplication<br/>Store]:::primary
    end

    subgraph "Outcomes"
        OK[Count Once]:::green
        SKIP[Skip Duplicate]:::orange
    end
...
```mermaid
flowchart LR
    C[Click arrives]:::rose --> BF{Bloom<br/>Filter}
    BF -->|"Definitely NOT<br/>in set"| NEW[New click<br/>Process it]:::green
    BF -->|"Possibly in set"| CHECK[Check Redis<br/>to confirm]:::orange
...
```mermaid
flowchart TB
    C[Click arrives<br/>click_id: abc123]:::rose

    subgraph Redis["Redis Operation"]
        CMD["SET abc123 1 NX EX 86400"]:::primary
    end

    C --> CMD
    CMD -->|"Returns OK"| NEW[New click<br/>Process it]:::green
    CMD -->|"Returns nil"| DUP[Duplicate<br/>Skip it]:::orange
...
flowchart TB
    ...
    C --> BF
    BF -->|"Definitely new"| P1[Process click<br/>Add to both]:::green
    BF -->|"Possibly exists"| RD
    RD -->|"Not in Redis"| P2[Process click<br/>Add to Redis]:::green
    RD -->|"In Redis"| SKIP[Skip duplicate]:::red
...
sequenceDiagram
    ...
    participant Window as Window 10:00-10:01

    Note over User: Click A occurs<br/>event_time = 10:00:30
    Note over User: Click B occurs<br/>event_time = 10:00:45
    Note over Server: Time passes...
    Note over Window: 10:01:00 - Window should close
    User->>Server: Click A finally arrives (10:01:15)
    Note over Window: 10:01:30 - Results emitted<br/>Missing Click A!
    User->>Server: Click B arrives (10:02:00)
    Note over Window: Click B is even later
```
...
flowchart LR
    subgraph "Event Stream"
        E1["Event<br/>10:00:30"]:::primary
        E2["Event<br/>10:00:45"]:::primary
        E3["Event<br/>10:01:10"]:::primary
        E4["Event<br/>10:01:20"]:::primary
    end

    subgraph "Watermark Calculation"
        WM["Watermark = 10:01:20 - 2min<br/>= 10:00:50"]:::orange
    end

    subgraph "Window Status"
        W1["Window 10:00-10:01<br/>Still OPEN"]:::green
    end
...
flowchart LR
    ...
    subgraph "Late Data Path"
		direction LR
        LATE[Late Events]:::orange --> SIDE[Side Output]:::orange
        SIDE --> BATCH[Batch Job]:::primary
        BATCH --> ACCURATE[Billing View<br/>Accurate]:::green
    end

    subgraph "Main Path"
		direction LR
        STREAM[Stream Processing]:::primary --> RT[Real-time View<br/>Approximate]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Tier 1: Real-time (< 2 min)"
        T1[Window closes at watermark]:::primary
        T1 --> D1[Dashboard shows<br/>preliminary results]:::green
    end

    subgraph "Tier 2: Late Data (2 min - 24 hr)"
        T2[Late events arrive]:::orange
        T2 --> D2[Upsert to update<br/>aggregations]:::orange
    end

    subgraph "Tier 3: Reconciliation (Daily)"
        T3[Batch job runs]:::purple
        T3 --> D3[Final verification<br/>for billing]:::purple
    end
...
sequenceDiagram
    ...
    KF->>FL: Read click events
    Note over FL: Process and aggregate
    FL->>CH: Write aggregation
    Note over CH: Write succeeds
    Note over FL: CRASH HERE!
    Note over KF: Offset NOT committed

    Note over FL: On restart...
    KF->>FL: Replay same events (offset was not committed)
    Note over FL: Process again
    FL->>CH: Write AGAIN
    Note over CH: Duplicate counts!
```
...
flowchart LR
    ...
    subgraph "On Replay (after crash)"
		direction LR
        R2[Read same click]:::primary --> D2{Dedup<br/>check}
        D2 -->|Already seen| S2[Skip]:::orange
    end

    subgraph "On First Processing"
		direction LR
        R1[Read click]:::primary --> D1{Dedup<br/>check}
        D1 -->|New| P1[Process + Store click_id]:::green
        P1 --> W1[Write aggregation]:::green
    end
...
flowchart LR
    subgraph "Single Transaction"
        BEGIN[BEGIN TRANSACTION]:::primary
        W1[Write aggregation update]:::green
        W2[Write click_id to dedup table]:::green
        COMMIT[COMMIT]:::primary
    end

    subgraph "After Commit"
        ACK[Commit Kafka offset]:::orange
    end

    BEGIN --> W1 --> W2 --> COMMIT --> ACK
...
flowchart TB
    subgraph "Atomic Transaction"
        READ[Read from input topic]:::primary
        PROCESS[Process events]:::primary
        WRITE[Write to output topic]:::primary
        COMMIT[Commit offset + writes]:::green
    end
...
```mermaid
flowchart LR
    subgraph "Stage 1: Kafka Transactions"
        IN[Input Topic]:::orange --> FL[Flink]:::primary
        FL --> OUT[Output Topic]:::orange
    end

    subgraph "Stage 2: Idempotent Sink"
        OUT --> SINK[ClickHouse Sink]:::primary
        SINK <--> DEDUP[(Dedup Store)]:::red
        SINK --> CH[(ClickHouse)]:::purple
    end
...
```mermaid
flowchart TB
    subgraph "Partitioned by Day"
        P1[Partition: 2024-01-15]:::primary
        P2[Partition: 2024-01-16]:::primary
        P3[Partition: 2024-01-17]:::primary
    end

    Q[Query: Last 24 hours]:::rose --> P2 & P3
    Q -.->|Skipped| P1
...
```mermaid
flowchart LR
    RAW[Raw Clicks]:::rose --> MIN[Minute Aggregations<br/>1.4B records/day]:::primary
    MIN --> HOUR[Hourly Aggregations<br/>24M records/day]:::orange
    HOUR --> DAY[Daily Aggregations<br/>1M records/day]:::green
...
```mermaid
flowchart TB
    subgraph "Stream Path (Real-time)"
        STREAM[Stream Processing]:::primary --> S_AGG[(Stream<br/>Aggregations)]:::orange
    end

    subgraph "Batch Path (Daily)"
        DL[(Data Lake<br/>Raw Events)]:::purple --> BATCH[Batch Job]:::primary
        BATCH --> B_AGG[(Batch<br/>Aggregations)]:::green
    end

    subgraph "Reconciliation"
        S_AGG --> COMPARE[Compare]:::primary
        B_AGG --> COMPARE
        COMPARE --> RESULT{Discrepancy"}
        RESULT -->|Within tolerance| OK[Use stream value]:::green
        RESULT -->|Above tolerance| ALERT[Alert + Use batch]:::red
    end
...
```mermaid
flowchart LR
    subgraph "Common Fraud Types"
        BOT[Bot Clicks<br/>Automated scripts]:::red
        FARM[Click Farms<br/>Paid humans]:::red
        COMP[Competitor Clicks<br/>Budget draining]:::red
        PUB[Publisher Fraud<br/>Fake traffic]:::red
    end
...
```mermaid
flowchart TB
    CLICK[Incoming Click]:::rose --> R1{IP Rate<br/>> 10/min"}
    R1 -->|Yes| SUS[Mark Suspicious]:::red
    R1 -->|No| R2{Known Bot<br/>User Agent"}
    R2 -->|Yes| SUS
    R2 -->|No| R3{Click Time<br/>< 100ms"}
    R3 -->|Yes| SUS
    R3 -->|No| OK[Process Normally]:::green

    SUS --> SEP[Separate Table<br/>Exclude from billing]:::orange
