---
id: "hld-interviews-search-aggregation-systems-design-a-web-crawler"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Search Aggregation Systems"
subSection: ""
title: "Design a Web Crawler"
slug: "hld-interviews-search-aggregation-systems-design-a-web-crawler"
summary: "Web crawlers serve many purposes beyond search engines. They power price comparison sites, archive the web for historical preservation, gather data for machine learning datasets, monitor brand mentions, and detect copyright violations."
eli10: "Imagine Design a Web Crawler as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design a Web Crawler Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
```mermaid
flowchart LR
    subgraph "Scale Summary"
        C[Crawl Rate<br/>400 pages/sec avg<br/>1,155 pages/sec peak]:::primary
        S[Storage<br/>500 TB pages<br/>10 TB metadata]:::purple
        B[Bandwidth<br/>1.6 Gbps download]:::orange
    end
...
flowchart LR
    subgraph "Core API Operations"
        direction TB
        ADD[POST /urls<br/>Add URLs to crawl]:::primary
        STATUS[GET /status<br/>System health and metrics]:::orange
        EXPORT[GET /pages<br/>Retrieve crawled content]:::green
    end

    ADD --> R1[URLs queued for crawling]:::secondary
    STATUS --> R2[Metrics and health info]:::secondary
    EXPORT --> R3[Page content and metadata]:::secondary
...
```mermaid
flowchart LR
    subgraph "The Crawl Loop"
        Q[URL Queue]:::primary --> F[Fetch Page]:::orange
        F --> P[Parse Content]:::primary
        P --> E[Extract Links]:::orange
        E --> Q
        P --> S[(Storage)]:::purple
    end
...
flowchart TB
    subgraph "URL Frontier"
        NEW[New URLs]:::rose --> DD[Deduplication<br/>Check if seen]:::orange
        DD --> PQ[Prioritizer<br/>Assign priority]:::primary

        PQ --> Q1[Queue: High Priority]:::green
        PQ --> Q2[Queue: Normal Priority]:::green
        PQ --> Q3[Queue: Low Priority]:::green

        subgraph "Per-Host Queues"
            H1[example.com]:::secondary
            H2[news.org]:::secondary
            H3[blog.net]:::secondary
        end

        Q1 & Q2 & Q3 --> H1 & H2 & H3

        H1 & H2 & H3 --> SEL[Host Selector<br/>Respects rate limits]:::primary
        SEL --> OUT[Next URL]:::rose
    end
...
flowchart TB
    subgraph "Fetcher Service"
        UF[URL Frontier]:::primary --> DNS[DNS Resolver<br/>Cached lookups]:::secondary
        DNS --> RC[Robots Checker<br/>Verify allowed]:::orange
        RC --> HTTP[HTTP Client<br/>Download page]:::primary
        HTTP --> VAL[Validator<br/>Check response]:::secondary
        VAL --> OUT[Content + Metadata]:::green
    end
...
```mermaid
flowchart TB
    START[Get URL from Frontier]:::rose --> DNS[Resolve DNS]:::secondary
    DNS --> ROBOT[Check robots.txt]:::orange

    ROBOT -->|allowed| FETCH[HTTP GET request]:::primary
    ROBOT -->|blocked| SKIP[Skip URL, mark blocked]:::red

    FETCH --> RESP{Response"}

    RESP -->|success| STORE[Store content]:::green
    RESP -->|redirect| REDIR[Follow redirect]:::orange
    RESP -->|error| RETRY{Retryable"}

    REDIR --> FETCH
    RETRY -->|yes| BACK[Back to queue with delay]:::orange
    RETRY -->|no| FAIL[Mark failed]:::red
...
```mermaid
flowchart TB
    UF[URL Frontier]:::primary

    subgraph "Fetcher Fleet"
        F1[Fetcher 1]:::secondary
        F2[Fetcher 2]:::secondary
        F3[Fetcher 3]:::secondary
        FN[Fetcher N]:::secondary
    end

    UF --> F1 & F2 & F3 & FN

    subgraph "Output"
        CS[(Content Store)]:::purple
        MQ[Parse Queue]:::green
    end
...
```mermaid
flowchart LR
    RAW[Raw HTML]:::rose --> PARSE[HTML Parser<br/>Build DOM]:::primary
    PARSE --> LINKS[Link Extractor<br/>Find URLs]:::secondary
    PARSE --> CONTENT[Content Extractor<br/>Extract text]:::secondary

    LINKS --> NORM[URL Normalizer]:::orange
    NORM --> FILT[Link Filter<br/>Remove unwanted]:::orange
    FILT --> FRONTIER[To URL Frontier]:::green

    CONTENT --> TEXT[Clean Text]:::green
    CONTENT --> META[Metadata]:::green
...
```mermaid
flowchart LR
    DOC[Document Text]:::rose --> TOK[Tokenize]:::secondary
    TOK --> HASH[Hash each token]:::secondary
    HASH --> AGG[Aggregate hashes]:::primary
    AGG --> SIM[64-bit SimHash]:::green

    SIM --> COMP[Compare to existing<br/>Hamming distance < 3]:::orange
    COMP -->|duplicate| SKIP[Skip or merge]:::red
    COMP -->|unique| STORE[Store normally]:::green
...
flowchart TB
    subgraph "URL Management"
        SEED[Seed URLs]:::rose
        API[URL API]:::rose
        FRONTIER[URL Frontier<br/>Priority queues + Per-host queues]:::primary
        URLDB[(URL Metadata DB)]:::purple
    end

    subgraph "Fetching"
        DNS[DNS Resolver<br/>Cached lookups]:::secondary
        ROBOT[(Robots.txt Cache)]:::purple
        FETCHERS[Fetcher Fleet<br/>N distributed workers]:::primary
    end

    subgraph "Processing"
        PARSER[HTML Parser]:::secondary
        EXTRACTOR[Link Extractor]:::secondary
        DEDUP[Duplicate Detector<br/>SimHash]:::orange
    end

    subgraph "Storage"
        CONTENT[(Content Store<br/>WARC files)]:::purple
        META[(URL Metadata)]:::purple
    end
...
flowchart LR
    subgraph "Storage Systems"
        direction TB
        KV[URL Metadata<br/>Key-value store<br/>Point lookups, updates]:::primary
        BLOB[Content Store<br/>Object storage<br/>Sequential writes, batch reads]:::orange
        CACHE[Caches<br/>In-memory<br/>Hot data, fast access]:::green
    end
...
```mermaid
flowchart TB
    subgraph "Back Queues: By Priority"
        B1[Back Queue 1<br/>Highest Priority URLs]:::green
        B2[Back Queue 2<br/>High Priority URLs]:::green
        B3[Back Queue 3<br/>Normal Priority URLs]:::green
        B4[Back Queue 4<br/>Low Priority URLs]:::green
    end

    subgraph "Front Queues: By Host"
        F1[example.com<br/>Queue]:::primary
        F2[news.org<br/>Queue]:::primary
        F3[blog.net<br/>Queue]:::primary
        FN[host-N<br/>Queue]:::primary
    end

    B1 & B2 & B3 & B4 --> ROUTE[Router<br/>By host hash]:::orange
    ROUTE --> F1 & F2 & F3 & FN

    subgraph "Selection"
        HEAP[Min-Heap<br/>By next-allowed-time]:::secondary
    end

    F1 & F2 & F3 & FN --> HEAP
    HEAP --> OUT[Next URL to Fetch]:::rose
...
```mermaid
flowchart LR
    subgraph "Adaptive Scheduling"
        CRAWL[Crawl Page]:::primary --> CHECK{Content<br/>Changed"}
        CHECK -->|yes| FASTER[Decrease interval<br/>×0.8]:::green
        CHECK -->|no| SLOWER[Increase interval<br/>×1.2]:::orange
        FASTER & SLOWER --> CLAMP[Clamp to min/max]:::secondary
        CLAMP --> SCHEDULE[Schedule next crawl]:::primary
    end
...
flowchart TB
    subgraph "Coordinator"
        ASSIGN[Domain Assignment<br/>Map domains to crawlers]:::primary
        MONITOR[Health Monitor<br/>Track crawler status]:::secondary
        REBAL[Rebalancer<br/>Handle failures]:::orange
    end

    subgraph "Crawlers"
        C1[Crawler 1<br/>Domains A-D]:::green
        C2[Crawler 2<br/>Domains E-H]:::green
        C3[Crawler 3<br/>Domains I-L]:::green
    end
...
```mermaid
flowchart LR
    REQ[Fetch Request]:::rose --> CHECK{robots.txt<br/>cached"}
    CHECK -->|yes, fresh| APPLY[Apply rules]:::green
    CHECK -->|no or stale| FETCH[Fetch robots.txt]:::primary
    FETCH --> PARSE[Parse rules]:::secondary
    PARSE --> CACHE[(Cache<br/>TTL: 24h)]:::purple
    CACHE --> APPLY
...
```mermaid
flowchart TB
    RESP[HTTP Response]:::rose --> STATUS{Status<br/>Code"}

    STATUS -->|200| OK[Process normally]:::green
    STATUS -->|301, 302| REDIRECT[Follow redirect<br/>Max 10 hops]:::orange
    STATUS -->|403| BLOCKED[Blocked - backoff]:::red
    STATUS -->|429| RATE[Rate limited - slow down]:::orange
    STATUS -->|500-599| SERVER[Server error - retry later]:::orange
    STATUS -->|timeout| TIMEOUT[Timeout - retry with backoff]:::red

    BLOCKED --> BACK1[Exponential backoff<br/>Skip domain temporarily]:::secondary
    RATE --> BACK2[Increase crawl delay<br/>for this domain]:::secondary
...
```mermaid
flowchart LR
    URL[New URL]:::rose --> NORM[Normalize URL]:::secondary
    NORM --> BLOOM{Bloom<br/>Filter}
    BLOOM -->|probably new| DB{Check<br/>Database}
    BLOOM -->|definitely seen| SKIP[Skip URL]:::red
    DB -->|exists| SKIP
    DB -->|new| ADD[Add to frontier<br/>and DB]:::green
...
```mermaid
flowchart TB
    DOC1[Document 1]:::primary --> SH1[SimHash: 1010...1101]:::secondary
    DOC2[Document 2<br/>Same with minor changes]:::primary --> SH2[SimHash: 1010...1100]:::secondary

    SH1 & SH2 --> COMPARE[XOR + popcount<br/>Hamming distance = 1]:::orange
    COMPARE --> RESULT[Near-duplicate!]:::green
...
```mermaid
flowchart LR
    FETCH[Fetch Page]:::primary --> CHECK{Meaningful<br/>content"}
    CHECK -->|yes| STORE[Store and process]:::green
    CHECK -->|no| TAG[Mark as needs-JS]:::orange
    TAG --> DEPRI[Lower priority or skip]:::secondary
...
```mermaid
flowchart TB
    URL[URL]:::rose --> SIMPLE{Needs JS<br/>Rendering"}
    SIMPLE -->|no| BASIC[Basic HTTP Fetch]:::green
    SIMPLE -->|yes| BROWSER[Headless Browser<br/>Chrome, Puppeteer]:::primary

    BROWSER --> WAIT[Wait for JS execution<br/>3-10 seconds]:::secondary
    WAIT --> EXTRACT[Extract rendered DOM]:::secondary
    BASIC & EXTRACT --> STORE[(Storage)]:::purple
...
```mermaid
flowchart TB
    URL[Extracted URL]:::rose --> DEPTH{Path depth<br/>> 15"}
    DEPTH -->|yes| SKIP[Skip URL]:::red
    DEPTH -->|no| DOMAIN{URLs from<br/>domain today<br/>> 100K"}
    DOMAIN -->|yes| SKIP
    DOMAIN -->|no| PATTERN{Matches trap<br/>pattern"}
    PATTERN -->|yes| SKIP
    PATTERN -->|no| ACCEPT[Accept URL]:::green
