---
id: "hld-interviews-location-based-services-design-food-delivery-service"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Location Based Services"
subSection: ""
title: "Design Food Delivery Service"
slug: "hld-interviews-location-based-services-design-food-delivery-service"
summary: "This system design problem touches on many interesting challenges: real-time location tracking, geo-spatial queries for finding nearby entities, coordinating multiple actors with different needs, optimizing competing constraints (fast delivery vs ..."
eli10: "Imagine Design Food Delivery Service as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Food Delivery Service Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    ...
    subgraph "Partner Features"
		direction LR
        P1[Receive Assignments]:::primary
        P2[Navigate to Pickup]:::primary
        P3[Complete Delivery]:::primary
    end

    subgraph "Restaurant Features"
		direction LR
        R1[View Orders]:::orange
        R2[Accept/Reject]:::orange
        R3[Update Status]:::orange
    end

    subgraph "Customer Features"
		direction LR
        C1[Browse Restaurants]:::rose
        C2[Place Orders]:::rose
        C3[Track Delivery]:::rose
    end
...
```mermaid
flowchart TB
    subgraph "Traffic Comparison"
        O[Orders<br/>600 QPS peak]:::orange
        L[Location Updates<br/>50,000 QPS peak]:::red
    end
...
```mermaid
flowchart LR
    subgraph "Storage Breakdown (Annual)"

        A[Orders<br/>7.3 TB]:::primary
        B[Location History<br/>1 TB / week]:::orange
        C[Restaurant Data<br/>~100 GB]:::secondary
    end
...
flowchart TB
    ...
    subgraph "Partner APIs"
        P1[POST /partners/id/location<br/>Send location]:::primary
    end

	subgraph "Restaurant APIs"
        R1[PUT /orders/id/status<br/>Update order status]:::orange
    end

    subgraph "Customer APIs"
        C1[GET /restaurants<br/>Search nearby]:::rose
        C2[POST /orders<br/>Place order]:::rose
        C3[GET /orders/id/tracking<br/>Track delivery]:::rose
    end
...
```mermaid
flowchart TB
    CA[Customer App]:::rose

    subgraph "Discovery Flow"
        DS[Discovery Service]:::primary
        LS[Location Service]:::orange
        RS[Restaurant Service]:::orange
        CACHE[(Redis Cache)]:::purple
    end

    subgraph "Data Stores"
        ES[(Elasticsearch<br/>Geo + Search)]:::purple
        PG[(PostgreSQL<br/>Restaurant Data)]:::purple
    end

    CA -->|"1. GET /restaurants"lat&lng"| DS
    DS -->|"2. Check cache"| CACHE
    DS -->|"3. Find nearby"| LS
    LS --> ES
    DS -->|"4. Get details"| RS
    RS --> PG
    DS -->|"5. Return ranked list"| CA
...
sequenceDiagram
    ...
    C->>DS: GET /restaurants"lat=X&lng=Y&cuisine=italian
    DS->>Cache: Check for cached results
    alt Cache Hit
        Cache-->>DS: Return cached restaurants
    else Cache Miss
        DS->>LS: Find restaurants within 5km
        Note over LS: Uses geo-spatial index<br/>Returns IDs + distances
        LS-->>DS: Restaurant IDs with distances
        DS->>RS: Get restaurant details (batch)
        Note over RS: Enriches with menus,<br/>ratings, prep times
        RS-->>DS: Full restaurant data
        DS->>DS: Apply filters, calculate ETAs, rank
        DS->>Cache: Cache results (TTL: 5 min)
    end
    DS-->>C: Ranked restaurant list with ETAs
```
...
flowchart TB
    CA[Customer App]:::rose
    RA[Restaurant App]:::orange

    subgraph "Order Flow"
        OS[Order Service]:::primary
        PS[Payment Service]:::secondary
        NS[Notification Service]:::secondary
    end

    subgraph "External"
        PG[Payment Gateway]:::purple
        PUSH[Push Service]:::purple
    end

    subgraph "Storage"
        DB[(PostgreSQL<br/>Orders)]:::purple
    end

    CA -->|"1. POST /orders"| OS
    OS -->|"2. Authorize"| PS
    PS --> PG
    OS -->|"3. Create order"| DB
    OS -->|"4. Notify"| NS
    NS --> PUSH
    NS -->|"5. New order!"| RA
...
sequenceDiagram
    ...
    C->>OS: POST /orders (items, address, payment)
    OS->>PS: Authorize payment
    Note over PS: Hold funds,<br/>don't charge yet
    PS-->>OS: Authorization token

    OS->>DB: Create order (status: PENDING)
    OS->>NS: Notify restaurant of new order
    NS->>RA: Push: New order #1234!
    OS-->>C: Order created (ID, ETA, total)

    Note over RA: Restaurant reviews order
    RA->>OS: Accept order
    OS->>DB: Update status: CONFIRMED
    OS->>NS: Notify customer
    NS->>C: Push: Order confirmed!

    Note over RA: Kitchen prepares food
    RA->>OS: Update status: PREPARING
    RA->>OS: Update status: READY_FOR_PICKUP
```
...
stateDiagram-v2
    ...
    CONFIRMED --> CANCELLED: Customer cancels<br/>(with penalty)

    PREPARING --> READY: Food ready
    PREPARING --> CANCELLED: Rare<br/>(full refund)
    ...
    %% Color classes
    classDef pending fill:#ffd43b,stroke:#000,color:#000
    classDef confirmed fill:#00ceff,stroke:#000,color:#000
    classDef preparing fill:#ffa94d,stroke:#000,color:#000
    classDef ready fill:#38d9a9,stroke:#000,color:#000
    classDef picked fill:#9775fa,stroke:#000,color:#000
...
flowchart TB
    ...
    subgraph "Assignment Flow"
        AS[Assignment Service]:::orange
        LTS[Location Tracking Service]:::orange
        PS[Partner Service]:::orange
    end

    subgraph "Partner Apps"
        PA1[Partner App 1]:::rose
        PA2[Partner App 2]:::rose
        PA3[Partner App N]:::rose
    end

    NS[Notification Service]:::secondary

    OS -->|"1. Order ready"| AS
    AS -->|"2. Get nearby partners"| LTS
    AS -->|"3. Get partner details"| PS
    AS -->|"4. Score and rank"| AS
    AS -->|"5. Assign to best"| NS
    NS --> PA1
    NS --> PA2
    NS --> PA3

    PA1 -->|Location updates| LTS
    PA2 -->|Location updates| LTS
    PA3 -->|Location updates| LTS
...
sequenceDiagram
    ...
    participant LTS as Location Tracking
    participant PS as Partner Service
    participant NS as Notification Service
    participant PA as Partner App

    OS->>AS: Order #1234 ready for assignment

    AS->>LTS: Get partners within 3km of restaurant
    LTS-->>AS: [Partner A, B, C, D, E] with locations

    AS->>PS: Get partner details (batch)
    PS-->>AS: Ratings, status, current orders

    Note over AS: Score each partner:<br/>distance, ETA, rating,<br/>idle time, acceptance rate

    AS->>AS: Rank by score, select top partner

    AS->>NS: Assign to Partner B
    NS->>PA: New delivery request!

    alt Partner Accepts (within 30s)
        PA->>AS: Accept assignment
        AS->>OS: Partner B assigned to order #1234
    else Partner Rejects or Timeout
        AS->>AS: Move to next partner
        AS->>NS: Assign to Partner C
    end
```
...
flowchart LR
    subgraph "Partners"
        PA[Partner Apps]:::rose
    end

    subgraph "Ingestion"
        LTS[Location Tracking<br/>Service]:::primary
    end

    subgraph "Storage & Streaming"
        REDIS[(Redis<br/>Current Locations)]:::purple
        KAFKA[Kafka<br/>Location Events]:::purple
    end

    subgraph "Delivery"
        WSG[WebSocket<br/>Gateway]:::secondary
    end

    subgraph "Customers"
        CA[Customer Apps]:::rose
    end

    PA -->|"50K updates/sec"| LTS
    LTS --> REDIS
    LTS --> KAFKA
    KAFKA --> WSG
    WSG <-->|"WebSocket"| CA
...
sequenceDiagram
    ...
    Note over CA: Customer opens tracking screen
    CA->>WSG: Subscribe to order #1234

    loop Every 5 seconds
        PA->>LTS: POST /location (lat, lng)
        LTS->>Redis: Update current location
        LTS->>Kafka: Publish location event
        Kafka->>WSG: Location update for order #1234
        WSG->>CA: Push via WebSocket
...
flowchart TB
    ...
    subgraph "Application Services"
        ...
        PS[Partner Service]:::orange
        AS[Assignment Service]:::orange
        LTS[Location Tracking]:::orange
        LS[Location Service]:::orange
        PAY[Payment Service]:::green
        NS[Notification Service]:::green
        ETA[ETA Service]:::green
    end

    subgraph "Data Stores"
        PG[(PostgreSQL<br/>Orders, Users)]:::purple
        REDIS[(Redis<br/>Location Cache)]:::purple
        ES[(Elasticsearch<br/>Restaurant Search)]:::purple
        KAFKA[Kafka<br/>Events]:::purple
    end

    subgraph "External Services"
        PGW[Payment Gateway]:::lightblue
        MAPS[Maps API]:::lightblue
        PUSH[Push Notifications]:::lightblue
    end
...
flowchart LR
    subgraph "Writes"
        W1[Orders → PostgreSQL]:::primary
        W2[Locations → Redis]:::orange
        W3[Events → Kafka]:::purple
    end

    subgraph "Reads"
        R1[Restaurant Search → Elasticsearch]:::green
        R2[Current Location → Redis]:::orange
        R3[Order Details → PostgreSQL]:::primary
    end
...
flowchart TB
    ...
    subgraph "Search Data"
        ...
        ES1[Restaurant search - Full text]:::green
        ES2[Geo queries - Nearby entities]:::green
    end

    subgraph "Time-series Data"
        TS[(Redis + TimescaleDB)]:::orange
        TS1[Current locations - Fast reads]:::orange
        TS2[Location history - Analytics]:::orange
    end

    subgraph "Relational Data"
        PG[(PostgreSQL)]:::primary
        PG1[Orders - ACID transactions]:::primary
        PG2[Users - Strong consistency]:::primary
        PG3[Restaurants - Complex relations]:::primary
    end
...
```mermaid
flowchart TD
    A[Order Ready]:::primary --> B[Query partners within 5km]:::orange
    B --> C[Filter: status = ONLINE]:::orange
    C --> D[Sort by distance]:::orange
    D --> E[Assign to closest]:::green
...
flowchart TD
    A[Order Ready]:::primary --> B[Query partners within 5km]:::orange
    B --> C[For each partner]:::orange
    C --> D[Calculate distance score]:::secondary
    C --> E[Calculate ETA score]:::secondary
    C --> F[Calculate rating score]:::secondary
    C --> G[Calculate fairness score]:::secondary
    D & E & F & G --> H[Weighted sum = Final score]:::orange
    H --> I[Sort by score]:::orange
    I --> J[Assign to highest score]:::green
...
```mermaid
flowchart TB
    subgraph "Partner Apps (250K)"
        PA[Partner Apps]:::rose
    end

    subgraph "Ingestion Layer"
        LB[Load Balancer]:::secondary
        LTS1[Location Service 1]:::primary
        LTS2[Location Service 2]:::primary
        LTSN[Location Service N]:::primary
    end

    subgraph "Storage Layer"
        REDIS[(Redis Cluster<br/>Current Locations)]:::purple
        KAFKA[Kafka<br/>Location Events]:::purple
    end

    subgraph "Processing Layer"
        STREAM[Stream Processor]:::orange
        WSG[WebSocket Gateway]:::orange
    end

    subgraph "Analytics"
        TS[(TimescaleDB<br/>Location History)]:::purple
    end
    ...
    PA --> LB
    LB --> LTS1 & LTS2 & LTSN
    LTS1 & LTS2 & LTSN --> REDIS
    LTS1 & LTS2 & LTSN --> KAFKA
    KAFKA --> STREAM --> TS
    KAFKA --> WSG --> CA
...
```mermaid
flowchart TD
    A[Monitor Zone]:::primary --> B{Demand/Supply > 2"}
    B -->|No| C[Surge = 1.0x]:::green
    B -->|Yes| D[Calculate Surge]:::orange
    D --> E[Display to Customer]:::orange
    E --> F{Customer Accepts"}
    F -->|Yes| G[Create Order]:::green
    F -->|No| H[Order Abandoned]:::red
...
```mermaid
flowchart TB
    A[New Order Ready]:::primary --> B{Partner has active order"}
    B -->|No| C[Single Order Assignment]:::orange
    B -->|Yes| D[Calculate Batch Score]:::orange
    D --> E{Score > Threshold"}
    E -->|Yes| F[Add to Batch]:::green
    E -->|No| C
    F --> G[Update ETAs for Both]:::green
...
```mermaid
flowchart TD
    A[No updates for 2 min]:::orange --> B[Mark AT_RISK]:::orange
    B --> C[Contact partner]:::primary
    C --> D{Response"}
    D -->|Yes| E[Continue delivery]:::green
    D -->|No, 5 min| F[Reassign to new partner]:::red
    F --> G[Notify customer]:::red
