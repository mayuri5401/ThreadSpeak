---
id: "hld-interviews-interview-patterns-removing-single-points-of-failure"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Removing Single Points of Failure"
slug: "hld-interviews-interview-patterns-removing-single-points-of-failure"
summary: "Your system has been running fine for months. Then one server fails. A single server. Suddenly your entire application is down, thousands of users are affected, and your on-call engineer is scrambling at 3 AM."
eli10: "Imagine Removing Single Points of Failure as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Removing Single Points of Failure Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
...
```mermaid
flowchart LR
    U[User]:::primary --> WS[Web Server]:::primary --> DB[(Single Database)]:::red

    DB -.-> Note[If this fails,<br/>everything fails]:::note

    classDef primary fill:#00ceff,stroke:#000,color:#000
    classDef red fill:#ff8787,stroke:#000,color:#000
    classDef note fill:#ffd43b,stroke:#000,color:#000
```
...
```mermaid
flowchart TD
    DB[(Database)]:::red --> |fails| OS[Order Service]:::red
    DB --> |fails| IS[Inventory Service]:::red
    OS --> |fails| CO[Checkout]:::red
    IS --> |fails| PP[Product Pages]:::red
...
```mermaid
flowchart TD
    U[Users]:::primary --> DNS[DNS]:::orange
    DNS --> LB[Load Balancer]:::orange
    LB --> S1[Server 1]:::primary
    LB --> S2[Server 2]:::primary
    S1 --> DB[(Primary DB)]:::purple
    S2 --> DB
    S1 --> C[Cache]:::orange
    S2 --> C
...
```mermaid
flowchart LR
    U[User]:::primary --> LB[Load Balancer]:::secondary
    LB --> S1[App Server 1]:::primary
    LB --> S2[App Server 2]:::primary
    LB --> S3[App Server 3]:::primary
    S1 --> DB[(Database Cluster)]:::purple
    S2 --> DB
    S3 --> DB
...
```mermaid
flowchart TD
    subgraph Failover["After Primary Fails"]
        U2[User]:::primary --> S2[(Standby DB<br/>now Primary)]:::green
    end

    subgraph Normal["Normal Operation"]
        U1[User]:::primary --> P1[(Primary DB)]:::green
        P1 -.-> |replicating| S1[(Standby DB)]:::orange
    end
...
```mermaid
flowchart TD
    U[User]:::primary --> DNS[DNS]:::secondary --> LB[Load Balancer]:::secondary
    LB --> |health check| S1[Server 1<br/>healthy]:::green
    LB --> |health check| S2[Server 2<br/>healthy]:::green
    LB -.-> |removed| S3[Server 3<br/>failed]:::red
...
```mermaid
flowchart TD
    VIP[Virtual IP: 10.0.0.100]:::orange --> LB1[LB 1 - Primary<br/>Owns the VIP]:::green
    VIP -.-> |failover| LB2[LB 2 - Standby<br/>Monitors LB 1]:::secondary
    LB1 --> Servers[Backend Servers]:::primary
    LB2 -.-> Servers
...
```mermaid
flowchart TD
    W[Writes]:::orange --> P[(Primary)]:::green
    P --> |replication| R1[(Replica 1)]:::primary
    P --> |replication| R2[(Replica 2)]:::primary
    P --> |replication| R3[(Replica 3)]:::primary
    R1 --> RD[Reads]:::orange
    R2 --> RD
    R3 --> RD
...
```mermaid
flowchart LR
    W1[Writes/Reads]:::orange --> P1[(Primary 1)]:::green
    P1 <--> |sync| P2[(Primary 2)]:::green
    P2 --> W2[Writes/Reads]:::orange
    P1 --> R1[(Replica 1)]:::primary
    P2 --> R2[(Replica 2)]:::primary
...
flowchart TD
    subgraph Region["Region: US-East"]
        subgraph ZA["Zone A"]
            S1[Server]:::primary
            DB1[(DB Primary)]:::green
            C1[Cache]:::secondary
        end
        subgraph ZB["Zone B"]
            S2[Server]:::primary
            DB2[(DB Replica)]:::purple
            C2[Cache]:::secondary
        end
        subgraph ZC["Zone C"]
            S3[Server]:::primary
            DB3[(DB Replica)]:::purple
            C3[Cache]:::secondary
        end
    end
...
```mermaid
flowchart TD
    DNS[Global DNS]:::orange --> R1[US-East Region]:::primary
    DNS --> R2[EU-West Region]:::primary
    R1 --> S1[Full Stack<br/>App + DB + Cache]:::secondary
    R2 --> S2[Full Stack<br/>App + DB + Cache]:::secondary
    S1 <--> |data sync| S2
...
```mermaid
flowchart TD
    D[example.com]:::orange --> P[Primary DNS<br/>Route 53]:::green
    D --> S[Secondary DNS<br/>Cloudflare]:::secondary
    P --> |health check| HC{Healthy"}:::orange
    HC --> |yes| P
    HC --> |no| S
...
```mermaid
flowchart LR
    U[Users]:::primary --> CDN1[Primary CDN<br/>80% traffic]:::green
    U --> CDN2[Secondary CDN<br/>20% traffic]:::secondary
    CDN1 --> O[Origin Servers]:::purple
    CDN2 --> O
...
flowchart TD
    subgraph Redis["Redis Cluster"]
        P1[Primary 1]:::green --> R1[Replica 1]:::primary
        P2[Primary 2]:::green --> R2[Replica 2]:::primary
        P3[Primary 3]:::green --> R3[Replica 3]:::primary
    end
...
```mermaid
flowchart TD
    P[(Primary<br/>Writes)]:::green --> R1[(Replica 1<br/>Reads)]:::primary
    P --> R2[(Replica 2<br/>Reads)]:::primary
    P --> R3[(Replica 3<br/>Reads)]:::primary
...
flowchart LR
    ...
    B1 --> T[Topic: orders<br/>replication factor: 3]:::green
    B2 --> T
    B3 --> T
...
```mermaid
flowchart LR
    U[Upload]:::orange --> P[Primary Region]:::green
    P --> |automatic replication| S[Secondary Region]:::secondary
