---
id: "hld-interviews-interview-patterns-distributed-counting-a-pattern-for-counting-at-scale"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Distributed Counting: A Pattern for Counting at Scale"
slug: "hld-interviews-interview-patterns-distributed-counting-a-pattern-for-counting-at-scale"
summary: "Counting sounds simple. Increment a number when something happens. How hard can it be\""
eli10: "Imagine Distributed Counting: A Pattern for Counting at Scale as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Distributed Counting: A Pattern for Counting at Scale Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart LR
    ...
    style E1 fill:#00ceff,stroke:#000,color:#000
    style E2 fill:#00ceff,stroke:#000,color:#000
    style E3 fill:#00ceff,stroke:#000,color:#000
    style E4 fill:#00ceff,stroke:#000,color:#000
    style E5 fill:#00ceff,stroke:#000,color:#000
```
...
sequenceDiagram
    ...
    U1->>DB: Read count (100)
    U2->>DB: Read count (100)
    U3->>DB: Read count (100)
    U1->>DB: Write count (101)
    U2->>DB: Write count (101)
    U3->>DB: Write count (101)

    Note over DB: Expected: 103<br/>Actual: 101<br/>Lost updates!
```
...
flowchart TD
    subgraph Distributed Database
        P1[Partition 1<br/>Normal Posts]:::green
        P2[Partition 2<br/>Normal Posts]:::green
        P3[Partition 3<br/>Viral Post]:::red
        P4[Partition 4<br/>Normal Posts]:::green
    end

    subgraph Traffic
        T1[10 writes/sec]:::green --> P1
        T2[15 writes/sec]:::green --> P2
        T3[10,000 writes/sec]:::red --> P3
        T4[12 writes/sec]:::green --> P4
    end
...
flowchart LR
    subgraph Approaches
        direction LR
        A[Exact<br/>Real-time]:::primary
        B[Exact<br/>Delayed]:::secondary
        C[Approximate<br/>Fast]:::orange
    end

    A --> A1[Single Counter<br/>Low scale only]
    B --> B1[Aggregated Counter<br/>High scale, seconds delay]
    C --> C1[Probabilistic<br/>Very high scale, ~1% error]
...
```mermaid
flowchart TD
    A[Read Current Count]:::primary --> B{Count Changed"}
    B -->|No| C[Write New Count]:::green
    B -->|Yes| D[Retry]:::orange
    D --> A
    C --> E[Success]:::green
...
flowchart TD
    subgraph "Write Path"
        W[Write Request]:::primary --> H[Hash User ID]:::secondary
        H --> S1[Shard 0<br/>Count: 2,341]:::purple
        H --> S2[Shard 1<br/>Count: 2,156]:::purple
        H --> S3[Shard 2<br/>Count: 2,289]:::purple
        H --> S4[Shard 3<br/>Count: 2,214]:::purple
    end

    subgraph "Read Path"
        R[Read Request]:::primary --> AGG[Aggregate]:::orange
        S1 --> AGG
        S2 --> AGG
        S3 --> AGG
        S4 --> AGG
        AGG --> T[Total: 9,000]:::green
    end
...
```mermaid
flowchart TD
    A[Incoming Write]:::primary --> B{Hot Entity"}
    B -->|No| C[Single Counter]:::green
    B -->|Yes| D[Sharded Counter]:::orange

    E[Monitor Write Rate]:::secondary --> F{> Threshold"}
    F -->|Yes| G[Activate Sharding]:::orange
    F -->|No| H[Keep Single]:::green
...
flowchart LR
    subgraph "Write Path (Fast)"
        W[Write Request]:::primary --> LS[Like Service]:::secondary
        LS --> DB[(Likes DB)]:::purple
        LS --> Q[Message Queue]:::orange
    end

    subgraph "Aggregation (Async)"
        Q --> AGG[Count Aggregator]:::secondary
        AGG --> CC[(Count Cache)]:::green
    end

    subgraph "Read Path (Fast)"
        R[Read Request]:::primary --> CC
    end
...
flowchart TD
    subgraph CMS["Count-Min Sketch"]
        I[Item X]:::primary --> H1[Hash 1]:::orange
        I --> H2[Hash 2]:::orange
        I --> H3[Hash 3]:::orange

        subgraph CA["Counter Array"]
            R1["Row 1: 0, 5, 3, 0, 2, 1, 0, 4"]:::purple
            R2["Row 2: 2, 0, 1, 6, 0, 3, 0, 2"]:::purple
            R3["Row 3: 0, 4, 0, 2, 5, 0, 1, 0"]:::purple
        end
    ...
    Q[Query X]:::primary --> MIN[Take Minimum]:::green
    MIN --> EST[Estimated Count]:::green
...
```mermaid
flowchart LR
    subgraph "HyperLogLog Intuition"
        I1[Item A]:::primary --> H[Hash]:::secondary
        I2[Item B]:::primary --> H
        I3[Item C]:::primary --> H

        H --> B1["Binary: 00101..."]:::orange
        H --> B2["Binary: 00001..."]:::orange
        H --> B3["Binary: 01001..."]:::orange

        B1 --> LZ[Track Leading Zeros]:::purple
        B2 --> LZ
        B3 --> LZ

        LZ --> EST["Max zeros = 4<br/>Estimate ≈ 2^4 = 16 uniques"]:::green
    end
...
flowchart TD
    Q1{Need exact count"}
    Q1 -->|Yes| Q2{High write rate"}
    Q1 -->|No| Q3{Need unique count"}

    Q2 -->|No| A1[Single Counter]:::green
    Q2 -->|Yes| Q4{Real-time needed"}

    Q4 -->|Yes| A2[Sharded Counters]:::orange
    Q4 -->|No| A3[Write-Behind]:::secondary

    Q3 -->|Yes| A4[HyperLogLog]:::purple
    Q3 -->|No| A5[Count-Min Sketch]:::purple
