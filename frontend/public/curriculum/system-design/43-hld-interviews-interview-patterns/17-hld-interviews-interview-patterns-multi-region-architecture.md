---
id: "hld-interviews-interview-patterns-multi-region-architecture"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Multi-Region Architecture"
slug: "hld-interviews-interview-patterns-multi-region-architecture"
summary: "Your service is running smoothly in a single data center. Then an earthquake hits, the power grid fails, or a fiber cable gets cut. Suddenly, millions of users cannot access your application."
eli10: "Imagine Multi-Region Architecture as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Multi-Region Architecture Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
...
```mermaid
flowchart LR
    subgraph "Single Region (Risky)"
        A[Users] --> B[Region A]
        B --> C[💥 Disaster]
        C --> D[Complete Outage]
    end

    style B fill:#ff8787,stroke:#000,color:#000
...
```mermaid
flowchart LR
    subgraph "Multi-Region (Resilient)"
        U[Users] --> R1[Region A]
        U --> R2[Region B]
        R1 --> X[💥 Disaster]
        R2 --> OK[Still Running]
    ...
    style OK fill:#69db7c,stroke:#000,color:#000
```
...
flowchart TB
    subgraph "Active-Passive"
        ...
        subgraph "Standby Region (Passive)"
            A2[App Servers<br/>Idle]
            D2[(Replica DB)]
        end

        GLB --> A1
        GLB -.->|Failover Only| A2
        A1 --> D1
        D1 -->|Async Replication| D2
    end

    style GLB fill:#00ceff,stroke:#000,color:#000
    style A1 fill:#69db7c,stroke:#000,color:#000
    style D1 fill:#9775fa,stroke:#000,color:#000
    style A2 fill:#ffa94d,stroke:#000,color:#000
    style D2 fill:#9775fa,stroke:#000,color:#000
```
...
flowchart TB
    ...
    style D1 fill:#9775fa,stroke:#000,color:#000
    style D2 fill:#9775fa,stroke:#000,color:#000
    style D3 fill:#9775fa,stroke:#000,color:#000
```
...
```mermaid
flowchart TB
    subgraph "Read Local, Write Global"

        subgraph "Region US (Primary)"
            A1[App Servers]
            D1[(Primary DB<br/>Reads + Writes)]
        end
    ...
    style D2 fill:#ffa94d,stroke:#000,color:#000
    style D3 fill:#ffa94d,stroke:#000,color:#000
```
...
flowchart TB
    ...
    style GLB fill:#00ceff,stroke:#000,color:#000
    style A1 fill:#38d9a9,stroke:#000,color:#000
    style A2 fill:#38d9a9,stroke:#000,color:#000
    style A3 fill:#38d9a9,stroke:#000,color:#000
    style D1 fill:#9775fa,stroke:#000,color:#000
    style D2 fill:#9775fa,stroke:#000,color:#000
    style D3 fill:#9775fa,stroke:#000,color:#000
```
...
sequenceDiagram
    ...
    C->>R1: Write Request
    R1->>R1: Write to Local DB
    R1->>C: Success ✓
    R1-->>R2: Replicate (async)
    Note over R2: Data arrives later
```
...
sequenceDiagram
    ...
    C->>R1: Write Request
    R1->>R1: Write to Local DB
    R1->>R2: Replicate (sync)
    R2->>R2: Write to Local DB
    R2->>R1: Ack ✓
    R1->>C: Success ✓
```
...
flowchart LR
    subgraph "Semi-Synchronous"
        P[Primary<br/>US-East]
        S1[Sync Replica<br/>US-West]
        S2[Async Replica<br/>EU]
        S3[Async Replica<br/>APAC]
    ...
    style S3 fill:#ffa94d,stroke:#000,color:#000
```
...
flowchart TB
    subgraph "GeoDNS Routing"
        U1[User in US] -->|DNS Query| DNS[GeoDNS]
        U2[User in EU] -->|DNS Query| DNS
        U3[User in Asia] -->|DNS Query| DNS

        DNS -->|Returns US IP| R1[US Region<br/>52.1.2.3]
        DNS -->|Returns EU IP| R2[EU Region<br/>34.5.6.7]
        DNS -->|Returns APAC IP| R3[APAC Region<br/>13.8.9.10]
    ...
    style DNS fill:#00ceff,stroke:#000,color:#000
    style R1 fill:#69db7c,stroke:#000,color:#000
    style R2 fill:#69db7c,stroke:#000,color:#000
    style R3 fill:#69db7c,stroke:#000,color:#000
```
...
flowchart TB
    ...
    style IP fill:#00ceff,stroke:#000,color:#000
    style R1 fill:#69db7c,stroke:#000,color:#000
    style R2 fill:#69db7c,stroke:#000,color:#000
    style R3 fill:#69db7c,stroke:#000,color:#000
```
...
flowchart TB
    subgraph "Application-Level Routing"
        ...
        GLB -->|User Home Region| R1[Home Region]
        GLB -->|Nearest Region| R2[Nearest Region]
        GLB -->|Least Loaded| R3[Available Region]
    end

    style GLB fill:#00ceff,stroke:#000,color:#000
    style R1 fill:#9775fa,stroke:#000,color:#000
    style R2 fill:#69db7c,stroke:#000,color:#000
...
flowchart TB
    subgraph "Health Check System"
        ...
        HC -->|Check /health| R1[Region US<br/>✓ Healthy]
        HC -->|Check /health| R2[Region EU<br/>✓ Healthy]
        HC -->|Check /health| R3[Region APAC<br/>✗ Unhealthy]
    ...
    style HC fill:#00ceff,stroke:#000,color:#000
    style R1 fill:#69db7c,stroke:#000,color:#000
    style R2 fill:#69db7c,stroke:#000,color:#000
    style R3 fill:#ff8787,stroke:#000,color:#000
    style GLB fill:#38d9a9,stroke:#000,color:#000
```
...
flowchart TD
    subgraph "Automatic Failover"
        T1[Traffic] --> R1[Region A<br/>Primary]
        R1 --> F[Failure Detected]
        F --> T2[Traffic] --> R2[Region B<br/>Takes Over]
    end
...
flowchart TB
    subgraph "Split-Brain Danger"
        ...
        R1[Region A<br/>'I am Primary']
        R2[Region B<br/>'I am Primary']

        R1 <-->|Cannot communicate| NP
        NP <-->|Cannot communicate| R2
    ...
    style NP fill:#ff8787,stroke:#000,color:#000
...
```mermaid
flowchart TB
    subgraph "Multi-Region Decision Tree"
        Q1{Need Global<br/>Low Latency"}
        Q2{Data Must<br/>Stay in Region"}
        Q3{Write-Heavy"}
        Q4{Strong Consistency<br/>Required"}

        Q1 -->|No| AP[Active-Passive]
        Q1 -->|Yes| Q2

        Q2 -->|Yes| GP[Geo-Partitioned]
        Q2 -->|No| Q3

        Q3 -->|Yes| Q4
        Q3 -->|No| RL[Read Local<br/>Write Global]

        Q4 -->|Yes| SS[Single Region +<br/>Sync Standby]
        Q4 -->|No| AA[Active-Active<br/>Eventual Consistency]
    end

    style Q1 fill:#00ceff,stroke:#000,color:#000
    style Q2 fill:#00ceff,stroke:#000,color:#000
    style Q3 fill:#00ceff,stroke:#000,color:#000
    style Q4 fill:#00ceff,stroke:#000,color:#000
    style AP fill:#69db7c,stroke:#000,color:#000
    style GP fill:#69db7c,stroke:#000,color:#000
    style RL fill:#69db7c,stroke:#000,color:#000
    style SS fill:#69db7c,stroke:#000,color:#000
    style AA fill:#69db7c,stroke:#000,color:#000
```
