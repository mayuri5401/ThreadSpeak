---
id: "hld-interviews-interview-patterns-real-time-media-streaming"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Interview Patterns"
subSection: ""
title: "Real-Time Media Streaming"
slug: "hld-interviews-interview-patterns-real-time-media-streaming"
summary: "Media streaming is one of the most demanding challenges in distributed systems. It combines massive data volumes (video is heavy), strict timing requirements (frames must arrive on schedule), and highly variable client conditions (networks change ..."
eli10: "Imagine Real-Time Media Streaming as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Real-Time Media Streaming Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
...
```mermaid
flowchart TD
    subgraph GOP [Group of Pictures]
        I[I-Frame<br/>Complete image]:::primary
        P1[P-Frame<br/>Differences from I]:::secondary
        P2[P-Frame<br/>Differences from P1]:::secondary
        B1[B-Frame<br/>Interpolated]:::orange
        P3[P-Frame]:::secondary
    end
...
flowchart TD
    subgraph Server
        V[Video Source]:::primary --> S[Segmenter]:::secondary
        S --> M[Manifest<br/>.m3u8]:::orange
        S --> SEG1[Segment 1<br/>.ts]:::purple
        S --> SEG2[Segment 2<br/>.ts]:::purple
        S --> SEG3[Segment 3<br/>.ts]:::purple
    end
...
flowchart LR
    ...
    subgraph With Media Server
        C[Browser]:::primary --> S[SFU Server]:::secondary
        S --> D[Browser]:::primary
        S --> E[Browser]:::primary
        S --> F[Browser]:::primary
    end
    ...
    subgraph Peer-to-Peer
        A[Browser A]:::primary <--> B[Browser B]:::primary
    end
...
flowchart TB
    subgraph Ingest
        S[Streamer/Camera]:::primary --> IS[Ingest Server]:::secondary
    end

    subgraph Processing
        IS --> TR[Transcoder]:::orange
        TR --> ST[Storage]:::purple
    end

    subgraph Distribution
        ST --> OR[Origin Server]:::secondary
        OR --> CDN[CDN Edge Servers]:::orange
    end

    subgraph Playback
        CDN --> V1[Viewer 1]:::primary
        CDN --> V2[Viewer 2]:::primary
        CDN --> V3[Viewer 3]:::primary
    end
...
```mermaid
flowchart LR
    I[Input Stream<br/>1080p60 20Mbps]:::primary

    I --> T[Transcoder]:::secondary

    T --> O1[1080p 6Mbps]:::orange
    T --> O2[720p 3Mbps]:::orange
    T --> O3[480p 1.5Mbps]:::orange
    T --> O4[360p 0.8Mbps]:::orange
    T --> O5[Audio only 128kbps]:::purple
...
```mermaid
flowchart LR
    TR[Transcoder]:::primary --> HOT[(Hot Storage<br/>SSD/Local)]:::orange
    HOT --> WARM[(Warm Storage<br/>S3/GCS)]:::secondary
    WARM --> COLD[(Cold Storage<br/>Glacier)]:::purple
...
flowchart TD
    ...
    subgraph Shield Layer
        SH1[Shield US]:::secondary
        SH2[Shield EU]:::secondary
        SH3[Shield Asia]:::secondary
    end

    subgraph Edge Layer
        E1[Edge NYC]:::orange
        E2[Edge LA]:::orange
        E3[Edge London]:::orange
        E4[Edge Frankfurt]:::orange
        E5[Edge Tokyo]:::orange
        E6[Edge Singapore]:::orange
    end
    ...
    E1 --> V1[Viewers]:::primary
    E2 --> V2[Viewers]:::primary
    E3 --> V3[Viewers]:::primary
...
flowchart TD
    subgraph Player
        M[Monitor bandwidth<br/>buffer level]:::primary
        A[ABR Algorithm]:::secondary
        D[Download segment]:::orange
        P[Play segment]:::purple
    end
...
flowchart LR
    subgraph Low Latency
        A[WebRTC<br/>200ms]:::primary
        B[LL-HLS<br/>2-5s]:::secondary
    end

    subgraph High Scale
        C[HLS/DASH<br/>15-30s]:::orange
    end

    A --> |Thousands| SCALE[Viewer Scale]
    B --> |Hundreds of thousands| SCALE
    C --> |Millions| SCALE
...
flowchart TD
    ...
    O --> CDN1[CDN A<br/>50% traffic]:::secondary
    O --> CDN2[CDN B<br/>30% traffic]:::secondary
    O --> CDN3[CDN C<br/>20% traffic]:::orange

    CDN1 --> V[Viewers]:::primary
    CDN2 --> V
    CDN3 --> V
...
flowchart TB
    subgraph Streamers
        S1[Streamer 1]:::primary
        S2[Streamer 2]:::primary
        S3[Streamer N]:::primary
    end

    subgraph Ingest Layer
        ILB[Ingest LB]:::secondary
        I1[Ingest 1]:::secondary
        I2[Ingest 2]:::secondary
        I3[Ingest N]:::secondary
    end

    subgraph Transcoding Layer
        TQ[(Transcode Queue)]:::orange
        T1[Transcoder 1]:::orange
        T2[Transcoder 2]:::orange
        T3[Transcoder N]:::orange
    end

    subgraph Storage
        SEG[(Segment Store<br/>S3)]:::purple
        META[(Metadata<br/>DynamoDB)]:::purple
    end

    subgraph Distribution
        OR[Origin Servers]:::secondary
        CDN[CDN]:::orange
    end
