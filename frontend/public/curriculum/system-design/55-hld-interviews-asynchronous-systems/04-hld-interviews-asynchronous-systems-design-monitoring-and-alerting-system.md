---
id: "hld-interviews-asynchronous-systems-design-monitoring-and-alerting-system"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Asynchronous Systems"
subSection: ""
title: "Design Monitoring and Alerting System"
slug: "hld-interviews-asynchronous-systems-design-monitoring-and-alerting-system"
summary: "In this chapter, we'll explore the high-level design of such a system including how it works, the key components involved, and the architecture behind real-time detection and alerting."
eli10: "Imagine Design Monitoring and Alerting System as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Monitoring and Alerting System Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
tags: ["HLD Interviews","Asynchronous Systems","System Design","Architecture"]
codeSnippet:
  language: "shell"
  explanation: "Production implementation for Design Monitoring and Alerting System"
  code: |
    metrics/{tenant}/{metric_name}/{YYYY}/{MM}/{DD}/chunk-file
---
...
The **Metric Collection Pipeline** gathers performance data from various systems and ensures it’s safely stored for querying and alerting.
 ...
 C(Metrics Agent)\n    B -- Push/Scrape --> C\n    N -- Push/Scrape --> C\n\n    C -- Batch & Push --> D[Ingestion Service]\n\n    D -- Stream/Buffer --> E(Kafka / Message Queue)\n\n    E -- Consume & Process --> F[Storage]"} -->
```mermaid
flowchart TB
...
The **Alerting Pipeline** continuously evaluates metric data against user-defined rules and notifies teams when thresholds are breached.
 ...
 F[Storage]\n\n    subgraph Alerting Pipeline\n        I[Alerting Engine]\n        J[Notification Service]\n    end\n\n    I -- Evaluate Rules --> G\n    I -- Trigger --> J\n    J -- Fan-out --> K[Email]\n    J -- Fan-out --> L[Slack]\n    J -- Fan-out --> M[PagerDuty]\n    J -- Fan-out --> N[SMS/Webhooks]\n\n    G -- Data for Alerting --> I"} -->
```mermaid
flowchart TB
    subgraph Query
        G[Query Service]
...
<details>
<summary>Approach 1 (Bad): Applications log metrics as plain text files.</summary>

Each application writes its metrics (e.g., `cpu_usage=50, timestamp=...`) to local log files. A separate batch process periodically scrapes and parses these files.

#### **Trade-offs:**

- **Pros:** Simple to implement initially within an app.
- **Cons:** Not real-time (metrics are delayed until file parsing). Requires complex log parsing logic. High I/O overhead on application servers. Difficult to scale and prone to data loss if files are rotated or servers crash before parsing. Lacks structure and metadata (labels).
</details>

<details>
<summary>Approach 2 (Good): Applications expose HTTP endpoints (/metrics).</summary>

Applications instrument their code to expose metrics in a standardized format (e.g., Prometheus exposition format) via an HTTP endpoint. A dedicated agent (like Prometheus's `scrape_config`) then periodically makes HTTP requests to these endpoints to pull metrics.

#### **Trade-offs:**

- **Pros:** Decouples metric collection from the application. Agents can discover targets. Standardized format improves interoperability. Real-timeish (depends on scrape interval).
- **Cons:** Requires agents to know where to find applications (service discovery). Pull-based model might struggle with very ephemeral services or when network partitions prevent scraping. Application still incurs overhead serving `/metrics` requests.
</details>

<details>
<summary>Approach 3 (Great): Use lightweight agents that push/pull metrics.</summary>

Deploy a dedicated, language-agnostic agent (like OpenTelemetry Collector or specialized exporters) alongside each application or on each host. These agents are configured to *either* pull metrics from the application *or* receive pushed metrics via a client library. They then batch, compress, and send these metrics (often using efficient formats like Protobuf over gRPC or HTTP/2) to the Ingestion Service. Metrics include rich metadata as tags/labels (e.g., `service="api"`, `host="host-1"`, `region="us-east-1"`).

#### **Trade-offs:**

- **Pros:** Highly efficient (batching, compression, protobuf). Flexible (push/pull). Agents can act as a local buffer, preventing data loss during network issues. Rich metadata (labels/tags) allows powerful querying and aggregation. Decouples instrumentation from transport. Standardized formats like OpenTelemetry offer future-proofing.
- **Cons:** Adds another component (the agent) to manage and deploy. Requires careful configuration of agents.
</details>
...
> **Should agents push data to the monitoring system, or should the system pull data from agents"**
 ...
 ExporterA\n        ClientB --> ExporterB\n        ClientC --> ExporterC\n\n        Prometheus -->|Scrape| ExporterA\n        Prometheus -->|Scrape| ExporterB\n        Prometheus -->|Scrape| ExporterC\n    end\n\n    subgraph Push_Model [Push-Based Model]\n        Server1[Server A]\n        Server2[Server B]\n        Server3[Server C]\n        Agent1[Agent A]\n        Agent2[Agent B]\n        Agent3[Agent C]\n        PushGateway[Push Endpoint / Kafka / Ingestion API]\n        TSDB[Time-Series DB]\n\n        Server1 --> Agent1 -->|Push| PushGateway\n        Server2 --> Agent2 -->|Push| PushGateway\n        Server3 --> Agent3 -->|Push| PushGateway\n\n        PushGateway --> TSDB\n    end"} -->
```mermaid
flowchart LR
    subgraph Pull_Model [Pull-Based Model]
        ClientA[Server A]
        ClientB[Server B]
        ClientC[Server C]
        ExporterA[Exporter A]
        ExporterB[Exporter B]
        ExporterC[Exporter C]
        Prometheus[(Monitoring Server Prometheus)]
...
### High-Level Architecture
 ...
 I[Ingestion Service]\n    B[Agent 2] --> I\n    C[Agent N] --> I\n\n    I --> K1(Kafka Partition 1)\n    I --> K2(Kafka Partition 2)\n    I --> KN(Kafka Partition N)\n\n    K1 -- Consume Batches --> W1[Worker 1]\n    K2 -- Consume Batches --> W2[Worker 2]\n    KN -- Consume Batches --> WN[Worker N]\n\n    W1 -- Vectorized Write --> TS[Time-Series Storage]\n    W2 -- Vectorized Write --> TS\n    WN -- Vectorized Write --> TS"} -->
```mermaid
flowchart LR
    A[Agent 1] --> I[Ingestion Service]
    B[Agent 2] --> I
    C[Agent N] --> I
...
```json
{
  "tenant": "acme",
  "metrics": [
    { "name": "cpu_usage", "value": 0.87, "timestamp": 1718964000, "labels": {"region":"us-east","service":"auth"} },
    { "name": "memory_usage", "value": 0.73, "timestamp": 1718964000, "labels": {"region":"us-east","service":"auth"} }
  ]
}
```
...
```shell
partition_key = hash(metric_name + sorted(labels))
```
...
### Core Components
 ...
 C\n    B -- \"What to check"\" --> C\n    C -- \"Condition met/not met\" --> D\n    D -- \"New Firing/Resolved Alerts\" --> E\n    E -- \"Grouped & Routed Alerts\" --> F\n    F --> G\n    F --> H\n    F --> I"} -->
```mermaid
flowchart TD
    subgraph "Data & Rules"
        A[Time-Series Database]
        B[Alert Rules Database]
    end

    subgraph "Alerting Pipeline"
        C{Alert Evaluator}
        D[Alert State Manager]
        E[Router & Grouper]
...
Every potential issue follows a clear, managed lifecycle, much like a smoke detector.
 ...
 Pending\n    Pending --> Firing : Condition holds for duration\n    Firing --> Resolved : Condition clears\n    Firing --> Notified : Notification sent\n    Notified --> Acknowledged : User acknowledges alert\n    Acknowledged --> Resolved : Issue fixed\n    Resolved --> [*]"} -->
```mermaid
stateDiagram-v2
    [*] --> Pending
    Pending --> Firing : Condition holds for duration
...
```shell
(job="auth-service", region="us-east", severity="critical")
```
...
```json
{
  ...
  "annotations": {
    ...
    "dashboard_url": "https://monitoring.example.com/dashboards/auth"
  }
}
```
...
> Silence all alerts for 
>
> `service=payments`
>
>  between 2 AM–3 AM UTC.
...
```shell
(metric_name, labels/tags) → [(timestamp₁, value₁), (timestamp₂, value₂), ...]
```
...
```shell
http_request_duration_seconds{job="frontend", status="200"} → 
[(t1, 0.23), (t2, 0.31), (t3, 0.27), …]
```
...
```shell
┌────────────────────────────────────────────────────────┐
│  Metric: cpu_usage{instance="web-12", region="us-east"} │
├──────────────┬──────────────┬──────────────┬────────────┤
│ Timestamp    │ Value        │ Delta Enc.   │ XOR Enc.   │
├──────────────┼──────────────┼──────────────┼────────────┤
│ 10:00:00     │ 82.5         │ —            │ —          │
│ 10:00:10     │ 82.7         │ +0.2         │ XOR bits   │
│ 10:00:20     │ 82.8         │ +0.1         │ XOR bits   │
│ 10:00:30     │ 82.6         │ -0.2         │ XOR bits   │
└──────────────┴──────────────┴──────────────┴────────────┘
```
...
```shell
http_requests_total{region, instance, endpoint, method, status}
```
