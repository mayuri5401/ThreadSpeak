---
id: "hld-interviews-social-media-systems-design-tinder"
trackId: "system-design"
trackTitle: "System Design (LLD & HLD)"
category: "HLD Interviews - Social Media Systems"
subSection: ""
title: "Design Tinder"
slug: "hld-interviews-social-media-systems-design-tinder"
summary: "The scale of modern dating apps is staggering. Tinder alone processes billions of swipes per day and has made over 75 billion matches since launch. Designing a system that can handle this load while keeping interactions feeling instant requires th..."
eli10: "Imagine Design Tinder as a well-organized city blueprint where every service and class has a single clear purpose, working together without chaos."
mentalModel: "Design Tinder Architecture Flow: Clients -> Load Balancer / API Gateway -> Services & Core Entities -> Storage & Cache."
difficulty: "Advanced"
estimatedMinutes: 15
...
flowchart TB
    subgraph "Core User Journey"
        direction LR
        A[Create Profile]:::primary --> B[Discover Profiles]:::primary
        B --> C[Swipe Like/Pass]:::primary
        C --> D[Match on<br/>Mutual Like]:::green
        D --> E[Chat with<br/>Matches]:::secondary
    end
...
flowchart LR
    subgraph "User Base"
        A[50M Total Users]:::purple
        B[10M Daily Active]:::primary
    end

    subgraph "Daily Activity"
        C[100 Swipes<br/>per User]:::orange
        D[1B Total<br/>Swipes/Day]:::green
    end
...
flowchart LR
    ...
    subgraph "Social APIs"
        S1[GET /matches<br/>List Matches]:::green
        S2[POST /messages<br/>Send Message]:::green
    end

    subgraph "Discovery APIs"
        D1[GET /recommendations<br/>Get Matches]:::orange
        D2[POST /swipes<br/>Like or Pass]:::orange
    end

    subgraph "Profile APIs"
        P1[PUT /users/profile<br/>Create/Update]:::primary
        P2[GET /users/profile<br/>View Profile]:::primary
    end
...
flowchart LR
    ...
    subgraph "Read Path (50K QPS peak)"
		direction LR
        R1[Client]:::rose --> R2[CDN]:::green
        R2 -.->|miss| R3[Cache Layer]:::orange
        R3 -.->|miss| R4[(Database)]:::purple
    end

    subgraph "Write Path (35K QPS peak)"
		direction LR
        W1[Client]:::rose --> W2[API Gateway]:::secondary
        W2 --> W3[Swipe Service]:::primary
        W3 --> W4[Match Detection]:::orange
        W3 --> W5[(Swipe DB)]:::purple
    end
...
sequenceDiagram
    ...
    Media->>S3: Generate pre-signed URL
    Media-->>Client: Return upload URL
    Client->>S3: Upload photo directly
    S3-->>Client: Upload complete
    Client->>Media: Trigger processing
    Media->>S3: Fetch and resize
    Media->>S3: Store thumbnails
    Media-->>Client: Return CDN URLs
    Client->>API: Save profile with photo URLs
    API->>Profile: Create profile
    Profile->>DB: Insert record
    DB-->>Profile: Confirm
    Profile-->>Client: Profile created
    Note over CDN,S3: Photos served via CDN
```
...
sequenceDiagram
    ...
    Client->>API: GET /recommendations (lat, long)
    API->>Rec: Forward request
    Rec->>Cache: Check cached recommendations
    alt Cache Hit
        Cache-->>Rec: Return cached profiles
    else Cache Miss
        Rec->>Loc: Find users within radius
        Loc-->>Rec: Return candidate user IDs
        Rec->>Cache: Get "already swiped" set
        Cache-->>Rec: Return swiped user IDs
        Note over Rec: Filter and rank candidates
        Rec->>Cache: Batch fetch profiles
        alt Profiles in cache
            Cache-->>Rec: Return profiles
        else Profiles not cached
            Rec->>DB: Fetch missing profiles
            DB-->>Rec: Return profiles
            Rec->>Cache: Populate cache
        end
        Rec->>Cache: Store recommendations
    end
    Rec-->>API: Return profile batch
    API-->>Client: 200 OK with profiles
```
...
sequenceDiagram
    ...
    A->>API: POST /swipes (like User B)
    API->>Swipe: Record swipe
    Swipe->>Redis: Check if B liked A
    alt B already liked A
        Redis-->>Swipe: Yes, B liked A
        Note over Swipe: MATCH DETECTED!
        par Async operations
            Swipe->>DB: Store swipe + match
            Swipe->>Notify: Send match notification
            Notify->>A: WebSocket: "It's a Match!"
            Notify->>B: WebSocket: "It's a Match!"
        end
        Swipe-->>A: 201 Created (is_match: true)
    else B has not liked A yet
        Redis-->>Swipe: No
        Swipe->>Redis: Store A's like (for B's future check)
        Swipe->>DB: Store swipe (async)
        Swipe-->>A: 201 Created (is_match: false)
    end
```
...
sequenceDiagram
    ...
    participant WS1 as WebSocket Gateway
    participant Msg as Messaging Service
    participant Redis as Connection Map
    participant DB as Message DB
    participant WS2 as WebSocket Gateway
    participant Recipient as Recipient Client
    participant Push as Push Service

    Note over Sender,WS1: Already connected via WebSocket
    Sender->>WS1: Send message
    WS1->>Msg: Forward message
    Msg->>Msg: Validate match exists
    Msg->>DB: Store message
    Msg->>Redis: Lookup recipient connection
    alt Recipient is online
        Redis-->>Msg: Recipient on WS2 server
        Msg->>WS2: Route message
        WS2->>Recipient: Deliver via WebSocket
        Recipient-->>WS2: Ack received
        Msg-->>Sender: Message delivered
    else Recipient is offline
        Redis-->>Msg: Recipient not connected
        Msg->>Push: Queue push notification
        Push->>Recipient: Send push notification
        Msg-->>Sender: Message sent (pending)
    end
```
...
flowchart TB
    subgraph Clients["Client Layer"]
        A[Mobile App<br/>iOS/Android]:::rose
    end

    subgraph Edge["Edge Layer"]
        B[CDN<br/>Photo Delivery]:::green
        C[API Gateway<br/>Auth + Routing]:::secondary
        D[WebSocket Gateway<br/>Real-time Connections]:::secondary
    end

    subgraph Services["Application Layer"]
        E[Profile Service]:::primary
        F[Recommendation Service]:::primary
        G[Location Service]:::orange
        H[Swipe Service]:::primary
        I[Messaging Service]:::primary
        J[Notification Service]:::green
    end
    ...
    subgraph Data["Storage Layer"]
        K[(Profile DB<br/>PostgreSQL)]:::purple
        L[(Swipe DB<br/>Cassandra)]:::purple
        M[(Message DB<br/>Cassandra)]:::purple
        N[(Geospatial Index<br/>Redis GEO)]:::purple
        P[(Object Storage<br/>S3)]:::purple
    end
...
flowchart TB
    subgraph "Data Types"
        P[Profiles<br/>Read-heavy, Structured]:::primary
        S[Swipes<br/>Write-heavy, Simple]:::orange
        M[Messages<br/>Append-only, Time-ordered]:::green
        L[Locations<br/>Geospatial, Frequent updates]:::rose
    end

    subgraph "Database Choices"
        PG[(PostgreSQL)]:::purple
        CASS[(Cassandra)]:::purple
        REDIS[(Redis)]:::red
    end
...
```mermaid
flowchart LR
    A[50 Million Users]:::rose --> B[Within Radius<br/>~10,000 candidates]:::orange
    B --> C[Match Preferences<br/>~3,000 candidates]:::primary
    C --> D[Not Already Swiped<br/>~2,500 candidates]:::primary
    D --> E[Mutual Interest<br/>~1,500 candidates]:::green
    E --> F[Top 20 Profiles<br/>Returned to User]:::green
...
```mermaid
flowchart LR
    A[User Request<br/>lat, long, radius]:::rose --> B[Redis GEO<br/>Fast initial filter]:::red
    B --> C[Get Candidate IDs<br/>~10,000 users]:::orange
    C --> D[Filter by Preferences<br/>Age, Gender]:::primary
    D --> E[Exclude Swiped<br/>Redis Set lookup]:::primary
    E --> F[Fetch Full Profiles<br/>Cache or PostgreSQL]:::purple
    F --> G[Return Top 20]:::green
...
```mermaid
flowchart TD
    A[Match Detected]:::green --> B[Notification Service]:::primary
    B --> C{Is User Online"}
    C -->|Yes, app active| D[WebSocket Delivery<br/>Instant in-app notification]:::green
    C -->|No, app backgrounded| E[Push Notification<br/>APNs / FCM]:::orange
    C -->|Offline| F[Store in Queue<br/>Deliver when online]:::rose
...
flowchart LR
    subgraph "Prevention"
        A[Rate Limiting]:::primary
        B[Bot Detection]:::primary
        C[Content Moderation]:::primary
    end

    subgraph "User Controls"
        D[Block User]:::orange
        E[Report Abuse]:::orange
        F[Privacy Settings]:::orange
    end

    subgraph "Enforcement"
        G[Automated Bans]:::red
        H[Human Review]:::red
        I[Account Suspension]:::red
    end
...
flowchart TB
    subgraph "Traffic Patterns"
        A[Morning<br/>Low: ~5K QPS]:::secondary
        B[Afternoon<br/>Medium: ~15K QPS]:::orange
        C[Evening Peak<br/>High: ~50K QPS]:::red
        D[Night<br/>Medium: ~20K QPS]:::orange
    end
