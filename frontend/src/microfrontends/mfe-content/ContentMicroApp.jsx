...
export default function ContentMicroApp({
  ...
  const selectedTopicIdRef = React.useRef(selectedTopicId);
  useEffect(() => {
  ...
  return (
    <div className={`relative pb-24 transition-all duration-300 ${getFontSizeClass()}`}>
      ...
      <div className="flex flex-col lg:flex-row items-start min-h-[calc(100vh-64px-52px)] w-full">
        ...
        {!isFocusMode && isSidebarCollapsed && (
          <div className="hidden lg:block sticky top-20 z-30 shrink-0 ml-4 mt-4">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-[#0B1222] hover:bg-slate-800 border border-slate-800 hover:border-emerald-500/50 text-slate-300 hover:text-white shadow-xl backdrop-blur-md transition group"
        ...
        {/* Main Topic Content Stage (Spacious & Clean) */}
        <main className={`flex-1 min-w-0 px-4 sm:px-8 lg:px-12 py-6 transition-all duration-300 ${isSidebarCollapsed || isFocusMode ? 'max-w-6xl mx-auto w-full' : 'w-full'}`}>
          <TopicViewer
            topic={selectedTopic}
      ...
      {/* =================================================================== */}
      {/* FIXED BOTTOM ACTION DOCK (ALOMASTER-STYLE FOOTER)                    */}
      {/* =================================================================== */}
      <CourseFooterDock
