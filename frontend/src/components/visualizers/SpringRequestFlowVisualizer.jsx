...
import { Play, Pause, ChevronRight, ChevronLeft, RotateCcw, Server, Shield, Send, Database, FileCode, CheckCircle2 } from 'lucide-react';
...
export default function SpringRequestFlowVisualizer() {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl border border-emerald-500/20 bg-[#0B1222]/80 space-y-6">
      {/* Visualizer Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-emerald-400 animate-pulse"></div>
            <h3 className="text-xl font-bold text-white tracking-wide">Spring MVC Request Pipeline &amp; Lifecycle Animator</h3>
              ...
              <div key={s.id} className="flex items-center flex-1">
                <button
                  ...
                  className={`w-full p-2.5 rounded-xl border text-center transition-all flex flex-col items-center gap-1.5 ${
                    isCurrent
                      ? 'bg-slate-900 border-emerald-400 shadow-lg shadow-emerald-500/20 ring-1 ring-emerald-400'
                      : isPassed
                      ? 'bg-slate-950/70 border-emerald-900/60 opacity-80'
                      : 'bg-slate-950/40 border-slate-800 opacity-50'
                  }`}
                >
                  <div className={`p-2 rounded-lg ${isCurrent ? 'bg-emerald-500 text-slate-950' : 'bg-slate-800 text-slate-300'}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <span className="text-[10px] font-mono text-slate-400 truncate w-full">
                    Step {idx + 1}
                  </span>
                  <span className={`text-xs font-semibold truncate w-full ${isCurrent ? 'text-white' : 'text-slate-400'}`}>
                    {s.badge}
                  </span>
                </button>
                {idx < steps.length - 1 && (
                  <div className={`w-3 h-0.5 mx-1 transition-colors ${idx < currentStep ? 'bg-emerald-500' : 'bg-slate-800'}`} />
                )}
              </div>
            );
          })}
      ...
      <div className={`p-5 rounded-2xl border ${active.color} space-y-4 shadow-xl transition-all`}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-xs font-mono px-3 py-1 rounded-full bg-slate-900 border border-slate-700 text-emerald-300">
            Active Processing Node
          </span>
        </div>

        {/* Action Description */}
        <div className="p-3.5 rounded-xl bg-[#080D18] border border-slate-800 space-y-1">
          <span className="text-[11px] font-mono text-slate-400 uppercase">Internal Operation:</span>
          <p className="text-sm font-mono text-cyan-300 font-semibold">{active.action}</p>
          <p className="text-xs text-slate-300 font-mono mt-1">{active.details}</p>
        </div>

        {/* Architectural Insight Takeaway */}
        <div className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-300">
          <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
          <div>
            <strong className="text-white">Architectural Takeaway:</strong> {active.takeaway}
          </div>
        </div>
      </div>
    </div>
